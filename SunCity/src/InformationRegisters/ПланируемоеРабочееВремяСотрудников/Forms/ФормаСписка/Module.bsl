...
Процедура ЗаполнитьГрафикРаботПоСотрудникам()
	...
	Для каждого РеквизитФормы Из РеквизитыФормы Цикл
		Если Найти(РеквизитФормы.Имя,"Добавлен")
			ИЛИ Найти(РеквизитФормы.Имя,"Выходной") Тогда
	...
	ДеньНедели = 0;
		...
		Для каждого ЭлементГруппа Из МассивЭлементов Цикл
				...
				Если НЕ ЭлементНайден Тогда
					...
					ТекстЗаголовокЭлемента = Прав(ЭлементГруппа.Имя, СтрДлина(ЭлементГруппа.Имя) - 2) + " " + Формат(НачалоНедели+86400*ДеньНедели, Нстр("ru='Л=ru; ДФ=dd.MM.yy'"));
					ЗаголовокЭлемента = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru ='%1'"), ТекстЗаголовокЭлемента);
