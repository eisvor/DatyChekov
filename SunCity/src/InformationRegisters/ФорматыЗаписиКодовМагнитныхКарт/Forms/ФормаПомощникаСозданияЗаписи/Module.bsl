...
Процедура ОповещениеПоискаПоМагнитномуКоду(МагнитныйКод, СтруктураРезультат) Экспорт
	...
	ИначеЕсли мТекущийНомерСтраницы = 3 Тогда
		...
		Если НЕ ПроверитьВхождениеВЗаданныйФормат(ДанныеСчитаннойДорожки) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Считанные данные карты не входят
...
Процедура ВыполнитьПроверкиПриНажатииДалее(Отказ)
	...
	Если мТекущийНомерСтраницы = 0 Тогда 
		...
		Если ПустаяСтрока(ДанныеПервойДорожки)
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Данные от считывателя магнитных карт не были распознаны,
	...
	ИначеЕсли мТекущийНомерСтраницы = 1 Тогда
		...
		Если ПустаяСтрока(КодКарты) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Выделите при помощи мыши искомую часть кода'"),,"КодКарты",,Отказ);
	...
	ИначеЕсли мТекущийНомерСтраницы = 2 Тогда
		...
		Если ПустаяСтрока(Префикс) 
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Заполните поля ""Префикс"" или ""Суффикс"",
		...
		Если (СтрДлина(Префикс + Суффикс) + КоличествоСимволов) > СтрДлина(ДанныеНужнойДорожки) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Длина полей ""Префикс"" и ""Суффикс""
		...
		Если НЕ ПроверитьВхождениеВЗаданныйФормат(ДанныеНужнойДорожки) Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Заданный код карты не входит
	...
	ИначеЕсли мТекущийНомерСтраницы = 3 Тогда
		...
		Если ВыделенныйКодКарты <> КодКарты Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Считанный по заданному формату код карты
...
Процедура СформироватьФорматКодаКарты(Отказ)
	...
	Если НомерДорожки = 0 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки,,"КодКарты",,Отказ);
