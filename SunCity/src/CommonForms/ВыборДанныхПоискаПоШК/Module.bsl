...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПоказатьОстатки") Тогда
		Элементы.ТаблицаВыбораОстаток.Видимость = Параметры.ПоказатьОстатки;
	КонецЕсли;
	...
	Если Параметры.Свойство("АдресВХранилище") Тогда
		...
		Если ЗначениеЗаполнено(Штрихкод) Тогда
			...
			Если ШтрихкодированиеРТКлиентСервер.ТелефонКорректен(Штрихкод) 
				ИЛИ ШтрихкодированиеРТКлиентСервер.ЕмаилКорректен(Штрихкод) Тогда
				
				Элементы.ТаблицаВыбораКодПоиска.Видимость = Истина;
				
				Если ШтрихкодированиеРТКлиентСервер.ТелефонКорректен(Штрихкод) Тогда
					Элементы.ТаблицаВыбораКодПоиска.Заголовок = НСтр("ru = 'Номер телефона'");
				Иначе
					Элементы.ТаблицаВыбораКодПоиска.Заголовок = НСтр("ru = 'Адрес ЭП'");
				КонецЕсли;
				
				СтрокаЗаголовка 				= НСтр("ru = 'Выбор дисконтной карты'");
				ПоказыватьОстатокНоменклатуры	= Ложь;
		...
		Для Каждого СтрокаХранилища Из ТаблицаВХранилище Цикл
			...
			Если ЗначениеЗаполнено(СтрокаХранилища.Карта) Тогда
				...
				КонецЕсли;
				НоваяСтрока.КодПоиска = СтрокаХранилища.Штрихкод;
...
Функция ПодготовитьСтруктуруВыбора()
	...
	СтруктураРезультат = ПодключаемоеОборудованиеРТКлиент.СтруктураДанныхПоиска();
	...
	Иначе
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(ТекущиеДанные.Карта) Тогда
				...
				Если ШтрихкодированиеРТКлиентСервер.ТелефонКорректен(ТекущиеДанные.КодПоиска, Истина) Тогда
					СтруктураРезультат.Вставить("НомерТелефона", ШтрихкодированиеРТКлиентСервер.ПодготовитьТелефон(ТекущиеДанные.КодПоиска));
				ИначеЕсли ШтрихкодированиеРТКлиентСервер.ЕмаилКорректен(ТекущиеДанные.КодПоиска) Тогда
					СтруктураРезультат.Вставить("АдресЭП", ТекущиеДанные.КодПоиска);
				КонецЕсли;
			...
			Иначе
				...
				Если ЗначениеЗаполнено(ТекущиеДанные.Серия) Тогда
					СтруктураСтроки.Вставить("Серия"                     , ТекущиеДанные.Серия);
					СтруктураСтроки.Вставить("ИспользоватьХарактеристики", ТекущиеДанные.ИспользоватьХарактеристики);
					СтруктураСтроки.Вставить("ВладелецХарактеристики"    , ТекущиеДанные.ВладелецХарактеристики);
				КонецЕсли;
			КонецЕсли;
...
Процедура ОбновитьОстаткиНоменклатурыВДереве()
	...
	ТаблицаОстатков = ЗапасыСервер.ОстаткиНоменклатурыПоМагазину(ТаблицаНоменклатурыДляОстатков.Выгрузить(), Магазин);
