...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МассивПараметров = СкладскиеОперацииКлиентСервер.МассивПараметровАнализаПересортицы(ПараметрыАнализаПересортицы);
...
Процедура ЗаписатьСервер(Отказ)
	...
	Если ПустаяСтрока(ПараметрыАнализаПересортицы) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,"СписокПараметров");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
