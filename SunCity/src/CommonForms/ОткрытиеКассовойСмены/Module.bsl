...
Процедура ОткрытьКассовуюСмену(Команда)
	...
	Если НЕ ЗначениеЗаполнено(КассаККМ)  Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, ЭтотОбъект, "КассаККМ", ,);
		Возврат;
	КонецЕсли;
	...
	Если ИспользоватьПодключаемоеОборудование И НЕ ИспользоватьКассуККМБезПодключенияОборудования И ЗначениеЗаполнено(ИдентификаторУстройства) Тогда
		...
		ДополнительныеПараметры_Оповещение = Новый Структура;
		ДополнительныеПараметры_Оповещение.Вставить("ОрганизацияСсылка", ИнтеграцияWebKassaВызовСервераПереопределяемый.ПолучитьРеквизитыКассы(КассаККМ).Владелец);
		
		Оповещение = Новый ОписаниеОповещения("ОткрытьКассовуюСменуЗавершение", ЭтотОбъект, ДополнительныеПараметры_Оповещение);
	...
	Иначе
		...
		ИначеЕсли ЗначениеЗаполнено(ТекстПереходящегоОстатка) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстПереходящегоОстатка);
		КонецЕсли;
