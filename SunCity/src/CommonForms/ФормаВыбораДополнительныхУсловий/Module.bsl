...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли Параметры.ПараметрыВнешнегоСоединения.ТипСоединения = "ВебСервис" Тогда
			...
			ПовторноИнициализироватьИдентификаторСсылки(ТаблицаБазыКорреспондента);
...
&НаСервере
Процедура ПовторноИнициализироватьИдентификаторСсылки(ТаблицаБазыКорреспондента)
	
	Для Каждого Строка Из ТаблицаБазыКорреспондента.Строки Цикл
		
		Если Строка.Строки.Количество() > 0 Тогда
			
			ПовторноИнициализироватьИдентификаторСсылки(Строка);
...
Функция ИдентификаторСсылки(Знач СтрокаВнутр)
	...
	СтрокаУИД = Сред(СтрокаВнутр, 1 + Найти(СтрокаВнутр, ":"), 32);
