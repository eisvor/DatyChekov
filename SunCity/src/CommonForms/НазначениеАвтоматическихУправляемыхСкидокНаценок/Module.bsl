...
// Обрабатывает внешнее событие поиска по штрихкоду.
//
// Параметры:
...
// Обрабатывает внешнее событие поиска по магнитному коду.
//
// Параметры:
//  Штрихкод - Строка - считанный магнитный код.
//  ДополнительныеПараметры - Структура - структура дополнительных параметров
//
&НаКлиенте
...
// Обрабатывает событие ввода кода одноразовой скидки.
//
// Параметры:
//  Результат - Структуара - структура результата ввода кода.
//  ДополнительныеПараметры - Структура - структура дополнительных параметров
//
&НаКлиенте
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		Если СписокОдноразовыхКодов.НайтиПоЗначению(Результат) = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(РезультатПоиска.ТекстСообщения) Тогда
				ОбщегоНазначенияКлиент.СообщитьПользователю(РезультатПоиска.ТекстСообщения);
			Иначе
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("СписокОдноразовыхКодов") Тогда
		...
		Для Каждого КодСкидки Из СписокОдноразовыхКодов Цикл
			...
			Если ЗначениеЗаполнено(РезультатПоиска.ТекстСообщения) Тогда
				ОбщегоНазначения.СообщитьПользователю(РезультатПоиска.ТекстСообщения);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура РассчитатьИнформациюОРасчетеСкидокПоДокументу(ДеревоСкидок)
	...
	Для Каждого СтрокаДерева Из ДеревоСкидок.Строки Цикл
		...
		Если СтрокаДерева.ЭтоГруппа Тогда
			...
			СтрокаДерева.ИндексКартинки = СкидкиНаценкиСервер.ИндексКартинкиДляГруппы(СтрокаДерева);
		...
		Иначе
			...
			СтрокаДерева.ИндексКартинки = СкидкиНаценкиСервер.ИндексКартинкиДляСкидки(СтрокаДерева);
			СтрокаДерева.Значение = СтрокаДерева.СкидкаНаценка;
...
Процедура ОтобразитьНадписьСегментаИсключения(СведенияДокумента)
	...
	|	И Товары.Номенклатура.ДоговорКонтрагента <> &ПустойДоговор
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
