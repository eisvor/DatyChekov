...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ОрганизацияЕГАИС") Тогда
...
Процедура ПоискПоШтрихкодуВыполнить(Команда)
	...
	ШтрихкодированиеИСРТКлиент.ПоказатьВводШтрихкода(Новый ОписаниеОповещения("ПоискПоШтрихкодуЗавершение", ЭтотОбъект));
...
Процедура ЗакрытьФормуПриСканировании()
	...
	Если РежимЗапросаТабачныхКодов Тогда
		...
		КонецЕсли;
	Иначе
		Если НЕ ИнтеграцияЕГАИСРТВызовСервера.ЭтоШтрихкодАкцизнойМарки(КодАкцизнойМарки) Тогда
			ЗаголовокИнформации = НСтр("ru = 'Штрихкод не соответствует формату.'");
			ОбщегоНазначенияРТКлиент.ВывестиИнформациюДляРМКУправляемой(ЗаголовокИнформации, КодАкцизнойМарки);
			Возврат;
		КонецЕсли;
	...
	Если НЕ РежимЗапросаТабачныхКодов Тогда
		ТекстОшибки = "";
		Если НЕ ИнтеграцияЕГАИСРТВызовСервера.ПроверитьУникальностьАкцизнойМарки(Операция, КодАкцизнойМарки, ТекстОшибки, ОрганизацияЕГАИС) Тогда
