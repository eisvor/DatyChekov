...
Функция СведенияМЧД(МЧД) Экспорт
	...
	Реквизиты = "ДатаВыдачи, ДатаОкончания, ДатаОбновленияСтатуса, СтатусВРеестреФНС, ДоверительЮЛ_КПП,
				|Подписана, Верна, Отозвана, ДатаОтзыва, ВариантЗаполненияПолномочий,
	...
	ЗаполнитьЗначенияСвойств(СведенияМЧД, РеквизитыМЧД,
		...
		|ДатаОтзыва, СовместныеПолномочия, НесколькоПредставителей,
	...
	Если ВерсияФорматаМЧД = МашиночитаемыеДоверенности.ФорматМЧД_2022_Версия_002() Тогда
		...
		Для Каждого СтрокаТаблицы Из Полномочия Цикл
			...
			Для Каждого ЭлементКод Из МассивКодов Цикл
				...
				Исключение
					НовоеПолномочие.Содержание = НСтр("ru = '<Не указано>'");
				КонецПопытки;
