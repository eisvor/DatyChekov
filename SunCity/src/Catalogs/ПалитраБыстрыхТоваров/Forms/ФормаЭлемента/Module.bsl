...
Функция ДобавитьНайденныеПозицииТоваров(СтруктураПараметров, ДополнительныеПараметры = Неопределено) Экспорт 
	...
	Если ТекущаяСтрока = Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		ИдентификаторСтроки = ТекущаяСтрока;
	КонецЕсли;
...
&НаКлиенте
Процедура Подключаемый_КартинкаБыстрыеТоварыШаблонНажатие(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ДеревоБыстрыхТоваровХарактеристикаПриИзменении(Элемент)
	...
	ТекущаяСтрока.Название = ТекущаяСтрока.Название + " " + Строка(ТекущаяСтрока.Характеристика);
	...
	Если Элементы.ДеревоБыстрыхТоваров.ВыделенныеСтроки.Количество() Тогда
		Для каждого ИндексВыделеннойСтроки Из Элементы.ДеревоБыстрыхТоваров.ВыделенныеСтроки Цикл
			Если ИндексВыделеннойСтроки = 0 Тогда
				...
				ТекстОшибки = НСтр("ru = 'Нельзя удалить корень списка ""Быстрых товаров"".'");
...
&НаКлиенте
Процедура Подключаемый_КартинкаПОНажатие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
...
Процедура ЗаполнитьДеревоБыстрыхТоваров(ОбъектЗаполнения = Неопределено)
	...
	ЭлементКорня.Название = Нстр("ru='В форме РМК'");
...
Процедура ЗаполнитьКнопкиНижнейПанели(Отказ, ПараметрыИнформации)
	...
	Если ЗначениеЗаполнено(НастройкаРМК) И НастройкаРМК.ИспользоватьРасширеннуюНастройкуКнопокНижнейПанели И НастройкаРМК.КнопкиНижнейПанели.Количество() > 0 Тогда
		...
		Для каждого СтрокаНастройкиКнопок Из НастройкаРМК.КнопкиНижнейПанели Цикл
			...
			ЭлементПолеКартинка.УстановитьДействие("Нажатие", "Подключаемый_КартинкаПОНажатие");
	...
	Если КоличествоБыстрыхТоваров > 0  Тогда
		...
		Для каждого СтрокаНастройкиКнопок Из СтрокиТЧВФормеРМК Цикл
			...
			ЭлементПолеКартинка.УстановитьДействие("Нажатие", "Подключаемый_КартинкаБыстрыеТоварыШаблонНажатие");
			ЭлементПолеКартинка.СочетаниеКлавиш =  СочетаниеКлавиш;
		...
		Если СтруктураПараметров.Номенклатура = СтрокаДерева.Номенклатура Тогда
			Если ЗначениеЗаполнено(СтруктураПараметров.Характеристика) Тогда
				Если СтруктураПараметров.Характеристика = СтрокаДерева.Характеристика Тогда
					ИскомаяСтрока = СтрокаДерева.ПолучитьИдентификатор();
					Прервать;
				КонецЕсли;
			Иначе
...
Функция ПоместитьВоВременноеХранилищеСервер()
	...
	ТаблицаТоваров.Колонки.Добавить("НомерСтроки", ОбщегоНазначенияРТКлиентСервер.ОписаниеТиповЧисла(15, 0));
	...
	ТаблицаТоваров.Колонки.Добавить("Цена", ОбщегоНазначенияРТКлиентСервер.ОписаниеТиповЧисла(15, 2));
