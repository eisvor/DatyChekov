...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.Заголовок = НСтр("ru = 'Редактирование шаблона печати этикетки'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если СохранятьИзменения Тогда
		Если ПолеТабличногоДокумента.ОбластьПечати = Неопределено Тогда
			ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(НСтр("ru = 'Не установлена область печати'"),
																				   ЭтаФорма,
		...
		Если Не РазрешитьАвтовысотуСтрок Тогда
			...
			Если НаличиеСтрокСАвтовысотой Тогда
				...
				ТекстВопроса = НСтр("ru = 'В шаблоне содержатся строки с автоопределением высоты.'")
							 + Символы.ПС
							 + НСтр("ru = 'Продолжить сохранение изменений?'");
...
&НаКлиенте
Процедура ОбновитьСоставПолей(Команда)

	ОбновитьСоставПолейНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьСоставПолейНаСервере()

	НоваяСКД = Обработки.ПечатьКодовМаркировкиИСМПТК.ПолучитьМакет("ПоляШаблонаИСМП");
	АдресВХранилище = ПоместитьВоВременноеХранилище(НоваяСКД, Новый УникальныйИдентификатор);
	КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(АдресВХранилище));	
...
Функция СтруктураМакетаШаблона()
	...
	Если ПараметрыШаблона.Получить(ИмяПараметраШтрихкод()) = Неопределено 
		ИЛИ ПараметрыШаблона.Получить(ИмяПараметраШтрихкодEAN()) = Неопределено Тогда
		Для Каждого Рисунок Из ОбластьМакетаЭтикетки.Рисунки Цикл
			...
			ИначеЕсли Лев(Рисунок.Имя,6) = ИмяПараметраШтрихкодEAN() Тогда
				ПараметрыШаблона.Вставить(ИмяПараметраШтрихкодEAN(), ПрефиксИмениПараметра + (СчетчикПараметров+1));		
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого Рисунок Из ОбластьМакетаЭтикетки.Рисунки Цикл
		Если Лев(Рисунок.Имя,8) = ИмяПараметраШтрихкод() 
			ИЛИ Лев(Рисунок.Имя,6) = ИмяПараметраШтрихкодEAN() Тогда
...
Процедура ПроверитьРазмещение(Отказ)
	...
	Если НЕ (ПолеТабличногоДокумента.ОбластьПечати.Лево = 0 
		...
		Если КоличествоПоГоризонтали <> МассивТаблиц.Количество() Тогда
			СообщениеПоГоризонтали = НСтр("ru = 'Максимальное количество по горизонтали: %1'");
			СообщениеПоГоризонтали = ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(СообщениеПоГоризонтали, МассивТаблиц.Количество());
			ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(СообщениеПоГоризонтали,, "КоличествоПоГоризонтали",, Отказ);
		КонецЕсли;
	...
	Если НЕ (ПолеТабличногоДокумента.ОбластьПечати.Верх = 0 
		...
		Если КоличествоПоВертикали <> МассивТаблиц.Количество() Тогда
			СообщениеПоВертикали = НСтр("ru = 'Максимальное количество по вертикали: %1'");
			СообщениеПоВертикали = ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(СообщениеПоВертикали, МассивТаблиц.Количество());
			ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(СообщениеПоВертикали,, "КоличествоПоВертикали",, Отказ);
		КонецЕсли;
...
Процедура ВставитьВШаблонРисунокШтрихкода(ИмяТекущейОбласти, ИмяПоляВШаблоне)
	...
	РисунокШтрихкода.Имя = ИмяПоляВШаблоне + СтрЗаменить(Новый УникальныйИдентификатор, "-", "_");
...
Процедура УстановитьКартинкуШтрихкода(РисунокШтрихкода)
	...
	//Определяем, какой штрихкод выводится в макет
	Если Лев(РисунокШтрихкода.Имя, 8) = "Штрихкод" Тогда
		ТипКодаШаблон = 24; //GS1-DataMatrix
	Иначе
		ТипКодаШаблон = 1; //EAN13
	КонецЕсли;
	
	ПримерШтрихкода = Неопределено;
	Коды = ПолучитьКоды();
	Для Каждого СтрокаТЧ Из Коды Цикл
		Если СтрокаТЧ.ТипКода = ТипКодаШаблон Тогда
	...
	//По умолчанию картинка во всех шаблонах генерируется в формате GS1-DataMatrix для кода маркировки и ЕАН13 для обычных штрихкодов
	ПараметрыШтрихкода.Вставить("ТипКода", ТипКодаШаблон); 
...
Процедура ВыбратьДоступнуюСтрокуВШаблон(ВыбраннаяСтрока)
	...
	Если ИмяПоляВШаблоне = ИмяПараметраШтрихкод() Или ИмяПоляВШаблоне = ИмяПараметраШтрихАвтопроверки() 
		ИЛИ ИмяПоляВШаблоне = ИмяПараметраШтрихкодEAN() Тогда
