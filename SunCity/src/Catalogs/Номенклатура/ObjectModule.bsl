...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЭтоГруппа Тогда
		...
		Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.ПодарочныйСертификат Тогда
			...
			КонецЕсли;
			ПроверяемыеРеквизиты.Добавить("ТипСрокаДействия");
		КонецЕсли;
		
		Если ИспользоватьСерийныеНомера Тогда
			ПроверяемыеРеквизиты.Добавить("ТипСерийногоНомера");
		КонецЕсли;
		...
		Если ТипСрокаДействия = Перечисления.СрокДействияПодарочныхСертификатов.ПериодПослеПродажи Тогда
			...
			ПроверяемыеРеквизиты.Добавить("КоличествоПериодовДействия");
		КонецЕсли;
		...
		КонецЕсли;
		Если ЭтоАгентскаяУслуга = Истина Тогда
		...
		Если КиЗГИСМ Тогда
			ПроверяемыеРеквизиты.Добавить("КиЗГИСМВид");
			ПроверяемыеРеквизиты.Добавить("КиЗГИСМСпособВыпускаВОборот");
			ПроверяемыеРеквизиты.Добавить("КиЗГИСМGTIN");
			ПроверяемыеРеквизиты.Добавить("КиЗГИСМРазмер");
		Иначе
			Если ЗначениеЗаполнено(КиЗГИСМGTIN)
				...
				ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "КиЗГИСМGTIN", "Объект", Отказ);
			КонецЕсли;
		...
		Если Ложь И ПродукцияМаркируемаяДляГИСМ //Отключенный функционал
			И НЕ ИнтеграцияГИСМ.ПодсистемаНеИспользуется() Тогда
			ПроверяемыеРеквизиты.Добавить("КодТНВЭД");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ЭтоГруппа Тогда
		Если ТипНоменклатуры <> ВидНоменклатуры.ТипНоменклатуры Тогда
			...
			ОсобенностьУчета = ВидНоменклатуры.ОсобенностьУчета;
		КонецЕсли;
