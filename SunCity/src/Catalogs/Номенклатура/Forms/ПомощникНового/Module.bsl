...
// Оповещение поиска товара по штрихкоду.
//
// Параметры:
//  Штрихкод - Строка - прочитанный штрикход. 
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПриИзмененииВидаНоменклатуры(Не ЗначениеЗаполнено(Параметры.ЗначениеКопирования));
...
Процедура ВидНоменклатурыПараметрыИменованияПриИзменении(Элемент)
	...
	ОчиститьСообщения();
	ПараметрыИменованияПриИзмененииВидаНоменклатурыСервер();
	УстановитьДоступностьРеквизитовНаКлиенте();
...
Процедура Подключаемый_ПараметрИменованияПриИзменении(Элемент)
	...
	ОпределитьВозможностьПереходаНаСледующуюСтраницу(Истина); 
...
&НаКлиенте
Процедура ДекорацияКартинкаПримерНаименованияНажатие(Элемент)
...
Процедура ПриИзмененииВидаНоменклатуры(ОбновлятьПризнакиНоменклатуры = Истина)
	...
	ОчиститьПравилаИменованияНаФорме();
	ЗаполнениеПоПравилу();
	...
	Если ИспользоватьИменованиеНоменклатурыПоПравилу Тогда
		...
		Если РезультатПроверкиПравила.ЕстьОшибки Тогда
			Если ОбновлятьПризнакиНоменклатуры Тогда
				...
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,Объект.ВидНоменклатуры));
			КонецЕсли;	
		...
		ИначеЕсли РезультатПроверкиПравила.Свойство("ПравилоНельзяИспользоватьДляНабора") Тогда
			Если ОбновлятьПризнакиНоменклатуры Тогда
				...
				ОбщегоНазначения.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,Объект.ВидНоменклатуры.НаборСвойств));
			КонецЕсли;	
			ИспользоватьИменованиеНоменклатурыПоПравилу = Ложь;
		КонецЕсли;
	КонецЕсли;	
		...
		Если Команда.Имя = "Далее" Тогда
			...
			Иначе
				...
				Иначе
					...
					УбратьПроверкуШтрихкодовДляУслуг();
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли Команда.Имя = "Назад" Тогда
			Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
...
Процедура ЗаполнитьТаблицуПараметровПравилаИменования()
	...
	СтруктураПараметров = РаботаСПравиламиИменования.СтруктураПараметровПравилаИменования(
		Объект.ВидНоменклатуры,
		?(ЭтаФорма.Свойства_ИспользоватьСвойства, ЭтаФорма.Свойства_ОписаниеДополнительныхРеквизитов, Неопределено));
	...
	РаботаСПравиламиИменования.ДобавитьЭлементыПравилаИменования(
		ЭтаФорма, Элементы.ГруппаПараметрыИменованияПраво, Элементы.ГруппаПараметрыИменованияЛево);
...
Процедура СформироватьНаименование()
	...
	Наименование = РаботаСПравиламиИменованияКлиентСервер.ПреобразоватьСтрокуПоПравилу(
		ПравилоИменования.Правило, ПараметрыПравилаИменования, "ИмяПараметра", "ДанныеПараметраСтрокой");
...
&НаСервере
Процедура ПараметрыИменованияПриИзмененииВидаНоменклатурыСервер()
...
Процедура ПоискДублей()
	...
	СтруктураПоискаДублей = РаботаСПравиламиИменования.СтруктураПоискаНоменклатуры(ЭтаФорма);
	...
	Если СтруктураПоискаДублей.РеквизитыЗаполнены Тогда
		...
		НадписьИнформацияДублирующиеПозиции =
			НСтр("ru = 'Обнаружены карточки с одинаковым наименованием (" + КоличествоДублирующихПозиций + ")'");
...
&НаСервере
Процедура ОпределитьВозможностьПереходаНаСледующуюСтраницу(ФормироватьНаименование = Ложь)
...
Процедура ОбновитьДекорацииПараметровИменования()
	...
	Для Каждого Строка Из ЭтаФорма.Свойства_ОписаниеДополнительныхРеквизитов Цикл
		...
		Если Элементы.Найти(ИмяГруппы) = Неопределено Тогда
			...
			ДекорацияПараметрИменования =
				Элементы.Добавить("Декорация" + Строка.ИмяРеквизитаЗначение, Тип("ДекорацияФормы"), НоваяГруппа);
			...
			СтрокиПараметрыИменования = ПараметрыПравилаИменования.НайтиСтроки(
				Новый Структура("ПутьКДанным", Строка.ИмяРеквизитаЗначение));
...
Функция ПроверитьШтрихкодыНоменклатуры()
	...
	Для Каждого Строка Из ШтрихкодыНоменклатуры Цикл
		...
		Если Не ЗначениеЗаполнено(Строка.Штрихкод) Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,"ШтрихкодыНоменклатурыШтрихкод");
			Отказ = Истина;
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(Строка.ТипШтрихкода) Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,"ШтрихкодыНоменклатурыТипШтрихкода");
			Отказ = Истина;
		КонецЕсли;
...
&НаСервере
Процедура ОчиститьПравилаИменованияНаФорме()
	
	МассивЭлементовКУдалению = Новый Массив;
	МассивРеквизитовКУдалению = Новый Массив;
	
	Для Каждого Параметр Из ПараметрыПравилаИменования Цикл
		Если Параметр.ЭтоДопРеквизит Или Параметр.ЭтоДопСведение Тогда
			МассивРеквизитовКУдалению.Добавить(Параметр.ИмяЭлемента);
		КонецЕсли;
	КонецЦикла;
	ИзменитьРеквизиты(,МассивРеквизитовКУдалению);
	ПараметрыПравилаИменования.Очистить();
	
	Для Каждого ЭлементИменования Из Элементы.ГруппаПараметрыИменованияЛево.ПодчиненныеЭлементы Цикл
		МассивЭлементовКУдалению.Добавить(ЭлементИменования);
	КонецЦикла;
	Для Каждого ЭлементИменования Из Элементы.ГруппаПараметрыИменованияПраво.ПодчиненныеЭлементы Цикл
		МассивЭлементовКУдалению.Добавить(ЭлементИменования);
	КонецЦикла;
...
&НаКлиенте
Процедура УбратьПроверкуШтрихкодовДляУслуг()
	
	ЭтоУслуга = Объект.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга");
	Для Каждого ЭлементТаблицы Из Элементы.РегистрСведенийНаборЗаписей.ПодчиненныеЭлементы Цикл
		ЭлементТаблицы.АвтоОтметкаНезаполненного = Не ЭтоУслуга;
		ЭлементТаблицы.ОтметкаНезаполненного = Не ЭтоУслуга;
	КонецЦикла;
