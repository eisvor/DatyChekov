...
Процедура СозданиеШтрихкода_Подтверждение(РезультатВопроса, СтруктураПараметровКлиента) Экспорт
	...
	Если НЕ СтруктураПараметровКлиента.Свойство("Штрихкод") Тогда
		...
		СтруктураПараметровКлиента.Вставить("ТипШтрихкода", ПодключаемоеОборудованиеРТКлиент.ТипШтрихкода(СтруктураПараметровКлиента.ДанныеПО));
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ВЕТИС - Отключенный функционал
	Если Ложь И НЕ ЗначениеЗаполнено(Параметры.Ключ) Тогда
		ОсобенностьУчета = Неопределено;
		Если Параметры.Свойство("ОсобенностьУчета", ОсобенностьУчета)
			И (ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПодконтрольнаяПродукцияВЕТИС
			...
			Или ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.КонсервированнаяПродукцияПодконтрольнаяВЕТИС) Тогда
	...
	РаботаСНоменклатурой.ПриСозданииНаСервереФормаНоменклатуры(ЭтотОбъект, Объект.Ссылка, Элементы.ГруппаВертикальНаименований, ДополнительныеПараметры);
	// Конец ЭлектронноеВзаимодействие.РаботаСНоменклатурой
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_Файл" И Параметр.Свойство("ВладелецФайла") Тогда
		...
		Если Параметр.ВладелецФайла = Объект.Ссылка Тогда
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ИмяСобытия = "Запись_ДополнительныеСведения" И АвтоматическиФормироватьНаименование Тогда
	...
	ИначеЕсли ИмяСобытия = "Запись_ВидНоменклатуры" Тогда
		ЭтотОбъект.Прочитать();
	ИначеЕсли ИмяСобытия = "ГиперссылкаОткрытьФормуНастройкиПараметровНоменклатурыИС"
		И Источник = ЭтотОбъект Тогда
		
		ВидПродукции = ИнтеграцияИСРТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(Объект.ОсобенностьУчета);
		...
		Если ЗначениеЗаполнено(Объект.НаборУпаковок)
			И НЕ Объект.НаборУпаковок = ПредопределенноеЗначение("Справочник.НаборыУпаковок.ИндивидуальныйДляНоменклатуры") Тогда
			ПараметрыОбработкиДействия.НаборУпаковок = Объект.НаборУпаковок;
		КонецЕсли;
		ПараметрыОбработкиДействия.УпаковкиВключены = ИспользоватьУпаковкиНоменклатуры;
...
Процедура СтранаПроисхожденияПриИзменении(Элемент)
	...
	Иначе
		Объект.ИмпортнаяАлкогольнаяПродукция =
			Объект.ОсобенностьУчета = ПредопределенноеЗначение("Перечисление.ОсобенностиУчетаНоменклатуры.АлкогольнаяПродукция");
	КонецЕсли;
	ПриИзмененииПараметраИменования(Элемент.Имя,Истина);
...
Процедура Подключаемый_РазрешитьРедактированиеРеквизитовОбъекта(Команда)
	...
	Если Не Объект.Ссылка.Пустая() Тогда
		...
		Если ЗаблокированныеРеквизиты.Количество() > 0 Тогда
			...
			ИнтеграцияИСРТКлиентСервер.ЗаполнитьДанныеНастройкиПараметровНоменклатурыИС(ЭтотОбъект, Истина);
...
Процедура УстановитьДоступностьРеквизитовНаСервере()
	...
	ОбщегоНазначенияРТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Номинал", "Видимость", Не Объект.ПроизвольныйНоминал);
	...
	КонецЕсли;
	ОбщегоНазначенияРТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДоговорКонтрагента", "Видимость", ЭтоАгентскаяУслуга);
	...
	Если ЭтоТовар И ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойПродукцииИСМП") Тогда
		ВидПродукции = ИнтеграцияИС.ВидПродукцииПоНоменклатуре(Объект.Ссылка);
		ЭтоМаркируемаяПродукцияИСМП = ОбщегоНазначенияИСКлиентСервер.ЭтоПродукцияИСМП(ВидПродукции, Ложь);
	КонецЕсли;
	...
	Элементы.КодТНВЭД.Видимость = Объект.ПродукцияМаркируемаяДляГИСМ 
									ИЛИ ЭтоМаркируемаяПродукцияИСМП 
									ИЛИ ПолучитьФункциональнуюОпцию("ИспользоватьУчетПрослеживаемыхИмпортныхТоваров");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.ТипНоменклатуры) Тогда
		МассивТекстов.Добавить(Новый ФорматированнаяСтрока(Строка(Объект.ТипНоменклатуры), Новый Шрифт(,, Истина)));
	Иначе
		МассивТекстов.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = '<не указан>'"), Новый Шрифт(,, Истина), ЦветаСтиля.ЦветОсобогоТекста));
	КонецЕсли;
	...
	Если Объект.ОсобенностьУчета <> Перечисления.ОсобенностиУчетаНоменклатуры.БезОсобенностейУчета Тогда
		...
		Если Объект.ВидНоменклатуры.ПродаетсяВРозлив Тогда
...
Процедура ПриИзмененииВидаНоменклатуры(Ответ = "")
	...
	СобытияФормИС.ПриИзмененииЭлемента(ЭтотОбъект, "ВидНоменклатуры", Неопределено);
...
Процедура ПриИзмененииИспользоватьСерийныеНомера()
	...
	Элементы.СерийныеНомера.Доступность                   = Объект.ИспользоватьСерийныеНомера;
	Элементы.СерийныеНомераКоманднаяПанель.Доступность    = Объект.ИспользоватьСерийныеНомера;
	Элементы.ЧастичноеПогашение.Доступность               = Объект.ИспользоватьСерийныеНомера;
	Элементы.ТипНоминала.Доступность                      = Объект.ИспользоватьСерийныеНомера;
	...
	Иначе
		...
		ОбщегоНазначенияРТКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Номинал", "Видимость", Истина);
		ЭлементСпискаКУдалению = Элементы.ТипСрокаДействия.СписокВыбора.НайтиПоЗначению(Перечисления.СрокДействияПодарочныхСертификатов.ПериодПослеПродажи);
		Если ЭлементСпискаКУдалению <> Неопределено Тогда
...
Процедура ЗаписатьШтрикходыНоменклатуры(ТекущийОбъект)
	...
	ОбменСКонтрагентамиРТ.ЗаписатьШтрикходыНоменклатуры(ТекущийОбъект.Ссылка, Штрихкоды);
КонецПроцедуры
...
Процедура ПолучитьGLNПоИННКППГИСМ(Команда) Экспорт
	...
	Если Не ЗначениеЗаполнено(Объект.ИНН) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""ИНН""'"), Объект.Ссылка, "Объект.ИНН");
		Возврат;
	КонецЕсли;
	
	МодульИнтеграцияГИСМКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ИнтеграцияГИСМКлиент");
	
	Сообщения = Новый Массив;
	Сообщения.Добавить(ИнтеграцияГИСМВызовСервера.СообщениеЗапросаGLN(Объект.Ссылка, Объект.ИНН, Объект.КПП));
	СообщенияПоОрганизациям = МодульИнтеграцияГИСМКлиент.СообщенияПоОрганизациям(Сообщения);
	
	Данные = МодульИнтеграцияГИСМКлиент.СообщенияСледующейОрганизацииКПодписанию(СообщенияПоОрганизациям);
	Если Данные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Контекст = Новый Структура;
	МодульИнтеграцияГИСМКлиент.Подписать(
		Данные.Сообщения,
		Данные.Организация,
...
Процедура ЗаполнитьОбъектПоДаннымВЕТИС(Параметры)
	...
	// Находим нужный уид номенклатуры
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
...
Функция ПолучитьСтруктуруПараметровПравилаИменования()
	...
	СтруктураПараметров = РаботаСПравиламиИменования.СтруктураПараметровПравилаИменования(Объект.ВидНоменклатуры,?(ЭтаФорма.Свойства_ИспользоватьСвойства,ЭтаФорма.Свойства_ОписаниеДополнительныхРеквизитов,Неопределено));	
...
Функция СформироватьНаименование()
	...
	Если ЗначенияПараметровИменования.Количество() = 0 Тогда	
		РезультатЗапроса = РаботаСПравиламиИменования.ЗначенияПараметровНоменклатуры(Объект.ВидНоменклатуры,СтруктураПараметровПравилаИменования.ПараметрыПравилаИменования,Объект.Ссылка);
...
Процедура ВидимостьПереходаКСериям()
	...
	Если НЕ Элементы.Найти("ФормаОбщаяКомандаОткрытьСписокСерий") = Неопределено Тогда
		...
		Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			...
			"ИспользоватьСерии, ИспользованиеСерий");
		Если ЗначениеЗаполнено(Объект.Ссылка)
			И ЗначениеЗаполнено(Объект.ВидНоменклатуры)
			И Реквизиты.ИспользоватьСерии Тогда
...
&НаСервере
Процедура ВидимостьПрослеживаемыйТовар()
	
	ПрослеживаемыйТовар = Ложь; // Объект.КодТНВЭД.ПрослеживаемыйТовар = Истина; //Отключенный функционал
	Элементы.ОсобенностиУчетаНоменклатуры.Видимость = НЕ ПрослеживаемыйТовар;
	Элементы.ДекорацияПрослеживаемыйТовар.Видимость = ПрослеживаемыйТовар;
...
Процедура ВидимостьПереходаКХарактеристикам()
	...
	Если НЕ Элементы.Найти("ФормаСправочникХарактеристикиНоменклатурыОткрытьПоЗначению") = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(Объект.Ссылка)
			И ЗначениеЗаполнено(Объект.ВидНоменклатуры)
			И Реквизиты.ИспользоватьХарактеристики Тогда
...
Процедура КодТНВЭДПриИзмененииСервер()
	...
	Объект.ЕдиницаИзмеренияТНВЭД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.КодТНВЭД, "ЕдиницаИзмерения");
