...
Процедура ЗагрузитьШаблонПоУмолчаниюОбработкаВопроса(Ответ, ДополнительныеПараметры) Экспорт
	...
	Если ПараметрОбъектМетаданных = "Документ.ЧекККМ"
		ИЛИ ПараметрОбъектМетаданных = "Документ.ПриходныйКассовыйОрдер"
		ИЛИ ПараметрОбъектМетаданных = "Документ.РасходныйКассовыйОрдер" Тогда
...
Процедура ИмпортироватьШаблонИзФайлаОбработкаВопроса(Ответ, ДополнительныеПараметры) Экспорт
	...
	ОбработчикОповещения = Новый ОписаниеОповещения(
										"ИмпортироватьШаблонИзФайлаОбработкаВыбораФайла",
										ЭтотОбъект,
...
&НаКлиенте
Процедура ИмпортироватьШаблонИзФайлаОбработкаВыбораФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ИмпортироватьШаблонИзФайлаОбработкаПомещенияФайлов(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ЭкспортироватьШаблонВФайлОбработкаВыбораФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ДобавитьТаблицуЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ДобавитьПолеВШаблон("Таблица", Результат.Значение, СтруктураТаблицы);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИмяОбъектаМетаданных = "ЧекККМ" Тогда
		Элементы.ФормаЗагрузитьШаблонРасширенный.Видимость = Истина;
	Иначе
	...
	Если МенеджерОбъекта <> Неопределено Тогда
		...
		Если ПараметрВыбораДокументаДляШаблонаЧека <> Неопределено Тогда
			Элементы.ДокументСсылка.ПараметрыВыбора = ПараметрВыбораДокументаДляШаблонаЧека;
		КонецЕсли;
	...
	Иначе
		Попытка
			...
			СформироватьМакетФискальногоЧека();
		Исключение
			СтрокаОшибки = НСтр("ru = 'Не удалось открыть сохраненный шаблон по причине:'") + Символы.ПС + ОписаниеОшибки();
			ОчиститьШаблонСервер();
...
Процедура КомпоновщикНастроекНастройкиВыборДоступныеПоляВыбораВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ВыбраннаяСтрока <> Неопределено Тогда
		...
		ДобавитьПолеВШаблон("Текст", ИмяПараметра, СтруктураПараметра);
	КонецЕсли;
...
Процедура ЗагрузитьШаблонРасширенный(Команда)
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Текущий шаблон будет заменен расширенным примером шаблона?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ИмпортироватьШаблонИзФайла(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ИмпортироватьШаблонИзФайлаОбработкаВопроса", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ЭкспортироватьШаблонВФайл(Команда)
	...
	ДиалогВыбораФайла.Расширение = "txt";
	ДиалогВыбораФайла.Фильтр = "Шаблон чека ККТ(*.txt)|*.txt";
	
	ДополнительныеПараметры = Новый Структура;
	ОбработчикОповещения = Новый ОписаниеОповещения(
										"ЭкспортироватьШаблонВФайлОбработкаВыбораФайла",
										ЭтотОбъект,
										ДополнительныеПараметры);
	ДиалогВыбораФайла.Показать(ОбработчикОповещения);
...
Процедура ДобавитьТаблицу(Команда)
	...
	Если СписокДоступныхТаблицСКД.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'В схеме компоновки нет доступных пользовательских таблиц'"));
	ИначеЕсли СписокДоступныхТаблицСКД.Количество() = 1 Тогда
		ДобавитьТаблицуЗавершение(СписокДоступныхТаблицСКД[0], Неопределено);
	Иначе
		ОбработчикОповещения = Новый ОписаниеОповещения("ДобавитьТаблицуЗавершение", ЭтотОбъект);
		СписокДоступныхТаблицСКД.ПоказатьВыборЭлемента(ОбработчикОповещения, НСтр("ru = 'Список доступных таблиц'"));
	КонецЕсли;
...
Процедура ЗагрузитьШаблонПоУмолчаниюСервер()
	...
	СтрокаВерхнийУровень = ДобавитьПараметрВДерево(ШаблонЧека, "ФискальныйДокумент", НСтр("ru = 'Фискальный документ'"), СтруктураПараметраГруппы);
		СтрокаШапка = ДобавитьПараметрВДерево(СтрокаВерхнийУровень, "ПараметрыШапкиЧека", НСтр("ru = 'Шапка'"), СтруктураПараметраГруппы);
			ЗаполнитьРазделШапки(СтрокаШапка);
	
		СтрокаСоставЧека = ДобавитьПараметрВДерево(СтрокаВерхнийУровень, "СоставЧека", НСтр("ru = 'Состав чека'"), СтруктураПараметраГруппы);	
			СтрокаПозицииЧека = ДобавитьПараметрВДерево(СтрокаСоставЧека, "ТаблицаТовары", НСтр("ru = 'Товары'"), СтруктураТаблицы);
				СтрокаФискальная = ДобавитьПараметрВДерево(СтрокаПозицииЧека, "ФискальныйРаздел", НСтр("ru = 'Фискальный раздел'"), СтруктураСоставнойСтроки);
					ЗаполнитьПоляФискальногоРаздела(СтрокаФискальная);
	
		СтрокаОплаты = ДобавитьПараметрВДерево(СтрокаВерхнийУровень, "ТаблицаОплат", НСтр("ru = 'Оплата'"), СтруктураПараметраГруппы);
...
Процедура ЗагрузитьШаблонРасширенныйСервер()
	...
	ВозвращатьСКД = Истина;
	СКД = Документы[ИмяОбъектаМетаданных].ПолучитьМакет("ПоляШаблонаЧекККТРасширенный");
	АдресСКД = ПоместитьВоВременноеХранилище(СКД, Новый УникальныйИдентификатор);
	ИнициализироватьКомпоновщикНастроек(КомпоновщикНастроек, АдресСКД);
	...
	СтрокаВерхнийУровень = ДобавитьПараметрВДерево(ШаблонЧека, "ФискальныйДокумент", НСтр("ru = 'Фискальный документ'"), СтруктураПараметраГруппы);
		...
		СтрокаСоставЧека = ДобавитьПараметрВДерево(СтрокаВерхнийУровень, "СоставЧека", НСтр("ru = 'Состав чека'"), СтруктураПараметраГруппы);	
			СтруктураВычисляемойСтроки.Префикс  = НСтр("ru = 'Карта: '") + " ";
			СтрокаКарты = ДобавитьПараметрВДерево(СтрокаСоставЧека, "Текст", НСтр("ru = 'ДисконтнаяКарта'"), СтруктураВычисляемойСтроки);
			...
			СтрокаПозицииЧека = ДобавитьПараметрВДерево(СтрокаСоставЧека, "ТаблицаТовары", НСтр("ru = 'Товары'"), СтруктураТаблицы);
				...
				КонецЦикла;
				СтрокаРазделительСтроки = ДобавитьПараметрВДерево(СтрокаПозицииЧека, "Текст", ТекстРазделителя, СтруктураНеВычисляемойСтроки);
				
				СтруктураВычисляемойСтроки.Префикс = НСтр("ru = 'Строка №:'") + " ";
				СтруктураВычисляемойСтроки.Постфикс = "";
				СтрокаНомерСтроки = ДобавитьПараметрВДерево(СтрокаПозицииЧека, "Текст", НСтр("ru = 'НомерСтрокиТовара'"), СтруктураВычисляемойСтроки);
				...
				СтруктураВычисляемойСтроки.Постфикс = "";
				СтрокаСкидки = ДобавитьПараметрВДерево(СтрокаПозицииЧека, "Текст", НСтр("ru = 'СуммаСкидки'"), СтруктураВычисляемойСтроки);
				
				СтруктураВычисляемойСтроки.Префикс = НСтр("ru = 'Сумма без скидки по строке:'") + " ";
				СтруктураВычисляемойСтроки.Постфикс = "";
				СтрокаСуммаБезСкидки = ДобавитьПараметрВДерево(СтрокаПозицииЧека, "Текст", НСтр("ru = 'СуммаБезСкидки'"), СтруктураВычисляемойСтроки);
				
				СтрокаШтрихкода = ДобавитьПараметрВДерево(СтрокаПозицииЧека, "Текст", НСтр("ru = 'Штрихкод'"), СтруктураСтрокиШтрихкода);
	...
	СтрокаВерхнийУровень = ДобавитьПараметрВДерево(ШаблонЧека, "НефискальныйДокумент", НСтр("ru = 'Нефискальный документ'"), СтруктураПоля);
		СтрокаСоставЧека = ДобавитьПараметрВДерево(СтрокаВерхнийУровень, "СоставЧека", НСтр("ru = 'Состав чека'"), СтруктураПоля);	
			
			СтрокаПозицииЧека = ДобавитьПараметрВДерево(СтрокаСоставЧека, "Таблица", НСтр("ru = 'БонусныеБаллыКНачислению'"), СтруктураТаблицы);
				СоставнаяСтрока = ДобавитьПараметрВДерево(СтрокаПозицииЧека, "СоставнаяСтрока", НСтр("ru = 'СоставнаяСтрока'"), СтруктураСоставнойСтроки);
				
					СтруктураВычисляемойСтроки.Префикс = НСтр("ru = 'По программе'") + " ";
					СтруктураВычисляемойСтроки.Постфикс =  " " + НСтр("ru = 'начислено:'") + " ";
					СтрокаБонуса = ДобавитьПараметрВДерево(СоставнаяСтрока, "Текст", НСтр("ru = 'БонуснаяПрограммаЛояльности'"), СтруктураВычисляемойСтроки);
					
					СтруктураВычисляемойСтроки.Префикс = "";
					СтруктураВычисляемойСтроки.Постфикс = " " + НСтр("ru = 'баллов.'");
					СтрокаБонуса = ДобавитьПараметрВДерево(СоставнаяСтрока, "Текст", НСтр("ru = 'КоличествоБонусныхБаллов'"), СтруктураВычисляемойСтроки);
	
			СтруктураВычисляемойСтроки.Префикс = НСтр("ru = 'Всего доступно баллов: '") + " ";
			СтруктураВычисляемойСтроки.Постфикс = "";
			СтрокаКарты = ДобавитьПараметрВДерево(СтрокаСоставЧека, "Текст", НСтр("ru = '%%Скидки.НакопленоБаллов%%'"), СтруктураВычисляемойСтроки);
			
			СтруктураВычисляемойСтроки.Префикс = НСтр("ru = 'Накопленная сумма продаж: '") + " ";
			СтруктураВычисляемойСтроки.Постфикс = "";
			СтрокаКарты = ДобавитьПараметрВДерево(СтрокаСоставЧека, "Текст", НСтр("ru = '%%Скидки.СуммаНакопления%%'"), СтруктураВычисляемойСтроки);
			
	СтрокаВерхнийУровень = ДобавитьПараметрВДерево(ШаблонЧека, "НефискальныйДокумент", НСтр("ru = 'Нефискальный документ'"), СтруктураПоля);
		СтрокаСоставЧека = ДобавитьПараметрВДерево(СтрокаВерхнийУровень, "СоставЧека", НСтр("ru = 'Состав чека'"), СтруктураПоля);	
			
			СтруктураСтрокиШтрихкода.Префикс = НСтр("ru = 'Промокод:'");
			СтруктураСтрокиШтрихкода.Постфикс = "";
			СтрокаШтрихкода = ДобавитьПараметрВДерево(СтрокаСоставЧека, "Текст", НСтр("ru = 'ИдентификаторЧекаДляСкидки'"), СтруктураСтрокиШтрихкода);
			СтрокаШтрихкода.ТипШтрихкода = 16;
...
Процедура ИмпортироватьШаблонИзФайлаСервер(АдресФайла)
	...
	Попытка
		...
		СформироватьМакетФискальногоЧека();
	Исключение
		ТекстОшибки = НСтр("ru = 'Ошибка загрузки шаблона по причине:'") + Символы.ПС + ОписаниеОшибки();
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Функция ДобавитьПолеВШаблон(ИмяПараметра, ПредставлениеПараметра, СтруктураПараметра)
		...
		Иначе
			...
			Если ТекущаяСтрокаДерева <> Неопределено Тогда
				Если ТекущаяСтрокаДерева.ИмяКолонки = "НефискальныйДокумент"
					...
					ТекстСообщения = ТекстСообщения + НСтр("ru = 'Раздел ""%1"" закрыт для редактирования. Добавлять строки в него нельзя.'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ТекущаяСтрокаДерева.Элемент);
					МожноДобавлять = Ложь;
				ИначеЕсли ИмяПараметра = "Таблица" И ТекущаяСтрокаДерева.ИмяКолонки <> "СоставЧека" Тогда
					ТекстСообщения = ТекстСообщения + НСтр("ru = 'Таблицу можно добавлять только в состав чека.'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ТекущаяСтрокаДерева.Элемент);
					МожноДобавлять = Ложь;
				ИначеЕсли ИмяПараметра = "СоставнаяСтрока"
					...
					ТекстСообщения = ТекстСообщения + НСтр("ru = 'Составную строку можно добавлять только в состав чека или в таблицу.'");
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ТекущаяСтрокаДерева.Элемент);
					МожноДобавлять = Ложь;
				КонецЕсли;
	...
	Если МожноДобавлять Тогда
		...
		Если ИмяПараметра = "НефискальныйДокумент" Тогда
			Результат = ДобавитьПолеВШаблонСервер(Результат, "СоставЧека", "Состав чека", СтруктураПараметра);
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(, ТекстСообщения);
	КонецЕсли;
...
Процедура ЗаполнитьРазделШапки(СтрокаШапка)
	...
	ДобавитьПараметрВДерево(СтрокаШапка, "КодСистемыНалогообложения", НСтр("ru = 'Код системы налогообложения'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ОтправительEmail", НСтр("ru = 'E-mail отправителя'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ПокупательEmail", НСтр("ru = 'E-mail получателя'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ПокупательНомер", НСтр("ru = 'Телефон получателя'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ТелефонПлатежногоАгента", НСтр("ru = 'Телефон платежного агента'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ТелефонОператораПоПриемуПлатежей", НСтр("ru = 'Телефон оператора по приему платежей'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ТелефонОператораПеревода", НСтр("ru = 'Телефон оператора перевода'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ТелефонПоставщика", НСтр("ru = 'Телефон поставщика услуг'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "АдресОператораПеревода", НСтр("ru = 'Адрес оператора перевода'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ИННОператораПеревода", НСтр("ru = 'ИНН оператора перевода'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "НаименованиеОператораПеревода", НСтр("ru = 'Наименование оператора перевода'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ТелефонБанковскогоАгента", НСтр("ru = 'Телефон банковского агента'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ТелефонБанковскогоСубагента", НСтр("ru = 'Телефон банковского субагента'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ОперацияПлатежногоАгента", НСтр("ru = 'Операция платежного агента'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ОперацияБанковскогоАгента", НСтр("ru = 'Операция банковского агента'"), СтруктураПредопределенногоПоля);
	ДобавитьПараметрВДерево(СтрокаШапка, "ОперацияБанковскогоСубагента", НСтр("ru = 'Операция банковского субагента'"), СтруктураПредопределенногоПоля);
...
Процедура ЗаполнитьПоляФискальногоРаздела(СтрокаФискальная)
	...
	СтруктураФискальногоПоля.Префикс = НСтр("ru = 'В т.ч. скидка:'") + " ";
	СтруктураФискальногоПоля.Постфикс = Символ(182);
	ДобавитьПараметрВДерево(СтрокаФискальная, "СуммаСкидок", "СуммаСкидок", СтруктураФискальногоПоля);
	
	СтруктураФискальногоПоля.Префикс = НСтр("ru = 'Ставка НДС:'") + " ";
...
Процедура СформироватьМакетФискальногоЧека(НовоеЗначениеМодифицированностиФормы = Истина)
	...
	Попытка
		СКД = ПолучитьИзВременногоХранилища(АдресСКД);
		ТаблицаДанных = УправлениеШаблонами.ТаблицаСКД(СКД, ДокументСсылка);
		...
		Если ИмяОбъектаМетаданных = "ЧекККМ" Тогда
			...
			СтруктураДанных.ЧекККМСсылка = ДокументСсылка;
			ПредопределенныеДанныеЧека = РозничныеПродажиСервер.ПодготовитьДанныеДляПробитияЧека(СтруктураДанных);
		ИначеЕсли ИмяОбъектаМетаданных = "РасходныйКассовыйОрдер"
			ИЛИ ИмяОбъектаМетаданных = "ПриходныйКассовыйОрдер" Тогда
			...
			ПредопределенныеДанныеЧека = Документы[ИмяОбъектаМетаданных].ПодготовитьДанныеДляПробитияЧека(ДокументСсылка, РаспределениеВыручкиПоСекциям)
		КонецЕсли;
	...
	ЦветСерый = Новый Цвет(220, 220, 220);
	...
	Для Каждого СтрокаДерева Из ШаблонДерево.Строки Цикл
		Если СтрокаДерева.ИмяКолонки = "ФискальныйДокумент" Тогда
			...
			Для Каждого СтрокаЧека Из СтрокаДерева.Строки Цикл
				...
				ИначеЕсли СтрокаЧека.ИмяКолонки = "ТаблицаОплат" Тогда
					...
					Область.Параметры.Текст = НСтр("ru = 'ИТОГО:'") + " " + Формат(Итого, "ЧДЦ=2; ЧН=0,00");
					ВыведеннаяОбласть = ПолеТабличногоДокумента.Вывести(Область);
					ВыведеннаяОбласть.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
					// Потом детализируется по типам оплаты.
					Если Наличные <> 0 Тогда
						Область.Параметры.Текст = " " + НСтр("ru = 'Наличными:'") + " " + Формат(Наличные, "ЧДЦ=2; ЧН=0,00");
						ВыведеннаяОбласть = ПолеТабличногоДокумента.Вывести(Область);
						ВыведеннаяОбласть.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
					КонецЕсли;
					Если Электронный1 <> 0 Тогда
						Область.Параметры.Текст = " " + НСтр("ru = 'Электронно:'") + " " + Формат(Электронный1, "ЧДЦ=2; ЧН=0,00");
						ВыведеннаяОбласть = ПолеТабличногоДокумента.Вывести(Область);
						ВыведеннаяОбласть.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
					КонецЕсли;
					Если Электронный2 <> 0 Тогда
						Область.Параметры.Текст = " " + НСтр("ru = 'Постоплата:'") + " " + Формат(Электронный2, "ЧДЦ=2; ЧН=0,00");
						ВыведеннаяОбласть = ПолеТабличногоДокумента.Вывести(Область);
						ВыведеннаяОбласть.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
					КонецЕсли;
					Если Электронный3 <> 0 Тогда
						Область.Параметры.Текст = " " + НСтр("ru = 'Предоплата:'") + " " + Формат(Электронный3, "ЧДЦ=2; ЧН=0,00");
						ВыведеннаяОбласть = ПолеТабличногоДокумента.Вывести(Область);
						ВыведеннаяОбласть.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
					КонецЕсли;
					Если Электронный4 <> 0 Тогда
						Область.Параметры.Текст = " " + НСтр("ru = 'Встречное предоставление:'") + " " + Формат(Электронный4, "ЧДЦ=2; ЧН=0,00");
						ВыведеннаяОбласть = ПолеТабличногоДокумента.Вывести(Область);
						ВыведеннаяОбласть.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
					КонецЕсли;
					Если Сдача > 0 Тогда
						Область.Параметры.Текст = НСтр("ru = 'Сдача:'") + " " + Формат(Сдача, "ЧДЦ=2; ЧН=0,00");
						ВыведеннаяОбласть = ПолеТабличногоДокумента.Вывести(Область);
						ВыведеннаяОбласть.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Переносить;
					КонецЕсли;
		...
		ИначеЕсли СтрокаДерева.ИмяКолонки = "Отрезка" Тогда
			...
			Если ШиринаЧека > 14 Тогда
				...
				КонецЦикла;
				ТекстОтрезки = ТекстОтрезки + НСтр("ru = 'Линия отреза'");
				Для Счетчик = 1 По ПоловинаДлины Цикл
					ТекстОтрезки = ТекстОтрезки + "_";
				КонецЦикла;
			Иначе
...
&НаСервере
Процедура ОбработатьСоставЧека(СтрокаОсновногоРаздела, Область, ОбластьШтрихкод, ПредопределенныеДанныеЧека, ТаблицаДанных)
			...
			Для Каждого ПоляСтрокиТаблицы Из СтрокаОсновногоРаздела.Строки Цикл
				...
				ИначеЕсли ПоляСтрокиТаблицы.ИмяКолонки = "Текст" Тогда
					Если ПоляСтрокиТаблицы.ВыводитьКакШтрихкод Тогда
						ОбработатьДанныеШтрихкода(ОбластьШтрихкод, Область, ПоляСтрокиТаблицы, СтрокаШапкиЧека, ТаблицаДанных, СтрокаВыборки, ИмяТаблицы);
					Иначе
		...
		Пока ИндексТекущейПозиции <= ИндексПоследнейПозиции Цикл
			...
			Для Каждого ПоляСтрокиТаблицы Из СтрокаОсновногоРаздела.Строки Цикл
				Если ПоляСтрокиТаблицы.ИмяКолонки = "ФискальныйРаздел" Тогда
					...
					ПредопределенныеДанныеЧека.ФискальнаяСуммаЧека = ПредопределенныеДанныеЧека.ФискальнаяСуммаЧека + СтрокаВыборки.Сумма;
					ОбработатьСоставнуюСтроку(Область, ПоляСтрокиТаблицы, СтрокаШапкиЧека, ТаблицаДанных, СтрокаВыборки, ИмяТаблицы);
				...
				ИначеЕсли ПоляСтрокиТаблицы.ИмяКолонки = "Текст" Тогда
					...
					КонецЕсли;
					Если ПоляСтрокиТаблицы.ВыводитьКакШтрихкод Тогда
						ОбработатьДанныеШтрихкода(ОбластьШтрихкод, Область, ПоляСтрокиТаблицы, СтрокаШапкиЧека, ТаблицаДанных, СтрокаДанных, ИмяТаблицы);
					Иначе
						ОбработатьТекстовуюСтроку(Область, ПоляСтрокиТаблицы, СтрокаШапкиЧека, ТаблицаДанных, СтрокаДанных, ИмяТаблицы);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
...
&НаСервере
Процедура ОбработатьТекстовуюСтроку(Область, ПоляСтроки, СтрокаШапкиЧека, ТаблицаДанных, СтрокаДанных = Неопределено, ИмяРаздела = "СоставЧека")
	
	ЗначениеСтроки = УправлениеШаблонами.ЗначениеТекстовогоПоля(ДокументСсылка, ПоляСтроки, СтрокаШапкиЧека, ТаблицаДанных, СтрокаДанных, ИмяРаздела);
...
&НаСервере
Процедура ОбработатьДанныеШтрихкода(ОбластьШтрихкода, ОбластьДополнительныхПолей, ПоляСтроки, СтрокаШапкиЧека, ТаблицаДанных, СтрокаДанных = Неопределено, ИмяРаздела = "СоставЧека")
...
&НаСервере
Процедура ОбработатьСоставнуюСтроку(Область, ПоляСтроки, СтрокаШапкиЧека, ТаблицаДанных, СтрокаДанных = Неопределено, ИмяРаздела = "СоставЧека")
	
	ТекстСтроки = "";
	Для Каждого ПараметрСтроки Из ПоляСтроки.Строки Цикл
		Если ЗначениеЗаполнено(ПараметрСтроки.ИмяКолонки) Тогда
			ЗначениеПоля = УправлениеШаблонами.ЗначениеТекстовогоПоля(ДокументСсылка, ПараметрСтроки, СтрокаШапкиЧека, ТаблицаДанных, СтрокаДанных, ИмяРаздела);
...
Функция ЗаполнитьИВывестиОбласть(Область, ЗначениеПараметра, ДанныеШаблона)
	...
	Область.Параметры.Текст = ДанныеШаблона.Префикс
								+ Формат(ЗначениеПараметра, ДанныеШаблона.Формат)
								+ ДанныеШаблона.Постфикс;
	...
	КонецЕсли;
	Если ДанныеШаблона.РазмещениеТекста = 1 Тогда
...
Функция ВывестиОтрезку(Область)
	...
	ВыведеннаяОбласть.РазмещениеТекста = ТипРазмещенияТекстаТабличногоДокумента.Обрезать;
	ВыведеннаяОбласть.ГоризонтальноеПоложение = ГоризонтальноеПоложение.Центр;
	ВыведеннаяОбласть.ЦветФона = Новый Цвет(220, 220, 220);
...
Процедура ИнициализироватьКомпоновщикНастроек(КомпоновщикНастроек, АдресСхемыКомпоновкиДанных, АдресНастроекКомпоновкиДанных = Неопределено)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Не удалось инициализировать компоновщик настроек по причине:'") + Символы.ПС + ОписаниеОшибки();
