...
&НаКлиенте
Процедура ОповещениеЗаполнитьШаблонЦенникаАвтопроверкиПоУмолчанию(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		ПоместитьВТабличныйДокументШаблонПоУмолчанию("ШаблонЦенникаАвтопроверкиПоУмолчанию");
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипШаблона = Перечисления.ТипыШаблонов.ДисконтнаяКарта Тогда
		
		Если СхемаКомпоновкиДанных = Неопределено Тогда
			СхемаКомпоновкиДанных = Обработки.ПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаДисконтнаяКарта");
		КонецЕсли;
		
		ЭтаФорма.Заголовок = НСтр("ru = 'Редактирование шаблона этикетки для дисконтной карты'");
		...
		Элементы.ФормаЗагрузитьШаблонДисконтнойКартыПоУмолчанию.Видимость = Истина;
		Элементы.ФормаЗагрузитьШаблонРегистрационнойКартыПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонИСМППоУмолчанию.Видимость = Ложь;
		
	ИначеЕсли ТипШаблона = Перечисления.ТипыШаблонов.РегистрационнаяКарта Тогда
		
		Если СхемаКомпоновкиДанных = Неопределено Тогда
			СхемаКомпоновкиДанных = Обработки.ПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаРегистрационнаяКарта");
		КонецЕсли;
		
		ЭтаФорма.Заголовок = НСтр("ru = 'Редактирование шаблона этикетки для регистрационной карты'");
		Элементы.ГруппаШаблоныПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонПодарочногоСертификатаПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонДисконтнойКартыПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонРегистрационнойКартыПоУмолчанию.Видимость = Истина;
		Элементы.ФормаЗагрузитьШаблонИСМППоУмолчанию.Видимость = Ложь;
		
	ИначеЕсли ТипШаблона = Перечисления.ТипыШаблонов.ПодарочныйСертификат Тогда
		
		Если СхемаКомпоновкиДанных = Неопределено Тогда
			СхемаКомпоновкиДанных = Обработки.ПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблонаПодарочныйСертификат");
		КонецЕсли;
		
		ЭтаФорма.Заголовок = НСтр("ru = 'Редактирование шаблона этикетки для подарочного сертификата'");
		Элементы.ГруппаШаблоныПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонПодарочногоСертификатаПоУмолчанию.Видимость = Истина;
		Элементы.ФормаЗагрузитьШаблонДисконтнойКартыПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонРегистрационнойКартыПоУмолчанию.Видимость = Ложь;
		Элементы.ФормаЗагрузитьШаблонИСМППоУмолчанию.Видимость = Ложь;

	ИначеЕсли ТипШаблона = Перечисления.ТипыШаблонов.ЭтикеткаКодМаркировкиИСМП Тогда
	...
	Если СохранятьИзменения Тогда
		...
		Если РазрешитьАвтовысотуСтрок ИЛИ НЕ НаличиеСтрокСАвтовысотой Тогда
			...
			Иначе
				...
				ОбработчикОповещения = Новый ОписаниеОповещения(
					"ОповещениеВопросаОНеСоответствииКоличества",
					ЭтотОбъект,
					ДополнительныеПараметры);
...
&НаКлиенте
Процедура ЗаполнитьШаблонЦенникаАвтопроверкиПоУмолчанию(Команда)
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗаполнитьШаблонЦенникаАвтопроверкиПоУмолчанию", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Редактируемый шаблон будет заменен на шаблон по умолчанию, продолжить?'"), РежимДиалогаВопрос.ДаНет);
...
Процедура ОткрытьНастройкиСхемыКомпоновкиДанных(Команда)
	...
	ПараметрыОткрытия.Вставить("АдресСхемыКомпоновкиДанных", АдресВХранилище);
	ПараметрыОткрытия.Вставить("ИсточникШаблонов", Ссылка);
...
Процедура ВыбратьДоступнуюСтрокуВШаблон(ВыбраннаяСтрока)
	...
	Если ИмяПоляВШаблоне = ИмяПараметраШтрихкод() Или ИмяПоляВШаблоне = ИмяПараметраШтрихкодСправкиБ() Или ИмяПоляВШаблоне = ИмяПараметраШтрихАвтопроверки() Тогда
			ДополнительныеПараметры = Новый Структура;
