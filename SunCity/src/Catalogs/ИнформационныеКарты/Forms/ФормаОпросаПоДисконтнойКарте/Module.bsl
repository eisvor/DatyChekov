...
&НаКлиенте
Процедура ПриЗакрытииСогласияНаОбработкуПерсональныхДанных(Результат, ДополнительныеПараметры) Экспорт
	
	ПолучитьСогласиеНаОбработкуПерсональныхДанных();
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьВладельцаКартыИТаблицуВопросовНаСервере(Отказ);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьДисконтнойКарты" И Источник = Карта Тогда
		ЗаполнитьВладельцаКартыИТаблицуВопросовНаСервере();
		ОбновитьВопрос();
		УправлениеИнтерфейсом();
	КонецЕсли;
...
Процедура СледующийРаздел(Команда)
	...
	Если НомерВопроса < ТаблицаВопросовИОтветов.Количество() Тогда
		...
		УправлениеИнтерфейсом();
	Иначе
		Ошибка = ЗаписатьДанныеОпроса();
		Если ЗначениеЗаполнено(Ошибка) Тогда
			ПоказатьПредупреждение(,Ошибка);
			Возврат;
		КонецЕсли;
		
		Если ТребуетсяСогласиеНаОбработкуПерсональныхДанных Тогда
			УправлениеИнтерфейсом();
			ОповещениеПриЗакрытии = Новый ОписаниеОповещения("ПриЗакрытииСогласияНаОбработкуПерсональныхДанных", ЭтотОбъект);
			ОткрытьФорму("Документ.СогласиеНаОбработкуПерсональныхДанных.ФормаОбъекта", Новый Структура("Субъект", ВладелецКарты),ЭтотОбъект,,,,ОповещениеПриЗакрытии);
		Иначе
			Закрыть();
...
&НаСервере
Процедура ЗаполнитьВладельцаКартыИТаблицуВопросовНаСервере(Отказ = Ложь)
	
	ВладелецКартыЗначение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Карта, "ВладелецКарты");
	Если Не ТипЗнч(ВладелецКартыЗначение) = Тип("СправочникСсылка.Контрагенты") Тогда
		ВладелецКарты = ВладелецКартыЗначение;
		ПолучитьСогласиеНаОбработкуПерсональныхДанных();
	Иначе
	...
	Для Каждого СтрокаТаблицыВопросов Из СтрокиТаблицы Цикл
		...
		Иначе
			...
			Если СтрокаТаблицыВопросов.Вопрос = НСтр("ru = 'Фамилия, Имя, Отчество'") Тогда
				...
				Иначе
					Возврат НСтр("ru = 'Необходимо заполнить ФИО покупателя'");
				КонецЕсли;
...
&НаСервере
Процедура ПолучитьСогласиеНаОбработкуПерсональныхДанных()
	
	Если ЗначениеЗаполнено(СогласиеНаОбработкуПерсональныхДанных) 
		И (СогласиеНаОбработкуПерсональныхДанных.Субъект) = ВладелецКарты Тогда
		ТребуетсяСогласиеНаОбработкуПерсональныхДанных = Ложь;
		Возврат;
	КонецЕсли;
	
	ИспользоватьСкрытиеПерсональныхДанных = ПолучитьФункциональнуюОпцию("ИспользоватьСкрытиеПерсональныхДанныхСубъектов");
	Если ИспользоватьСкрытиеПерсональныхДанных Тогда
		СведенияОСогласии = ЗащитаПерсональныхДанных.ДействующееСогласиеНаОбработкуПерсональныхДанных(ВладелецКарты);
		Если ЗначениеЗаполнено(СведенияОСогласии) Тогда
			СогласиеНаОбработкуПерсональныхДанных = СведенияОСогласии.ДокументОснование;
		Иначе
			СогласиеНаОбработкуПерсональныхДанных = Неопределено;
			ТребуетсяСогласиеНаОбработкуПерсональныхДанных = Истина;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
