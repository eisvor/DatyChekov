...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДополнениеЗаголовка = "";
	ИспользуютсяАвтоматическиеСкидки = ПолучитьФункциональнуюОпцию("ИспользоватьАвтоматическиеСкидкиВПродажах");
	Если Объект.Ссылка.Пустая() Тогда
		Если НЕ (Параметры.Свойство("ЗначенияЗаполнения") И Параметры.ЗначенияЗаполнения.Свойство("ТипКарты")) Тогда
			Если ИспользуютсяАвтоматическиеСкидки Тогда
				Объект.ТипКарты = Перечисления.ТипыИнформационныхКарт.Дисконтная;
			Иначе
				Объект.ТипКарты = Перечисления.ТипыИнформационныхКарт.Регистрационная;
			КонецЕсли;
		КонецЕсли;
		
		ДополнениеЗаголовка = " " + НСтр("ru = '(создание)'");
	Иначе
		Если НЕ Объект.ВидКарты = Перечисления.ВидыИнформационныхКарт.Магнитная Тогда
			
			ДанныеШтрихКода = ПодключаемоеОборудованиеРТ.ШтрихкодТовара(Объект.Ссылка);
	...
	СкрытиеПерсональныхДанныхСубъектов = ПолучитьФункциональнуюОпцию("ИспользоватьСкрытиеПерсональныхДанныхСубъектов");
	ЭтоДисконтнаяКарта = Объект.ТипКарты = Перечисления.ТипыИнформационныхКарт.Дисконтная;
	
	Заголовок = ?(ЭтоДисконтнаяКарта,
		НСтр("ru = 'Дисконтная карта'"), НСтр("ru = 'Регистрационная карта'")) + ДополнениеЗаголовка;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("ЗаписьДисконтнойКарты", Неопределено, Объект.Ссылка);
...
Процедура ПриИзмененииВидаКарты(Элемент)
	...
	Иначе
		Если Элемент.Имя = "ФлагКодКарты" Тогда
			ФлагШтрихКод = Истина;
			Флаг = Элементы.ФлагШтрихКод;
		Иначе
			ФлагКодКарты = Истина;
			Флаг = Элементы.ФлагКодКарты;
		КонецЕсли;
		ПриИзмененииВидаКарты(Флаг);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступностьСервер()
	...
	Элементы.КодКарты.АвтоОтметкаНезаполненного = ФлагКодКарты;
	
	Элементы.ШтрихкодКарты.ТолькоПросмотр = НЕ ФлагШтрихКод;
	Элементы.ШтрихкодКарты.АвтоОтметкаНезаполненного = ФлагШтрихКод;
	Элементы.ТипШтрихкодаКарты.ТолькоПросмотр = НЕ ФлагШтрихКод;
	Элементы.ТипШтрихкодаКарты.АвтоОтметкаНезаполненного = ФлагШтрихКод;
...
Процедура ОбновитьКоды(СтруктураПараметровКлиента)
	...
	Если СтруктураПараметровКлиента.ТипДанныхПО = "Штрихкод" Тогда
		...
		ТипШтрихкодаКарты = ПодключаемоеОборудованиеРТКлиент.ТипШтрихкода(ШтрихкодКарты);
	Иначе
		ФлагКодКарты = Истина;
...
Процедура СформироватьСписокАвтоНаименованийКартыКлиент()
	...
	Если Не СкрытиеПерсональныхДанныхСубъектов Тогда
...
Процедура КартаДляНакопленийПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.КартаДляНакоплений) Тогда
		...
		Если Объект.БонуснаяПрограммаЛояльности <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.КартаДляНакоплений, "БонуснаяПрограммаЛояльности") Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(СтрокаОшибки, , "КартаДляНакоплений", "Объект");
