...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если СчетчикПолей = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не добавлено ни одного поля ни в одной из доступных дорожек.'"), , , , Отказ);
	КонецЕсли;
...
Процедура ВзятьПрефикс1(Команда)
	...
	Иначе
		ОчиститьСообщения();
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Выделите мышкой участок кода.'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ОпределитьКоординатыПоля(Оповещение, ДанныеДорожки, Префикс, Суффикс, Разделитель, НКол, ККол)
	...
	Если НЕ ПустаяСтрока(Префикс)
		...
		Если НКол < 1 Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Выделенный участок кода не должен пересекаться с суффиксом, префиксом или разделителем блоков.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
				...
				Для каждого текПоле Из Объект["ПоляДорожки"+Строка(ВремИндекс)] Цикл
					Если текПоле.Поле = Поле Тогда
						...
						Если СчетчикДублей > 1 Тогда
							...
							ОбщегоНазначенияКлиент.СообщитьПользователю(СтрСообщение
...
Процедура ПроверитьШаблонЗавершение(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если НЕ ЭтотОбъект.Записать() Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не удалось записать шаблон'"));
			Возврат;
		КонецЕсли;
