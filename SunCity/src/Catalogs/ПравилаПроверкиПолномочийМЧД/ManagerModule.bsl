...
Процедура ЗаписатьПравило(Доверенность, Форма) Экспорт
	...
	ВариантЗаполненияПолномочий = 
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Доверенность, "ВариантЗаполненияПолномочий");
		
	ПолномочияОграничены = Не МашиночитаемыеДоверенности.ЭтоМЧДСПолнымиПолномочиями(Доверенность);
		
	Если Не ПолномочияОграничены 
		ИЛИ ВариантЗаполненияПолномочий = Перечисления.ВариантыЗаполненияПолномочийМЧД.КлассификаторФНС Тогда
		Возврат;
	КонецЕсли;
