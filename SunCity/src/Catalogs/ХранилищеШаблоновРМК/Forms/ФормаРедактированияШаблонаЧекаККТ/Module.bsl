...
&НаКлиенте
// АПК:399-выкл. обработчик оповещения
Процедура ОповещениеВыбораШаблонаЧека(ОтветФормы, ДополнительныеПараметры) Экспорт // АПК:78 - обработчик оповещения
// АПК:399-вкл.
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ОбрабатыватьОбъектВФормеРедактирования", ОбрабатыватьОбъектВФормеРедактирования)
		И ОбрабатыватьОбъектВФормеРедактирования Тогда
		
		Если Параметры.Свойство("Ключ") И НЕ Параметры.Ключ.Пустая() Тогда
			ХранилищеШаблоновРМКОбъект = Параметры.Ключ.ПолучитьОбъект();
		ИначеЕсли Параметры.Свойство("ЗначениеКопирования") И НЕ Параметры.ЗначениеКопирования.Пустая() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			...
			Ложь);
	Иначе
		Если Параметры.Свойство("АдресШаблона") И НЕ ПустаяСтрока(Параметры.АдресШаблона) Тогда
		...
		КонецЕсли;
		Если Параметры.Свойство("Ссылка") И НЕ Параметры.Ссылка.Пустая() Тогда
	...
	ИспользоватьПромокодыСуществует = Метаданные.ФункциональныеОпции.Найти("ИспользоватьПромокоды") <> Неопределено;
	Если ИспользоватьПромокодыСуществует Тогда
...
Процедура ДобавитьНефискальныйДокумент(Команда)
	...
	ДобавитьПолеВШаблон("НефискальныйДокумент", НСтр("ru = 'Нефискальный документ'",
		ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), СтруктураПоля);
...
Функция ДобавитьПолеВШаблон(ИмяПараметра, ПредставлениеПараметра, СтруктураПараметра)
	...
	Если МожноДобавлять Тогда
		...
		Если ИмяПараметра = "НефискальныйДокумент" Тогда
			...
			Результат = ДобавитьПолеВШаблонСервер(Результат, "СоставЧека", НСтр("ru = 'Состав чека'"),
				СтруктураПараметра);
				
			Если ИспользоватьПромокоды Тогда
				РезультатПромокоды = ДобавитьПолеВШаблонСервер(Результат,
					"Промокоды", НСтр("ru = 'Промокоды к выдаче'"), СтруктураПараметра);
				...
				РезультатПромокодыКВыдаче = ДобавитьПолеВШаблонСервер(РезультатПромокоды,
					"Таблица", "ПромокодыКВыдаче", СтруктураТаблицыПромокодов);
				...
				РезультатПромокод = ДобавитьПолеВШаблонСервер(РезультатПромокодыКВыдаче,
					"Текст", НСтр("ru = 'ПромокодЗначениеПромокода'"), СтруктураПоляПромокод);
			КонецЕсли;
		КонецЕсли;
