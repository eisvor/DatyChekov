...
//
// Параметры: 
//  Результат - Строка - результат вопроса.
//  Параметры - Структура - структура дополнительных параметров.
//
&НаКлиенте
Процедура ВопросИзменитьРеквизитБанка(Результат, Параметры) Экспорт

	Если Результат = "ОтменитьВвод" Тогда
		БИКБанка = "";
	...
	ИначеЕсли Результат = "ВыбратьИзСписка" Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АвтоНаименованиеБанковскогоСчета();
...
Процедура ОбработкаВыбора(РезультатВыбора, ИсточникВыбора)
	...
	Если (ИсточникВыбора.ИмяФормы = "Справочник.УдалитьБанковскиеСчетаОрганизаций.Форма.РеквизитыБанка") Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПустаяСтрока(Объект.Наименование) Тогда
		Объект.Наименование = АвтоНаименованиеБанковскогоСчета();
	КонецЕсли;
КонецПроцедуры
...
Процедура РеквизитБанкаОткрытие(ИмяЭлемента)
    ...
    // &ЗамерПроизводительности
	ОценкаПроизводительностиРТКлиент.НачатьЗамер(
		     Истина, "Справочник.УдалитьБанковскиеСчетаОрганизаций.Форма.РеквизитыБанка.Открытие");
	...
	ОткрытьФорму("Справочник.УдалитьБанковскиеСчетаОрганизаций.Форма.РеквизитыБанка",СтруктураПараметров, ЭтаФорма);
	...
	Если (ИмяЭлемента = "БИКБанка") Тогда
		Если НЕ Объект.РучноеИзменениеРеквизитовБанка Тогда
			Если НЕ ЗаполнитьРеквизитыБанкаПоБИК("Банк", БИКБанка, Истина) Тогда
				...
				Оповещение = Новый ОписаниеОповещения("ВопросИзменитьРеквизитБанка", ЭтотОбъект);
