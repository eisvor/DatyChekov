...
// Оповещение очистки таблицы кнопок.
//
// Параметры:
...
// Оповещение результат открытия формы конструктора запроса.
//
// Параметры:
...
// Результат ответа на вопрос записи при открытии палитры.
//
// Параметры:
...
// Результат ответа на вопрос записи при выборе палитры.
//
// Параметры:
//  Результат - КодВозвратаДиалога - результат ответа на вопрос.
//  ДополнительныеПараметры - Структура - структура дополнительных параметров.
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПриИзмененииИспользоватьРасширеннуюНастройкуКнопокНижнейПанели(ЭтаФорма);
...
Процедура ИспользоватьРасширеннуюНастройкуКнопокНижнейПанелиПриИзменении(Элемент)
	...
	Если НЕ Объект.ИспользоватьРасширеннуюНастройкуКнопокНижнейПанели Тогда
		Объект.МаксимальноеКоличествоКнопокВОдномРяду = 0;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьВерхнююПанель(Команда)
	ЗаполнитьВерхнююПанельНаСервере();
КонецПроцедуры
...
Процедура ЗаполнитьТаблицуКнопокИзМакета()
	...
	Для НомерСтрокиМакета = 2 По КолСтрок Цикл
		...
		//ИнтеграцияИСМПТК
		//Вывод команды Коды маркировки - только если включен учет маркированной продукции в базе
		УчетМаркировкиИСМПТК = Константы.ВестиУчетМаркируемойПродукцииИСМПТК.Получить();
		ИмяКомандыИзМакета = Макет.Область(НомерСтрокиМакета,2,НомерСтрокиМакета,2).Текст;
		Если Не УчетМаркировкиИСМПТК И ИмяКомандыИзМакета = "КомандаПО99КодыМаркировки" Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьТекстЗапросаТабло()
	...
	Если ТаблицаВыбранныхРеквизитов.Количество() > 0 Тогда
		...
		Для каждого СтрокаТаблицы Из ТаблицаВыбранныхРеквизитов Цикл
			...
			Текст = Текст + Разделитель + Символы.ПС + НСтр("ru = '	Номенклатура.'") + СтрокаТаблицы.Реквизит + " КАК Поле" + Формат(НомерСтроки, "ЧЦ=2; ЧВН=");
			Разделитель = ",";
		КонецЦикла;
	...
	Для Каждого Строка Из Объект.КнопкиНижнейПанели Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(Нстр("ru='Преобразование настроек РМК'"),
				УровеньЖурналаРегистрации.Предупреждение,,, Нстр("ru='Не удалось заполнить оформление настройки'") + Строка.ЗаголовокКнопки);
		КонецПопытки;
	КонецЦикла;
...
Процедура ПриИзмененииИспользоватьРасширеннуюНастройкуКнопокНижнейПанели(Форма)
	
	Форма.Элементы.ГруппаФункциональныеКнопкиНижнейПанели.Доступность =
		Форма.Объект.ИспользоватьРасширеннуюНастройкуКнопокНижнейПанели;
		
	Форма.Элементы.МаксимальноеКоличествоКнопокВОдномРяду.Доступность =
		Форма.Объект.ИспользоватьРасширеннуюНастройкуКнопокНижнейПанели;
	Форма.Элементы.МаксимальноеКоличествоКнопокВОдномРяду.АвтоОтметкаНезаполненного =
		Форма.Объект.ИспользоватьРасширеннуюНастройкуКнопокНижнейПанели;
	Форма.Элементы.МаксимальноеКоличествоКнопокВОдномРяду.ОтметкаНезаполненного =
		Форма.Объект.ИспользоватьРасширеннуюНастройкуКнопокНижнейПанели;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьВерхнююПанельНаСервере()
	
	Объект.КнопкиВерхнейПанели.Очистить();
	СформироватьИменаКнопокВерхнейПанели();
