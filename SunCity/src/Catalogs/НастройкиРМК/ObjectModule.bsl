...
Процедура ОбновитьКнопкиВерхнейПанели(ДанныеИзменились = Ложь) Экспорт
	
	Макет 	 = Справочники.НастройкиРМК.ПолучитьМакет("СтандартныеДействияКнопокВерхнейПанели");
	КолСтрок = Макет.ВысотаТаблицы;
	
	Для НомерСтрокиМакета = -КолСтрок По -2 Цикл
		
		НоваяКнопка = Новый Структура("ИмяКоманды, ИмяКнопки, ЗаголовокКнопки, ДоступнаКнопка, ДоступнаКоманда, Шрифт, Цвет");
		НоваяКнопка.ИмяКоманды          = Макет.Область(-НомерСтрокиМакета,1,-НомерСтрокиМакета,1).Текст;	
		НоваяКнопка.ИмяКнопки           = Макет.Область(-НомерСтрокиМакета,2,-НомерСтрокиМакета,2).Текст;
		НоваяКнопка.ЗаголовокКнопки     = Макет.Область(-НомерСтрокиМакета,3,-НомерСтрокиМакета,3).Текст;
		НоваяКнопка.ДоступнаКнопка      = Булево(Макет.Область(-НомерСтрокиМакета,4,-НомерСтрокиМакета,4).Текст);
		НоваяКнопка.ДоступнаКоманда     = Булево(Макет.Область(-НомерСтрокиМакета,5,-НомерСтрокиМакета,5).Текст);
		
		НоваяКнопка.Шрифт 				= ОбщегоНазначения.ЗначениеВСтрокуXML(WindowsШрифты.ШрифтДиалоговИМеню);
		НоваяКнопка.Цвет 				= ОбщегоНазначения.ЗначениеВСтрокуXML(Новый Цвет(0,0,0));
		
		Если КнопкиВерхнейПанели.Найти(НоваяКнопка.ИмяКоманды, "ИмяКоманды") <> Неопределено Тогда
			Прервать;
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(КнопкиВерхнейПанели.Добавить(), НоваяКнопка);
