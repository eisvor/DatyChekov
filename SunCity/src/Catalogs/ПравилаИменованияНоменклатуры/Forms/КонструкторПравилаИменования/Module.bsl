...
Процедура ПроверитьЗаполнениеПравила(ЗакрыватьФорму = Ложь)
	...
	Если СтруктураПравила.ЕстьОшибки Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Обнаружены ошибки:'"));
...
Процедура ОбновитьДеревоРеквизитовИСведенийСервер()
	...
	ДеревоРеквизитовИСведений = РаботаСПравиламиИменования.ДеревоРеквизитовИСведений(НаборыСвойств,ТаблицаДоступныхПараметров);
...
Процедура ОбновитьДеревоРеквизитовИСведений()
	...
	Если СвойстваКОбработке.Количество() > 0 Тогда
		ЗаголовокФормы = Нстр("ru='Указать путь к данным (%1)'");
		Элементы.НадписьУказатьПутьКДанным.Заголовок = СтрШаблон(ЗаголовокФормы, СвойстваКОбработке.Количество());
