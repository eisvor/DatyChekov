...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЭтоНовый() Тогда
		ЗначенияКодовВИнформационнойБазе = ОбщегоНазначенияРТ.ЗначенияРеквизитовОбъекта(Ссылка, Новый Структура("ИНН,КПП,ОГРН,КодПоОКПО, ЮрФизЛицо"));
	КонецЕсли;
		...
		Если ЗначениеЗаполнено(ГоловнаяОрганизация) Тогда
			...
			Иначе
				...
				Если ПараметрыГоловной.ИНН = ИНН Тогда
					...
					КонецЕсли;
				Иначе
					ТекстСообщения = НСтр("ru = 'БИН головной организации должен совпадать с БИН обособленного подразделения'");
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "ГоловнаяОрганизация", , Отказ);
				КонецЕсли;
