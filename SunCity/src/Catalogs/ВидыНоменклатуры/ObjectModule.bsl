...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ИспользоватьСерии Тогда
		ИспользованиеСерий = Перечисления.ВариантыВеденияДополнительныхДанныхПоНоменклатуре.НеИспользовать;
	КонецЕсли;
	...
	ПродукцияМаркируемаяДляГИСМ = ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПродукцияИзНатуральногоМеха;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЭтоГруппа Тогда
		...
		Если (ПродукцияМаркируемаяДляГИСМ Или КиЗГИСМ)
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если НЕ ЭтоГруппа Тогда
		...
		КонецЕсли;
		//ИнтеграцияИСМПТК
		СписокОсобенностейРК = ИнтеграцияИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДляОтбораМаркируемойПродукции();
		Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар 
			И Не СписокОсобенностейРК.Найти(ОсобенностьУчета) = Неопределено Тогда
			ПризнакПредметаРасчета = Справочники.ПризнакиПредметовРасчета.Товар;
		КонецЕсли;
...
Функция НаборСвойствНужноИзменить(НаборСвойств)
	...
	Результат = ОбщегоНазначенияРТ.ЗначенияРеквизитовОбъекта(НаборСвойств,
