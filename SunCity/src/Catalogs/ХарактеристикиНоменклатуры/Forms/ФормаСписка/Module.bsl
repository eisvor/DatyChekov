...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Отбор") И Параметры.Отбор.Свойство("Владелец") Тогда
		...
		Если ОбщиеХарактеристики = Неопределено Тогда
			
			Если ЗначениеЗаполнено(Владелец) Тогда
				ЗаголовокШаблон		 = НСтр("ru = 'Характеристики (не используются): %1'", ОбщегоНазначения.КодОсновногоЯзыка());
				ЭтотОбъект.Заголовок = СтрШаблон(ЗаголовокШаблон, Строка(Владелец));
			Иначе
				ЭтотОбъект.Заголовок = НСтр("ru = 'Характеристики (не используются)'", ОбщегоНазначения.КодОсновногоЯзыка());
			КонецЕсли;
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ВладелецДляОтбора) Тогда
				...
				ЭтотОбъект.Заголовок = СтрШаблон(ЗаголовокШаблон, Строка(ВладелецДляОтбора));
			Иначе
	...
	Элементы.Владелец.Видимость = НЕ ЗначениеЗаполнено(Владелец);
