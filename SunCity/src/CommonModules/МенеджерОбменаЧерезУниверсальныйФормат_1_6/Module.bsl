// Конвертация РТ 2.3.4 от 22.04.2020 18:28:19
...
Процедура ПОД_Документ_ВозвратТоваровПоставщику_Отправка_ПриОбработке(ДанныеИБ, ИспользованиеПКО, КомпонентыОбмена)
	...
	|	ЕСТЬNULL(ВозвратТоваровПоставщикуСерии.Количество, ВозвратТоваровПоставщикуТовары.Количество) КАК Количество,
	|	ЕСТЬNULL(ВозвратТоваровПоставщикуСерии.Количество, ВозвратТоваровПоставщикуТовары.КоличествоУпаковок) КАК КоличествоУпаковок,
	|	ВозвратТоваровПоставщикуСерии.Серия КАК Серия,
	|	ВозвратТоваровПоставщикуТовары.Цена КАК Цена,
	|	ВЫБОР КОГДА ВозвратТоваровПоставщикуСерии.Серия ЕСТЬ NULL
	|		ТОГДА ВозвратТоваровПоставщикуТовары.Сумма
	|		ИНАЧЕ ВЫРАЗИТЬ(ВозвратТоваровПоставщикуТовары.Цена * ВозвратТоваровПоставщикуСерии.Количество КАК ЧИСЛО(15,2))
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	...
	|		ИНАЧЕ ВЫРАЗИТЬ(ВозвратТоваровПоставщикуТовары.СуммаНДС / ВозвратТоваровПоставщикуТовары.Количество КАК ЧИСЛО(15,2))
	|	КОНЕЦ КАК СуммаНДС,
...
Процедура ПОД_Документ_ОтчетОРозничныхПродажах_Отправка_ПриОбработке(ДанныеИБ, ИспользованиеПКО, КомпонентыОбмена)
	...
	|	Товары.Упаковка,
	|	ВЫБОР КОГДА ОтчетОРозничныхПродажахСерии.Серия ЕСТЬ NULL
	|		ТОГДА Товары.Сумма
	|		ИНАЧЕ ВЫРАЗИТЬ(Товары.Цена * ОтчетОРозничныхПродажахСерии.Количество КАК ЧИСЛО(15,2))
	|	КОНЕЦ КАК Сумма,
	|	Товары.Цена,
	|	Товары.СтавкаНДС,
	|	ЕСТЬNULL(ОтчетОРозничныхПродажахСерии.Количество, Товары.Количество) КАК Количество,
	|	ЕСТЬNULL(ОтчетОРозничныхПродажахСерии.Количество, Товары.КоличествоУпаковок) КАК КоличествоУпаковок,
...
Процедура ПОД_Документ_РеализацияТоваровУслуг_Отправка_ПриОбработке(ДанныеИБ, ИспользованиеПКО, КомпонентыОбмена)
	УстановитьПравилоДляДокумента(ДанныеИБ.Ссылка, ИспользованиеПКО.Документ_РеализацияТоваровУслуг_Отправка, "РеализацияТоваров"," И СправочникНоменклатура.ДоговорКонтрагента = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)");
...
КонецПроцедуры
#КонецОбласти
#Область Справочник_СостояниеОтгрузкиЗаказа_Отправка
...
КонецПроцедуры
#КонецОбласти
#Область Справочник_СостоянияЭД_Отправка
...
Процедура ПКО_Документ_ОРП_Отправка_СтарыеВерсииФормата_ПриОтправкеДанных(ДанныеИБ, ДанныеXDTO, КомпонентыОбмена, СтекВыгрузки)
	...
	|	ОплатаКартами.ЭквайринговыйТерминал.Код КАК Номер,
	|	ОплатаКартами.ЭквайринговыйТерминал.Ссылка КАК Ссылка,
	...
	|	ОплатаКартами.ЭквайринговыйТерминал.Код,
	|	ОплатаКартами.ЭквайринговыйТерминал.Ссылка,
...
Процедура ПКО_Документ_ОтчетОРозничныхПродажах_Отправка_ПриОтправкеДанных(ДанныеИБ, ДанныеXDTO, КомпонентыОбмена, СтекВыгрузки)
	...
	Если НЕ РезультатыЗапроса[1].Пустой() Тогда
		...
		Для Каждого Строка ИЗ ОплатаКартами Цикл
			...
			СтруктураДанныеДоговора = Новый Структура("Организация, Контрагент, ВидДоговора, ВалютаВзаиморасчетов, РасчетыВУсловныхЕдиницах",
								ДанныеИБ.Организация, Строка.Эквайрер, "Прочее", Валюта , Ложь);
			Строка.ЭквайринговыйТерминал = Новый Структура("Значение, ИмяПКО", Строка.Ссылка, "Справочник_ЭквайринговыеТерминалы_Отправка");;
			Строка.ДоговорЭквайринга = Новый Структура("Значение, ИмяПКО", СтруктураДанныеДоговора, "Справочник_ДоговорыКонтрагентов_ИзСтруктуры");	
		КонецЦикла;
		ДанныеXDTO.Вставить("ОплатаПлатежнымиКартами", ОплатаКартами);
	КонецЕсли;
...
Процедура ДобавитьПКО_Справочник_БанковскиеСчета_Отправка(ПравилаКонвертации)
	...
	ДобавитьПКС(СвойстваШапки, "Владелец",     "Владелец", 1, "Справочник_Контрагенты_Отправка");
...
Процедура ПКО_Справочник_ШтрихкодыНоменклатуры_Получение_ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КонвертацияСвойств, КомпонентыОбмена)
	...
	ПолученныеДанные.Отбор.Штрихкод.Установить(ПолученныеДанные.ДополнительныеСвойства.Штрихкод, Истина);
