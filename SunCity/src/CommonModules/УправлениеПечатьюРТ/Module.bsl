////////////////////////////////////////////////////////////////////////////////
// УправлениеПечатьюРТ содержит процедуры и функции для работы 
...
КонецФункции // СтруктураДанныхЦенниковИЭтикеток()
...
Функция СтруктураПараметровПечатиЦенниковИЭтикеток() Экспорт
	...
	СтруктураПараметров.Вставить("ИмяПроцедурыПодготовкиСтруктурыДанных", "УправлениеПечатьюРТВызовСервера.СтруктураДанныхЦенниковИЭтикеток");
...
Функция СтруктураПараметровПечатиЧекаДляФР(МассивДокументов, РабочееМесто) Экспорт
	...
	|	ИЛИ КассыККМ.ТипКассы = ЗНАЧЕНИЕ(Перечисление.ТипыКассККМ.СетевоеОборудование)
...
КонецФункции // СтруктураПараметровПечатиЧекаДляФР()
...
//
// Параметры:
//  МассивДокументов - Массив - массив документов РКО.
//
// Возвращаемое значение:
//  Соответствие - соответствие физичиского лица в им. падаже к документу РКО.
//
...
Процедура ПередОтправкойПоПочте(ПараметрыОтправки, ПараметрыВывода, ОбъектыПечати, ПечатныеФормы) Экспорт
	...
	ЗаполнитьПолучателейПисьмаИзДокумента(ПараметрыОтправки, ПараметрыВывода, ОбъектыПечати, ПечатныеФормы);
	
КонецПроцедуры

Функция МакетСхемыКомпоновкиДанных(ТипЦенника = "") Экспорт
	
	Возврат Обработки.ПечатьЭтикетокИЦенников.ПолучитьМакет("ПоляШаблона");
...
Процедура ОбработкаПодбораТоваровДляПечатиЭтикетокИЦенниковЗавершение(Результат, ПараметрыОперации, МассивТоваровДляПечати) Экспорт
	...
	ТаблицаТоваров = ПолучитьИзВременногоХранилища(Результат.АдресТоваровВХранилище);
	...
	СтруктураНастроек.ИсходныеДанные = ТаблицаТоваров;
	
	СтруктураНастроек.ПараметрыДанных.Вставить("ВидЦены", Неопределено);
	СтруктураНастроек.ПараметрыДанных.Вставить("Магазин", ПараметрыОперации);
	СтруктураНастроек.ПараметрыДанных.Вставить("ПравилоЦенообразования", ПараметрыОперации.ПравилоЦенообразования);
	СтруктураНастроек.ПараметрыДанных.Вставить("МагазинДляЦен", ПараметрыОперации);
	СтруктураНастроек.ПараметрыДанных.Вставить("ЦеныПоВидуЦены", Ложь);
	СтруктураНастроек.ПараметрыДанных.Вставить("ЦеныНаДату", ТекущаяДатаСеанса());
	СтруктураНастроек.ПараметрыДанных.Вставить("ЦеныНазначенныеДействующие", Истина);
	СтруктураНастроек.ПараметрыДанных.Вставить("ВидМинимальныхЦенПродажи", ПараметрыОперации.ВидМинимальныхЦенПродажи);
	СтруктураНастроек.ПараметрыДанных.Вставить("ЦеныМинимальные", Ложь);
	СтруктураНастроек.ПараметрыДанных.Вставить("ИспользоватьАссортимент", Ложь);
	СтруктураНастроек.ПараметрыДанных.Вставить("ИспользоватьСправкиБ", Ложь);
	
	СтруктураРезультата = Обработки.ПечатьЭтикетокИЦенников.ПодготовитьСтруктуруДанных(СтруктураНастроек);
	
	ДобавитьСкладИОрганизациюПродажи(СтруктураРезультата.ТаблицаТоваров, ПараметрыОперации, МенеджерОборудованияВызовСервера.ПолучитьРабочееМестоКлиента());
	
	Для Каждого Товар Из СтруктураРезультата.ТаблицаТоваров Цикл
		...
		СтруктураТовара.ОрганизацияБПО    = Товар.ОрганизацияПродажи;
...
Процедура ДобавитьСкладИОрганизациюПродажи(ТаблицаТоваров, Магазин, РабочееМесто) Экспорт
	...
	|	МАКСИМУМ(Подзапрос.Организация) КАК Организация,
	|	МАКСИМУМ(Подзапрос.ОрганизацияБезМеста) КАК ОрганизацияБезМеста
	|ПОМЕСТИТЬ СкладыОрганизацииПродаж
	|ИЗ
	|	(ВЫБРАТЬ 
	...
	|		NULL КАК ОрганизацияБезМеста
	|	ИЗ
	|		ТаблицаТоваров КАК ТаблицаТоваров
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаспределениеНоменклатуры КАК РаспределениеНоменклатуры
	|			ПО (РаспределениеНоменклатуры.Магазин = &Магазин)
	|				И (РаспределениеНоменклатуры.ТоварнаяГруппа = ТаблицаТоваров.Номенклатура.ТоварнаяГруппа)
	...
	|			ПО (РаспределениеНоменклатуры.Магазин = &Магазин)
	|				И (РаспределениеНоменклатуры.ТоварнаяГруппа = ТаблицаТоваров.Номенклатура.ТоварнаяГруппа)
	|				И (РаспределениеНоменклатуры.РабочееМесто = &ПустоеМесто)) КАК Подзапрос
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	&ТекстКолонок,
	|	ВЫБОР
	|		КОГДА СкладыОрганизацииПродаж.Склад ЕСТЬ NULL
	|			ТОГДА ВЫБОР
	|					КОГДА СкладыОрганизацииПродаж.СкладБезМеста ЕСТЬ NULL
	|						ТОГДА &СкладМагазина
	|					ИНАЧЕ СкладыОрганизацииПродаж.СкладБезМеста
	|				КОНЕЦ
	|		ИНАЧЕ СкладыОрганизацииПродаж.Склад
	|	КОНЕЦ КАК СкладПродажи,
	|	ВЫБОР
	|		КОГДА СкладыОрганизацииПродаж.Организация ЕСТЬ NULL
	|			ТОГДА ВЫБОР
	|					КОГДА СкладыОрганизацииПродаж.ОрганизацияБезМеста ЕСТЬ NULL
	|						ТОГДА &ОрганизацияМагазина
	|					ИНАЧЕ СкладыОрганизацииПродаж.ОрганизацияБезМеста
	|				КОНЕЦ
	|		ИНАЧЕ СкладыОрганизацииПродаж.Организация
	|	КОНЕЦ КАК ОрганизацияПродажи
	|ИЗ
	|	ТаблицаТоваров КАК ТаблицаТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ СкладыОрганизацииПродаж КАК СкладыОрганизацииПродаж
	|		ПО ТаблицаТоваров.Номенклатура = СкладыОрганизацииПродаж.Номенклатура";
...
Функция ДанныеДляФормированияПечатныхФорм(ПараметрыПечати, СоответствиеШаблоновИПолей) Экспорт
	...
	ИсходныеДанные.Колонки.Добавить("ШтрихАвтопроверки", Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(128)));
	ИсходныеДанные.Колонки.Добавить("Справка2", Новый ОписаниеТипов("СправочникСсылка.Справки2ЕГАИС"));
	...
	Для Каждого Товар Из ПараметрыПечати.Товары Цикл
		...
		Строка.Упаковка = Товар.УпаковкаБПО;
		Строка.Организация = Товар.ОрганизацияБПО;
	КонецЦикла;
	
	СтруктураНастроек.ИсходныеДанные = ИсходныеДанные;
	
	Магазин = ПараметрыПечати.ТорговыйОбъект;
	
	СтруктураНастроек.ПараметрыДанных.Вставить("ВидЦены", Неопределено);
	СтруктураНастроек.ПараметрыДанных.Вставить("Магазин", Магазин);
	СтруктураНастроек.ПараметрыДанных.Вставить("ПравилоЦенообразования", Магазин.ПравилоЦенообразования);
	СтруктураНастроек.ПараметрыДанных.Вставить("МагазинДляЦен", Магазин);
	СтруктураНастроек.ПараметрыДанных.Вставить("ЦеныПоВидуЦены", Ложь);
	СтруктураНастроек.ПараметрыДанных.Вставить("ЦеныНаДату", ТекущаяДатаСеанса());
	СтруктураНастроек.ПараметрыДанных.Вставить("ЦеныНазначенныеДействующие", Истина);
	СтруктураНастроек.ПараметрыДанных.Вставить("ВидМинимальныхЦенПродажи", Магазин.ВидМинимальныхЦенПродажи);
	СтруктураНастроек.ПараметрыДанных.Вставить("ЦеныМинимальные", Ложь);
	СтруктураНастроек.ПараметрыДанных.Вставить("ИспользоватьАссортимент", Ложь);
	СтруктураНастроек.ПараметрыДанных.Вставить("ИспользоватьСправкиБ", Ложь);
	
	ДанныеНоменклатурыДляФормированияПечатныхФорм = Новый Массив;
	Для Каждого Шаблон Из СоответствиеШаблоновИПолей Цикл;
		...
		Для Каждого Поле Из СписокПолей Цикл
			СтруктураНастроек.ОбязательныеПоля.Добавить(Поле);
		КонецЦикла;
		
		ТаблицаДанных = Обработки.ПечатьЭтикетокИЦенников.ПодготовитьСтруктуруДанных(СтруктураНастроек, СтруктураШаблона);
...
//  АдресХранилища - Строка - адрес данных в хранилище.
//
...
//
// Параметры
// 	Период  - Дата - тип дата, в данном параметре передается
...
// Печатает опись номенклатуры по входящим данным
//
// Параметры:
//  ТабличныйДокумент - ТабличныйДокумнет - Табличный документ, в который выводится опись номенклатуры.
//  ОбъектыПечати - Массив - массив объектов для печати.
...
//  ПервыйДокумент - Булево - признак того что печатается первый документ.
//  ПараметрыПечати - Структура - параметры печати документов.
//
...
// 
// Параметры:
//  СтруктураДокументов - Структура - структура документов для печати.
//  Устройство - СправочникСсылка.ПодключаемоеОборудование - оборудование родитель печати.
//
// Возвращаемое значение:
...
//
// Параметры:
//  СтруктураНастроек - Структура - структура для подготовки данных.
//  СтруктураШаблона - Структура - структура данных для печати.
//
// Возвращаемое значение:
...
Процедура ПередДобавлениемКомандПечати(ИмяФормы, КомандыПечати, СтандартнаяОбработка) Экспорт
	...
	Если ИмяФормы = "Справочник.ШтрихкодыУпаковокТоваров.Форма.ФормаЭлемента"
		ИЛИ ИмяФормы = "Справочник.ШтрихкодыУпаковокТоваров.Форма.ФормаСписка"
		ИЛИ ИмяФормы = "Справочник.ШтрихкодыУпаковокТоваров.Форма.ФормаВыбора" Тогда
		...
		КомандаПечати.Представление = НСтр("ru = 'Печать: Этикетка (обувь)';");
...
// Переопределяет параметры отправки печатных форм при подготовке письма.
// Используется для заполнения получателей писем из контрагентов документа.
//
// Параметры:
//  ПараметрыОтправки - Структура - коллекция параметров:
//   * Получатель - Массив - коллекция имен получателей;
...
//   * ТабличныйДокумент - ТабличныйДокумент - печатная форма.
// 
Процедура ЗаполнитьПолучателейПисьмаИзДокумента(ПараметрыОтправки, ПараметрыВывода, ОбъектыПечати, ПечатныеФормы) Экспорт
	
	МассивОбъектов = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(ОбъектыПечати);
	...
	ПараметрыОтправки.Получатель = АдресаЭлектроннойПочты(Получатель);
...
Функция АдресаЭлектроннойПочты(Контрагент)
	...
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ВТ_ФизЛица.Выбран, ИСТИНА),
	...
	|		ПО Контрагенты.ФизЛицо = ФизическиеЛицаКонтактнаяИнформация.Ссылка
	...
	|		ПО Контрагенты.ФизЛицо = ВТ_ФизЛица.Контрагент";
	
	ВыборкаКонтактнойИнформации = Запрос.Выполнить().Выбрать();
	
	Пока ВыборкаКонтактнойИнформации.Следующий() Цикл
		Если ЗначениеЗаполнено(ВыборкаКонтактнойИнформации.Адрес) Тогда
			ПараметрыПолучателя = НовыеПараметрыПолучателя();
			ЗаполнитьЗначенияСвойств(ПараметрыПолучателя, ВыборкаКонтактнойИнформации);
...
Функция НовыеПараметрыПолучателя()
	...
	Результат.Вставить("Выбран"                      , Истина);
...
Функция ЗаполнитьПечатныеФормы(ИмяМакета, ПараметрыПечати, СтруктураДокументов, Устройство)
	...
	Иначе // Тип("Массив")
		ИмяМакета = ОбщегоНазначения.СкопироватьРекурсивно(ИмяМакета);
	КонецЕсли;
	
	ВременнаяКоллекцияДляОднойПечатнойФормы = ПодготовитьКоллекциюПечатныхФорм(ИмяМакета);
...
Процедура ЗаполнитьТабличныеДокументыПечатнойФормы(ПараметрыПечати, СтруктураДокументов, КоллекцияПечатныхФорм, ПараметрыВывода, Устройство)
	...
	СтруктураНастроек.ПараметрыДанных.Вставить("ЦеныПоВидуЦены", Ложь);
	СтруктураНастроек.ПараметрыДанных.Вставить("ЦеныНаДату", ОбщегоНазначения.ТекущаяДатаПользователя());
