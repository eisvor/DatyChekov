...
//
// Параметры:
//  ДокументУстановкиЦен - ДокументОбъект - документ для которого необходимо осуществить проверку.
...
// Формирует движения по РС "Цены номенклатуры".
// 
// Параметры:
...
// Формирует движения по РС "Действующие цены номенклатуры".
// 
// Параметры:
//  ДополнительныеСвойства - Структура - структура дополнительных свойств для формирования движений.
//  Движения - Движения - движения документа для отражения в регистре "Расчеты с поставщиками".
...
#Область ПроцедурыИФункцииДляОбслуживанияВидовЦенЗаполняемыПоДаннымИБ
...
//
Функция ОбязательныеПоляСхемыКомпоновкиДанных() Экспорт
	...
	Если ЦенообразованиеПереопределяемый.ИспользуютсяВалюты() Тогда
		...
		Поля.Вставить("Валюта", Новый ОписаниеТипов("Строка"));
...
Функция ПроверитьСхемуКомпоновкиДанных(СхемаКомпоновкиДанных, СообщениеОбОшибке = Неопределено) Экспорт
	...
	Если СхемаКомпоновкиДанных <> Неопределено И СхемаКомпоновкиДанных.НаборыДанных.Количество() = 1 Тогда
		...
		Поля = ОбязательныеПоляСхемыКомпоновкиДанных();
...
#Область ПроцедурыИФункцииПроведенияДокументов
...
//
// Параметры:
//  ДокументСсылка - ДокументСсылка - документы для проведения.
...
//
// Параметры:
//  Объект - ДокументОбъект - документ объект по которому ведется запись движений.
//
...
//
// Параметры:
//  Объект - ДокументОбъект - документ объект по которому выполняется запись движений.
//
...
//  ВалютаНач      - СправочникСсылка.Валюты - определяет валюты из которой надо пересчитать.
//  ВалютаКон      - СправочникСсылка.Валюты - определяет валюты в которую надо пересчитать.
//  ПоКурсуНач     - Число - курс из которого надо пересчитать;
//  ПоКурсуКон     - Число - курс в который надо пересчитать;
//  ПоКратностьНач - Число - кратность из которого надо пересчитать (по умолчанию = 1);
//  ПоКратностьКон - Число - кратность в который надо пересчитать  (по умолчанию = 1);
//
// Возвращаемое значение: 
...
Функция ПересчитатьИзВалютыВВалюту(Сумма, ВалютаНач, ВалютаКон, ПоКурсуНач, ПоКурсуКон, 
	...
	Если ПоКурсуНач     = 0 
	 ...
	 Или ПоКратностьКон = 0 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'ПересчитатьИзВалютыВВалюту(): при пересчете обнаружен нулевой курс.'"));
		Возврат 0;
	КонецЕсли;
...
#Область ПроцедурыИФункцииСогласованияУстановкиЦенНоменклатуры
