...
// Выполняет расчет скидок по документу.Вызывается из форм документов.
//
// Параметры:
//  Объект - ДокументОбъект - документ по которому рассчитываются скидки.
//  ВходныеПараметры - Структура - параметры расчета скидок.
//
// Возвращаемое значение:
//  ДеревоЗначений - дерево скидок по документу.
//
...
// 
// Параметры:
//  РеквизитыДокумента - Структура - структура реквизитов документа.
//
// Возвращаемое значение:
//  Число - сумма накоплений по карте.
//
Функция СуммаНакопленияПоКартеДляПечатиЧека(РеквизитыДокумента) Экспорт
	...
	Иначе
		ТаблицаИсторииКарт = МаркетинговыеАкцииСервер.РасширеннаяИсторияДисконтнойКарты(ДатаОкончания, РеквизитыДокумента.ДисконтнаяКарта, Ложь);
		Запрос = Новый Запрос;
...
//
// Параметры:
//  Форма - ДокументОбъект - документ в котором выполняется перенос.
//  ЦенаВключаетНДС - Булево - признак включения НДС в цену.
//  ПримененныеСкидкиНаценки - ДеревоЗначений - дерево с примененными скидками.
//  КоличествоПеренесенных - Число - количество перенесенных строк.
//  ВыводитьСообщения - Булево - признак вывода сообщений пользователю.
//  КонтролироватьВидНалога - Булево - признак контроля вида налога.
//
Процедура ПеренестиСкидкиПодаркиВТовары(Форма, ЦенаВключаетНДС, ПримененныеСкидкиНаценки = Неопределено, КоличествоПеренесенных = 0, ВыводитьСообщения = Истина, КонтролироватьВидНалога = Ложь, МассивСообщений = Неопределено) Экспорт
	...
	Для Каждого СтрокаПодарка Из ТаблицаСтрокПереносимыхПодарков Цикл
		...
		Если Учитывать = Истина Тогда
			...
			ИначеЕсли СтрокаПодарка.Сумма <= 0 Тогда
				...
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, СтрокаПодарка.Номенклатура);
			ИначеЕсли ЗначениеЗаполнено(ВидПродукции)
					И ((ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ИмяФормы")
					И Форма.ИмяФормы <> "Обработка.РМКУправляемыйРежим.Форма.Форма")
					ИЛИ (ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "Ссылка")
					И (ТипЗнч(Форма.Ссылка) = Тип("ДокументСсылка.ЗаказПокупателя")
					ИЛИ ТипЗнч(Форма.Ссылка) = Тип("ДокументСсылка.ЧекККМ")))) Тогда
				СтрокаСообщения = НСтр("ru = 'Подарок ""%1"" нельзя учесть как продажу: для ""%2"" операция поддерживается только при продаже через РМК'");
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, СтрокаПодарка.Номенклатура, ВидПродукции);
		...
		Если УдалятьСтроку Тогда
			...
			КонецЕсли;
			Если МассивСообщений <> Неопределено И ЗначениеЗаполнено(СтрокаСообщения) Тогда
	...
	Для Каждого СтрокаТоваров Из Объект.Товары Цикл
		...
		Если СтрокаТоваров.ПродажаПодарка
			...
			ИЛИ ЗначениеЗаполнено(СтрокаТоваров.ДоговорКонтрагента) Тогда
	...
	Если ПодаркиИтогСумма > ОбщаяСумма Тогда
		...
		ВидыПродукции = ИнтеграцияИСРТ.ВидыПродукцииВТоварах(Объект.Подарки);
		
		Если ВидыПродукции.Найти(Перечисления.ВидыПродукцииИС.Табак) <> Неопределено Тогда
			Объект.Подарки.Очистить();
			ДополнительноеОписание = НСтр("ru = 'Табачную продукцию можно дарить, учитывая только как продажу.'")
		КонецЕсли;
		
		СтрокаСообщения = НСтр("ru = 'Подарки нельзя учесть как продажу: сумма подарков превышает итоговую сумму продажи.
		                            |Дополнительное описание: %ДополнительноеОписание%'");
		СтрокаСообщения = СтрЗаменить(СтрокаСообщения, "%ДополнительноеОписание%", ДополнительноеОписание);
	...
	ИначеЕсли ПодаркиИтогСумма = ОбщаяСумма Тогда
		...
		Если ВидыПродукции.Найти(Перечисления.ВидыПродукцииИС.Табак) <> Неопределено Тогда
			Объект.Подарки.Очистить();
			ДополнительноеОписание = НСтр("ru = 'Табачную продукцию можно дарить, только учитывая как продажу.'")
		КонецЕсли;
	...
	Для Каждого СтрокаПодарка Из ТаблицаСтрокПереносимыхПодарков Цикл
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаПодарка, "ШтрихкодУпаковки")
			...
			АкцизныеМаркиСтрокаТЧ = Объект.АкцизныеМарки.Добавить();
			АкцизныеМаркиСтрокаТЧ.КлючСвязи        = СтрокаТоваров.КлючСвязи;
			АкцизныеМаркиСтрокаТЧ.КодАкцизнойМарки = СтрокаПодарка.ШтрихкодУпаковки;
			АкцизныеМаркиСтрокаТЧ.АкцизнаяМарка    = СтрокаПодарка.ШтрихкодУпаковки;
			АкцизныеМаркиСтрокаТЧ.ШтрихкодУпаковки = СтрокаПодарка.ШтрихкодУпаковки;
			АкцизныеМаркиСтрокаТЧ.Справка2         = СтрокаПодарка.Справка2;
			
			СтрокаТоваров.Штрихкод                 = СтрокаПодарка.ШтрихкодУпаковки;
	...
	Если ПеренесенМаркируемыйТовар Тогда
		ОбработкаТабличнойЧастиТоварыСервер.ЗаполнитьМаркируемаяПродукцияГосИС(Объект.Товары);
		ПроверкаИПодборПродукцииИС.ЗаполнитьКешШтрихкодовУпаковок(Форма, ПроверкаИПодборПродукцииИСМПРТ.НастройкиИсточникаКешаЧека());
	...
	СтруктураДействий = Новый Структура;
	ПараметрыПересчетаНДС = ОбработкаТабличнойЧастиТоварыКлиентСервер.СтруктураПересчетаСуммыНДСВСтрокеТЧ(Объект);
...
// Выполняет удаление строк из табличной части объекта "Товары",
// в которых значение поля ПродажаПодарка установлено равным Истина.
//
// Параметры:
//  Объект - ДокументОбъект - документ в котором очищаются подарки.
//
Процедура ОчиститьТоварыОтПодарков(Объект) Экспорт
	...
	Для Каждого СтрокаТовары Из СтрокиТовары Цикл
		МассивКлючей.Добавить(СтрокаТовары.КлючСвязи);
		Объект.Товары.Удалить(СтрокаТовары);
	КонецЦикла;
	
	Если МассивКлючей.Количество() 
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "АкцизныеМарки") Тогда
		РозничныеПродажиСервер.УдалитьСтрокиИзТабличнойЧастиПоКлючам("АкцизныеМарки", "КлючСвязи", МассивКлючей,, Объект);
	КонецЕсли;
	
КонецПроцедуры

// Анализирует все подарки из одноименной табличной части документа "ЧекККМ".
// Если условие для переноса в товары не выполняется, то выдается соответствующее сообщение.
//
// Параметры: 
//  Объект - ДокументОбъект - основной реквизит "Объект" формы документа "ЧекККМ".
//  ЕстьОшибки - Булево - признак наличия ошибок во время выполнения процедуры.
//  
...
// Проверяет существование запрета продажи для товаров табличной части в указанном магазине на указанную дату.
//
// Параметры: 
...
//
// Возвращаемое значение:
//  Булево - Истина, когда запрет продажи установлен, Ложь, когда запрета нет.
// 
Функция ЗапрещенаПродажаТовара(Номенклатура, Магазин, ДатаРасчета) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЗапретРозничнойПродажиАлкоголя") Тогда
		...
		|ПОМЕСТИТЬ СкидкиНаценкиНоменклатуры
		|ИЗ
		...
		|		ПО (НоменклатураСегмента.Номенклатура = &Номенклатура)
		|			И (СкидкиНаценки.СпособПредоставления = ЗНАЧЕНИЕ(Перечисление.СпособыПредоставленияСкидокНаценок.ЗапретРозничнойПродажи))
		|			И (СкидкиНаценки.СтатусДействия = ЗНАЧЕНИЕ(Перечисление.СтатусыДействияСкидок.Действует))
		|			И СкидкиНаценки.СегментНоменклатурыПредоставления = НоменклатураСегмента.Сегмент
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СкидкиНаценкиНоменклатуры.Ссылка КАК СкидкаНаценка,
		|	СкидкиНаценкиНоменклатуры.Сегмент КАК Сегмент,
		...
		|				И Магазин В (&СписокМагазинов)
		|				И СкидкаНаценка В
		|					(ВЫБРАТЬ
		|						СкидкиНаценкиНоменклатуры.Ссылка
		|					ИЗ
		|						СкидкиНаценкиНоменклатуры КАК СкидкиНаценкиНоменклатуры)) КАК ДействиеСкидок
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СкидкиНаценкиНоменклатуры КАК СкидкиНаценкиНоменклатуры
		|		ПО (СкидкиНаценкиНоменклатуры.Ссылка = ДействиеСкидок.СкидкаНаценка)
		...
		|		ПО (ТЧУсловия.Ссылка = СкидкиНаценкиНоменклатуры.Ссылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ДействиеСкидок.Номенклатура КАК Номенклатура
		|ИЗ
		...
		СписокМагазинов.Добавить(Магазин);
		СписокМагазинов.Добавить(Справочники.Магазины.ПустаяСсылка());
...
// 
// Параметры:
//  МассивКарт - Массив - массив карт для выбора карты.
//  ДатаРасчета - Дата - дата расчета скидки.
//  СегментОтграничения - СправочникСсылка.СегментыНоменклатуры - сегмент ограничения номенклатуры.
//  Регистратор - ДокументСсылка - документ регистратор предыдущей покупки.
//
// Возвращаемое значение:
//  Структура - данные предыдущей покупки.
//
...
//
// Параметры:
//  Объект - ДокументОбъект.ЧекККМ, ДокументОбъект.Реализация Товаров - документ для отображения скидки.
...
Процедура ОбновитьОтображениеСкидки(Объект, ИмяОсновнойТЧ = "Товары", ИмяТЧ = "СкидкиНаценки") Экспорт
	...
	Для каждого СтрокаСкидкиНаценки Из Объект[ИмяТЧ] Цикл
		...
		СтрокаСкидкиНаценки.СкидкаЗапрещенаКПросмору = НЕ ОбщегоНазначенияРТВызовСервера.РеквизитДоступен(СтрокаСкидкиНаценки, "СкидкаНаценка", "Справочник.СкидкиНаценки");
...
// Формирует текст запроса для динамического списка номенклатуры в обработке "Подбор товаров"
...
Функция ТекстЗапросаДинамическогоСпискаДляПодбора() Экспорт
	...
	ТекстЗапроса="ВЫБРАТЬ РАЗРЕШЕННЫЕ
				...
				|	СправочникНоменклатура.ДоговорКонтрагента КАК ДоговорКонтрагента
				|ИЗ
...
//
// Параметры:
//  СтруктураШаблона - ДеревоЗначений - структура шаблона чека.
//  ДокументСсылка - ДокументСсылка - документ чек.
//
...
// Заполняет цены и суммы строк таблицы товаров
// в соответствии с данными документа-основания.
// 
// Параметры:
//  Объект - ДокументОбъект - документ-основания по которому заполняются цены и суммы.
//  РежимРМК - Булево - признак работы в режиме РМК.
//
Процедура ПодготовитьСкидкиИОплатуБонусамиКСторнированиюПриВозврате(Объект, РежимРМК = Ложь, ТолькоБонусы = Ложь) Экспорт
	...
	Если РежимРМК ИЛИ ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЧекККМ") Тогда
		Если ЗначениеЗаполнено(Объект.ЧекККМПродажа) Тогда
			...
			|	И НЕ Товары.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПодарочныйСертификат)
			|
			|СГРУППИРОВАТЬ ПО
			|	Товары.Номенклатура,
...
// Выполняет проверку соответствия дисконтной карты возврата
// и дисконтных карт документов продажи.
//
// Параметры:
//  Объект - ДокументОбъект - документ по которому выполняется проверка.
//
// Возвращаемое значение:
//  ТаблицаЗначений - таблица значений расхождение дисконтных карт возврата и продаж.
//
...
// Выполняет активирование скидок по идентификатору предыдущего чека.
//
// Параметры:
//  Чек - ДокументСсылка.ЧекККМ - документ по которому активируются скидки.
//  Магазин - СправочникСсылка.Магазины - магазин по которому активируются скидки.
//  ДатаАктивации - Дата - дата активации скидок.
//
...
// Выполняет деактивацию скидок по идентификатору предыдущего чека.
//
// Параметры:
//  Чек - ДокументСсылка.ЧекККМ - чек по которому деактивируются скидки.
//
...
// Выполняет погашение всех скидок по идентификатору предыдущего чека.
...
//
// Параметры:
//  ДокументОбъект - ДокументОбъект - документ в котором необходимо погасить однократные скидки.
//
...
//
// Параметры:
//  КодСкидки - Число - код скидки для купона.
//  ДатаЗапроса - Дата - дата проверки.
//
// Возвращаемое значение:
//  ТаблицаЗначений - скидки для купона.
//
...
Функция РассчитатьПоДокументу(Объект, ВходныеПараметры, ТипОбъекта)
	...
	ДисконтныеКарты = МаркетинговыеАкцииСервер.ИсторияДисконтнойКарты(ДатаОбъекта, Объект.ДисконтнаяКарта, Истина);
	...
	Если ЗначениеЗаполнено(ВладелецКарты) Тогда
		ПараметрыРасчета.Вставить("ДеньРождения", МаркетинговыеАкцииСервер.БлижайшийДеньРождения(ДатаОбъекта, ВладелецКарты));
	КонецЕсли;
	...
	Если ТипОбъекта = "РеализацияТоваров" ИЛИ 
			ТипОбъекта = "ЗаказПокупателя" Тогда
