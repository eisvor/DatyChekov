...
Процедура ИнициализироватьДокументМаркировкаТоваров(ДокументОбъект, ДанныеЗаполнения = Неопределено) Экспорт
	...
	ДокументОбъект.Организация   = ЗначениеНастроекПовтИсп.ОрганизацияПоУмолчанию(ДокументОбъект.Организация);
	ДокументОбъект.Магазин       = ПараметрыСеанса.ТекущийМагазин;
	ДокументОбъект.Склад         = ЗначениеНастроекПовтИсп.СкладПоступленияПоУмолчанию(ДокументОбъект.Магазин, Перечисления.ТипыСкладов.СкладскоеПомещение, ДокументОбъект.Склад);
...
Процедура ОбработкаПроверкиЗаполненияМаркировкаТоваров(Отказ, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов, ДокументОбъект) Экспорт
	...
	Для каждого СтрТабл из ДокументОбъект.Товары Цикл
		Если СтрТабл.СтатусУказанияСерий = 0 Тогда
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Товар%", ОбщегоНазначенияРТ.ПредставлениеНоменклатуры(СтрТабл.Номенклатура, СтрТабл.Характеристика));
...
Процедура ЗаполнитьСпискиВыбораРеквизитовШапки(Форма) Экспорт
	...
	ИначеЕсли Форма.Элементы.ОперацияМаркировки.СписокВыбора.Количество() = 0 Тогда
		...
		ВызватьИсключение НСтр("ru='Перед созданием маркировки необходимо ввести контрольные идентификационные знаки в справочник ""Номенклатура"".'");
...
Процедура ЗаполнитьСтатусСерииВСтроке(Магазин, ТекущаяСтрока, СерииКиЗ) Экспорт
	...
	|ГДЕ
	|	ВидыНоменклатурыПолитикиУчетаСерий.Ссылка = &Ссылка
	|	И ВидыНоменклатурыПолитикиУчетаСерий.Магазин = &Магазин
	|	И ВидыНоменклатурыПолитикиУчетаСерий.ПолитикаУчетаСерий.УказыватьПриМаркировкеПродукцииДляГИСМ";
	
	Запрос.УстановитьПараметр("Ссылка", ТекущаяСтрока.Номенклатура.ВидНоменклатуры);
	...
	Если Результат.Пустой() Тогда
		...
		ТекущаяСтрока.СтатусУказанияСерийСписываемаяСерия = 0;
		ТекущаяСтрока.Серия = Неопределено;
		ТекущаяСтрока.СписываемаяСерия = Неопределено;
		
		СтурктураПоиска = Новый Структура;
		СтурктураПоиска.Вставить("Номенклатура", ТекущаяСтрока.НоменклатураКиЗ);
		СтурктураПоиска.Вставить("Характеристика", ТекущаяСтрока.ХарактеристикаКиЗ);
		
		МассивСтрокКиЗ = СерииКиЗ.НайтиСтроки(СтурктураПоиска);
		Для каждого СтрокаКиЗ Из СерииКиЗ Цикл
			СерииКиЗ.Удалить(СтрокаКиЗ);
		КонецЦикла;
		
	Иначе
		
		ТекущаяСтрока.СтатусУказанияСерий = ?(ЗначениеЗаполнено(ТекущаяСтрока.Серия),2,1);
		ТекущаяСтрока.СтатусУказанияСерийСписываемаяСерия = ?(ЗначениеЗаполнено(ТекущаяСтрока.СписываемаяСерия),2,1);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьСтатусыСерииВСтроках(ДокументОбъект) Экспорт
	...
	Запрос.УстановитьПараметр("МассивНоменклатуры", ДокументОбъект.Товары.Выгрузить().ВыгрузитьКолонку("Номенклатура"));
	Запрос.УстановитьПараметр("Магазин", ДокументОбъект.Магазин);
	
	Результат = Запрос.Выполнить();
	МассивВидовНоменклатуры = Результат.Выгрузить().ВыгрузитьКолонку("ВидНоменклатуры");
	
	Для каждого СтрокаТовары Из ДокументОбъект.Товары Цикл
		
		ВидНоменклатурыВСтроке = СтрокаТовары.Номенклатура.ВидНоменклатуры;
		Если МассивВидовНоменклатуры.Найти(ВидНоменклатурыВСтроке) = Неопределено Тогда
			
			СтрокаТовары.СтатусУказанияСерий = 0;
			СтрокаТовары.СтатусУказанияСерийСписываемаяСерия = 0;
			СтрокаТовары.Серия = Неопределено;
			СтрокаТовары.СписываемаяСерия = Неопределено;
			
			СтурктураПоиска = Новый Структура;
			СтурктураПоиска.Вставить("Номенклатура", СтрокаТовары.НоменклатураКиЗ);
			СтурктураПоиска.Вставить("Характеристика", СтрокаТовары.ХарактеристикаКиЗ);
			МассивСтрокКиЗ = ДокументОбъект.СерииКиЗ.НайтиСтроки(СтурктураПоиска);
			Для каждого СтрокаКиЗ Из ДокументОбъект.СерииКиЗ Цикл
				ДокументОбъект.СерииКиЗ.Удалить(СтрокаКиЗ);
			КонецЦикла;
		Иначе
			
			СтрокаТовары.СтатусУказанияСерий = ?(ЗначениеЗаполнено(СтрокаТовары.Серия),2,1);
			СтрокаТовары.СтатусУказанияСерийСписываемаяСерия = ?(ЗначениеЗаполнено(СтрокаТовары.СписываемаяСерия),2,1);
...
Процедура ПриПолученииФормыДокумента(ИмяДокумента, ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт
	...
	Если ИмяДокумента = "МаркировкаТоваровГИСМ" Тогда
		Если ВидФормы = "ФормаОбъекта" Тогда
			ВыбраннаяФорма = "ФормаДокументаРТ";
...
Процедура МассивGTINМаркированногоТовара(Номенклатура, Характеристика, СписокGTINРезультат) Экспорт
	...
	СписокGTINРезультат =  Новый Массив;
...
Процедура ПроверитьСоответствияGTIN(Отказ, ДокументОбъект)
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонСообщения,
			ОбщегоНазначенияРТ.ПредставлениеНоменклатуры(Выборка.НоменклатураКиЗ, Выборка.ХарактеристикаКиЗ),
			СокрЛП(Выборка.GTINКИЗ),
			ОбщегоНазначенияРТ.ПредставлениеНоменклатуры(Выборка.Номенклатура, Выборка.Характеристика),
			СокрЛП(Выборка.GTIN));
...
Функция ПараметрыУказанияСерий() Экспорт
	...
	СтруктураПараметров.Вставить("Сырье");
