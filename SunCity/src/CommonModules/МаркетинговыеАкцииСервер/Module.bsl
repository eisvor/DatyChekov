////////////////////////////////////////////////////////////////////////////////
// МаркетинговыеСервер содержит процедуры и функции 
...
// Получает историю замен дисконтной карты, включая даты и документы замены.
//
// Параметры:
//  ДатаРасчета - Дата -дата расчета;
//  ДисконтнаяКарта - СправочникСсылка.ИнформационныеКарты - карта для которой находим историю.
//  ТолькоПредкиКарты - Булево - признак поиска только родителей карты.
//
// Возвращаемое значение:
//  ТаблицаЗначений - таблица значений, включающую историю замен дисконтной карты.
//
Функция РасширеннаяИсторияДисконтнойКарты(ДатаРасчета, ДисконтнаяКарта, ТолькоПредкиКарты) Экспорт
...
//
// Возвращаемое значение:
//  Массив - массив карт, включающий саму карту.
//
Функция ИсторияДисконтнойКарты(ДатаРасчета, ДисконтнаяКарта, ТолькоПредкиКарты) Экспорт
...
//
// Параметры:
//  ДатаРасчета - Дата - дата на которую рассчитывается ближайший день рождения
//  Получатель - СправочникСсылка.ФизическиеЛица - получатель для которого ищется день рождения.
//
// Возвращаемое значение:
//  Дата - день рождения получателя.
//
Функция БлижайшийДеньРождения(ДатаРасчета, Получатель) Экспорт
...
КонецФункции // БлижайшийДеньРождения()

// Заполняем приемник по данным подарочного сертификата.
//
// Параметры:
//  СерийныйНомер - СправочникСсылка.СерийныеНомера - серийный номер подарочного сертификата.
//
// Возвращаемое значение
//  Структура - структура данных подарочного сертификата.
//
Функция ДанныеПодарочногоСертификата(СерийныйНомер) Экспорт
	...
	|ИЗ
	|	РегистрНакопления.ПодарочныеСертификаты.Обороты(, , , НомерСертификата = &Ссылка) КАК ПодарочныеСертификатыОбороты
	|ГДЕ
	...
	|	ВЫБОР
	|		КОГДА СерийныеНомера.Владелец.ПроизвольныйНоминал
	|			ТОГДА ЕстьNULL(ПроизвольныеНоминалы.Номинал, 0)
	|		ИНАЧЕ СерийныеНомера.Владелец.Номинал
	|	КОНЕЦ КАК Номинал,
	|	СерийныеНомера.Владелец.ЧастичноеПогашение КАК ЧастичноеПогашение,
	|	СерийныеНомера.Владелец.ПроизвольныйНоминал КАК ПроизвольныйНоминал,
	|	ПодарочныеСертификатыОстатки.СуммаОстаток КАК Остаток
	|ИЗ
	|	Справочник.СерийныеНомера КАК СерийныеНомера
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПодарочныеСертификаты.Остатки(, НомерСертификата = &Ссылка) КАК ПодарочныеСертификатыОстатки
	|		ПО (ПодарочныеСертификатыОстатки.НомерСертификата = СерийныеНомера.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПроизвольныеНоминалы КАК ПроизвольныеНоминалы
	|		ПО СерийныеНомера.Ссылка = ПроизвольныеНоминалы.НомерСертификата
	|ГДЕ
	...
	Если Выборка.Следующий() Тогда
		...
		Возврат ОбщегоНазначенияРТ.СтруктураПоСтрокеВыборки(Результат, Выборка);
...
#Область ПроцедурыПроверкиЗаполнения

// Проверяет условие, что в табличной части Погашение подарочных сертификатов, должны быть только подарочные
// сертификаты.
//
// Параметры:
//  ДокументОбъект - ДокументОбъект.ПогашениеПодарочныхСертификатов - документ в котором проверяется условие.
...
// Проверяет реализацию и погашение серийных номеров.
//
// Параметры:
...
//
// Параметры:
//  ДокументОбъект - ДокументОбъект - объект документа Маркетинговая акция.
//  Отказ -Булево - Переменная отвечающая за прерывание проведения.
//
...
//
// Параметры:
//  СправочникОбъект - СправочникОбъект - объект справочника Информационные карты.
//  Отказ - Булево - Переменная отвечающая за прерывание проведения.
//
...
#Область ПроцедурыПроверкиПодарочныхСертификатов

// Получает параметры объекта - Документа.
// 
// Параметры:
//  ДокументОбъект - ДокументОбъект,
//                   Обработка.РМКУправляемыйРежим - документ или обработка для получения параметров.
//  ДокументОбъект_Параметризованный - Неопределено - Возвращаемый параметр, документ или Неопределено.
//  ЭтоДокумент - Булево - признак документа.
//  ЭтоНовый - Булево - признак новый документ или нет.
//  Дата - Дата - дата в разрезе которой получаются параметры.
//
...
// Проверяет реализации и погашения серийных номеров.
//
// Параметры:
//  ДокументОбъект - ДокументОбъект - объект проводимого документа документа.
//  ИмяТабличнойЧасти - Строка - имя табличной части Товары.
//  ИмяТабличнойЧастиСерийныеНомера - Строка - имя табличной части Серийные номера.
...
// Параметры:
//  ДокументОбъект - ДокументОбъект - объект проводимого документа документа.
//  ИмяТабличнойЧасти  - Строка - наименование табличной части серийные номера.
//  Отказ - Булево - переменная отвечающая за прерывание проведения.
...
// Проверяет заполненность табличной части серийные номера.
//
// Параметры:
//  ДокументОбъект - ДокументОбъект - объект проводимого документа.
//  ИмяТабличнойЧасти - Строка - имя табличной части Товары.
//  ИмяТабличнойЧастиСерийныеНомера - Строка - имя табличной части Серийные номера.
//  Отказ - Булево - переменная отвечающая за прерывание операции.
//  ТекстОшибки - Строка - текст возвращаемой ошибки.
//  ОсобыйВыводСообщения - Булево - особый вывод сообщения об ошибки
//
Процедура ПроверитьЗаполнениеТабличнойЧастиСерийныеНомера(
	...
	Пока Выборка.Следующий() Цикл
		...
		Если -Выборка.Количество = Выборка.КоличествоСерийныхНомеров Тогда
			Продолжить;
		КонецЕсли;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТабСерийныеНомераКоличество.СерийныйНомер КАК СерийныйНомер,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТабПовторяющиесяСерийныеНомера.СерийныйНомер КАК СерийныйНомер,
	|	ТабСерийныеНомера.КлючСвязиСерийныхНомеров КАК КлючСвязиСерийныхНомеров
	...
	Пока ВыборкаВидОперации.Следующий() Цикл
		ВыборкаНоменклатура = ВыборкаВидОперации.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаНоменклатура.Следующий() Цикл
			...
			Пока ВыборкаСерийныйНомер.Следующий() Цикл
				...
				Если ВыборкаНомерСтроки.Количество() = 1 Тогда
					Продолжить;
				КонецЕсли;
...
//  Отказ - Булево - переменная отвечающая за прерывание операции.
//  ТекстОшибки - Строка - текст возвращаемой ошибки.
//  Дата - Дата - дата проверки документа.
//
...
//  Движения - движения документа для отражения в регистре "Действия скидок наценок".
...
// Добавляет в движения документа данные для регистра РегистрацияЗаменыКартПокупателей.
//
// Параметры:
//  ДополнительныеСвойства - Структура - структура дополнительных свойств для формирования движений.
//  Движения - движения документа для отражения в регистре "Регистрация замены карт покупателей".
//  Отказ - Булево - признак отказа от выполнения операции.
//
...
#Область ПроцедурыОтчетыПоМаркетинговымОперациям

// Возвращает количество недель в интервале.
//
// Параметры:
//  ДатаНачалаИнтервала -Дата - Дата начала интервала.
//  ДатаКонцаИнтервала - Дата - Дата конца интервала.
//
// Возвращаемое значение:
...
//  Интервал - Перечисления.Периодичность - интервал отчета по диаграмме.
//
...
//
// Параметры:
//  Интервал - Перечисления.Периодичность - интервал диаграммы.
//
// Возвращаемое значение:
//  Строка - описание интервала диаграммы.
//
Функция ОписаниеИнтервалаДиаграммы(Интервал) Экспорт 
...
//
// Параметры:
//  ВходящиеПараметры - Структура - структура с полями:
//  	* НачалоПериода   - Дата - Начало отчета.
//  	* КонецПериода    - Дата - Окончание отчета.
//  	* ДатаНачалаАкции - Дата - Начало акции.
//  	* ДатаКонцаАкции  - Дата - Окончание акции.
//  Интервал - Перечисления.Периодичность - интервал на котором рассчитывается количество периодов.
//  КоличествоПериодовДоАкции - Число - определяемое количество периодов до акции.
//  КоличествоПериодовАкции - Число - определяемое количество периодов акции.
//  КоличествоПериодовПослеАкции - Число - определяемое количество периодов после акции.
//  ОписаниеПериодаПриведения - Строка - Описание периода усреднения.
//
...
//
// Параметры:
//  МаркетинговаяАкция - ДокументСсылка.МаркетинговыеАкции - Документ маркетинговая акция.
//
// Возвращаемое значение:
//  ТаблицаЗначений - список сегментов акции.
//
...
// Возвращает параметр компоновки данных по имени.
//
// Параметры:
//  КомпоновщикНастроек - КомпоновщикНастроекКомпоновкиДанных - компоновщик настроек компоновки данных.
//  ИмяПараметра - Строка - наименование параметра для поиска
//
// Возвращаемые параметры:
//  ПолеКомпоновкиДанных - параметр компоновки данных по имени.
//
...
Процедура УстановитьОтборВКомпоновщикеПоСписку(КомпоновщикНастроек, ИмяПоля, СписокЗначенийОтбора, ОтключатьОтборПриПустомСписке = Истина) Экспорт
	...
	Если НайденЭлементОтбора Тогда
		...
		Если НЕ ЭлементПользовательскогоОтбораПоля = Неопределено Тогда
			ЗаполнитьЗначенияСвойств(ЭлементПользовательскогоОтбораПоля, ЭлементОтбораПоля, "ВидСравнения, Использование, ПравоеЗначение");
		КонецЕсли;
...
//  КомпоновщикНастроек - КомпоновщикНастроекКомпоновкиДанных - Компоновщик Настроек отчета.
//  МаркетинговаяАкция - ДокументСсылка.МаркетинговыеАкции - Документ маркетинговая акция.
//  Периоды - Структура - структура периодов сегментов.
//
...
// Параметры:
//  КомпоновщикНастроек - КомпоновщикНастроекКомпоновкиДанных - Компоновщик Настроек отчета.
//  ОтборИсточник - Отбор - отбор источник заполнения.
//
...
// Параметры:
//  КомпоновщикНастроек - КомпоновщикНастроекКомпоновкиДанных - Компоновщик Настроек отчета.
//  ПараметрыДанныхИсточника - Массив - параметры данных.
//
...
// Подбирает серийный номер по введенному тексту.
//
// Параметры:
//  Текст - Строка - введенный текст
//  МассивИспользуемыхСерийныхНомеров - Массив - выбранные серийные номера.
//  Номенклатура - Номенклатура.Ссылка - номенклатура поиска.
//  ТипСерийногоНомера - Перечисления.ТипыСерийныхНомеровСертификатов - тип серийного номера номенклатуры.
//
// ВозвращаемоеЗначение
//  Массив - массив серийных номеров.
//
Функция СерийныеНомераПоТексту(Текст, МассивИспользуемыхСерийныхНомеров, Номенклатура = Неопределено, ТипСерийногоНомера = Неопределено) Экспорт
...
// Обрабатывает серийные номера и открывает выборку номера.
//
// Параметры:
//  СтруктураПараметров - Структура - структура параметров серийных номеров.
//  Текст - Строка - введенный текст.
//  СтандартнаяОбработка - Булево - флаг стандартной обработки выбора. Здесь сбрасывается.
//
// Возвращаемое значение:
//  Массив - массив серийных номеров.
//
Функция ОбработатьВведенныйТекстСерийногоНомера(СтруктураПараметров, Текст, СтандартнаяОбработка) Экспорт
	...
	МассивСерийныхНомеров = МаркетинговыеАкцииСерверПривилегированный.МассивСерийныхНомеров(СтруктураПараметров, Текст);
	
	Если МассивСерийныхНомеров.Количество() = 0 Тогда
		Возврат Неопределено;
	ИначеЕсли МассивСерийныхНомеров.Количество() = 1 Тогда
		Возврат ДанныеПодарочногоСертификата(МассивСерийныхНомеров[0]);
...
//
// Параметры:
//  Текст - Строка - введенный текст, по которому подбирается серийный номер.
//  ТекстАвтоПодбора - Строка - текст авто подбора.
//  МассивИспользуемыхСерийныхНомеров - Массив - массив выбранных серийных номеров.
//  Номенклатура - СправочникСсылка.Номенклатура - номенклатура поиска.
//  ТипСерийногоНомера - Перечисления.ТипыСерийныхНомеровСертификатов - тип серийного номера номенклатуры.
//
// ВозвращаемоеЗначение:
//  Массив - массив серийных номеров по тексту.
//
