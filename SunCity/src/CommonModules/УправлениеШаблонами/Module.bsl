////////////////////////////////////////////////////////////////////////////////
// УправлениеШаблонами содержит процедуры и функции для работы 
// с шаблонами и обработки действий пользователя с щаблонами.
//  
////////////////////////////////////////////////////////////////////////////////
...
// Вставляет в текстовый макет параметры области.
//
// Параметры:
...
//  Компоновщик - КомпоновщикНастроекКомпоновкиДанных - компоновщик настроек.
//  ИдентификаторыПолейСКД - Структура - идентификаторы полей СКД.
//  ИмяМакета - Строка - наименование макета.
//
// Возвращаемое значение:
//  ДеревоЗначений - параметры области в текстовом макете.
//
...
// Производит заполнение структуры параметров по строке табличной части.
//
// Параметры:
...
Процедура ЗаполнитьСтруктуруПараметровШаблонаПоСтрокеТЧ(СтрокаТаблицыЗначений, Структура, ШаблонЧека) Экспорт
	...
	Для каждого ЗначениеКолонки Из СтрокаТаблицыЗначений Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Заполнение структуры параметров шаблона'"),
...
// Заполняет структуру параметров значениями составных строк.
// Рекурсивная.
//
// Параметры:
	...
	Для каждого СтрокаТЗ Из ТЗ Цикл
		...
		Для каждого Строка Из СтрокаТЗ.СоставнаяСтрока.Строки Цикл
			...
			Если Строка.ТипЭлемента = "СтрокаТекста" И Строка.Вычислять Тогда
				Попытка
					Значение = ОбщегоНазначения.ВычислитьВБезопасномРежиме(СтрЗаменить(СтрЗаменить(СтрЗаменить(Строка.Элемент,".",""),"%",""),"Структура","Структура."));
				Исключение
...
// Заполняет структуру параметров, которые заданы пользователем
// или являются составными строками. Рекурсивная.
//
// Параметры:
//  Шаблон - ДеревоЗначений - Шаблон. В строках шаблона могут находиться параметры.
...
//
Функция ИдентификаторыПользовательскихПолей(Шаблон, Структура = Неопределено) Экспорт
	...
	Для каждого Строка Из Шаблон.Строки Цикл
		...
		Если ТипЭлемента = "Таблица"
			...
			ИдентификаторыПользовательскихПолей(Строка, Структура);
		КонецЕсли;
...
// Выполняет преобразование шаблона (строки дерева) в массив текстовых строк, заполненный по структуре
// параметров.
//
// Параметры:
//  СтрокаДерева - СтрокаДереваЗначений - строка шаблона
//  СтруктураПараметров - Структура - параметры, которые необходимо подставить в макет шаблона.
//
// Возвращаемое значение:
//  Массив - результирующий массив текстовых строк, полученный по макету для строки дерева.
//
Функция МассивСтрокДляСтрокиДереваШаблона(СтрокаДерева, СтруктураПараметров) Экспорт
...
Процедура ОбработатьСоставЧека(ПараметрыЧека, СтрокаОсновногоРаздела, ТаблицаДанных, ФискальныеПозицииЧека = Неопределено) Экспорт
	...
	ИначеЕсли СтрокаОсновногоРаздела.ИмяКолонки = "ТаблицаТовары" Тогда
		...
		Пока ИндексТекущейПозиции <= ИндексПоследнейПозиции Цикл
			...
			Если СтрокаВыборки.Свойство("ФискальнаяСтрока") Тогда
				Для Каждого ПоляСтрокиТаблицы Из СтрокаОсновногоРаздела.Строки Цикл
			...
			Иначе
				ПараметрыЧека.ПозицииЧека.Добавить(СтрокаВыборки);
			КонецЕсли;
...
//Проверяет тип шаблона объекта.
//
// Параметры:
//  Объект - ЛюбойОбъект - объект для проверки шаблона.
//  ИмяШаблона - Строка - имя шаблона строкой.
//  ПроверяемыйТип - Проивзвольный - проверямый тип шаблона.
...
Процедура ВТаблицеВычисляемыхПолейЗаполнитьДополнительные(ТаблицаВычисляемыхПолей)
	...
	Для Каждого Стр Из ТаблицаВычисляемыхПолей Цикл
		...
		ИначеЕсли ВРЕГ(Стр.Поле) = ВРЕГ("СистемныеПоля.ТекущаяДата") Тогда
			Стр.Значение = Формат(текДата, НСтр("ru='ДФ=dd.MM.yyyy'"));
			
		ИначеЕсли ВРЕГ(Стр.Поле) = ВРЕГ("СистемныеПоля.ТекущееВремя") Тогда
			Стр.Значение = Формат(текДата, НСтр("ru='ДФ=ЧЧ:мм:сс'"));
...
Процедура ОбработатьТекстовуюСтроку(ПараметрыЧека, ПоляСтроки, СтрокаШапкиЧека, ТаблицаДанных, СтрокаДанных = Неопределено, ИмяРаздела = "СоставЧека")
	...
	Если ЗначениеЗаполнено(ЗначениеСтроки)
		...
		ТекстСтроки = ПоляСтроки.Префикс
					...
					+ ПоляСтроки.Постфикс;
		СтрокаПозицииЧека = МенеджерОборудованияКлиентСервер.ПараметрыТекстовойСтрокиЧека(ТекстСтроки);
...
Процедура ОбработатьДанныеШтрихкода(ПараметрыЧека, ПоляСтроки, СтрокаШапкиЧека, ТаблицаДанных, СтрокаДанных = Неопределено, ИмяРаздела = "СоставЧека")
	...
	Если ЗначениеЗаполнено(ЗначениеСтроки)
		...
		Если ЗначениеЗаполнено(ПоляСтроки.Префикс) Тогда
			СтрокаПозицииЧека = МенеджерОборудованияКлиентСервер.ПараметрыТекстовойСтрокиЧека(ПоляСтроки.Префикс);
			ПараметрыЧека.ПозицииЧека.Добавить(СтрокаПозицииЧека);
		КонецЕсли;
	
		СтрокаПозицииЧека = МенеджерОборудованияКлиентСервер.ПараметрыШтрихкодВСтрокеЧека(
			ТипШтрихкодаРТ, 
			ЗначениеСтроки, 
			МенеджерОборудованияКлиентСервер.ШтрихкодВBase64(ЗначениеСтроки));
		ПараметрыЧека.ПозицииЧека.Добавить(СтрокаПозицииЧека);
		
		Если ЗначениеЗаполнено(ПоляСтроки.ПостФикс) Тогда
			СтрокаПозицииЧека = МенеджерОборудованияКлиентСервер.ПараметрыТекстовойСтрокиЧека(ПоляСтроки.ПостФикс);
			ПараметрыЧека.ПозицииЧека.Добавить(СтрокаПозицииЧека);
		КонецЕсли;
