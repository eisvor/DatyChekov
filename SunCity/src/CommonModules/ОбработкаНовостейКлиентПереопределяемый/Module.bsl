...
Процедура ОбработкаНажатияВТекстеНовости(
	...
	ГдеМаркерОтключения = СтрНайти(ВРег(ДанныеСобытия.Href), ВРег("news1C:feed."));
	Если ГдеМаркерОтключения > 0 Тогда
	// Формат гиперссылки: news1C:feed.{КодЛентыНовостей}.unsubscribe
	Точка1 = СтрНайти(ДанныеСобытия.Href, ".", НаправлениеПоиска.СНачала, ГдеМаркерОтключения, 1);
	Точка2 = СтрНайти(ДанныеСобытия.Href, ".", НаправлениеПоиска.СНачала, ГдеМаркерОтключения, 2);
	Если (Точка1 > 0) И (Точка2 > 0) И (Точка2 > Точка1) Тогда
		КодЛентыНовостей = Сред(ДанныеСобытия.Href, Точка1+1, Точка2-Точка1-1);
		ОбработкаНовостейВызовСервера.ИзменитьПользовательскуюВидимостьЛентНовостей(
			ОбработкаНовостейВызовСервера.ПолучитьЛентуНовостейПоКоду(КодЛентыНовостей), // Найти ленту новостей по коду
			Ложь, // Отключить
			Неопределено); // Для текущего пользователя.
			
		КодОсновногоЯзыка = ОбщегоНазначенияКлиент.КодОсновногоЯзыка();
		ПоказатьОповещениеПользователя(
			НСтр("ru='Отключение ленты новостей'", КодОсновногоЯзыка),
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Лента новостей с кодом [%1] отключена.'", КодОсновногоЯзыка),
				КодЛентыНовостей));
		СтандартнаяОбработкаПлатформой = Ложь;
		СтандартнаяОбработкаПодсистемой = Ложь;
	КонецЕсли;
