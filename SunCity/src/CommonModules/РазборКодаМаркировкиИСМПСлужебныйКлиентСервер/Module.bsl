...
Функция ДлинаСерийногоНомераКодаМаркировки(ВидПродукции, Шаблон = Неопределено) Экспорт
	...
	ИначеЕсли ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.МолочнаяПродукцияБезВЕТИС")
		...
		Или ВидПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ИгрыИИгрушкиДляДетей")
			И Шаблон = ПредопределенноеЗначение("Перечисление.ШаблоныКодовМаркировкиСУЗ.ИгрыИИгрушкиДляДетей") Тогда
		Результат = 6;
	Иначе
		Результат = 13;
	КонецЕсли;
