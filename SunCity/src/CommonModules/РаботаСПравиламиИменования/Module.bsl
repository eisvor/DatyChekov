////////////////////////////////////////////////////////////////////////////////
// РаботаСПравиламиИменования содержит процедуры и функции для работы 
...
//
Процедура УстановитьПараметрыВыбораПравилаИменования(НаборСвойств, ЭлементПравилоИменования, ВыполнятьПроверкуПравила = Истина) Экспорт
	
	Если НЕ РаботаСПравиламиИменованияВызовСервера.ИспользоватьПравилаИменования() Тогда
		Возврат;
	КонецЕсли;
	...
	ТаблицаДоступныхПараметров = ДоступныеПараметры();
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеРеквизитыИСведения") Тогда
		...
		Пока Выборка.Следующий() Цикл
			Если ВыполнятьПроверкуПравила Тогда
				РезультатПроверкиПравила = ПроверитьИспользованиеПравилаИменования(
					Выборка.ПравилоИменования, ТаблицаДоступныхПараметров);
			КонецЕсли;
			
			Если НЕ ВыполнятьПроверкуПравила ИЛИ НЕ РезультатПроверкиПравила.ЕстьОшибки Тогда 
		...
		МассивПараметровВыбора.Добавить(
			Новый ПараметрВыбора("Отбор.Ссылка", Новый ФиксированныйМассив(МассивДоступныхПравилИменования)));
...
//  ДеревоЗначений - дерево реквизитов и сведений.
//
Функция ДеревоРеквизитовИСведений(НаборыСвойств, ТаблицаДоступныхПараметров) Экспорт
	
	ТаблицаДоступныхПараметров.Очистить();
	
	Дерево = ПустоеДеревоОператоров();
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Если ГруппаОператоров <> Неопределено Тогда
					ДобавитьОператор(
						Дерево,
						ГруппаОператоров,
						ВыборкаДетальныеЗаписи.ИндексКартинки,
						ВыборкаДетальныеЗаписи.ИмяПараметра,
						ВыборкаДетальныеЗаписи.ПутьКДанным,ВыборкаДетальныеЗаписи.НаборСвойств);
				КонецЕсли;
...
Процедура ДобавитьЭлементыПравилаИменования(Форма,ГруппаПараметровИменования,ГруппаДекорацийПараметровИменования = Неопределено) Экспорт
	...
	Для Каждого Строка Из Форма.ПараметрыПравилаИменования Цикл
		...
		НовыйЭлемент.Формат = Формат("ДФ=dd.MM.yyyy; БЛ=Нет; БИ=Да");
		НовыйЭлемент.ФорматРедактирования = Формат("ДФ=dd.MM.yyyy; БЛ=Нет; БИ=Да");
		...
		Если ГруппаДекорацийПараметровИменования <> Неопределено Тогда
			ДекорацияПараметрИменования = Форма.Элементы.Добавить("Декорация" + Строка.ИмяЭлемента,
				Тип("ДекорацияФормы"), ГруппаДекорацийПараметровИменования);
...
//
// Возвращаемое значение:
//  Структура - структура информации о параметрах правила именования.
//
Функция СтруктураПараметровПравилаИменования(ВидНоменклатуры, ОписаниеДополнительныхРеквизитов = Неопределено, ПропускатьВидНоменклатуры = Ложь) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВТ_ПараметрыПравила.НомерСтроки,
...
//
// Возвращаемое значение:
//  Структура - поиска позиций номенклатур.
//
Функция СтруктураПоискаНоменклатуры(Форма, ЭтоПоиск = Ложь) Экспорт
...
//
// Возвращаемое значение:
//  ТаблицаЗначений - найденные позиции номенклатуры.
//
...
Процедура ЗаполнитьИменаДополнительныхРеквизитовИСведений() Экспорт
	...
	Попытка
		...
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("Справочник.НаборыДополнительныхРеквизитовИСведений");
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
...
//
// Возвращаемое значение:
//  РезультатЗапроса - параметры именования номенклатуры.
//
Функция ЗначенияПараметровНоменклатуры(ВидНоменклатуры, ПараметрыПравилаИменования, Номенклатура = Неопределено) Экспорт
...
//
// Возвращаемое значение:
//  РезультатЗапроса - списки выбора параметров именования номенклатуры.
//
Функция СпискиВыбораПараметровПоиска(Форма, ВидНоменклатуры) Экспорт
...
//
// Параметры:
//	Источник - ПланВидовХарактеристикОбъект.ДополнительныеРеквизитыИСведения - планы видов характеристик для контроля.
...
//
// Возвращаемое значение:
//  Булево - признак использования номенклатуры по правилу.
//
Функция ИспользоватьИменованиеНоменклатурыПоПравилу(ВидНоменклатуры,Номенклатура = Неопределено) Экспорт
	
	ПравилоИменования = ВидНоменклатуры.ПравилоИменования;
	
	Если Не (ПолучитьФункциональнуюОпцию("ИспользоватьПравилаИменованияНоменклатуры")
			И ЗначениеЗаполнено(ПравилоИменования)) Тогда
		Возврат Ложь;
	КонецЕсли;
...
//	ТаблицаДоступныхПараметров - ТаблицаЗначений - см. РаботаСПравиламиИменования.ДоступныеПараметры 
...
//
// Возвращаемое значение:
//  Структура - структура проверки и использования правил именования.
//
Функция ПроверитьИспользованиеПравилаИменования(ПравилоИменования,ТаблицаДоступныхПараметров = Неопределено,НаборСвойств = Неопределено) Экспорт
	...
	СтруктураПравила = РаботаСПравиламиИменованияКлиентСервер.ПолучитьСтруктуруПравилаИменования(
		ТекущееПравило, ПравилоИменования.НаборыСвойств, ТаблицаДоступныхПараметров);
...
//
// Возвращаемое значение:
//  ТаблицаЗначений - таблица доступных параметров.
//
Функция ДоступныеПараметры() Экспорт
	
	ТаблицаДоступныхПараметров = Новый ТаблицаЗначений;
	ТаблицаДоступныхПараметров.Колонки.Добавить("ИмяПараметра",
		Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(100)));
	ТаблицаДоступныхПараметров.Колонки.Добавить("ПутьКДанным",
		Новый ОписаниеТипов("Строка",,,,Новый КвалификаторыСтроки(300)));
	ТаблицаДоступныхПараметров.Колонки.Добавить("НаборСвойств",
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеРеквизитыИСведения") Тогда
		...
		|ПОМЕСТИТЬ НаборыДополнительныхРеквизитовИСведений
		|ИЗ
		|	Справочник.НаборыДополнительныхРеквизитовИСведений КАК НаборыДополнительныхРеквизитовИСведений
		|ГДЕ
		|	НЕ НаборыДополнительныхРеквизитовИСведений.ПометкаУдаления
		|	И НаборыДополнительныхРеквизитовИСведений.Родитель = &НаборРеквизитовИСведений
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Ссылка
		|;
		...
		|	Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеСведения КАК НаборыДополнительныхРеквизитовИСведенийДополнительныеСведения
		|ГДЕ
		|	НаборыДополнительныхРеквизитовИСведенийДополнительныеСведения.Ссылка В
		|			(ВЫБРАТЬ
		|				НаборыДополнительныхРеквизитовИСведений.Ссылка
		|			ИЗ
		|				НаборыДополнительныхРеквизитовИСведений)
		...
		Запрос.УстановитьПараметр("НаборРеквизитовИСведений",
			Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_Номенклатура);
...
// 
Функция ПустоеДеревоОператоров()
...
Функция РеквизитИспользуется(ИмяРеквизита)
	...
	Если ИмяРеквизита = "НаименованиеПолное"
		...
		Или ИмяРеквизита = "ФайлКартинки"
		Или ИмяРеквизита = "КодВидаНоменклатурнойКлассификации" Тогда
		Возврат Ложь;
	ИначеЕсли Лев(ИмяРеквизита,7) = "Удалить" Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ИмяРеквизита = "ДоговорКонтрагента"
