...
// Определяет наличие валютного учета в конфигурации.
//
...
// Возвращает значение валюты розничных продаж, валюты регламентированного учета.
//
// Возвращаемое значение:
//  СправочникСсылка.Валюты, Неопределено - значение валюты продаж.
//
...
// Возвращает сформированную структуру данных о текущем объекте ценообразования.
//
// Возвращаемое значение:
//  Структура - структура данных  о текущем объекте ценообразования, содержит параметры:
//				*ИмяМетаданныхОбъекта - Строка - имя объекта метаданных.
//				*Ссылка - ЛюбаяСсылка - объект ценообразования.
//				*ПредставлениеОбъекта - Строка - строковое представление объекта метаданных.
//
...
// Обработчик события "Обработка заполнения" документа "Установка цен номенклатуры".
//
// Параметры: 
//  Объект - ДокументОбъект.УстановкаЦенНоменклатуры - документ для обработки заполнения.
//  ДанныеЗаполнения - Структура, ДокументСсылка - данные к заполнению.
//  СтандартнаяОбработка - Булево - признак стандартной обработки данных.
//
		...
		Если ДанныеЗаполнения.Свойство("Ссылка") Тогда
			...
			Если ТипЗнч(ДанныеЗаполнения.Ссылка) = Тип("ДокументСсылка.ПоступлениеТоваров") Тогда
				...
				Если ДанныеЗаполнения.Действие = "УстановкаЦенПоИзмененнымЦенам" Тогда
					...
					|////////////////////////////////////////////////////////////////////////////////
					|ВЫБРАТЬ РАЗРЕШЕННЫЕ
					|	ПравилаЦенообразования.ВидЦен КАК ВидЦены
					|ПОМЕСТИТЬ ВидыЦенПоПравиламЦенообразования
					|ИЗ
					...
					|ГДЕ
					|	ПравилаЦенообразованияДополнительныеВидыЦен.Ссылка = &ПравилоЦенообразования
					|;
					|
					|////////////////////////////////////////////////////////////////////////////////
					|ВЫБРАТЬ РАЗРЕШЕННЫЕ
					|	ВидыЦенПоПравиламЦенообразования.ВидЦены КАК ВидЦены
					|ПОМЕСТИТЬ ВидыЦенДляУстановкиЦен
					|ИЗ
					|	ВидыЦенПоПравиламЦенообразования КАК ВидыЦенПоПравиламЦенообразования
					|
					|ОБЪЕДИНИТЬ ВСЕ
					|
					|ВЫБРАТЬ РАЗЛИЧНЫЕ
					|	ВидыЦенВлияющиеВидыЦен.ВлияющийВидЦен
					|ИЗ
					|	Справочник.ВидыЦен.ВлияющиеВидыЦен КАК ВидыЦенВлияющиеВидыЦен
					|ГДЕ
					|	ВидыЦенВлияющиеВидыЦен.Ссылка В
					|			(ВЫБРАТЬ
					|				ВидыЦенПоПравиламЦенообразования.ВидЦены
					|			ИЗ
					|				ВидыЦенПоПравиламЦенообразования КАК ВидыЦенПоПравиламЦенообразования)
					|;
					|
					...
					|	ВТ_ТаблицаУстановкиЦен.Упаковка КАК Упаковка,
					|	ВТ_ТаблицаУстановкиЦен.ВидЦены КАК ВидЦены
					|ИЗ
					...
					Запрос.УстановитьПараметр("ПравилоЦенообразования", ДанныеЗаполнения.Ссылка.Магазин.ПравилоЦенообразования);
					
					РезультатыЗапроса = Запрос.ВыполнитьПакет();
					ТаблицаТоваров    = РезультатыЗапроса[6].Выгрузить();
					ТаблицаВидовЦен   = РезультатыЗапроса[7].Выгрузить();
				...
				ИначеЕсли ДанныеЗаполнения.Действие = "УстановкаЦенПоЦенаМагазина" Тогда 
					...
					|ИЗ
					|	ВидыЦенДляУстановкиЦенПриПоступлении КАК ВидыЦенДляУстановкиЦен
					...
					РезультатыЗапроса = Запрос.ВыполнитьПакет();
					ТаблицаТоваров    = РезультатыЗапроса[4].Выгрузить();
					ТаблицаВидовЦен   = РезультатыЗапроса[5].Выгрузить();
...
Процедура ОбработкаЗаполненияПримененияЦенНоменклатуры(Объект, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Объект.ОбъектЦенообразования  = ЗначениеНастроекПовтИсп.МагазинПоУмолчанию(Объект.ОбъектЦенообразования);	
...
//
// Параметры:
//  ЭтаФорма - УправляемаяФорма - форма с которой вызывается функция.
//  Объект - ОбработкаОбъект - обработка с данными для заполнения.
//
// Возвращаемое значение:
