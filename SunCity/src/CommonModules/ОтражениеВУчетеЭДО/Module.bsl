...
Процедура ОтразитьДанныеЭлектронногоДокументаСКонтролем(ДанныеДокумента, СпособОбработки = Неопределено) Экспорт
	...
	Если ПроверитьДокументНаПеремещениеТоваров(ДанныеДокумента) Тогда
		Возврат;
	КонецЕсли;
...
Функция ПроверитьДокументНаПеремещениеТоваров(ДанныеДокумента)
	...
	|ГДЕ
	|	ЭлектронныйДокументИсходящийЭДО.ИдентификаторСвязи = &ИдентификаторСвязи
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент = ВТИсходящийЭД.Ссылка
	|ИТОГИ ПО
	|	ЭлектронныйДокумент";
	
	Запрос.УстановитьПараметр("ИдентификаторСвязи", ДанныеДокумента.ЭлектронныйДокумент.ИдентификаторСвязи);
	РезультатЗапроса = Запрос.Выполнить();
	
	ВыборкаЭД = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
	
	СсылкаНаПеремещение = Документы.ПеремещениеТоваров.ПустаяСсылка();
	СсылкаНаСчетФактуруВыданную = Документы.СчетФактураПолученный.ПустаяСсылка();
	РТ_ЭтоСчетФактураВыданная = Ложь;
	РТ_ЭтоПеремещениеТоваров = Ложь;
	
	Пока ВыборкаЭД.Следующий() Цикл
		ТекЭД = ВыборкаЭД.ЭлектронныйДокумент;
		ВыборкаОбъектУчета = ВыборкаЭД.Выбрать();
		Пока ВыборкаОбъектУчета.Следующий() Цикл
			
			Если ТипЗнч(ВыборкаОбъектУчета.ОбъектУчета) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
				РТ_ЭтоПеремещениеТоваров = Истина;
				СсылкаНаПеремещение = ВыборкаОбъектУчета.ОбъектУчета;
			КонецЕсли;
			
			Если ТипЗнч(ВыборкаОбъектУчета.ОбъектУчета) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
				ОснованиеСчетФактуры = ВыборкаОбъектУчета.ОбъектУчета.ДокументОснование;
				Если ТипЗнч(ОснованиеСчетФактуры) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
					РТ_ЭтоСчетФактураВыданная = Истина;
					СсылкаНаСчетФактуруВыданную = ВыборкаОбъектУчета.ОбъектУчета;
					Если Не ЗначениеЗаполнено(СсылкаНаПеремещение) Тогда
						СсылкаНаПеремещение = ОснованиеСчетФактуры;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
	...
	Если РТ_ЭтоПеремещениеТоваров ИЛИ РТ_ЭтоСчетФактураВыданная Тогда
		ДанныеДокумента.ОтключитьКонтрольОтраженияВУчете = Истина;
		Результат = Истина;
		Если ТипЗнч(ДанныеДокумента.ЭлектронныйДокумент) = Тип("ДокументСсылка.ЭлектронныйДокументВходящийЭДО") Тогда
			НачатьТранзакцию();
			Попытка
				
				Если РТ_ЭтоПеремещениеТоваров Тогда
					
					МенеджерЗаписи = РегистрыСведений.ОбъектыУчетаДокументовЭДО.СоздатьМенеджерЗаписи();
					МенеджерЗаписи.ЭлектронныйДокумент = ДанныеДокумента.ЭлектронныйДокумент;
					МенеджерЗаписи.ОбъектУчета = СсылкаНаПеремещение;
					МенеджерЗаписи.Записать(Истина);
					
					ВладелецНоменклатуры = СсылкаНаПеремещение.Организация.СобственныйКонтрагент;
					Для Каждого СтрокаТовара Из СсылкаНаПеремещение.Товары Цикл
						
						НоменклатураИБ = СопоставлениеНоменклатурыКонтрагентовКлиентСервер.НоваяНоменклатураИнформационнойБазы();
						ЗаполнитьЗначенияСвойств(НоменклатураИБ, СтрокаТовара);
						ИдентификаторДляСопоставления = СопоставлениеНоменклатурыКонтрагентовКлиентСервер.ИдентификаторНоменклатурыКонтрагентаПоНоменклатуреИБ(НоменклатураИБ);
						...
						СопоставлениеНоменклатурыКонтрагентов.УстановитьСоответствиеНоменклатуры(ОписаниеНоменклатуры, НоменклатураИБ);
					КонецЦикла;
					
				КонецЕсли;
				
				Если РТ_ЭтоСчетФактураВыданная Тогда
					
					СтруктураРеквизитов = Новый Структура("Номер,Дата,Исправление,НомерИсправления");
					РеквизитыСФВыданной = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаСчетФактуруВыданную, СтруктураРеквизитов);
					
					ДанныеСчетаФактуры = ЗакупкиСервер.ДанныеСчетаФактурыСтруктурой(
					СсылкаНаПеремещение,
					СсылкаНаПеремещение.ОрганизацияПолучатель,
					СсылкаНаПеремещение.Организация,
					РеквизитыСФВыданной.Номер,
					РеквизитыСФВыданной.Дата);
					ДанныеСчетаФактуры.Вставить("СФВыданный", СсылкаНаСчетФактуруВыданную);
					
					СчетаФактурыПолученные = Документы.СчетФактураПолученный.СчетаФактурыПоОснованию(СсылкаНаПеремещение,
					СсылкаНаПеремещение.ОрганизацияПолучатель,,,Истина);
					КоличествоСФПолученных = СчетаФактурыПолученные.Количество();
					
					РазличаетсяНомерИсправления = Ложь;
					Если РеквизитыСФВыданной.Исправление Тогда
						СчетаФактурыПолученныеИсправления = Документы.СчетФактураПолученный.СчетаФактурыПоОснованию(СсылкаНаПеремещение,
						СсылкаНаПеремещение.ОрганизацияПолучатель);
						Если СчетаФактурыПолученныеИсправления.Количество() > 0 Тогда
							РазличаетсяНомерИсправления = СокрЛП(СчетаФактурыПолученныеИсправления[0].НомерИсправления) <> СокрЛП(СсылкаНаСчетФактуруВыданную.НомерИсправления);
							Если Не РазличаетсяНомерИсправления Тогда
								СчетФактураПолученный = СчетаФактурыПолученныеИсправления[0].Ссылка;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
					
					Если КоличествоСФПолученных = 0 ИЛИ РазличаетсяНомерИсправления Тогда
						СчетФактураПолученныйОбъект = Документы.СчетФактураПолученный.СоздатьДокумент();
						СчетФактураПолученныйОбъект.ПолученВЭлектронномВиде = Истина;
						СчетФактураПолученныйОбъект.Заполнить(ДанныеСчетаФактуры);
						Если РеквизитыСФВыданной.Исправление Тогда
							СчетФактураПолученныйОбъект.Исправление = Истина;
							СчетФактураПолученныйОбъект.НомерИсправления = РеквизитыСФВыданной.НомерИсправления;
							СчетФактураПолученныйОбъект.ДатаИсправления = РеквизитыСФВыданной.Дата;
							Если КоличествоСФПолученных > 0 Тогда
								СчетФактураПолученныйОбъект.СчетФактураОснование = СчетаФактурыПолученные[0].Ссылка;
							КонецЕсли;
						КонецЕсли;
						СчетФактураПолученныйОбъект.Записать(РежимЗаписиДокумента.Проведение);
						СчетФактураПолученный = СчетФактураПолученныйОбъект.Ссылка;
					ИначеЕсли Не ЗначениеЗаполнено(СчетФактураПолученный) Тогда
						СчетФактураПолученный = СчетаФактурыПолученные[0].Ссылка;
					КонецЕсли;
					
					Если ДанныеДокумента.ВидДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.УПД
						И НЕ РТ_ЭтоПеремещениеТоваров 
						И ЗначениеЗаполнено(СсылкаНаПеремещение) Тогда
					...
					МенеджерЗаписи = РегистрыСведений.ОбъектыУчетаДокументовЭДО.СоздатьМенеджерЗаписи();
					МенеджерЗаписи.ЭлектронныйДокумент = ДанныеДокумента.ЭлектронныйДокумент;
					МенеджерЗаписи.ОбъектУчета = СчетФактураПолученный;
					МенеджерЗаписи.Записать(Истина);
