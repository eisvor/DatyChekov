...
Процедура АвторизацияПользователяЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТипЗнч(Результат[Организация]) = Тип("Строка") Тогда
		...
		Если ДополнительныеПараметры.ПараметрыСканирования.РежимБРМК Тогда
			ДополнительныеПараметры.РезультатОбработкиШтрихкода.ДанныеШтрихкода.ТекстОшибки = Результат[Организация];
			ДополнительныеПараметры.РезультатОбработкиШтрихкода.ТекстОшибки                 = Результат[Организация];
			ДополнительныеПараметры.РезультатОбработкиШтрихкода.ЕстьОшибки                  = Истина;
			ОповещениеЗавершения = Новый ОписаниеОповещения(ДополнительныеПараметры.ЗавершениеОбработки, ДополнительныеПараметры.Форма);
			ВыполнитьОбработкуОповещения(ОповещениеЗавершения, ДополнительныеПараметры.РезультатОбработкиШтрихкода.ДанныеШтрихкода);
		Иначе
