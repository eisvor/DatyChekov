...
Процедура ЗакончитьОжиданиеОбработкиЗаявкиНаПодключениеЭДО_1СЭДО(Знач РезультатОперации, Знач Контекст) Экспорт
	...
	Иначе
		...
		КонецЕсли;
			ОбщегоНазначенияБЭДКлиент.НачатьВыполнениеОбработкиОповещения(Новый ОписаниеОповещения(
			"ЗакончитьОжиданиеОбработкиЗаявкиНаПодключениеЭДО_1СЭДОПродолжение", ЭтотОбъект, Контекст));
	КонецЕсли;
