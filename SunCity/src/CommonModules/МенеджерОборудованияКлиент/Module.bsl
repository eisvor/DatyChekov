...
Процедура ОбработкаВнешнегоСобытия(Источник, Событие, Данные) Экспорт
	...
	Результат = ОбработатьСобытиеОтУстройства(ОписаниеСобытия, ОписаниеОшибки);
	Если Не Результат Тогда
		...
		Иначе
			...
			Иначе
				...
				Иначе
					...
					ТипЧека = ?(ТипРасчета = 1, Ложь, Истина);
					ПолучательИИН = ?(ВходныеПараметры.Свойство("ПолучательИИН"), Строка(ВходныеПараметры.ПолучательИИН), "");
					ПокупательEmail = ?(ВходныеПараметры.Свойство("ПокупательEmail"), Строка(ВходныеПараметры.ПокупательEmail), "");
					ПокупательНомер = ?(ВходныеПараметры.Свойство("ПокупательНомер"), Строка(ВходныеПараметры.ПокупательНомер), "");
					ВремВходныеПараметры = Новый Массив();
...
Функция ПечатьЧекаПоШаблону(ОбщийМодульОборудования, ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры) Экспорт
	...
	Иначе
		...
		ТипЧека =  ?(ТипРасчета = 1, Ложь, Истина);
		// Открываем чек
		Параметры.Вставить("ПолучательИИН", ?(ВходныеПараметры.Свойство("ПолучательИИН"), Строка(ВходныеПараметры.ПолучательИИН), ""));
		Параметры.Вставить("ПокупательEmail", ?(ВходныеПараметры.Свойство("ПокупательEmail"), Строка(ВходныеПараметры.ПокупательEmail), ""));
		Параметры.Вставить("ПокупательНомер", ?(ВходныеПараметры.Свойство("ПокупательНомер"), Строка(ВходныеПараметры.ПокупательНомер), ""));
	...
	Если Результат Тогда
		...
		Для ИндексМассива = 0 По ПозицииЧека.Количество() - 1 Цикл
			...
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				ИдентификаторВС 	= ?(ПозицияЧека.Свойство("ИдентификаторВС"),	Строка(ПозицияЧека.ИдентификаторВС),	"");
				...
				Если ЗначениеЗаполнено(ИдентификаторВС) Тогда 
					Штрихкод = "";
				КонецЕсли;
				...
				Если ПозицияЧека.ДанныеКодаТоварнойНоменклатуры.Свойство("КодМаркировкиBase64") Тогда
					КодМаркировки = ШтрихкодированиеОбщегоНазначенияИСКлиентСервер.Base64ВШтрихкод(ПозицияЧека.ДанныеКодаТоварнойНоменклатуры.КодМаркировкиBase64);
				...
				Если НЕ ОбщийМодульОборудования.НапечататьФискальнуюСтроку(ОбъектДрайвера, Параметры, ПараметрыПодключения,
									Наименование, Количество, Цена, Сумма, НомерСекции, СтавкаНДС, ИдентификаторВС, Штрихкод, ВыходныеПараметры) Тогда
...
Функция ПолучитьДоступноеККТДляФискализации(РеквизитыЧека, СписокУстройств)
	...
	ОбщегоНазначенияРМККлиентПереопределяемый.ДоступноеККТДляФискализации(РеквизитыЧека, СписокУстройств, ИдентификаторУстройстваККТ, СтандартнаяОбработка);
