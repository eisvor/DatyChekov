...
Функция ПодключитьУстройство(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Если ТипОборудованияИспользуемый = "ККТ" И РевизияИнтерфейса < 2002 Тогда  
		СообщениеПользователю = НСтр("ru='Данный стандарт подключения драйверов не поддерживется. Необходимо обновить драйвер ККТ.'");
		Результат = Ложь;
		СформироватьОшибкуДрайвера(ВыходныеПараметры, "Подключить", СообщениеПользователю);
		Возврат Результат;
	КонецЕсли;
	...
	Если ПараметрыПодключения.ТипОборудования = "ККТ" Тогда
		...
		Попытка
			Если РевизияИнтерфейса >= 3000 Тогда // Версия стандарта 3.0 (ОФД 1.1)
				...
				КонецЕсли;
			Иначе
				Если РевизияИнтерфейса >= 2002 Тогда // Версия стандарта 2.2 (ОФД 1.05)
...
Функция ОтчетБезГашения(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Если ПараметрыПодключения.ТипОборудования = "ККТ" Тогда
		...
		Попытка
			Если ПараметрыПодключения.РевизияИнтерфейса > 2001 Тогда // Версия стандарта 2.2 (ОФД 1.05/1.1)
...
Функция ПечатьТекста(ОбъектДрайвера, Параметры, ПараметрыПодключения, СтрокаТекста, ВыходныеПараметры) Экспорт
	...
	ИначеЕсли ПараметрыПодключения.ТипОборудования = "ККТ" Или ПараметрыПодключения.ТипОборудования = "ПринтерЧеков" Тогда
		...
		//ИнтеграцияИСМПТК
		//Для исключения из печатной строки чека непередаваеых символов XML в составе кодов маркировки
		Если ОбщегоНазначенияИСМПТКВызовСервера.ПроверитьЗначениеОбщейФОМаркировки() Тогда
			СтрокаТекста = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиент.ПреобразоватьСтрокуСНедопустимымиСимволами(СтрокаТекста);
		КонецЕсли;
...
Функция ОткрытьЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТипЧека, ФискальныйЧек, ВыходныеПараметры, ПараметрыОткрытияЧека = Неопределено) Экспорт
	...
	Попытка
		...
		Если ПараметрыПодключения.Свойство("РевизияИнтерфейса") И ПараметрыПодключения.РевизияИнтерфейса >= 2008 Тогда
			...
			ПолучательИИН, ПокупательEmail, ПокупательНомер);
...
Функция ЗакрытьЧек(ОбъектДрайвера, Параметры, ПараметрыПодключения, ТаблицаОплат, ВыходныеПараметры) Экспорт
	...
	Попытка
		Если ПараметрыПодключения.РевизияИнтерфейса >=2009 Тогда
			Ответ = ОбъектДрайвера.ЗакрытьЧек(ПараметрыПодключения.ИДУстройства, СуммаНаличнойОплаты, СуммаБезналичнойОплаты1, СуммаБезналичнойОплаты2, СуммаБезналичнойОплаты3, СуммаБезналичнойОплаты4);
		Иначе
			Ответ = ОбъектДрайвера.ЗакрытьЧек(ПараметрыПодключения.ИДУстройства, СуммаНаличнойОплаты, СуммаБезналичнойОплаты1 + СуммаБезналичнойОплаты4, СуммаБезналичнойОплаты2, СуммаБезналичнойОплаты3);
		КонецЕсли;
		Если НЕ Ответ Тогда
			Результат = Ложь;
			ВыходныеПараметры.Очистить();
...
Функция НапечататьФискальнуюСтроку(ОбъектДрайвера, Параметры, ПараметрыПодключения, Наименование, Количество, Цена, Сумма,
                                   НомерСекции, СтавкаНДС, ИдентификаторВС, Штрихкод, ВыходныеПараметры) Экспорт
	Результат = Истина;
	КодЕдиницыИзмерения =		Строка(Параметры.КодЕдиницыИзмерения);
	КодМаркировки =				Параметры.КодМаркировки;
	ОсвобожденныйОборотНДС =	Параметры.ОсвобожденныйОборотНДС;
	Попытка
		
		КодМаркировки = ?(КодМаркировки = Неопределено, "", КодМаркировки);
		
		Если ПараметрыПодключения.Свойство("РевизияИнтерфейса") И ПараметрыПодключения.РевизияИнтерфейса = 2006 Тогда
		...
		ИначеЕсли ПараметрыПодключения.Свойство("РевизияИнтерфейса") И ПараметрыПодключения.РевизияИнтерфейса >= 2007 Тогда
			...
			Ответ = ОбъектДрайвера.НапечататьФискСтроку(ПараметрыПодключения.ИДУстройства, Наименование, Количество, Цена,
			Сумма, НомерСекции, СтавкаНДС, ОсвобожденныйОборотНДС, КодЕдиницыИзмерения, КодМаркировки, ИдентификаторВС, ШтрихКод);
	...
	Попытка
		...
		Иначе
			Для Каждого СтрокаДанных Из МассивДанных Цикл
				...
				Если Не СтрокаДанных.Свойство("ШтрихкодBase64") Тогда
					СтрокаДанных.Вставить("ШтрихкодBase64"  , МенеджерОборудованияКлиентСервер.ШтрихкодВBase64(СтрокаДанных.Штрихкод));
				ИначеЕсли ПустаяСтрока(СтрокаДанных.ШтрихкодBase64) Тогда
					СтрокаДанных.ШтрихкодBase64 = МенеджерОборудованияКлиентСервер.ШтрихкодВBase64(СтрокаДанных.Штрихкод);
				Иначе
					СтрокаДанных.Штрихкод = МенеджерОборудованияКлиентСервер.Base64ВШтрихкод(СтрокаДанных.ШтрихкодBase64);
				КонецЕсли;
				
				Если Не СтрокаДанных.Свойство("ШтрихкодУпаковкиBase64") Тогда 
					СтрокаДанных.Вставить("ШтрихкодУпаковкиBase64"  , МенеджерОборудованияКлиентСервер.ШтрихкодВBase64(СтрокаДанных.ШтрихкодУпаковки));
				ИначеЕсли ПустаяСтрока(СтрокаДанных.ШтрихкодУпаковкиBase64) Тогда
					СтрокаДанных.ШтрихкодУпаковкиBase64 = МенеджерОборудованияКлиентСервер.ШтрихкодВBase64(СтрокаДанных.ШтрихкодУпаковки);
				Иначе
					СтрокаДанных.ШтрихкодУпаковки = МенеджерОборудованияКлиентСервер.Base64ВШтрихкод(СтрокаДанных.ШтрихкодУпаковкиBase64);
				КонецЕсли;
