////////////////////////////////////////////////////////////////////////////////
// ЗакупкиСервер. Содержит вспомогательные процедуры и функции
//  обработки закупок.
////////////////////////////////////////////////////////////////////////////////
...
// Формирует движения по регистру ЗаказыТоваров.
//
// Параметры:
//  ДополнительныеСвойства - Структура - структура дополнительных свойств для формирования движений.
//  Движения - движения документа для отражения в регистре "Заказы товаров".
...
// Формирует движения по регистру ЗакупкиТоваров.
//
// Параметры:
//  ДополнительныеСвойства - Структура - структура дополнительных свойств для формирования движений.
//  Движения - движения документа для отражения в регистре "Закупки товаров".
//  Отказ - Булево - признак отказа от выполнения операции.
//
...
// Отражает передаваемого поставщика товаров как действующего.
//
// Параметры:
//  Ссылка - СправочникСсылка.Магазины,
//           СправочникСсылка.Контрагенты - поставщик товаров для отражения действующим.
//
...
#Область ПроцедурыИФункцииДляОбслуживанияВзаиморасчетовСПоставщиками

// Обновляет итоговые показатели взаиморасчетов по документу.
//
// Параметры:
//  Объект - ДокументОбъект.ВозвратТоваровПоставщику,
//           ДокументОбъект.ЗаказПоставщику,
//           ДокументОбъект.ОтчетКомитентуОПродажах,
//           ДокументОбъект.ОтчетКомитентуОСписании,
//           ДокументОбъект.ПоступлениеТоваров - документ для обновления итоговых показателей.
//
// Возвращаемое значение:
//  Структура - структура обновленных итоговых показателей взаиморасчетов по документу.
// 
Функция ОбновитьИтоговыеПоказателиВзаиморасчетовПоДокументу(Объект) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(РасчетыСПоставщиками.СуммаПриход, 0) КАК СуммаПриход,
	|	ВЫБОР
	...
	|				ИЛИ ВЫБОР
	|					КОГДА ДокументРасчета ССЫЛКА Документ.ВозвратТоваровПоставщику
	|							И ДокументРасчета.ДокументОснование = &ДокументРасчета
	|						ТОГДА ИСТИНА
	|				КОНЕЦ)
	|				И Магазин = &Магазин
	...
	|ГДЕ
	|	РасчетыСПоставщиками.Регистратор <> РасчетыСПоставщиками.ДокументРасчета
	|	И (РасчетыСПоставщиками.Регистратор ССЫЛКА Документ.РасходныйКассовыйОрдер
	|			ИЛИ РасчетыСПоставщиками.Регистратор ССЫЛКА Документ.РегистрацияБезналичнойОплаты)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЗапросКОплате.КОплате КАК КОплате,
	|	ЗапросКОплате.СуммаПриход - ЕСТЬNULL(Взаимозачет.ОплаченоВзаимозачет, 0) КАК Оплачено,
	...
	|		СУММА(РасчетыСПоставщикамиСумма.СуммаПриход) КАК СуммаПриход
	|	ИЗ
	|		(ВЫБРАТЬ
	|			МАКСИМУМ(РасчетыСПоставщиками.Период) КАК Период
	|		ИЗ
	|			РасчетыСПоставщиками КАК РасчетыСПоставщиками) КАК СрезПоследних
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РасчетыСПоставщиками КАК РасчетыСПоставщиками
	|			ПО СрезПоследних.Период = РасчетыСПоставщиками.Период,
	|		РасчетыСПоставщиками КАК РасчетыСПоставщикамиСумма
	|	
	|	СГРУППИРОВАТЬ ПО
	|		РасчетыСПоставщиками.КОплате) КАК ЗапросКОплате
	...
	|				ИЛИ ВЫБОР
	|					КОГДА РасчетыСПоставщикамиОстатки.ДокументРасчета ССЫЛКА Документ.ВозвратТоваровПоставщику
	|							И РасчетыСПоставщикамиОстатки.ДокументРасчета.ДокументОснование = &ДокументРасчета
	|						ТОГДА ИСТИНА
	|				КОНЕЦ)
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|		ВЫБОР
	|			КОГДА РасчетыСПоставщикамиОстатки.ДокументРасчета ССЫЛКА Документ.ВозвратТоваровПоставщику
	|				ТОГДА РасчетыСПоставщикамиОстатки.ДокументРасчета.ДокументОснование
	|			ИНАЧЕ РасчетыСПоставщикамиОстатки.ДокументРасчета
	...
	|	
	|	ИМЕЮЩИЕ
	|		СУММА(РасчетыСПоставщикамиОстатки.СуммаОстаток) < 0) КАК ВложенныйЗапрос";
...
// Осуществляет подбор взаиморасчет после выбора документа.
//
// Параметры:
//  Объект - ДокументОбъект.ЗаказПоставщику,
//           ДокументОбъект.ПоступлениеТоваров - документ по которому будет осуществлен подбор взаиморасчетов.
//  ВыбранноеЗначение - Структура - структура значений адресов хранений данных в хранилище.
...
// Сортирует ТЧ "Этапы оплаты" по дате
//
// Параметры:
//  Объект - ДокументОбъект.ЗаказПоставщику,
//           ДокументОбъект.ОтчетКомитентуОПродажах,
//           ДокументОбъект.ПоступлениеТоваров - документ в котором необходимо провести 
//                                               сортировку табличной части "Этапы оплаты".
//  Отказ - Булево - признак отказа от выполнения операции.
//
...
// Проверяет ТЧ "Этапы оплаты" на ошибки заполнения.
//
// Параметры:
//  Объект - ДокументОбъект.ЗаказПоставщику,
//           ДокументОбъект.ОтчетКомитентуОПродажах,
//           ДокументОбъект.ПоступлениеТоваров - документ в котором необходимо проверить 
//                                               табличную часть "Этапы оплаты".
//  Отказ - Булево - признак отказа от выполнения операции.
//
Процедура ПроверитьТабличнуюЧастьЭтапыОплат(Объект, Отказ) Экспорт
	...
	Если Объект.ЭтапыОплат.Количество() = 1 И Объект.ЭтапыОплат[0].ПроцентОплаты = 100 Тогда
		СуммаДокумента = ОбработкаТабличнойЧастиТоварыКлиентСервер.СуммаДокумента(Объект.Товары, Объект.ЦенаВключаетНДС);
...
// Добавленяет строку этапа оплаты в ТЧ "Этапы оплаты".
//
// Параметры:
//  Параметры - Структура - структура параметров для выполнения операции.
//
...
// Получает сумму неоплаченного этапа.
//
// Параметры:
//  ДокументОснование - ДокументОбъект.ЗаявкаНаРасходованиеДенежныхСредств,
//                      ДокументОбъект.РасходныйКассовыйОрдер,
//                      ДокументОбъект.РегистрацияБезналичнойОплаты - документ по которому необходимо 
//                                                                    получить сумму неоплаченного этапа.
//  ФормаОплаты - ПеречислениеСсылка.ФормыОплаты - форма оплаты по документу основанию.
//
// Возвращаемое значение:
//  Число - сумма неоплаченного этапа оплаты.
//
...
// Рассчитывает статусы этапов оплат.
// 
// Параметры:
//  Объект - ДокументОбъект.ЗаказПоставщику,
//           ДокументОбъект.ОтчетКомитентуОПродажах,
//           ДокументОбъект.ОтчетКомитентуОСписании,
//           ДокументОбъект.ПоступлениеТоваров - документ в котором необходимо рассчитать
//                                               статусы этапов оплат.
//
Процедура РассчитатьСтатусОплатыПоЭтапамОплаты(Объект) Экспорт
	...
	Для каждого Этап Из ЭтапыОплат Цикл
		...
		Если Этап.ФормаОплаты = Перечисления.ФормыОплаты.Взаимозачет Тогда
			Этап.СтатусОплаты = НСтр("ru ='Взаимозачет'");
			Продолжить;
		КонецЕсли;
		...
		Если СуммаОплачено >= СуммаЭтапа Тогда
			Этап.СтатусОплаты = НСтр("ru ='Оплачен'");
		ИначеЕсли СуммаОплачено > 0 И СуммаОплачено < СуммаЭтапа Тогда
			Этап.СтатусОплаты = НСтр("ru ='Оплачен частично'");
		ИначеЕсли СуммаОплачено = 0 Тогда
			Этап.СтатусОплаты = НСтр("ru ='Не оплачен'");
		КонецЕсли;
	...
	Если ПоследнийЭтап <> Неопределено И СуммаОплачено > 0 И ЭтапыОплат.Итог("ПроцентОплаты") = 100 Тогда
		ПоследнийЭтап.СтатусОплаты = НСтр("ru ='Переплата'");
	КонецЕсли;
	
КонецПроцедуры

// Получает текст запроса отчета График оплаты поставок.
//
// Параметры: 
//  СтруктураОтборов - Структура - структура отборов отчета.
//
// ВозвращаемоеЗначение:
//  Строка - текст запроса отчета.
//
Функция ПолучитьТекстЗапросаГрафикаОплатыПоставок(СтруктураОтборов = Неопределено) Экспорт
	...
	|	ЗаказПоставщикуЭтапыОплат.Ссылка.Контрагент КАК Контрагент,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПоступленияТоваров КАК ПоступленияТоваров
	...
	|	ЭтапыОплат.ВидПлатежа КАК ВидПлатежа,
	|	ЭтапыОплат.ДатаПлатежа КАК ДатаПлатежа,
	|	ЭтапыОплат.ОтсрочкаПлатежа КАК ОтсрочкаПлатежа,
	...
	|	ЭтапыОплат.Закрыт КАК Закрыт,
	|	РасчетыСПоставщиками.КОплате КАК КОплате,
	|	ЭтапыОплат.Магазин КАК Магазин,
	|	ЭтапыОплат.НомерСтроки КАК НомерСтроки,
...
//
// Параметры:
//  Документ - ДокументСсылка.ЗаказПоставщику,
//             ДокументСсылка.ПоступлениеТоваров - документ для обновления состояний.
...
Процедура ОбновитьСостояниеОплатыПоступления(Документ, Удаление = Ложь) Экспорт
	...
	РезультатПросроченаОплата = Результат[4];
...
//
// Параметры:
//  Документ - ДокументОбъект.ЗаказПоставщику,
//             ДокументОбъект.ПоступлениеТоваров,
//             ДокументОбъект.ЗаказПоставщику,
//             ДокументОбъект.ПоступлениеТоваров - документ для обновления состояний.
...
Функция ПараметрыПредставленияСчетаФактуры(Основание, Организация, НеТребуется = Ложь, РазделятьДокументы = Ложь, ПредставлениеДокумента = Ложь) Экспорт
	...
	Если СчетаФактуры.Количество() > 0 Тогда
		...
		РазрешеныИсправления = ПравоДоступа("Добавление", Метаданные.Документы.СчетФактураПолученный);
	...
	ИначеЕсли РазделятьДокументы Тогда
		ТекстСчетФактура = НСтр("ru='Создать полученный счет-фактуру'");
		ТекстСписок = "";
		ГиперссылкаСчетФактура = Истина;
	...
	Если РазрешеныИсправления Тогда
		Если НЕ (Пользователи.РолиДоступны("ДобавлениеИзменениеПоступленийТоваровВозвратовПоставщику")
			ИЛИ Пользователи.РолиДоступны("ДобавлениеИзменениеПоступленийТоваровВозвратовТоваровПоставщикам")) Тогда
			РазрешеныИсправления = Ложь;
		КонецЕсли;
	КонецЕсли;
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("ТекстСчетФактура", ТекстСчетФактура);
	СтруктураПараметров.Вставить("ТекстСписок", ТекстСписок);
	СтруктураПараметров.Вставить("ГиперссылкаСчетФактура", ГиперссылкаСчетФактура);
	СтруктураПараметров.Вставить("РазрешенВводНового", РазрешенВводНового);
	СтруктураПараметров.Вставить("РазрешеныИсправления", РазрешеныИсправления);
...
//
Функция ПредставлениеСчетаФактуры(Номер, Дата, Проведен, ПредставлениеДокумента=Ложь)
	
	Возврат ?(ПредставлениеДокумента = Истина, Нстр("ru='Счет-фактура полученный '"),"") + СтрШаблон(
	Нстр("ru = '№%1 от %2 г. %3'"),
	СокрЛП(Номер),
	Формат(Дата, "ДЛФ = D"),
	?(НЕ Проведен, " (не проведен)", ""));
...
// Рассчитывает сумма комиссионного вознаграждения.
//
// Параметры:
//  Объект - ДокументОбъект.ОтчетКомитентуОПродажах.
//
Процедура РассчитатьСуммуКомиссионногоВознаграждения(Объект) Экспорт
	...
	Для Каждого СтрокаТоваров Из Объект.Товары Цикл
		Если Объект.СпособРасчетаКомиссионногоВознаграждения = Перечисления.СпособыРасчетаКомиссионногоВознаграждения.ПроцентОтРазностиСуммыПродажиИСуммыКомитента Тогда
			...
			Иначе
				СтрокаТоваров.СуммаВознаграждения = (СтрокаТоваров.Сумма - СтрокаТоваров.СуммаЗакупки) * Объект.ПроцентКомиссионногоВознаграждения / 100;
			КонецЕсли;
		ИначеЕсли Объект.СпособРасчетаКомиссионногоВознаграждения = Перечисления.СпособыРасчетаКомиссионногоВознаграждения.ПроцентОтСуммыПродажи Тогда
			СтрокаТоваров.СуммаВознаграждения = СтрокаТоваров.Сумма * Объект.ПроцентКомиссионногоВознаграждения / 100;
		Иначе
			СтрокаТоваров.СуммаВознаграждения = 0;
		КонецЕсли;
	КонецЦикла;
	...
	ПроцентНДС = ОбработкаТабличнойЧастиТоварыСерверПовтИсп.СтавкаНДСЧислом(Объект.СтавкаНДСВознаграждения);
	РассчитатьСуммуНДСВознаграждения(Объект, ПроцентНДС);
	
КонецПроцедуры

// Находит пересекающиеся периоды отчетов. Объектом может быть, 
// как ДокументОбъект.ОтчетКомитентуОПродажах 
// или ДокументОбъект.ОтчетКомитентуОСписании.
// Так и ДанныеФормыСтруктура этих документов.
//
// Параметры:
//  ПроверяемыйОбъект - ДокументОбъект.ОтчетКомитентуОПродажах,
//                      ДокументОбъект.ОтчетКомитентуОСписании,
//                      ДанныеФормыСтруктура.ОтчетКомитентуОПродажах,
//                      ДанныеФормыСтруктура.ОтчетКомитентуОСписании.
//  ИмяДокумента - Строка - наименование проверяемого документа.
//  ПрефиксПоля - Строка - префикс поля.
//
// ВозвращаемоеЗначение:
//  Булево - признак пересечения периодов отчетов.
//
Функция НайтиПересекающиесяПериодыОтчетов(ПроверяемыйОбъект, ИмяДокумента, ПрефиксПоля = "") Экспорт
	...
	Иначе
		...
		Запрос.УстановитьПараметр("Магазин", ПроверяемыйОбъект.Магазин);
		...
		|	И ОтчетКомитенту.Магазин = &Магазин
...
Процедура РассчитатьСуммуНДСВознаграждения(Объект, ПроцентНДС)
