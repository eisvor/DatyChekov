				...
				Для Каждого СтрокаТЗНастроек Из ТаблицаНастроек Цикл
					...
					ИначеЕсли ТипЗнч(СтрокаТЗНастроек.НачальноеЗначение) = Тип("Число") Тогда
						...
						СтрокаТЗНастроек.УсловиеВыполнено =
							?(СтрокаТЗНастроек.НачальноеЗначение = 0, Истина, ЗначениеРеквизитаДокумента >= СтрокаТЗНастроек.НачальноеЗначение)
							И ?(СтрокаТЗНастроек.КонечноеЗначение = 0, Истина, ЗначениеРеквизитаДокумента <= СтрокаТЗНастроек.КонечноеЗначение);
			...
			ИначеЕсли ТаблицаНастроек.Количество() > 0 Тогда // Упрощенная настройка
				...
				Для Каждого СтрокаТЗНастроек Из ТаблицаНастроек Цикл
					...
					ИначеЕсли ТипЗнч(СтрокаТЗНастроек.НачальноеЗначение) = Тип("Булево") 
						И ТипЗнч(ЗначениеРеквизитаДокумента) = Тип("Булево") Тогда
							
						СтрокаТЗНастроек.УсловиеВыполнено = ЗначениеРеквизитаДокумента = СтрокаТЗНастроек.НачальноеЗначение;
						
					ИначеЕсли ТипЗнч(СтрокаТЗНастроек.НачальноеЗначение) = Тип("Число") 
						И ТипЗнч(ЗначениеРеквизитаДокумента) = Тип("Число") Тогда
...
Функция СведенияОСтатусахДоверенностей(СправочникМЧД, ДанныеМЧД)
	...
	Для Каждого ДанныеДоверенности Из ДанныеМЧД Цикл
		...
		Если СведенияИзРеестра.Ошибка И ЗначениеЗаполнено(МЧД) И ЭтоНереестроваяМЧД(МЧД) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если СведенияИзРеестра.Ошибка Тогда
			...
			Если ЗначениеЗаполнено(МЧД) И Не ЭтоНереестроваяМЧД(МЧД) Тогда
				СтатусМЧД.Сведения = СправочникМЧД(МЧД).СведенияМЧД(МЧД);
...
Процедура ЗаполнитьПротоколПроверкиДоверенности(ПроверкаДоверенности, ПараметрыПроверки, ИННПредставителя, ДатаПодписи)
	...
	Иначе
		ПроверкаПолномочий.Успех = ЗначениеЗаполнено(ПараметрыПроверки.СведенияМЧД.Ссылка) И ЭтоМЧДСПолнымиПолномочиями(ПараметрыПроверки.СведенияМЧД.Ссылка)
			Или Не ЗначениеЗаполнено(ПараметрыПроверки.ЭлектронныйДокумент);
