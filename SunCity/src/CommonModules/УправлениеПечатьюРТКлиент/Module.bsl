...
// Вызывается для клиентской обработки печати ценников и этикеток.
// 
// Параметры:
...
Функция ОбработкаКомандыПечатиЦенниковИЭтикеток(ПараметрыПечати) Экспорт
	...
	МассивНепроведенныхДокументов = ФормированиеПечатныхФормВызовСервера.МассивНепроведенныхДокументов(МассивДокументов);
...
//
// Параметры:
//  РезультатВопроса - КодВозвратаДиалога - код возврата диалога.
//  ДополнительныеПараметры - Структура - структура дополнительных параметров.
// 
Функция ОповещениеВопросОПроведенииДокументовЦенникиИЭтикетки(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
// Вызывается перед формированием печатной формы.
// Установка цен номенклатуры.
//
// Параметры:
//  ПараметрыПечати - Структура - параметры функции печати.
//
...
// Вызывается перед формированием печатных форм инкассации.
...
// Вызывается перед формированием печатных форм КМ3.
...
// Вызывается для печати товарного чека на ФР.
//
// Параметры:
//  ПараметрыПечати - Структура - параметры функции печати.
//
// Возращаемое значение:
//  Булево - результат обработки команды печати.
//
Функция ОбработкаКомандыПечатиТоварногоЧекаДляФР(ПараметрыПечати) Экспорт
	...
	МассивНепроведенныхДокументов = ФормированиеПечатныхФормВызовСервера.МассивНепроведенныхДокументов(ПараметрыПечати.ОбъектыПечати);
...
//  Булево - результат обработки команды печати.
//
Функция ОбработкаКомандыПечатиКопияЧекаДляФР(ПараметрыПечати) Экспорт
	...
	Если МассивНепроведенныхДокументов.Количество() > 0 Тогда
		...
		ОбработчикОповещения = Новый ОписаниеОповещения("ОповещениеВопросОПроведенииДокументовКопияЧекаДляФР", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(ОбработчикОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗавершитьПечатьКопииЧекаДляФР(ПараметрыПечати)
	КонецЕсли;
...
//  ДополнительныеПараметры - Структура - структура дополнительных параметров.
//
Процедура ОповещениеВопросОПроведенииДокументовКопияЧекаДляФР(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		МассивНепроведенныхДокументов = ФормированиеПечатныхФормВызовСервера.ПровестиДокументы(ДополнительныеПараметры.МассивНепроведенныхДокументов);
		Если МассивНепроведенныхДокументов.Количество() = 0 Тогда
			ЗавершитьПечатьКопииЧекаДляФР(ДополнительныеПараметры)
		КонецЕсли; 
	КонецЕсли;
...
//
// Параметры:
//  Результат - Булево - результат открытия формы.
//  ПараметрыФормы - Структура - параметры печатной формы.
//
...
//  ПараметрыПечати - Структура - параметры функции печати.
//
Функция ОбработкаКомандыПечатиЗаявленияПриВозврате(ПараметрыПечати, ТаблицаТоваров = Неопределено) Экспорт
	...
	Если НЕ ТаблицаТоваров = Неопределено Тогда
		ПараметрыПечати.ДополнительныеПараметры.Вставить("ТаблицаТоваров", ТаблицаТоваров);
	КонецЕсли;
...
//  Результат - ТабличныйДокумент - табличный документ по которому ведется расчет.
//  КэшВыделеннойОбласти - Структура - значения выделенных полей.
//  НеобходимоВычислятьНаСервере - Булево - признак необходимости вычислять на севере.
// 
...
// 
// Параметры:
//  Форма - УправляемаяФорма - форма печати.
...
//
// Параметры:
//  Элемент - ЭлементФормы - выбранный элемент формы.
//  ВыбранноеЗначение - Значение - выбранное значение перед обработкой.
//  СтандартнаяОбработка - Булево - признак стандартной обработки.
//  СоответствиеОрганизаций - Соответствие - соответсвие организаций.
//  Организация - СправочникСсылка.Организации - выбранная организация.
//  ВключатьОбособленныеПодразделения - Булево - признак использования обособленных подразделений.
//
...
//  Элемент - ЭлементФормы - выбранный элемент формы.
//  СтандартнаяОбработка - Булево - признак стандартной обработки.
//  ПолеОрганизация - СправочникСсылка.Организации - выбранная организация.
//  СоответствиеОрганизаций - Соответствие - соответсвие организаций.
//
...
Процедура ОбработкаКомандыПечатиЦенниковИЭтикетокНаПринтереЭтикеток(ПараметрыПечати) Экспорт
	...
	ДанныеДляПринтераЭтикеток = УправлениеПечатьюРТВызовСервера.СтруктураДанныхЦенниковИЭтикетокДляПринтераЭтикеток(Обработка, ПараметрыПечати.МенеджерПечати, ПараметрыПечати.ТекущийРазмер);
...
Процедура ОткрытьФормуПодбора(ПараметрыОткрытия) Экспорт
	...
	Если Не ЗначениеЗаполнено(ПараметрыОткрытия.ТорговыйОбъект) Тогда
		ТорговыйОбъект = ПараметрыОткрытия.ТорговыйОбъект;
	Иначе
		ТорговыйОбъект = Неопределено;
	КонецЕсли;
	ПараметрыФормы.Вставить("Магазин", ТорговыйОбъект);
	...
	ОткрытьФорму("Обработка.ПодборТоваров.Форма", ПараметрыФормы, ПараметрыОткрытия.ФормаВладелец,,,, ПараметрыОткрытия.ОбработчикЗакрытия);
...
Процедура ЗавершитьПечатьКопииЧекаДляФР(ПараметрДействия)
...
Процедура ЗавершитьПечатьТоварногоЧекаДляФР(ПараметрДействия)
	...
	СтруктураПараметровПечати = УправлениеПечатьюРТВызовСервера.СтруктураПараметровПечатиЧекаДляФР(ПараметрДействия.ОбъектыПечати, ПараметрДействия.РабочееМесто);
	
	Если СтруктураПараметровПечати = Неопределено 
		ИЛИ СтруктураПараметровПечати.ШиринаЛенты = 0 Тогда
