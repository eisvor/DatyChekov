...
// Начинает фоновое выполнение загрузки операций по ЮKassa.
//
// Параметры:
//  ПараметрыЗагрузки - Структура - параметры загрузки операций.
//   * Период - СтандартныйПериод, Структура - Период за который будут выбираться операции по ЮKassa.
...
//
Процедура НачатьЗагрузкуОперацийПоОнлайнОплате(ВыводитьОкноОжидания = Ложь) Экспорт
	
	ПараметрыЗагрузки = НовыйПараметрыЗагрузки();
	ПараметрыЗагрузки.ПоказыватьОповещениеПользователя = ВыводитьОкноОжидания;
	
	ДлительнаяОперация = ОнлайнОплатыРТВызовСервера.ЗагрузитьОперацииОнлайнОплатыВФоне(ПараметрыЗагрузки);
	
	ОповещениеПослеЗагрузки = Новый ОписаниеОповещения("ЗагрузитьОперацииПоОнлайнОплатеЗавершение", ЭтотОбъект);
...
// Обработчик оповещения завершения загрузки операций по ЮKassa.
//
// Параметры:
// 	Результат - Структура - Результат выполнения фонового задания.
// 	ДопПараметры - Струкутра - Дополнительные параметры.
//
Процедура ЗагрузитьОперацииПоОнлайнОплатеЗавершение(Результат, ДопПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если ПоказыватьОповещениеПользователя ИЛИ СчетчикЗагруженныхДокументов > 0 Тогда
			...
			Если СчетчикЗагруженныхДокументов Тогда
				Оповестить("ИнтеграцияСЮKassa_ЗагруженыДокументы", СчетчикЗагруженныхДокументов);
			КонецЕсли;
...
// Инициализирует пустую структуру параметров загрузки операций из ЮKassa.
...
// Обработчик оповещения о загрузке операций из ЮKassa в формах списков.
//
// Параметры:
...
Процедура ОбработкаОповещения_ФормаСписка(Список, ИмяСобытия, Параметр, Источник) Экспорт
	...
	Если ИмяСобытия = "ИнтеграцияСЮKassa_ЗагруженыДокументы" Тогда
		Список.Обновить();
	КонецЕсли;
...
Процедура ЭлементФормыНастроекСоздание(Контекст, Элемент, СтандартнаяОбработка) Экспорт
	...
	ПараметрыФормы.Вставить("Основание", ОнлайнОплатыРТВызовСервера.ДанныеЭквайераПоУмолчанию());
