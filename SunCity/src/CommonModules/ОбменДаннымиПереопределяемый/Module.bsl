...
Процедура ПолучитьПланыОбмена(ПланыОбменаПодсистемы) Экспорт
	...
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменРозницаУправлениеТорговымПредприятием);
...
Процедура РегистрацияИзмененийНачальнойВыгрузкиДанных(Знач Получатель, СтандартнаяОбработка, Отбор) Экспорт
	...
	ИнтеграцияУРМКРТ.РегистрацияИзмененийНачальнойВыгрузкиДанных(Получатель, СтандартнаяОбработка, Отбор);
	ОбменДаннымиРТ.РегистрацияИзмененийНачальнойВыгрузкиДанных(Получатель, СтандартнаяОбработка, Отбор);
...
Процедура ПриНастройкеПодчиненногоУзлаРИБ() Экспорт
	...
	Если ОбщегоНазначения.ЭтоАвтономноеРабочееМесто() Тогда
		МассивРабочихМест = МенеджерОборудованияВызовСервера.НайтиРабочиеМестаПоИД("00000000-0000-0000-0000-000000000000");
		
		Если МассивРабочихМест.Количество() = 1 Тогда
			...
			РабочееМестоОбъект = МассивРабочихМест[0].ПолучитьОбъект();
			РабочееМестоОбъект.Код = ИдентификаторКлиента;
			РабочееМестоОбъект.ИмяКомпьютера = ИмяКомпьютера;
			РабочееМестоОбъект.Наименование = РабочееМестоОбъект.Наименование + "(" + РабочееМестоОбъект.ИмяКомпьютера + ")";
...
Функция ЗаписатьВводОстатков(ОбъектВводОстатков, РежимЗаписи) Экспорт
	...
	Попытка
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен данными.Проведение документа ""Ввод начальных остатков узла""'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Информация);
				
	Исключение
		
		Инфо = ИнформацияОбОшибке();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен данными.Проведение документа ""Ввод начальных остатков узла""'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,Инфо);
