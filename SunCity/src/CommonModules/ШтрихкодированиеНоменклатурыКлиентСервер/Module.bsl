	...
	Если НастройкиИспользованияСерий.ИспользоватьНомерСерии
		...
		Если НастройкиИспользованияСерий.ИспользоватьНомерКИЗГИСМСерии
			...
			Иначе
				...
				ТекстСообщения = НСтр("ru = 'При маркировке неперсонифицированными КиЗ номер серии должен быть сгенирирован по TID RFID-метки. При генерации произошла ошибка: %ТекстОшибки%. Обратитесь к администратору.'");
		...
		Если Не ПустаяСтрока(ОбрабатываемаяСтрока.Номер)
			И Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ОбрабатываемаяСтрока.Номер,,Ложь) Тогда
			ОбрабатываемаяСтрока.Номер = "";
			ТекстСообщения = НСтр("ru = 'При маркировке неперсонифицированными КиЗ номер серии должен содержать только цифры.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		КонецЕсли;
