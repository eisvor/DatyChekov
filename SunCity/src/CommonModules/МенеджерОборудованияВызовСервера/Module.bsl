...
// * ОбработчикДрайвера - ОбщийМодуль - 
...
//
// Параметры: 
//   ПомещенныеФайлы - Массив Из ОписаниеПередаваемогоФайла - Массив, содержащий элементы типа ОписаниеПередаваемогоФайла, из которых читается содержание.
//   Кодировка - Строка - Кодировка
// Возвращаемое значение: 
//   Структура:
//    * Успешно - булево.
//    * СодержаниеФайлов - Массив Из Структура - Структуры с ключами ОписаниеПереданногоФайла и ТекстСодержания или Неопределено, в случае Успешно = Ложь.
//    * ТекстОшибки - строка.
//
...
Процедура СоздатьСообщениеФискализации(ИдентификаторЧека, ОрганизацияИИН = Неопределено) Экспорт
	...
	Если СистемаВзаимодействия.ИнформационнаяБазаЗарегистрирована() Тогда
		...
		Если ЗначениеЗаполнено(ИдентификаторОбсужденияСтрока) Тогда
			...
			КонецЕсли;
			НовоеСообщение.Текст = Новый ФорматированнаяСтрока(ТекстСообщения);
...
Функция ПодготовитьДанныеФискализацииЧека(ОбщиеПараметры, ПараметрыПодключения) Экспорт
	...
	Если ОбщиеПараметры.Свойство("СформироватьЧекКоррекции") И ОбщиеПараметры.СформироватьЧекКоррекции Тогда
		...
		Иначе // Для ФФД 1.05 и исправления ошибок ККТ - КассовыйЧек 
...
// Записывает в XML данные агента
//
// Параметры:
//  ЗаписьXML - ЗаписьXML -
//  ДанныеАгента - Структура - см. МенеджерОборудованияКлиентСервер.ПараметрыДанныеАгента()
			...
			Если НЕ ПустаяСтрока(РеквизитКодаТовара) И Не ПустаяСтрока(ДанныеКодаТовара.ШтрихкодBase64) Тогда // Запись тега 1163
				Если ПустаяСтрока(Позиция.КонтрольнаяМарка) Тогда
					...
					Если ДанныеКодаТовара.ТипИдентификатораТовара = Перечисления.ТипыИдентификаторовТовараККТ.ИзделияИзНатуральногоМеха Тогда
						ЗаписьXML.ЗаписатьАтрибут(ДанныеКодаТовара.НаименованиеРеквизита, XMLСтрока(Base64ВШтрихкод(ДанныеКодаТовара.ШтрихкодBase64)));
					Иначе
...
Функция ВыполненаПроверкаОбязательностиИПравильностиЗаполненияТэгов(Параметры, ИдентификаторУстройства, ОписаниеОшибки) Экспорт
	...
	Если ИспользуетсяПодсистемыФискальныхУстройств() Тогда
		МодульФорматноЛогическийКонтрольВызовСервера = ОбщегоНазначения.ОбщийМодуль("ФорматноЛогическийКонтроль");
