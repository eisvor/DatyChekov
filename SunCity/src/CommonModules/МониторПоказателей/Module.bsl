////////////////////////////////////////////////////////////////////////////////
// МониторПоказателей содержит процедуры и функции 
// для работы с монитором показателей.
//  
////////////////////////////////////////////////////////////////////////////////
...
Процедура ПолучитьДанныеОПродажах(ТекущийОтчет, ПараметрыЗадания, МассивРезультат)
	...
	Если РазрезАналитики = СокрЛП("Магазин") Тогда 
		...
		|ИЗ
		|	РегистрНакопления.Продажи.Обороты(&НачалоПериода, &КонецПериода, Авто, Магазин В (&Магазины)) КАК ПродажиОбороты
		|
		|СГРУППИРОВАТЬ ПО
	...
	ИначеЕсли РазрезАналитики = СокрЛП("Товарная группа") Тогда
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.Продажи.Обороты(&НачалоПериода, &КонецПериода, Авто, ) КАК ПродажиОбороты
	...
	ИначеЕсли РазрезАналитики = СокрЛП("Иерархия номенклатуры") Тогда
		...
		ЗапросНоменклатура.УстановитьПараметр("ТекстВнеГруппы", НСтр("ru='Вне группы'"));
		...
		|			ТОГДА &ТекстПрочиеГруппыНоменклатуры = ""Прочие группы номенклатуры""
	...
	Запрос.УстановитьПараметр("ТекстПрочиеГруппыНоменклатуры", НСтр("ru='Прочие группы номенклатуры'"));
	...
	Структура.Вставить("Заголовок", 		НСтр("ru='Сумма продаж'"));
...
Процедура ПолучитьДанныеОКоличествеЧеков(ТекущийОтчет, ПараметрыЗадания, МассивРезультат)
	...
	Структура.Вставить("Заголовок", 		НСтр("ru='Количество чеков'"));
...
Процедура ПолучитьДанныеОСреднемЧеке(ТекущийОтчет, ПараметрыЗадания, МассивРезультат)
	...
	Структура.Вставить("Заголовок", 		НСтр("ru='Средний чек'"));
...
Процедура ПолучитьДанныеОбОстаткахДСККМ(ТекущийОтчет, ПараметрыЗадания, МассивРезультат)
	...
	Структура.Вставить("Заголовок", 		НСтр("ru='Остатки денежных средств в кассах ККМ'"));
...
Процедура ПолучитьДанныеОбОстаткахДС(ТекущийОтчет, ПараметрыЗадания, МассивРезультат)
	...
	Структура.Вставить("Заголовок", 		НСтр("ru='Остатки денежных средств в кассах'"));
...
Процедура ПолучитьДанныеОПродажахПоКонтрагентам(ТекущийОтчет, ПараметрыЗадания, МассивРезультат)
	...
	Структура.Вставить("Заголовок", 		НСтр("ru='Продажи по контрагентам'"));
...
Процедура ПолучитьДанныеОДвиженииДенежныхСредств(ТекущийОтчет, ПараметрыЗадания, МассивРезультат)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПлатежныеКарты.Ссылка КАК Документ,
	|	СУММА(ПлатежныеКарты.Сумма) КАК Сумма
	|ПОМЕСТИТЬ ПлатежныеКарты
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.ОплатаПлатежнымиКартами КАК ПлатежныеКарты
	|ГДЕ
	|	ПлатежныеКарты.Ссылка.Проведен
	|	И ПлатежныеКарты.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|
	|СГРУППИРОВАТЬ ПО
	|	ПлатежныеКарты.Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Документ
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	БанковскиеКредиты.Ссылка КАК Документ,
	|	СУММА(БанковскиеКредиты.Сумма) КАК Сумма
	|ПОМЕСТИТЬ БанковскиеКредиты
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.ОплатаБанковскимиКредитами КАК БанковскиеКредиты
	|ГДЕ
	|	БанковскиеКредиты.Ссылка.Проведен
	|	И БанковскиеКредиты.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|
	|СГРУППИРОВАТЬ ПО
	|	БанковскиеКредиты.Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Документ
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПодарочныеСертификаты.Ссылка КАК Документ,
	|	СУММА(ПодарочныеСертификаты.ПодарочныйСертификат.Номинал * ПодарочныеСертификаты.Количество) КАК Сумма
	...
	|ГДЕ
	|	ПодарочныеСертификаты.Ссылка.Проведен
	|	И ПодарочныеСертификаты.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ПлатежныеКарты.Сумма КАК Сумма,
	|	ПлатежныеКарты.Документ КАК Документ,
	|	ПлатежныеКарты.Документ.Дата КАК Дата
	|ПОМЕСТИТЬ ДвиженияДСОтчетыОПродажах
	|ИЗ
	|	ПлатежныеКарты КАК ПлатежныеКарты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОтчетыПоПродажам КАК ОтчетыПоПродажам
	...
	|	БанковскиеКредиты.Сумма,
	|	БанковскиеКредиты.Документ,
	|	БанковскиеКредиты.Документ.Дата
	|ИЗ
	|	БанковскиеКредиты КАК БанковскиеКредиты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОтчетыПоПродажам КАК ОтчетыПоПродажам
	...
	|	ПодарочныеСертификаты.Документ,
	|	ПодарочныеСертификаты.Документ.Дата
	|ИЗ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПлатежныеКарты КАК ПлатежныеКарты
	|		ПО ОтчетыПоПродажам.Документ = ПлатежныеКарты.Документ
	|		ЛЕВОЕ СОЕДИНЕНИЕ БанковскиеКредиты КАК БанковскиеКредиты
	|		ПО ОтчетыПоПродажам.Документ = БанковскиеКредиты.Документ
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПодарочныеСертификаты КАК ПодарочныеСертификаты
	|		ПО ОтчетыПоПродажам.Документ = ПодарочныеСертификаты.Документ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	Структура.Вставить("Заголовок", 		НСтр("ru='Движения денежных средств по типам оплат'"));
...
Процедура ПолучитьДанныеОЗадолженностиПоставщикам(ТекущийОтчет, ПараметрыЗадания, МассивРезультат)
	...
	Запрос.УстановитьПараметр("НачалоПериода", 	НачалоДня(Период.ДатаНачала));
	Запрос.УстановитьПараметр("КонецПериода", 	КонецДня(Период.ДатаОкончания));
	Запрос.УстановитьПараметр("ТекстПрочиеПоставщики", НСтр("ru='Прочие поставщики'"));
	...
	Структура.Вставить("Заголовок", 		НСтр("ru='Задолженность поставщикам'"));
