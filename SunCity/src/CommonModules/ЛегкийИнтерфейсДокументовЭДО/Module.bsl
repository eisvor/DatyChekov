...
Процедура ЗаполнитьДанныеКомпонентаПанельОтраженияВУчете(ДанныеКомпонента, СвойстваДокумента,
	...
	Если СвойстваДокумента.ЭтоНовыйДокумент 
		...
		Для Каждого ОбъектУчета Из ОбъектыУчета Цикл
			НовыйОбъектУчета = ДанныеКомпонента.ОбъектыУчетаДокумента.Добавить();
	...
	Для Каждого ОбъектУчета Из ОбъектыУчетаДокумента Цикл
		НовыйОбъектУчета = ДанныеКомпонента.ОбъектыУчетаДокумента.Добавить();
		ЗаполнитьЗначенияСвойств(НовыйОбъектУчета, ОбъектУчета);
	КонецЦикла;
...
// Возвращаемое значение:
//  Структура:
//  * ОбъектыУчетаДокумента - См. НовыйНаборДанныхОбъектовУчетаДокумента
...
Функция НовыеДанныеКомпонентаПанельОтраженияВУчете() Экспорт
	...
	ДанныеКомпонента = Новый Структура;
	ДанныеКомпонента.Вставить("ОбъектыУчетаДокумента", НовыйНаборДанныхОбъектовУчетаДокумента());
...
Функция ПредставлениеДанныхПоДаннымКомпонентаПанельИтогов(ДанныеКомпонента) Экспорт
	...
	Если ЗначениеЗаполнено(ДанныеКомпонента.ВалютаДокумента) Тогда
		ЗнакВалюты = ОбщегоНазначенияБЭД.ЗнакВалютыПоКоду(ДанныеКомпонента.ВалютаДокумента);
	КонецЕсли;
...
//  * ОбъектУчета - ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО
//  * СпособОбработки - Строка
Функция НовыйНаборДанныхОбъектовУчетаДокумента()

	ДанныеОбъектовУчета = Новый ТаблицаЗначений;
	ДанныеОбъектовУчета.Колонки.Добавить("ОбъектУчета", 
		Метаданные.ОпределяемыеТипы.ОснованияЭлектронныхДокументовЭДО.Тип);
	ДанныеОбъектовУчета.Колонки.Добавить("СпособОбработки", ОбщегоНазначения.ОписаниеТипаСтрока(150));
...
Процедура НастроитьПанельПакетаКарточкиДокумента(Форма, ДанныеКомпонента)
	...
	Форма.ЭтоПакетДокументов = ЗначениеЗаполнено(ДанныеКомпонента.ИдентификаторПакета)
		И Форма.СоставПакета.Количество() > 1;
...
Процедура НастроитьПанельИтоговКарточкиДокумента(Форма, ДанныеКомпонента)
	...
	ПредставлениеДанныхКомпонента = ПредставлениеДанныхПоДаннымКомпонентаПанельИтогов(ДанныеКомпонента);
...
Процедура НастроитьПанельОтраженияВУчетеКарточкиДокумента(Форма, ДанныеКомпонента)
	...
	Форма.ОбъектыУчетаДокумента.Загрузить(ДанныеКомпонента.ОбъектыУчетаДокумента);
	Форма.Основания.ЗагрузитьЗначения(ДанныеКомпонента.ОбъектыУчетаДокумента.ВыгрузитьКолонку("ОбъектУчета"));
	...
	ПредставлениеДанныхКомпонента = ПредставлениеДанныхПоДаннымКомпонентаПанельОтраженияВУчете(ДанныеКомпонента);
	...
	ОбъектыУчетаДокумента = ДанныеКомпонента.ОбъектыУчетаДокумента;
	...
	Если ОдинОбъектУчета Тогда
		Форма.Элементы.КнопкаОткрытьДокументУчета1.Заголовок = Строка(ОбъектыУчетаДокумента[0].ОбъектУчета);
	КонецЕсли;

	Если ДваОбъектаУчета Тогда
		Форма.Элементы.КнопкаОткрытьДокументУчета1.Заголовок = Строка(ОбъектыУчетаДокумента[0].ОбъектУчета);
		Форма.Элементы.КнопкаОткрытьДокументУчета2.Заголовок = Строка(ОбъектыУчетаДокумента[1].ОбъектУчета);
	КонецЕсли;
