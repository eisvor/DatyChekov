////////////////////////////////////////////////////////////////////////////////
// КомпоновкаДанныхСервер содержит процедуры и функции 
...
// Формирует отчет-расшифровку, по описанию, подготовленному соответствующей функцией клиентского модуля.
//
// Параметры:
//  Форма - Форма - форма, в которой расположен отчет.
//  ИмяРеквизитаФормы - Строка - имя реквизита формы типа ОтчетОбъект.
//  ОписаниеОбработкиРасшифровки - ОписаниеОбработкиРасшифровкиКомпоновкиДанных - значение, сформированное
//                                                                                соответствующей клиентской функцией.
//
...
// Формирует отчет, расположенный в форме настройками, которые сделал пользователь.
//
// Параметры:
//  Форма - Форма - форма, в которой расположен отчет.
//  ИмяРеквизитаФормы - Строка - имя реквизита формы типа ОтчетОбъект.
...
// Инициализирует отчет, расположенный в форме настройками по умолчанию и устанавливает контекстные отборы.
//
// Параметры:
//  Форма - Форма - форма, в которой расположен отчет.
//  ИмяРеквизитаФормы - Строка - имя реквизита формы типа ОтчетОбъект.
//  ИмяОтчета - Строка - имя формируемого отчета в метаданных.
//  ИмяВариантаНастроек - Строка - имя варианта отчета в настройках отчета.
//  Отбор - Структура - стрктура ключ и значение отбора.
//
...
//  ПользовательскиеНастройкиМодифицированы - Булево - обязательный к установке признак модификации пользовательских
//                                                     настроек отчета.
//
Процедура УстановитьПараметрыВалютыОтчета(КомпоновщикНастроек, ПользовательскиеНастройкиМодифицированы) Экспорт
	...
	Если ПараметрДанныеОтчета <> Неопределено Тогда
		ВалютаУправленческогоУчета = "Тг.";
		ВалютаРегламентированногоУчета = "Тг.";
...
// Возвращает место в порядке.
//
// Параметры:
//  ДанныеГрупповойОбработкиКомпоновкиДанных - ДанныеГрупповойОбработкиКомпоновкиДанных - данные для определения места в порядке.
//  ПорядокСортировки - Строка - номер порядка сортировки строкой.
//
// Возвращаемое значение:
//  Число - индекс места в порядке.
//
...
//
// Возвращаемое значение:
//  МакетКомпоновкиДанных - возвращает макет компоновки данных для схемы компоновки данных.
//
...
//
// Параметры:
//  КомпоновщикНастроек - КомпоновщикНастроек - компоновщик настроек из которых будет удален параметр.
...
//
// Параметры:
//  СхемаКомпоновкиДанных - СхемаКомпоновкиДанных - схема компоновки данных для которой получается строковое описание параметров СКД.
//  НастройкиКомпоновкиДанных - НастройкиКомпоновкиДанных - настройки компоновки данных.
//
// Возвращаемое значение:
//  Строка - описание параметров настройки компоновки данных.
//
