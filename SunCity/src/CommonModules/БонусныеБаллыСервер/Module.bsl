...
// 
// Параметры:
//  ПравилоНачисления - СправочникСсылка.ПравилаНачисленияБонусныхБаллов - правило начисления бонусных баллов.
//
Процедура ВыполнитьАвтоматическоеНачислениеИСписаниеРегламентноеЗадание(ПравилоНачисления) Экспорт
	...
	Попытка
		ВыполнитьАвтоматическоеНачислениеИСписание(ПравилоНачисления);
	Исключение
...
// Формирует таблицы начисления и списания бонусных баллов
// по указанному правилу начисления на определенную дату.
//
// Параметры:
//  ПравилоНачисления - СправочникСсылка.ПравилаНачисленияБонусныхБаллов - правило начисления бонусных баллов.
//  Дата - Дата - период времени за который формируются таблицы начисления и списания бонусных баллов.
//
// Возвращаемое значение:
//  ТаблицаЗначений - данные по начисленным и списанным бонусным баллам по определенному правилу начисления на указанную дату.
//
...
// Формирует остатки бонусных баллов по указанной дисконтной карте на определенную дату.
//
// Параметры:
//  ДисконтнаяКарта - СправочникСсылка.ИнформационнаяКарта - дисконтная карта по которой будут сформированы остатки.
//  ДатаЗапроса - Дата - дата на которую отображаются остатки по дисконтной карте.
//
// Возвращаемое значение:
//  ТаблицаЗначений - данные по остаткам и движениям бонусных баллов по указанной дисконтной карте.
//
...
Функция ТаблицаОстаткиИДвиженияБонусныхБаллов(ДисконтнаяКарта, ДатаЗапроса) Экспорт
	...
	|	&Сейчас КАК Период,
	|	ВЫБОР
	|		КОГДА БонусныеБаллыОстаткиИОбороты.НачисленоОстаток - ВЫБОР
	|				КОГДА БонусныеБаллыОстаткиИОбороты.КСписаниюОстаток > 0
	|					ТОГДА БонусныеБаллыОстаткиИОбороты.КСписаниюОстаток
	|				ИНАЧЕ 0
	|			КОНЕЦ >= 0
	|			ТОГДА БонусныеБаллыОстаткиИОбороты.НачисленоОстаток - ВЫБОР
	|					КОГДА БонусныеБаллыОстаткиИОбороты.КСписаниюОстаток > 0
	|						ТОГДА БонусныеБаллыОстаткиИОбороты.КСписаниюОстаток
	|					ИНАЧЕ 0
	|				КОНЕЦ
	...
	|	ВЫБОР
	|		КОГДА БонусныеБаллыОстаткиИОбороты.КСписаниюКонечныйОстаток < 0
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоСторно,
	|	ВЫБОР
	|		КОГДА БонусныеБаллыОстаткиИОбороты.НачисленоКонечныйОстаток - ВЫБОР
	|				КОГДА БонусныеБаллыОстаткиИОбороты.КСписаниюКонечныйОстаток > 0
	|					ТОГДА БонусныеБаллыОстаткиИОбороты.КСписаниюКонечныйОстаток
	|				ИНАЧЕ 0
	|			КОНЕЦ >= 0
	|			ТОГДА БонусныеБаллыОстаткиИОбороты.НачисленоКонечныйОстаток - ВЫБОР
	|					КОГДА БонусныеБаллыОстаткиИОбороты.КСписаниюКонечныйОстаток > 0
	|						ТОГДА БонусныеБаллыОстаткиИОбороты.КСписаниюКонечныйОстаток
	|					ИНАЧЕ 0
	|				КОНЕЦ
	...
	Запрос.УстановитьПараметр("Сейчас", НСтр("ru = 'Сейчас'"));
	...
	Если ЗначениеЗаполнено(КартаДляНакоплений) Тогда
		ДисконтныеКарты = МаркетинговыеАкцииСервер.ИсторияДисконтнойКарты(ДатаЗапроса, КартаДляНакоплений, Истина);
	Иначе
		ДисконтныеКарты = МаркетинговыеАкцииСервер.ИсторияДисконтнойКарты(ДатаЗапроса, ДисконтнаяКарта, Истина);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Заголовок = НСтр("ru = 'Списание через'");
		Баллы = -Изменение;
		
		Если Изменение > 0 Тогда
			Если Выборка.ЭтоСторно Тогда
				Заголовок = НСтр("ru = 'Сторнирование через'");
			Иначе
				Заголовок = НСтр("ru = 'Начисление через'");
			КонецЕсли;
			Баллы = Изменение;
		КонецЕсли;
		
		РазницаСтрокой = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(ПредставлениеДней, РазницаДней);
		ПериодСтрокой = Формат(Выборка.Период,"ДЛФ=D");
		БаллыСтрокой = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(ПредставлениеБаллов, Баллы);
		
		НоваяСтрока.Период = СтрШаблон(НСтр("ru = '%1 %2 (%3): %4'"), Заголовок, РазницаСтрокой, ПериодСтрокой, БаллыСтрокой);
...
// Формирование движений по регистру "История подарочных сертификатов".
//
// Параметры:
//	ДополнительныеСвойства - Структура - структура дополнительный свойств таблицы бонусных баллов.
//	Движения - Коллекция движений - движения, которые отразят изменения бонусных баллов.
...
// Вызывается из подписки на событие СторнироватьБонусныеБаллыПоДокументу
// перед записью документа.
//
// Входящие параметры:
//  Источник - ДокументОбъект.ВозвратТоваровОтПокупателя, ДокументОбъект.ЧекККМ - документ основание сторнирования бонусных баллов.
//  Отказ - Булево - Признак отказа от записи.Если в теле процедуры-обработчика установить 
//    данному параметру значение Истина, то запись выполнена не будет и будет вызвано исключение.
//  РежимЗаписи - В параметр передается режим записи документа. Позволяет определить в теле процедуры режим записи.
//    Изменение значения параметра позволяет изменить режим записи.
//  РежимПроведения - РежимПроведенияДокумента - В данный параметр передается режим проведения.
...
// Рассчитывает дату начала и дату окончания действия бонусной скидки
// на указанную дату, а также получает значения остальных реквизитов элемента.
//
// Входящие параметры:
//  СкидкаНаценка - СправочникСсылка.СкидкиНаценки - ссылка на элемент справочника параметры которого рассчитываются.
//  Дата - Дата - дата в соответствии с которой рассчитываются дата начала действия и дата окончания.
//
// Возвращаемое значение:
//  Структура - структура, содержащая значения реквизитов.
//
...
// Подготавливает пустую таблицу бонусов
// при расчете.
//
// Возвращаемое значение:
//  ТаблицаЗначений - пустая таблица бонусов при расчете скидок.
//
Функция ПустаяТаблицаБонусовПриРасчетеСкидок() Экспорт
	
	ТаблицаДанных = Новый ТаблицаЗначений;
	ТаблицаДанных.Колонки.Добавить("КлючСвязи",                 ОбщегоНазначенияРТКлиентСервер.ОписаниеТиповЧисла(5, 0));
	ТаблицаДанных.Колонки.Добавить("КоличествоБонусныхБаллов",  ОбщегоНазначенияРТКлиентСервер.ОписаниеТиповЧисла(15, 2));
	ТаблицаДанных.Колонки.Добавить("Расшифровка",               Новый ОписаниеТипов("ТаблицаЗначений"));
	ТаблицаДанных.Колонки.Добавить("РеквизитДопУпорядочивания", ОбщегоНазначенияРТКлиентСервер.ОписаниеТиповЧисла(5, 0));
	
	Возврат ТаблицаДанных;
	
КонецФункции

// Подготавливает пустую таблицу расшифровки
// для расчета бонусов.
//
// Возвращаемое значение:
//  ТаблицаЗначений - пустая таблица расшифровки бонусов.
//
...
//
// Параметры:
//  Объект - ДокументОбъект.ЧекККМ, ДокументОбъект.РеализацияТоваров - документ по которому будут обновлены бонусы.
//  ИмяОсновнойТЧ - Строка - Имя основной табличной части.
//  ИмяТЧ - Строка - Имя табличной части.
//  ИмяКлюча - Строка - Имя ключа.
//
...
// Очищает суммы бонусов в таблице товаров
// и увеличивает общую сумму.
//
// Параметры:
//  Объект - ДокументОбъект.ЧекККМ, ОбработкаОбъект.РМКУправляемыйРежим - объект в котором будут очищены суммы бонусов.
//
Процедура ОчиститьОплатуБонусамиВТоварах(Объект) Экспорт
	...
	Для Каждого СтрокаТовара Из Объект.Товары Цикл
		Если СтрокаТовара.СуммаСкидкиОплатыБонусом <> 0 Тогда
			...
			СтрокаТовара.СуммаНДС = ОбработкаТабличнойЧастиТоварыКлиентСервер.СуммаНДС(СтрокаТовара.Сумма, СтрокаТовара.СтавкаНДС, Объект.ЦенаВключаетНДС);
...
// Возвращает накопленные баллы для печати чека.
//
// Параметры:
//  РеквизитыДокумента - Структура - структура реквизитов документа.
//
// Возвращаемое значение:
//  Число - количество накопленных баллов.
//
Функция КоличествоНакопленныхБалловДляПечатиЧека(РеквизитыДокумента) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(КартаДляНакоплений) Тогда
			ДисконтныеКарты = МаркетинговыеАкцииСервер.ИсторияДисконтнойКарты(ДатаЗапроса, КартаДляНакоплений, Истина);
		Иначе
			ДисконтныеКарты = МаркетинговыеАкцииСервер.ИсторияДисконтнойКарты(ДатаЗапроса, РеквизитыДокумента.ДисконтнаяКарта, Истина);
		КонецЕсли;
		Запрос.УстановитьПараметр("ДисконтныеКарты", ДисконтныеКарты);
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
...
// Распределяет сумму оплаты и количество бонусов
// по вспомогательной таблице товаров при оплате бонусами.
//
// Параметры:
//	ТаблицаРаспределения - ТаблицаЗначений - таблица с товарами, в которой производится распределение.
...
// Рассчитывает максимальные суммы по сегменту и ценовым группам
// и подготавливает таблицу для распределения оплаты бонусными баллами.
//
// Параметры:
//  ТабличнаяЧастьТовары - ТабличнаяЧасть - список товаров по которой будет подготовлено распределение бонусными баллами.
//  БонуснаяПрограммаЛояльности - СправочникСсылка.БонуснаяПрограммаЛояльности - бонусная программа лояльности по которой будут рассчитаны скидки.
//  Магазин - магазин по которому определяются минимальные цены и сегменты номенклатуры.
//
// Возвращаемое значение:
//  ТаблицаЗначений - таблица для распределения оплат бонусными баллами.
//
Функция ТаблицаМаксимальныхСуммОплаты(ТабличнаяЧастьТовары, БонуснаяПрограммаЛояльности, Магазин) Экспорт
	...
	|				И Товары.Номенклатура.ДоговорКонтрагента <> ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)))
	|////////////////////////////////////////////////////////////////////////////////
	|";
...
// Объединяет таблицу бонусных баллов и таблицу оплат бонусами.
//
// Параметры:
//  ТаблицаБонусныеБаллы - ТаблицаЗначений - таблица бонусных баллов.
//  ТаблицаОплатыБонусами - ТаблицаЗначений - таблица оплат бонусными баллами.
//  РезультатЗапроса - ТаблицаЗначений - 
//  ПериодБонусов - Дата - период за которую рассчитываются бонусы.
//  ЭтоОтчетОПродажах - Булево - признак отчета о продажах.
//  ЕстьЗамены - Булево - признак наличия замен в оплатах.
//  ТаблицаСоответствияЗамен - ТаблицаЗначений - таблица соответствия замен в таблицах бонусных баллов и оплат бонусами.
//
...
// Проводит отчеты о розничных продажах при обмене данными.
//
// Параметры:
//  МассивОтчетовОРозничныхПродажах - Массив - отчеты о розничных продажах для проведения.
//  ТекущийУзел - ПланыОбменаСсылка.ПоМагазину - текущий узел обмена.
//
	...
	Если МассивОтчетовОРозничныхПродажах.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка
				...
				ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("Документ.ОтчетОРозничныхПродажах");
			...
			Исключение
				...
				|При загрузке данных из магазина: %2'");
				ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщенияОбОшибке, КраткоеПредставлениеОшибки(Инфо), ТекущийУзел);
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен данными.Загрузка данных'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.Документы.ОтчетОРозничныхПродажах, Выборка.Ссылка, ТекстСообщенияОбОшибке);
...
Процедура ВыполнитьАвтоматическоеНачислениеИСписание(ПравилоНачисления)
