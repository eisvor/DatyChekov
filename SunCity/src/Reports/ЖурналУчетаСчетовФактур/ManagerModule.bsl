...
Процедура ЗаполнитьПодстрочникРНПТ(ЗаписьОтчета, ПараметрыСтрокиРНПТ, СтруктураПараметров, СекцияСтрокаРНПТ, ТабличныйДокумент, ИмяОбласти) Экспорт
	...
	Если ЗначениеЗаполнено(СтруктураПараметров.МассивСвРегНом)
		...
		Для ЭлементСвРегНом = 1 По СтруктураПараметров.МассивСвРегНом.Количество() - 1 Цикл
			...
			ПараметрыСтрокиРНПТ.РегНомер     = СтруктураРНПТ.РегНомер;
			ПараметрыСтрокиРНПТ.КолПрослеж   = СтруктураРНПТ.КолПрослеж;
			ПараметрыСтрокиРНПТ.СтоимПрослеж = СтруктураРНПТ.СтоимПрослеж;
			ПараметрыСтрокиРНПТ.ЕдПрослеж    = СтруктураРНПТ.ЕдПрослеж;
...
Функция ПолучитьЗаписиЖурналаСчетовФактур(СтруктураПараметров)
	...
	СведенияОРНПТ = ПолучитьСведенияОРНПТ(Запрос);
	СтруктураПараметров.Вставить("СведенияОРНПТ", СведенияОРНПТ);
	
	Возврат Новый Структура("ВыставленныеСчетаФактуры, ПолученныеСчетаФактуры, СведенияОРНПТ", ВыставленныеСчетаФактуры, ПолученныеСчетаФактуры, СведенияОРНПТ);
...
Процедура ЗаполнитьСтрокуЖурналаУчетаСчетовФактур(ПараметрыСтроки, ЗаписьЖурналаУчетаСчетовФактур, СтруктураПараметров)
	...
	ПараметрыСтроки.ДатаПередачиПолучения = Формат(ЗаписьЖурналаУчетаСчетовФактур.ДатаВыставления, Нстр("ru='ДФ=dd.MM.yyyy'"));
	Если СтруктураПараметров.ВерсияПостановленияНДС1137 >= 3 Тогда
		...
		Если ЗаписьЖурналаУчетаСчетовФактур.Исправление Тогда
			ПараметрыСтроки.НомерДатаИсправления = Формат(ЗаписьЖурналаУчетаСчетовФактур.ДатаИсправления, Нстр("ru='ДФ=dd.MM.yyyy'"))
	...
	Иначе
		...
		ПараметрыСтроки.ДатаСчетаФактуры      = Формат(ЗаписьЖурналаУчетаСчетовФактур.ДатаСоставления, Нстр("ru='ДФ=dd.MM.yyyy'"));
		ПараметрыСтроки.ДатаИсправления       = Формат(ЗаписьЖурналаУчетаСчетовФактур.ДатаИсправления, Нстр("ru='ДФ=dd.MM.yyyy'"));
	...
	Если СтруктураПараметров.ВерсияПостановленияНДС1137 > 3 Тогда
		...
		Если СтруктураПараметров.Свойство("СведенияОРНПТ") Тогда
		...
		Если ЗначениеЗаполнено(МассивСвРегНом) Тогда
			...
			ПараметрыСтроки.РегНомер		= ПервыйРНПТ.РегНомер;
			ПараметрыСтроки.КолПрослеж		= ПервыйРНПТ.КолПрослеж;
			ПараметрыСтроки.СтоимПрослеж	= ПервыйРНПТ.СтоимПрослеж;
			ПараметрыСтроки.ЕдПрослеж		= ПервыйРНПТ.ЕдПрослеж;
			СтруктураПараметров.Вставить("МассивСвРегНом", МассивСвРегНом);
		Иначе
			ПараметрыСтроки.РегНомер		= "";
			ПараметрыСтроки.КолПрослеж		= "";
			ПараметрыСтроки.СтоимПрослеж	= "";
			ПараметрыСтроки.ЕдПрослеж		= "";
...
Функция ВыводитьСтроку(Знач СтруктураПараметров, Строка, ЭтоИтог = Ложь)
...
Функция ПолучитьСведенияОРНПТ(Запрос)
	
	СведенияОРНПТ = НовыйСведенияОРНПТ();
	
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫРАЗИТЬ(ВыданныеСчета.СчетФактура КАК Документ.СчетФактураВыданный) КАК СчетФактура
	|ПОМЕСТИТЬ ВсегоСчетаФактуры
	|ИЗ
	|	ВыданныеСчета КАК ВыданныеСчета
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВЫРАЗИТЬ(РЕЗУЛЬТАТ.Ссылка КАК Документ.СчетФактураПолученный)
	|ИЗ
	|	РЕЗУЛЬТАТ КАК РЕЗУЛЬТАТ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВсегоСчетаФактуры.СчетФактура КАК СчетФактура,
	|	ВсегоСчетаФактуры.СчетФактура.ДокументОснование КАК ДокументОснование
	|ПОМЕСТИТЬ СчетФактурыИОснования
	|ИЗ
	|	ВсегоСчетаФактуры КАК ВсегоСчетаФактуры
	|ГДЕ
	|	ВсегоСчетаФактуры.СчетФактура ССЫЛКА Документ.СчетФактураВыданный
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ПО ВсегоСчетаФактуры.СчетФактура = СчетФактураПолученныйДокументыОснования.Ссылка
	|			И (ВсегоСчетаФактуры.СчетФактура ССЫЛКА Документ.СчетФактураПолученный)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СчетФактурыИОснования.СчетФактура КАК СчетФактура,
	|	СчетФактурыИОснования.ДокументОснование КАК ДокументОснование,
	|	ЗакупкиТоваров.Номенклатура КАК Номенклатура,
	|	ЗакупкиТоваров.Характеристика КАК Характеристика,
	|	ЗакупкиТоваров.Количество КАК Количество,
	|	ЗакупкиТоваров.Сумма КАК Сумма,
	|	ВЫБОР
	|		КОГДА СчетФактурыИОснования.ДокументОснование ССЫЛКА Документ.ПеремещениеТоваров
	|				ИЛИ СчетФактурыИОснования.ДокументОснование ССЫЛКА Документ.ПередачаТоваровМеждуОрганизациями
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ СчетФактурыИОснования.ДокументОснование.ЦенаВключаетНДС
	|	КОНЕЦ КАК ЦенаВключаетНДС
	|ПОМЕСТИТЬ СчетФактурыИОснованияССуммами
	|ИЗ
	|	СчетФактурыИОснования КАК СчетФактурыИОснования
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ЗакупкиТоваров КАК ЗакупкиТоваров
	|		ПО СчетФактурыИОснования.ДокументОснование = ЗакупкиТоваров.Регистратор
	|			И (СчетФактурыИОснования.СчетФактура ССЫЛКА Документ.СчетФактураПолученный)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	СчетФактурыИОснования.СчетФактура,
	|	СчетФактурыИОснования.ДокументОснование,
	...
	|				ИНАЧЕ СчетФактурыИОснования.ДокументОснование.ЦенаВключаетНДС
	|			КОНЕЦ
	|			ТОГДА Продажи.Стоимость - Продажи.НДС
	...
	|		ПО СчетФактурыИОснования.ДокументОснование = Продажи.Регистратор
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СчетФактурыИОснованияССуммами.СчетФактура КАК Ссылка,
	|	ТоварыОрганизаций.НомерГТД КАК РегНомер,
	|	ТоварыОрганизаций.КоличествоПоРНПТ КАК КолПрослеж,
	|	ВЫБОР
	|		КОГДА ТоварыОрганизаций.Номенклатура.ЕдиницаИзмеренияТНВЭД = ЗНАЧЕНИЕ(Справочник.БазовыеЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА ТоварыОрганизаций.Номенклатура.ЕдиницаИзмерения.Наименование
	|		ИНАЧЕ ТоварыОрганизаций.Номенклатура.ЕдиницаИзмеренияТНВЭД.Наименование
	|	КОНЕЦ КАК ЕдПрослеж,
	|	ТоварыОрганизаций.Номенклатура.СтавкаНДС КАК СтавкаНДС,
	|	СчетФактурыИОснованияССуммами.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	СчетФактурыИОснованияССуммами.Сумма / ВЫБОР
	|		КОГДА СчетФактурыИОснованияССуммами.Количество = 0
	|			ТОГДА 1
	|		ИНАЧЕ СчетФактурыИОснованияССуммами.Количество
	|	КОНЕЦ * ТоварыОрганизаций.Количество КАК СтоимПрослеж
	|ИЗ
	|	СчетФактурыИОснованияССуммами КАК СчетФактурыИОснованияССуммами
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыОрганизаций КАК ТоварыОрганизаций
	|		ПО СчетФактурыИОснованияССуммами.ДокументОснование = ТоварыОрганизаций.Регистратор
	|			И (ТоварыОрганизаций.КоличествоПоРНПТ <> 0)
	|			И СчетФактурыИОснованияССуммами.Номенклатура = ТоварыОрганизаций.Номенклатура
	|			И СчетФактурыИОснованияССуммами.Характеристика = ТоварыОрганизаций.Характеристика
	|ИТОГИ ПО
	|	Ссылка";
	
	ВыборкаСФ = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
	
	Пока ВыборкаСФ.Следующий() Цикл
		
		НоваяСтрокаСФ = СведенияОРНПТ.Добавить();
		НоваяСтрокаСФ.Ссылка = ВыборкаСФ.Ссылка;
		ВыборкаРегНом = ВыборкаСФ.Выбрать();
		Пока ВыборкаРегНом.Следующий() Цикл
			СтруктураРегНом = Новый Структура("РегНомер, КолПрослеж, СтоимПрослеж, ЕдПрослеж");
			ЗаполнитьЗначенияСвойств(СтруктураРегНом, ВыборкаРегНом);
			Если ТипЗнч(ВыборкаСФ.Ссылка) = Тип("ДокументСсылка.СчетФактураПолученный") Тогда
				РеквизитыСФ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВыборкаСФ.Ссылка, "Организация, Дата");
				Если УчетНДС.ПрименяетсяНДС(РеквизитыСФ.Дата, РеквизитыСФ.Организация) = Истина Тогда
					Если ВыборкаРегНом.ЦенаВключаетНДС Тогда
						СтруктураРегНом.СтоимПрослеж = ВыборкаРегНом.СтоимПрослеж - Окр(ОбработкаТабличнойЧастиТоварыКлиентСервер.СуммаНДС(ВыборкаРегНом.СтоимПрослеж, ВыборкаРегНом.СтавкаНДС), 2)
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			НоваяСтрокаСФ.СвРегНом.Добавить(СтруктураРегНом);
		КонецЦикла;
