...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДанныеДляОтображения") Тогда
		ДанныеДляОтображения = Параметры.ДанныеДляОтображения;
		ЗагрузитьПереданныеНастройкиФормы(ДанныеДляОтображения);
	Иначе
...
Процедура ВыполнитьОбработчикАвтообновления(ВыполнитьСразу = Неопределено)
	...
	Если Период.ДатаОкончания < Период.ДатаНачала Тогда 
		...
		ПутьКДанным = Период;
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки, , ПутьКДанным);
		Возврат;
	КонецЕсли;
...
&НаСервере
Процедура ЗагрузитьПереданныеНастройкиФормы(ДанныеДляОтображения)
	
	ТаблицаРазделов = ДанныеДляОтображения.ВыбранныеОтчеты.Получить();
	
	СписокОтчетов.Очистить();
	СписокОтчетов.Загрузить(ТаблицаРазделов);
	
	КратностьЧисел = Перечисления.ВидыПредставленияЧисел.БезИзменений;
	ТочностьРасчетаДробнойЧасти = "0";
	ПериодАвтообновления = 30;
	КоличествоКолонок = 2;
	
	ЗаполнитьЗначенияСвойств(ЭтаФорма, ДанныеДляОтображения,
		"РазрезАналитики, Период, Периодичность");
	
	СписокМагазинов.ЗагрузитьЗначения(ДанныеДляОтображения.СписокМагазинов);
...
Процедура ОбновитьФормуОтчета()
	...
	МассивВсехРеквизитов = ПолучитьРеквизиты();
	МассивНеудаляемыхРеквизитов = ПолучитьНеудаляемыеРеквизиты();
	ОбновитьМассивВсехРеквизитов(МассивВсехРеквизитов, МассивНеудаляемыхРеквизитов);	
	
	Если МассивВсехРеквизитов.Количество() > 0 Тогда
		МассивУдаляемыхРеквизитов = Новый Массив;
		Для Каждого СуществующийРеквизит Из МассивВсехРеквизитов Цикл 
	...
	ГруппаМониторПоказателей = ВертикальнаяГруппа("ГруппаМониторПоказателей", НСтр("ru='ГруппаМониторПоказателей'"));
...
Процедура ОбновитьМассивВсехРеквизитов(МассивВсехРеквизитов, МассивНеудаляемыхРеквизитов)
	...
	Для Каждого НеудаляемыйРеквизит Из МассивНеудаляемыхРеквизитов Цикл 
		Для Каждого СуществующийРеквизит Из МассивВсехРеквизитов Цикл 
			Если СуществующийРеквизит.Имя = НеудаляемыйРеквизит Тогда
				Индекс = МассивВсехРеквизитов.Найти(СуществующийРеквизит);
				Прервать;
			Иначе
				Индекс = Неопределено;
			КонецЕсли;
		...
		Если Индекс <> Неопределено Тогда
			МассивВсехРеквизитов.Удалить(Индекс);
		КонецЕсли;
	КонецЦикла;
...
Процедура ВывестиДанныеОтчетаНаСервере()
	...
	Иначе
		...
		Исключение  
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Заполнение отчета ""Монитор показателей магазина""'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура НарисоватьГрафик(СтруктураПараметров)
	...
	ВычислитьТекущуюКолонкуИСтроку();
	...
	Если Данные.Количество() = 0 Тогда 
		...
		ПолеКартинки.ТекстНевыбраннойКартинки 	= Нстр("ru='Нет данных для заполнения отчета'");
	...
	Если График.ТипДиаграммы = ТипДиаграммы.Круговая 
		Или График.ТипДиаграммы = ТипДиаграммы.КруговаяОбъемная Тогда
...
&НаСервере
Процедура ВычислитьТекущуюКолонкуИСтроку()
...
Процедура ЗаполнитьПустыеОбласти()
	...
	Если ДополнитьОбласти Тогда 
		...
		Пока ТекущаяКолонка < КоличествоКолонок + 1 Цикл
			...
			ГруппаДляДиаграммы 	= ГоризонтальнаяГруппа(ИмяГруппы, Нстр("ru='Пустая вертикальная группа'") + ТекущаяСтрока + ТекущаяКолонка, РодительскаяГруппа);
...
Функция СформироватьТекстЗаголовка(ЗаголовокДиаграммы, КратностьЧисел)
	...
	Если ЗаголовокДиаграммы = СокрЛП(Нстр("ru='Количество чеков'")) Тогда 
		ЕдиницаИзмерения = "шт.";
	Иначе 
		ЕдиницаИзмерения = "тг.";
	КонецЕсли;
...
Функция СформироватьПредставлениеПериода(Дата, Периодичность)
	...
	Если Периодичность = Перечисления.Периодичность.Год Тогда 
		ФорматДаты = Формат(Дата, "ДФ=гггг");
	ИначеЕсли Периодичность = Перечисления.Периодичность.Квартал Тогда 
		ФорматДаты = Формат(Дата, "ДФ='к ""кв"" гг'");
	ИначеЕсли Периодичность = Перечисления.Периодичность.Месяц Тогда 
		ФорматДаты = Формат(Дата, "ДФ='MMM гг'");
	ИначеЕсли Периодичность = Перечисления.Периодичность.Неделя Тогда 
		ФорматДаты = Формат(Дата, "ДФ='dd МММ гг'");
