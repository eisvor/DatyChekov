...
Процедура ОбновитьДанныеКомпонентов(ДанныеКомпонентов)
	...
	НастроитьПанелиПросмотра(ДанныеКомпонентов);
...
Процедура ЗаполнитьАсинхронныеДанныеПослеОжидания(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Успех Тогда
		...
		Если Не Результат.ОперацияПолученияДанных.ИдентификаторЗадания = ОперацияПолученияДанных.ИдентификаторЗадания Тогда
			Возврат;
		КонецЕсли;
...
&НаСервере
Процедура НастроитьПанелиПросмотра(ДанныеКомпонентов)
...
Процедура НастроитьПанельИтогов(ДанныеКомпонента)
	...
	Если ДанныеКомпонента.АсинхронноеПолучение Тогда
		Элементы.ГруппаСтраницыИтогов.ТекущаяСтраница = Элементы.СтраницаОжиданиеИтогов;
		Элементы.СтраницаЗагруженныеИтоги.Видимость = Ложь;
		Элементы.СтраницаОжиданиеИтогов.Видимость = Истина;
		Возврат;
	КонецЕсли;
	
	ПредставлениеДанныхКомпонента = 
		ЛегкийИнтерфейсДокументовЭДО.ПредставлениеДанныхПоДаннымКомпонентаПанельИтогов(ДанныеКомпонента);
	
	Элементы.ГруппаСтраницыИтогов.ТекущаяСтраница = Элементы.СтраницаЗагруженныеИтоги;
	Элементы.СтраницаЗагруженныеИтоги.Видимость = Истина;
	Элементы.СтраницаОжиданиеИтогов.Видимость = Ложь;

	Элементы.ГруппаРеквизитыСуммаНДС.Видимость = ЗначениеЗаполнено(ПредставлениеДанныхКомпонента.СуммаДокументаСтрокой);
	Элементы.НадписьСуммаЗначение.Заголовок = ПредставлениеДанныхКомпонента.СуммаДокументаСтрокой;
	Элементы.КартинкаПанельИтоговРазделитель1.Видимость = Элементы.ГруппаРеквизитыСуммаНДС.Видимость;
...
Процедура НастроитьПанельОтраженияВУчете(ДанныеКомпонента)
	...
	ОбъектыУчетаТекущегоДокумента.Загрузить(ДанныеКомпонента.ОбъектыУчетаДокумента);
	...
	ПредставлениеДанныхКомпонента = 
		ЛегкийИнтерфейсДокументовЭДО.ПредставлениеДанныхПоДаннымКомпонентаПанельОтраженияВУчете(ДанныеКомпонента);
...
Процедура НастроитьПанельВизуализации(ДанныеКомпонента)
	...
	Если ДанныеКомпонента.ТипДанных = Перечисления.ТипыДанныхВизуализацииДокументаЭДО.ИмяФайла Тогда
	...
	ИначеЕсли ДанныеКомпонента.ТипДанных = Перечисления.ТипыДанныхВизуализацииДокументаЭДО.HTMLДокумент Тогда
		
		ПредпросмотрHTML = ДанныеКомпонента.Данные;
		
		Элементы.СтраницыПредпросмотр.ТекущаяСтраница = Элементы.СтраницаHTMLДокумент;
	
	ИначеЕсли ДанныеКомпонента.ТипДанных = Перечисления.ТипыДанныхВизуализацииДокументаЭДО.ТабличныйДокумент Тогда
