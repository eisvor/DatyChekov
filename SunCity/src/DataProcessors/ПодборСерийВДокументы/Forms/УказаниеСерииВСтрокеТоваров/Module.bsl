...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВладелецСерии = НастройкиИспользованияСерий.ВладелецСерии;
	...
	Если ЭтоПеремаркировкаТоваровГИСМ Тогда
		...
		Если ОсобенностьУчетаНоменклатуры <> Перечисления.ОсобенностиУчетаНоменклатуры.ПродукцияИзНатуральногоМеха Тогда
...
Функция НайтиСоздатьСерию()
	...
	ЭлементБлокировки = Блокировка.Добавить("Справочник.СерииНоменклатуры");
...
Функция СоздатьСерию()
	...
	Если Не ПравоДобавленияСерий Тогда
		...
		ПредставлениеСерии = ПредставлениеСерии(НастройкиИспользованияСерий, ЭтаФорма);
	...
	Иначе
		...
		СерияОбъект.ВладелецСерии   = ВладелецСерии;
		СерияОбъект.ГоденДо         = ГоденДо;
...
&НаСервере
Функция ПредставлениеСерии(ПараметрыШаблона, ЗначенияРеквизитов, ДобавитьСловоНовая = Ложь)
