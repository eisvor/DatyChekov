...
Процедура ОповещениеПоискаПоШтрихкоду(Штрихкод, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ПустаяСтрока(Штрихкод) Тогда
		...
		СтруктураДанныхШтрихкода = ОбработкаТабличнойЧастиТоварыКлиентСервер.СтруктураДанныхШтрихкода(Штрихкод, 1);
		...
		Если ЗакрытьФорму И Не ДоступностьПравНаДобавлениеСерий(Объект.Серии, ЕстьПравоДоступаДобавление) Тогда
			ПоказатьПредупреждение(, НСтр("ru='Недостаточно прав на добавление серий.
										|Серии не будут зарегистрированы.'"),,НСтр("ru='Добавление серий'"));
			Возврат;
		КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыУчетнойПолитикиСерий = Новый ФиксированнаяСтруктура(ЗначениеНастроекПовтИсп.
		ПараметрыПолитикиУчетаСерийПоВидуНоменклатуры(РеквизитыНоменклатуры.ВидНоменклатуры, Параметры.Магазин, Номенклатура));
		
	ВладелецСерии = ПараметрыУчетнойПолитикиСерий.ВладелецСерии;
	...
	ТоварПредставление = СтрЗаменить(ТоварПредставление,"%ПредставлениеТовара%",ОбщегоНазначенияРТ.ПредставлениеНоменклатуры(Параметры.Номенклатура, Параметры.Характеристика));
	...
	Если ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ПродукцияИзНатуральногоМеха Тогда
	...
	Если ВидНоменклатуры = ВладелецСерии Тогда
		СтрокаЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Серии вида номенклатуры (%1)'"), ВидНоменклатуры);
		
		Элементы.ГруппаСерии.Заголовок = СтрокаЗаголовка; 
	КонецЕсли;
	
	ЕстьПравоДоступаДобавление = ПравоДоступа("Добавление", Метаданные.Справочники.СерииНоменклатуры);
	Если Не ЕстьПравоДоступаДобавление Тогда
		Элементы.СерииСерия.Доступность = Ложь;
	КонецЕсли;
...
Процедура СерииНомерНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТекущиеДанные = Элементы.Серии.ТекущиеДанные;
	ВводимыйРеквизит = "Номер";
	ПроверяемыйРеквизит = "ГоденДо";
	
	ПараметрыПоиска = Новый Структура;
	ПараметрыПоиска.Вставить(ВводимыйРеквизит, "");
	ПараметрыПоиска.Вставить(ПроверяемыйРеквизит,Неопределено);
	ПараметрыПоиска.Вставить("ВладелецСерии",ПараметрыУчетнойПолитикиСерий.ВладелецСерии);
	...
	Наименование = "<Новая>" + " " + Формат(ТекущиеДанные.ГоденДо,ПараметрыУчетнойПолитикиСерий.ФорматнаяСтрокаСрокаГодности);
	...
	ПоказатьВыборИзСписка(ОписаниеОповещения, СписокВыбора, Элемент, СписокВыбора.Количество()-1);
...
Процедура СерииПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	Если Не ОтменаРедактирования Тогда
		...
		Элементы.ДекорацияНедостаточноПрав.Видимость = Не ДоступностьПравНаДобавлениеСерий(Объект.Серии, ЕстьПравоДоступаДобавление);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗавершитьВводСерий(Команда)
	...
	Если Не ДоступностьПравНаДобавлениеСерий(Объект.Серии, ЕстьПравоДоступаДобавление) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Недостаточно прав на добавление серий.
										|Серии не будут зарегистрированы.'"),,НСтр("ru='Добавление серий'"));
		Возврат;
	КонецЕсли;
...
Функция ОбработатьШтрихкодыСервер(ДанныеШтрихкодов, КэшированныеЗначения, ИдентификаторТекущейСтроки = Неопределено)
	...
	Если ШтрихкодыПоТипам.БезТипа.Количество() > 0 Тогда
		...
		|		И СерииНоменклатуры.ВладелецСерии = &ВладелецСерии
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		Запрос.УстановитьПараметр("ВладелецСерии"  ,ПараметрыУчетнойПолитикиСерий.ВладелецСерии);
...
Процедура ПолучитьИнформациюПоКиЗДобавитьСерию(НомерКиЗ, ИдентификаторТекущейСтроки, КэшированныеЗначения)
	...
	ОбработаннаяСтрока = ПодключаемоеОборудованиеРТ.НоваяСерияПоИнформацииПоКиЗ(ИнформацияПоКиЗ, ПараметрыОбработки, Объект.Серии, ИдентификаторТекущейСтроки);
...
Процедура СохранитьВводСерийСервер()
	...
	Для Каждого СтрТабл Из ТаблицаСерий Цикл
		...
		Если СтрТабл.Количество > 0 Тогда
			...
			Если ЕстьПравоДоступаДобавление Тогда
				НомерСерии = СтрТабл.Номер;
				...
				Если Не ЗначениеЗаполнено(СтрТабл.Серия) Тогда
					...
					СправочникОбъект.ВладелецСерии   = ПараметрыУчетнойПолитикиСерий.ВладелецСерии;
					СправочникОбъект.ГоденДо         = СтрТабл.ГоденДо;
					СправочникОбъект.НомерКИЗГИСМ    = СтрТабл.НомерКИЗГИСМ;
					
					Если ЗначениеЗаполнено(СправочникОбъект.НомерКиЗГИСМ) 
						И Не ЗначениеЗаполнено(СправочникОбъект.Номер) Тогда 
						
						НомерСерии = СправочникОбъект.НомерКИЗГИСМ;
					КонецЕсли;
					
					СправочникОбъект.Номер = НомерСерии;
...
Процедура ИзменитьСериюКлиент()
	...
	ОценкаПроизводительностиРТКлиент.НачатьЗамер(
		Истина,
		"Справочник.СерииНоменклатуры.Форма.ФормаЭлемента.Открытие");
	
	ЗначенияЗаполненияСерии = Новый Структура;
	ЗначенияЗаполненияСерии.Вставить("ВладелецСерии", ВладелецСерии);
	...
	ОткрытьФорму("Справочник.СерииНоменклатуры.Форма.ФормаЭлемента",
		ПараметрыФормыСерии,
		ЭтаФорма,,,,,
		РежимОткрытияОкна);
...
Функция ДоступностьПравНаДобавлениеСерий(Серии, ЕстьПравоДоступаДобавление)
	
	Результат = Истина;
	Если Не ЕстьПравоДоступаДобавление Тогда
		
		РегистрацияНовыхСерий = Ложь;
