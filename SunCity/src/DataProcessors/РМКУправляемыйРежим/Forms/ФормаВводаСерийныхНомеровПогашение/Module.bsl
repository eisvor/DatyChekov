...
Процедура ОбработатьДанныеПоКодуСервер(СтруктураРезультат) Экспорт
	...
	ИначеЕсли СтрокаРезультата.Свойство("Номенклатура") Тогда
		...
		Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.ПодарочныйСертификат Тогда
			ДобавитьНайденныеПозицииТоваровСервер(СтрокаРезультата);
		Иначе
...
Функция ДобавитьНайденныеПозицииТоваровСервер(СтруктураПараметров, ДополнительныеПараметры = Неопределено)
	...
	РеквизитыСертификата = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтруктураПараметров.Номенклатура, "Номинал, ИспользоватьСерийныеНомера, ЧастичноеПогашение, ПроизвольныйНоминал");
	
	ПодарочныйСертификат = СтруктураПараметров.Номенклатура;
	...
	СтрокаПогашения.СерийныйНомер = СерийныйНомер;
	ЗаполнитьЗначенияСвойств(СтрокаПогашения, РеквизитыСертификата);
	ДанныеСертификата = ПолучитьДанныеПоСертификатуНаСервере(ПодарочныйСертификат, СерийныйНомер);
	СтрокаПогашения.Остаток = ДанныеСертификата.Остаток;
	Если РеквизитыСертификата.ПроизвольныйНоминал Тогда
		СтрокаПОгашения.Номинал = ДанныеСертификата.Номинал;
	КонецЕсли;
...
Функция ДобавитьНайденныеСерийныеНомера(СтруктураНомера) Экспорт
	...
	Если СтрокиССерийнымНомером.Количество() = 0  Тогда
		...
		ПодарочныйСертификат = СтруктураНомера.Номенклатура;
		СерийныйНомер = СтруктураНомера.СерийныйНомер;
		
		РеквизитыСертификата = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПодарочныйСертификат, "Номинал, ИспользоватьСерийныеНомера, ЧастичноеПогашение, ПроизвольныйНоминал");
		
		СтрокаПогашения = Список.Добавить();
		СтрокаПогашения.ПодарочныйСертификат = ПодарочныйСертификат;
		СтрокаПогашения.СерийныйНомер = СерийныйНомер;
		ЗаполнитьЗначенияСвойств(СтрокаПогашения, РеквизитыСертификата);
		ДанныеСертификата = ПолучитьДанныеПоСертификатуНаСервере(ПодарочныйСертификат, СерийныйНомер);
		СтрокаПогашения.Остаток = ДанныеСертификата.Остаток;
...
Процедура ЗаполнитьСписокВыбора(МассивСерийныхНомеров)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПодарочныеСертификаты.Остатки(, НомерСертификата В (&МассивСерийныхНомеров)) КАК ПодарочныеСертификатыОстатки
	|		ПО СерийныеНомера.Владелец = ПодарочныеСертификатыОстатки.ПодарочныйСертификат
	|			И СерийныеНомера.Ссылка = ПодарочныеСертификатыОстатки.НомерСертификата
	|ГДЕ
...
Процедура ЗаполнитьДанныеСтрокиСервер(ИдентификаторСтроки)
	...
	Если СтрокаСписка <> Неопределено Тогда
		...
		РеквизитыСертификата = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаСписка.ПодарочныйСертификат, "Номинал, ИспользоватьСерийныеНомера, ЧастичноеПогашение, ПроизвольныйНоминал");
		
		СтрокаСписка.СерийныйНомер = Справочники.СерийныеНомера.ПустаяСсылка();
		ЗаполнитьЗначенияСвойств(СтрокаСписка, РеквизитыСертификата);
