...
Процедура ОбработатьДанныеПоКодуСервер(СтруктураРезультат) Экспорт
	...
	Если СтрокаРезультата.Свойство("Карта") Тогда
		...
		ПодключаемоеОборудованиеРТ.ВставитьПредупреждениеОНевозможностиОбработкиКарт(
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьРезервированиеПоЗаказамПокупателей =
		ПолучитьФункциональнуюОпцию("ИспользоватьРезервированиеПоЗаказамПокупателей");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПустаяСтрока(ВидПоискаПриОткрытии) Тогда
		...
		ИначеЕсли ИмяТекущейСтраницы = "СтраницаТоварыРасширение" Тогда
			ЭтотОбъект.ТекущийЭлемент = Элементы.ТоварыРасширение;
		ИначеЕсли ИмяТекущейСтраницы = "СтраницаТоварыГруппы" Тогда
			ЭтотОбъект.ТекущийЭлемент = Элементы.ТоварыГруппы;
		ИначеЕсли ИмяТекущейСтраницы = "СтраницаТоварыГруппыРасширение" Тогда
			ЭтотОбъект.ТекущийЭлемент = Элементы.ТоварыГруппыРасширение;
		КонецЕсли;
	Иначе
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	СохраненныйРодитель = Настройки.Получить("РодительАктивизированнойНоменклатуры");
	
	Если Не СохраненноеИмяТекущейСтраницы = Неопределено
		И Не СохраненноеИмяТекущейСтраницы = ИмяТекущейСтраницы Тогда
		
		ИмяТекущейСтраницы = СохраненноеИмяТекущейСтраницы;
		РодительАктивизированнойНоменклатуры = СохраненныйРодитель;
		ПоказРасширения = Найти(ИмяТекущейСтраницы, "Расширение") > 0;
		
		ТекущаяСтраница = Элементы.Найти(ИмяТекущейСтраницы);
		...
		Если Найти(ИмяТекущейСтраницы, "СтраницаТоварыГруппы") > 0 Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗакрытьФормуПодбораИзНовогоРМК" Тогда
		Если Открыта() Тогда
			Закрыть();
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьНадписьПутиВДереве(Номенклатура = Неопределено)
	...
	Если ИмяТекущейСтраницы = "СтраницаТовары" ИЛИ ИмяТекущейСтраницы = "СтраницаТоварыРасширение" Тогда
		...
		Если НЕ ТекущаяСтрока = Неопределено И ЗначениеЗаполнено(ТекущаяСтрока.Родитель) Тогда
			...
			Исключение
				РодительАктивизированнойНоменклатуры = Справочники.Номенклатура.ПустаяСсылка();
...
Процедура ОбработатьВыборНоменклатуры(СтандартнаяОбработка)
	...
	ИмяДинамическогоСписка = СтрЗаменить(ИмяТекущейСтраницы, "Страница", "");
	ДинамическийСписок = Элементы.Найти(ИмяДинамическогоСписка);
	...
	КонецЕсли;
	Номенклатура = ДинамическийСписок.ТекущаяСтрока;
	Если Не ЗначениеЗаполнено(Номенклатура) Тогда
		Возврат;
	КонецЕсли;
	СтруктураИспользованияХарактеристикПоНоменклатуре =
		ПодборТоваровВызовСервера.СтруктураИспользованияХарактеристикПоНоменклатуре(Номенклатура);
	СтандартнаяОбработка = Ложь;
...
Процедура ВыполнитьРасширенныйПоискПоСКД()
	...
	МаркетинговыеАкцииСервер.УстановитьОтборВКомпоновщикеПоСписку(Товары.КомпоновщикНастроек,
...
Процедура ВыполнитьПоискВезде()
	...
	|	КодыТоваровSKU.Номенклатура КАК Номенклатура,
	|	КодыТоваровSKU.Характеристика КАК Характеристика
	|ПОМЕСТИТЬ ТаблицаПоSKU
	|ИЗ
	|	РегистрСведений.КодыТоваровSKU КАК КодыТоваровSKU
	|ГДЕ
	|	&ПоискПоSKU
	|	И КодыТоваровSKU.SKU = &ПолеПоискаЧисло
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|
	|ВЫБРАТЬ
	|	ТаблицаПоSKU.Номенклатура
	|ИЗ
	|	ТаблицаПоSKU КАК ТаблицаПоSKU
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Запрос.УстановитьПараметр("ПоискПоSKU", ПоискПоSKU);
	Запрос.УстановитьПараметр("ПолеПоискаЧисло", ПолеПоискаЧисло);
	
	Результат = Запрос.ВыполнитьПакет();
	РезультатТовары = Результат[6].Выгрузить();
	РезультатХарактеристики = Результат[7].Выгрузить();
...
Процедура РассчитатьОстаткиИЦены()
	...
	Иначе
		...
		ДинамическийСписокНоменклатуры = Элементы.Найти(ИмяДинамическогоСпискаНоменклатуры);
		Если ТипЗнч(ДинамическийСписокНоменклатуры) = Тип("ТаблицаФормы") Тогда
			ТекущаяСтрока = ДинамическийСписокНоменклатуры.ТекущаяСтрока;
		КонецЕсли;
		Если ТекущаяСтрока = Неопределено Тогда
			ТаблицаОстаткиЦены.Очистить();
			Возврат;
		...
		Номенклатура = ТекущаяСтрока;
		ХарактеристикаТекущаяСтрока = Неопределено;
		ИмяДинамическогоСписка = СтрЗаменить(ИмяТекущейСтраницы, "Страница", "");
		ДинамическийСписок = Элементы.Найти(ИмяДинамическогоСписка);
		Если ТипЗнч(ДинамическийСписок) = Тип("ТаблицаФормы") Тогда
			ХарактеристикаТекущаяСтрока = ДинамическийСписок.ТекущаяСтрока;
		КонецЕсли;
		Если ХарактеристикаТекущаяСтрока = Неопределено Тогда
			Характеристика = Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка();
		Иначе
			Характеристика = Характеристики.НайтиПоИдентификатору(ХарактеристикаТекущаяСтрока).Ссылка;
		КонецЕсли;
	КонецЕсли;
...
Функция ДанныеПоЦенообразованиюМагазина()
	...
	ЦенообразованиеМагазина = Новый Структура("ВидМинимальныхЦенПродажи");
