...
Процедура ОбработатьДисконтнуюКарту()
	...
	Если ЗначениеЗаполнено(ДисконтнаяКарта) Тогда
		...
		СписокТелефонов = ПодключаемоеОборудованиеРТ.ТелефоныОбъектаДляЧека(ДисконтнаяКарта);
...
Функция ПодготовитьДанные(Знач Телефон)
	...
	Возврат ШтрихкодированиеРТКлиентСервер.ПодготовитьТелефон(Телефон);
...
Функция ДанныеКорректны(Данные)
	...
	Возврат ШтрихкодированиеРТКлиентСервер.ТелефонКорректен(НСтр("ru = '7'") + Данные);
