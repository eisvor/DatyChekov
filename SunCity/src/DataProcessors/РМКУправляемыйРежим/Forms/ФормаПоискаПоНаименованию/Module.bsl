...
Процедура ПерейтиКСпискуНоменклатуры(Команда)
	...
	ТекстЗаголовка = НСтр("ru = 'Найдено позиций: %1'");
	ТекстЗаголовка = СтрШаблон(ТекстЗаголовка, ТаблицаНоменклатуры.Количество());
	
	Элементы.ГруппаТаблицаНоменклатуры.Видимость  = Истина;
	Элементы.ГруппаТаблицаХарактеристик.Видимость = Ложь;
	Элементы.НадписьИнформацияПоиска.Заголовок    = ТекстЗаголовка;
...
Процедура ПоискНоменклатуры()
	...
	ТекстЗаголовка = НСтр("ru = 'Найдено позиций: %1'");
	ТекстЗаголовка = СтрШаблон(ТекстЗаголовка, Выборка.Количество());
