...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РежимПодбораСУчетомЗапрещенныхКПродаже ИЛИ РежимПодбораСУчетомАссортимента Тогда
		...
		Если УсловиеАссортимента = "РазрешеныЗакупки" Тогда
			Элементы.СкрыватьЗапрещенные.Заголовок = Нстр("ru='Скрывать запрещенные к закупке'");
		Иначе
			Элементы.СкрыватьЗапрещенные.Заголовок = Нстр("ru='Скрывать запрещенные к продаже'");
		КонецЕсли;
	Иначе
	...
	Если Параметры.Свойство("ДоговорКонтрагента") Тогда
		...
		ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(
			ПодборТаблицаНоменклатура,
			"Номенклатура.ДоговорКонтрагента",
			ДоговорКонтрагента,
			Истина,
			ВидСравненияКомпоновкиДанных.Равно);
	Иначе
	...
	РаботаСНоменклатурой.ПриСозданииНаСервереФормаСпискаНоменклатуры(
		ЭтотОбъект, 
		Элементы.ПодборТаблицаНоменклатура.КоманднаяПанель, 
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = РаботаСНоменклатуройКлиент.ОписаниеОповещенийПодсистемы().ЗагрузкаНоменклатуры Тогда
		Если Параметр.СозданныеОбъекты.Количество() > 0 Тогда
			...
			Номенклатура = Параметр.СозданныеОбъекты[0].Номенклатура;
			РеквизитыНоменклатуры = РеквизитыНоменклатуры(
				Номенклатура,
				"Родитель, ВидНоменклатуры, ТоварнаяКатегория, Марка");
			
			НовыйОбъект = Новый Структура;
			НовыйОбъект.Вставить("Номенклатура", Номенклатура);
			НовыйОбъект.Вставить("Родитель", РеквизитыНоменклатуры.Родитель);
			НовыйОбъект.Вставить("ВидНоменклатуры", РеквизитыНоменклатуры.ВидНоменклатуры);
			НовыйОбъект.Вставить("ТоварнаяКатегория", РеквизитыНоменклатуры.ТоварнаяКатегория);
			НовыйОбъект.Вставить("Марка", РеквизитыНоменклатуры.Марка);
			
			ЗавершитьОбработкуДанныхПоКодуКлиент(Новый Структура("АктивизироватьСтроку", НовыйОбъект));
		КонецЕсли;
	КонецЕсли;
	// Конец ЭлектронноеВзаимодействие.РаботаСНоменклатурой
КонецПроцедуры
...
Процедура КорзинаПриИзменении()
	...
	ПозицииПрописью = НРег(ЧислоПрописью(Объект.Корзина.Количество(), Нстр("ru='Л = ru_RU; ДП = Ложь; НП = Истина; НД = Ложь;'"), НСтр("ru = 'позиция,позиции,позиций,ж,,,,,0'")));
...
Процедура ПодборТаблицаНоменклатураВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Иначе
		...
		Иначе
			...
			СтруктураПараметры.Вставить("ДоговорКонтрагента",    Элементы.ПодборТаблицаНоменклатура.ТекущиеДанные.ДоговорКонтрагента);
...
Процедура ПерейтиКРодителю(Команда)
	...
	Если Элементы.ВариантыНавигации.ТекущаяСтраница = Элементы.ВариантыНавигации.ПодчиненныеЭлементы.НавигацияВидыНоменклатуры Тогда
		Элементы.ВидыНоменклатуры.ТекущаяСтрока = ПодборТоваровВызовСервера.ВидНоменклатуры(ТекущиеДанные.Номенклатура);
	ИначеЕсли Элементы.ВариантыНавигации.ТекущаяСтраница = Элементы.ВариантыНавигации.ПодчиненныеЭлементы.НавигацияИерархияНоменклатуры Тогда
		Элементы.ИерархияНоменклатуры.ТекущаяСтрока = ПодборТоваровВызовСервера.РодительНоменклатуры(ТекущиеДанные.Номенклатура);
	ИначеЕсли Элементы.ВариантыНавигации.ТекущаяСтраница = Элементы.ВариантыНавигации.ПодчиненныеЭлементы.НавигацияСегментыНоменклатуры Тогда
		Элементы.СегментыНоменклатуры.ТекущаяСтрока = ПодборТоваровВызовСервера.СегментНоменклатуры(ТекущиеДанные.Номенклатура);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПодборТекущаяНоменклатура) Тогда
		...
		Если ПодборТекущаяНоменклатураТипНоменклатуры <> ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга") Тогда	
			Если ТекущийРежимОтображенияОстатков = 2 Тогда
				Если РежимПодбораВЗакупки Тогда
					СтруктураИнформацииОТоваре = ПодборТоваровВызовСервера.ЦенаЗакупки(ПодборТекущаяНоменклатура, ПодборТекущаяХарактеристика, Магазин, Контрагент, СсылкаНаПоступление);						
				Иначе
					СтруктураИнформацииОТоваре = ПодборТоваровВызовСервера.ЦенаПродажи(ПодборТекущаяНоменклатура, ПодборТекущаяХарактеристика, Магазин, РежимПодбораСУчетомМинимальныхЦен);
				КонецЕсли;
			ИначеЕсли ТекущийРежимОтображенияОстатков = 1 Тогда
				Если РежимПодбораВЗакупки Тогда
					СтруктураИнформацииОТоваре = ПодборТоваровВызовСервера.ЦенаЗакупкиИОстаткиНоменклатуры(ПодборТекущаяНоменклатура, ПодборТекущаяХарактеристика, Магазин, Истина, Контрагент, СсылкаНаПоступление);
				Иначе					
					СтруктураИнформацииОТоваре = ПодборТоваровВызовСервера.ЦенаПродажиИОстаткиНоменклатуры(ПодборТекущаяНоменклатура, ПодборТекущаяХарактеристика, Магазин, Истина, РежимПодбораСУчетомМинимальныхЦен);
				КонецЕсли;
			Иначе
				Если РежимПодбораВЗакупки Тогда
					СтруктураИнформацииОТоваре = ПодборТоваровВызовСервера.ЦенаЗакупкиИОстаткиНоменклатуры(ПодборТекущаяНоменклатура, ПодборТекущаяХарактеристика, Магазин, Ложь, Контрагент, СсылкаНаПоступление);
				Иначе
					СтруктураИнформацииОТоваре = ПодборТоваровВызовСервера.ЦенаПродажиИОстаткиНоменклатуры(ПодборТекущаяНоменклатура, ПодборТекущаяХарактеристика, Магазин, Ложь, РежимПодбораСУчетомМинимальныхЦен);
				КонецЕсли;
			КонецЕсли;
		Иначе
			Если РежимПодбораВЗакупки Тогда
...
Процедура ДобавитьВКорзину(СтруктураПараметры, КоличествоУпаковок)
	...
	Если ЗапрашиватьКоличество Тогда
		...
		ПараметрыФормы.Вставить("ДоговорКонтрагента",  СтруктураПараметры.ДоговорКонтрагента);
	...
	Иначе
		...
		НоваяСтрока.Вставить("ДоговорКонтрагента",    СтруктураПараметры.ДоговорКонтрагента);
			...
			Если НеРазрешатьРазличныеДоговорыПлатежногоАгента Тогда
				Если ДоговорКонтрагента = Неопределено Тогда
					...
					ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(
						...
						Истина,
						ВидСравненияКомпоновкиДанных.Равно);
				ИначеЕсли Не ДоговорКонтрагента = НоваяСтрока.ДоговорКонтрагента Тогда
		...
		Если РежимПодбораСУчетомАссортимента Тогда
			Если РежимПодбораСУчетомЗапрещенныхКПродаже И СкрыватьЗапрещенные Тогда
				Если НЕ ПродажиОптовые Тогда
					...
					СвойстваСписка.ТекстЗапроса = АссортиментСервер.ТекстЗапросаДинамическогоСпискаДляПодбора(УсловиеАссортимента, Истина);
					ОбщегоНазначения.УстановитьСвойстваДинамическогоСписка(Элементы.Список, СвойстваСписка);
				Иначе
					СвойстваСписка = ОбщегоНазначения.СтруктураСвойствДинамическогоСписка();
					СвойстваСписка.ОсновнаяТаблица = "Справочник.Номенклатура";
					СвойстваСписка.ДинамическоеСчитываниеДанных = Истина;
					СвойстваСписка.ТекстЗапроса = АссортиментСервер.ТекстЗапросаДинамическогоСпискаДляПодбора(УсловиеАссортимента, Ложь);
					ОбщегоНазначения.УстановитьСвойстваДинамическогоСписка(Элементы.Список, СвойстваСписка);
				КонецЕсли;
				//
...
Процедура ЗавершитьОбработкуДанныхПоКодуКлиент(СтруктураПараметров)
	...
	Если СтруктураПараметров.Свойство("АктивизироватьСтроку") Тогда
		...
		СтруктураСтроки.Вставить("ТоварнаяКатегория");
		СтруктураСтроки.Вставить("Марка");
		
		ЗаполнитьЗначенияСвойств(СтруктураСтроки, СтруктураПараметров.АктивизироватьСтроку);
		...
		ИначеЕсли Элементы.ВариантыНавигации.ТекущаяСтраница = Элементы.ВариантыНавигации.ПодчиненныеЭлементы.НавигацияВидыНоменклатуры Тогда
			...
			ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(
				...
				ВидСравненияКомпоновкиДанных.Равно);
		ИначеЕсли Элементы.ВариантыНавигации.ТекущаяСтраница = Элементы.ВариантыНавигации.ПодчиненныеЭлементы.НавигацияТоварныеМарки Тогда
			Элементы.ТоварныеМарки.ТекущаяСтрока = СтруктураСтроки.Марка;
			ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(
				ПодборТаблицаНоменклатура,
				"Марка",
				ТекущаяНавигация.ПоТоварнойМарке,
				ЗначениеЗаполнено(ТекущаяНавигация.ПоТоварнойМарке),
				ВидСравненияКомпоновкиДанных.Равно);
		ИначеЕсли Элементы.ВариантыНавигации.ТекущаяСтраница = Элементы.ВариантыНавигации.ПодчиненныеЭлементы.НавигацияТоварныеКатегории Тогда
			Элементы.ТоварныеКатегории.ТекущаяСтрока = СтруктураСтроки.ТоварнаяКатегория;
			ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(
				ПодборТаблицаНоменклатура,
				"ТоварнаяКатегория",
				ТекущаяНавигация.ПоТоварнойКатегории,
				ЗначениеЗаполнено(ТекущаяНавигация.ПоТоварнойКатегории),
				ВидСравненияКомпоновкиДанных.Равно);
		КонецЕсли;
...
&НаСервере
Функция РеквизитыНоменклатуры(Номенклатура, Реквизиты)
