...
Процедура ДекорацияСинхронизацияДанныхОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	Если НавигационнаяСсылкаФорматированнойСтроки = "Синхронизация" Тогда
        ...
        // &ЗамерПроизводительности
        ОценкаПроизводительностиРТКлиент.НачатьЗамер(
                 Истина, "ОбщаяФорма.НастройкиСинхронизацииДанных.Открытие"); 
	...
	Если ЭтоПереходДалее Тогда
		...
		Если Не ПустаяСтрока(СтрокаПерехода.ИмяОбработчикаПриПереходеДалее) Тогда
			...
			Попытка
				...
				#Иначе
					Отказ = Истина;
					ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Операция не поддерживается в ВебКлиенте'"));
				#КонецЕсли
			Исключение
...
Процедура Подключаемый_СтраницаПараметрыСвертки_ПриПереходеДалее(Отказ)
	...
	Если Не ЗначениеЗаполнено(Объект.ДатаСверткиИБ) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,,"ДатаСверткиИБ", "Объект.ДатаСверткиИБ", Отказ);
		Возврат;
	КонецЕсли;
...
Функция ЕстьУзлыРИБ(ДатаСверткиИБ)
	...
	Запрос.Текст = 
		...
		|	И ОбменРозницаРозница.ДатаНачалаВыгрузкиДокументов < &ДатаСвертки";
...
Процедура ИзменитьДатуНачалаВыгрузкиДокументов(Отказ)
	...
	|ИЗ
	|	ПланОбмена.ОбменРозницаУправлениеТорговлей22 КАК ОбменУТ10
	|ГДЕ
	...
	Запрос.УстановитьПараметр("ЭтотУзелУТ10", ПланыОбмена.ОбменРозницаУправлениеТорговлей22.ЭтотУзел());
