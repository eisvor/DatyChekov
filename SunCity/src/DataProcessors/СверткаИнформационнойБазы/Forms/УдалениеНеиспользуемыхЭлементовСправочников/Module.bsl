...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("АвтоТест") Тогда // Возврат при получении формы для анализа.
		Возврат;
	КонецЕсли;
	МассивИсключаемыхСправочников = ПолучитьИсключаемыеСправочники();
...
Процедура ПриОткрытии(Отказ)
	...
	Если ОбщегоНазначенияКлиент.ПредлагатьСоздаватьРезервныеКопии() Тогда
		...
		ФорматированнаяСтрока = СтроковыеФункцииКлиент.ФорматированнаяСтрока(Текст);
		Элементы.ДекорацияПредупреждение.Заголовок = ФорматированнаяСтрока;
	КонецЕсли;
...
Процедура ДекорацияЖурналРегистрацииНажатие(Элемент)
    ...
    ОценкаПроизводительностиРТКлиент.НачатьЗамер(
             Истина, "Обработка.ЖурналРегистрации.Форма.ЖурналРегистрации.Открытие");
...
Процедура СписокСправочниковПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если Элемент.ТекущийЭлемент.Имя = "СписокСправочниковЕстьОтборКУдалению"
		Или Элемент.ТекущийЭлемент.Имя = "СписокСправочниковЕстьОтборКИсключению" Тогда
...
Процедура ВыполнитьПоиск(Команда)
	...
	Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы.ФормаВыполнитьПоиск.Видимость = Ложь;
...
Процедура НастроитьУсловияОтбора(Команда)
    ...
    // &ЗамерПроизводительности
    ОценкаПроизводительностиРТКлиент.НачатьЗамер(
             Истина, "Обработка.СверткаИнформационнойБазы.Форма.ФормаНастройкаОтбора.Открытие");
	...
	ОткрытьФорму("Обработка.СверткаИнформационнойБазы.Форма.ФормаНастройкаОтбора", ПараметрыФормыОтбора,,,,, Новый ОписаниеОповещения("НастроитьУсловияОтбораЗавершение", ЭтотОбъект, Новый Структура("СтрокаТаблицыСправочников", СтрокаТаблицыСправочников)), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ЗаписатьВЖурналСообщитьПользователю(ПараметрыЖурнала, УровеньЖурнала, ИмяСобытия, Знач Комментарий = "", ИнформацияОбОшибке = Неопределено) Экспорт
	...
	Если УровеньЖурнала = УровеньЖурналаРегистрации.Ошибка 
		...
		ОбщегоНазначения.СообщитьПользователю(СокрЛП(ТестСообщенияПользователю),ПараметрыЖурнала.Данные);
	КонецЕсли;
...
Процедура ОбработчикОжиданияУдаление()
	...
	КонецЦикла;
	Отказ = Ложь;
	ДатаВремяЗапуска = ОбщегоНазначенияКлиент.ДатаСеанса();
	...
	Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы.ФормаЗакрыть.КнопкаПоУмолчанию = Истина;
	Элементы.ФормаКоманднаяПанель.ПодчиненныеЭлементы.ФормаЗакрыть.Заголовок = "Закрыть";
КонецПроцедуры
...
Процедура НайтиИУдалитьНеиспользуемыеЭлементыСправочников(МассивПараметров, Отказ)
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не удалось снять монопольный режим'"));
	КонецПопытки;
...
Процедура НайтиИУдалитьНеиспользуемыеЭлементыСправочника(СтруктураПараметров, ИсключенияПоискаСсылок, 
	...
	Если СтруктураПараметров.ЕстьОтборКУдалению ИЛИ СтруктураПараметров.ЕстьОтборКИсключению Тогда
		...
		ИначеЕсли НЕ СтруктураПараметров.ЕстьОтборКУдалению И СтруктураПараметров.ЕстьОтборКИсключению Тогда
			...
			|ИЗ Справочник."+ИмяСправочника+" КАК Справочник
			|ГДЕ (НЕ Справочник.Предопределенный)
			|И Ссылка НЕ В (ВЫБРАТЬ Ссылка ИЗ РезультатОтбораКИсключению)";
			Запрос.Выполнить();
	...
	Пока НЕ РезультатЗапроса.Пустой() Цикл // Обработка порциями
		...
		Для Каждого СтрокаТаблицыСсылок Из ТабСсылок Цикл
			...
			Если ПодчиненныеСправочники.Количество() > 0 
				...
				Если ПодчиненныеСправочники[СтрокаТаблицыСсылок.Метаданные.Имя] <> Неопределено 
					И СтрокаТаблицыСсылок.Данные.Владелец = СтрокаТаблицыСсылок.Ссылка Тогда
...
Процедура УдалитьЭлементСправочника(Ссылка, СчетчикУдаленныхОбъектов)
	...
	Исключение
		...
		ЗаписатьВЖурналСообщитьПользователю(ПараметрыЖурнала,УровеньЖурнала,"",ИмяСобытия,ОписаниеОшибки());
		ОтменитьТранзакцию();
	...
	ЗаписатьВЖурналСообщитьПользователю(ПараметрыЖурнала, УровеньЖурнала, "", ИмяСобытия);
...
Функция ПолучитьИсключаемыеСправочники()
	...
	МассивИсключений.Добавить(МетаданныеСправочники.ИдентификаторыОбъектовМетаданных);
	МассивИсключений.Добавить(МетаданныеСправочники.ИдентификаторыОбъектовРасширений);
	МассивИсключений.Добавить(МетаданныеСправочники.КлассификаторБанков);
	МассивИсключений.Добавить(МетаданныеСправочники.КлючевыеОперации);
	МассивИсключений.Добавить(МетаданныеСправочники.ОчередьЗаданий);
	МассивИсключений.Добавить(МетаданныеСправочники.ПоставляемыеДанные);
	МассивИсключений.Добавить(МетаданныеСправочники.ПредопределенныеВариантыОтчетов);
	МассивИсключений.Добавить(МетаданныеСправочники.СообщенияСистемы);
	МассивИсключений.Добавить(МетаданныеСправочники.СценарииОбменовДанными);
	МассивИсключений.Добавить(МетаданныеСправочники.ТомаХраненияФайлов);
	МассивИсключений.Добавить(МетаданныеСправочники.ШаблоныЗаданийОчереди);
