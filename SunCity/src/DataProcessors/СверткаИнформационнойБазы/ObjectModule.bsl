...
//
Процедура ЗаписатьВЖурналСообщитьПользователю(ПараметрыЖурнала, УровеньЖурнала, ИмяСобытия, Знач Комментарий = "", ИнформацияОбОшибке = Неопределено)
...
Процедура СформироватьДокументыВводОстатковПоНакопительнымСкидкам()
	...
	ДокОст.Ответственный =  Пользователи.ТекущийПользователь();
...
Процедура СформироватьДокументыВводНачальныхОстатковУзла()
	...
	Если НЕ Результат.Пустой() Тогда
		...
		МодульОбменДаннымиПереопределяемый.ЗаполнитьОстатки(МассивМагазины, ДанныеДляЗаполненияВводаОстатков,, Отказ);
		Попытка
			...
			КонецЕсли; 
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Свертка ИБ'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура УдалитьДвиженияПоРегиструСРегистратором(ТипОбъектаМД, ИмяРегистра, МассивПропускаемыхДокументов, МассивУдаляемыхДокументов, Периодический = Истина)
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ТипОбъектаМД+"."+ИмяРегистра);
	
	ТекстУсловий = "";
	Если МассивПропускаемыхДокументов.Количество() > 0 Тогда
		
		ИменаПропускаемыхДокументов = Новый Массив;
		
		Для каждого ПропускаемыйДокумент Из МассивПропускаемыхДокументов Цикл
			Если Не ОбъектМетаданных.СтандартныеРеквизиты.Регистратор.Тип.СодержитТип(ТипЗнч(ПропускаемыйДокумент)) Тогда
				Продолжить;
			КонецЕсли;
			ИменаПропускаемыхДокументов.Добавить(ПропускаемыйДокумент.Метаданные().Имя);
		КонецЦикла;
		
		ИменаПропускаемыхДокументов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ИменаПропускаемыхДокументов);
			
		Если Периодический Тогда
			ТекстУсловий = ТекстУсловий + Символы.ПС + "Регистр.Период <= &ДатаКон";
		Иначе
			Для Каждого ИмяПропускаемогоДокумента Из ИменаПропускаемыхДокументов Цикл
				
				ТекстУсловий  = ТекстУсловий + Символы.ПС + СтрШаблон("ВЫБОР
					|	КОГДА Регистр.Регистратор ССЫЛКА Документ.%1
					|		ТОГДА ВЫРАЗИТЬ(Регистр.Регистратор КАК Документ.%1).Дата <= &Дата
					|		ИНАЧЕ ", ИмяПропускаемогоДокумента);
				
			КонецЦикла;
			
			ТекстУсловий = ТекстУсловий + "Ложь";
			
			ТекстКонец = "";
	
			Для СчетчикИмен = 1 По ИменаПропускаемыхДокументов.Количество() Цикл
				ТекстКонец = ТекстКонец + " КОНЕЦ";
			КонецЦикла;
			
			ТекстУсловий = ТекстУсловий + ТекстКонец;
		КонецЕсли;
	КонецЕсли;
	...
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, ТекстУсловий);
	...
	Если МассивУдаляемыхДокументов.Количество()>0 Тогда
		...
		Для каждого УдаляемыйДокумент Из МассивУдаляемыхДокументов Цикл
			Если Не ОбъектМетаданных.СтандартныеРеквизиты.Регистратор.Тип.СодержитТип(ТипЗнч(УдаляемыйДокумент)) Тогда
				Продолжить;
			КонецЕсли;
			ИменаУдаляемыхДокументов.Добавить(УдаляемыйДокумент.Метаданные().Имя);
		КонецЦикла;
		
		ИменаУдаляемыхДокументов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ИменаУдаляемыхДокументов);
			
		Если Периодический Тогда
			ТекстУсловий = ТекстУсловий + Символы.ПС + "Регистр.Период <= &ДатаКон";
		Иначе
			Для Каждого ИмяУдаляемогоДокумента Из ИменаУдаляемыхДокументов Цикл
				
				ТекстУсловий  = ТекстУсловий + Символы.ПС + СтрШаблон("ВЫБОР
					|	КОГДА Регистр.Регистратор ССЫЛКА Документ.%1
					|		ТОГДА ВЫРАЗИТЬ(Регистр.Регистратор КАК Документ.%1).Дата <= &Дата
					|		ИНАЧЕ ", ИмяУдаляемогоДокумента);
			...
			Для СчетчикИмен = 1 По ИменаУдаляемыхДокументов.Количество() Цикл
				ТекстКонец = ТекстКонец + " КОНЕЦ";
			КонецЦикла;
...
Процедура УдалитьДвиженияПоДатуСвертки() Экспорт
	...
	Для Каждого ТипДокумента Из МассивТипов Цикл
		...
		|ГДЕ Дата > &КонДата И Комментарий ПОДОБНО &Комментарий_Удалить
		|";
	КонецЦикла;
	Запрос.УстановитьПараметр("КонДата", ДатаСверткиИБ);
	Запрос.УстановитьПараметр("Комментарий_Удалить", "%Удалить при свертке базы%");
	Запрос.Текст = ТекстЗапроса;
