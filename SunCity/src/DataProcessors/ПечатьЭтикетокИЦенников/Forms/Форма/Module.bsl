...
Процедура МагазинПриИзменении(Элемент)
	...
	Если Элементы.СтраницыТиповПечати.ТекущаяСтраница = Элементы.СтраницаТовары Тогда
		...
		СтруктураПараметров.Вставить("ПересчитатьЦены");
		СтруктураПараметров.Вставить("ТекстСобытия");
		СтруктураПараметров.Вставить("ТекстОповещения");
		
		МагазинПриИзмененииСервер(СтруктураПараметров);
		
		Если НЕ ПустаяСтрока(СтруктураПараметров.ТекстСобытия) Тогда
			ПоказатьОповещениеПользователя(СтруктураПараметров.ТекстСобытия, , СтруктураПараметров.ТекстОповещения);
		КонецЕсли;
...
Процедура Печать(Команда)
	...
	ИначеЕсли Элементы.СтраницыТиповПечати.ТекущаяСтраница = Элементы.СтраницаДисконтныеКарты Тогда
		...
		ПараметрКоманды = Новый Массив;   // Добавим объект обработки печати ценников и этикеток в массив параметров печати.
		ПараметрКоманды.Добавить(ПредопределенноеЗначение("Справочник.ИнформационныеКарты.ПустаяСсылка"));
...
&НаСервере
Процедура МагазинПриИзмененииСервер(СтруктураПараметров = Неопределено)
	...
	ЗаполнитьОрганизациюВСтроках();
	
	Если СтруктураПараметров <> Неопределено И СтруктураПараметров.Свойство("ПересчитатьЦены") Тогда
		ПересчитатьЦеныСервер(СтруктураПараметров.ТекстСобытия, СтруктураПараметров.ТекстОповещения);
	КонецЕсли;
...
Функция ПолучитьПустуюТаблицуТоваров()
	...
	ТаблицаТоваров.Колонки.Добавить("Организация",Новый ОписаниеТипов("СправочникСсылка.Организации"),"Организация");
	ТаблицаТоваров.Колонки.Добавить("Количество", ОбщегоНазначенияРТКлиентСервер.ОписаниеТиповЧисла(15, 2));
	КвалификаторыСтроки = Новый КвалификаторыСтроки(200);
	ТаблицаТоваров.Колонки.Добавить("ШтрихАвтопроверки", Новый ОписаниеТипов("Строка",,,,КвалификаторыСтроки), "ШтрихАвтопроверки");
...
&НаСервере
Процедура ЗаполнитьОрганизациюВСтроках()
	
	ТаблицаТоваровСОрганизацией = Объект.Товары.Выгрузить(, "Номенклатура");
	
	ДобавитьСкладИОрганизациюПродажи(
		ТаблицаТоваровСОрганизацией,
		Объект.Магазин,
		МенеджерОборудованияВызовСервера.ПолучитьРабочееМестоКлиента());
		
	ТаблицаТоваровСОрганизацией.Индексы.Добавить("Номенклатура");
	
	Для каждого Строка Из Объект.Товары Цикл
		
		НайденныеСтроки = ТаблицаТоваровСОрганизацией.НайтиСтроки(Новый Структура("Номенклатура", Строка.Номенклатура));
		
		Строка.Организация = НайденныеСтроки[0].ОрганизацияПродажи;
...
&НаКлиенте
Функция ПродолжитьОбработатьДанныеПоКодуКлиент(СтруктураРезультат)
	
	СтрокаРезультата = СтруктураРезультат.ЗначенияПоиска[0];
	ДанныеПО = СтруктураРезультат.ДанныеПО;
	...
	ИначеЕсли СтрокаРезультата.Свойство("СерийныйНомер") Тогда
		...
		ИначеЕсли ИмяТаблицыВыборки = "ПодарочныеСертификаты" Тогда
...
Функция ПолучитьПараметрыПечати(РежимПечати)
	...
	ПараметрыПечати.Вставить("ПодарочныеСертификаты", ОбщегоНазначения.ТаблицаЗначенийВМассив(Объект.ПодарочныеСертификаты.Выгрузить()));
	ПараметрыПечати.Вставить("ДисконтныеКарты", ОбщегоНазначения.ТаблицаЗначенийВМассив(Объект.ДисконтныеКарты.Выгрузить()));
	ПараметрыПечати.Вставить("РегистрационныеКарты", ОбщегоНазначения.ТаблицаЗначенийВМассив(Объект.РегистрационныеКарты.Выгрузить()));
	ПараметрыПечати.Вставить("ВидМинимальныхЦенПродажи",   Объект.ВидМинимальныхЦенПродажи);
	ПараметрыПечати.Вставить("ВидЦены", Объект.ВидЦены);
	ПараметрыПечати.Вставить("ИспользоватьСправкиБ", Объект.ИспользоватьСправкиБ);
	ПараметрыПечати.Вставить("Магазин",          Объект.Магазин);
	ПараметрыПечати.Вставить("НажатаПечать",       Объект.НажатаПечать);
	ПараметрыПечати.Вставить("ПечатьИзДокумента",          Объект.ПечатьИзДокумента);
	ПараметрыПечати.Вставить("ПечатьИзОбработки",         Объект.ПечатьИзОбработки);
	ПараметрыПечати.Вставить("ПравилоЦенообразования", Объект.ПравилоЦенообразования);
	ПараметрыПечати.Вставить("УчитыватьЦеныМинимальные",    Объект.УчитыватьЦеныМинимальные);
	ПараметрыПечати.Вставить("Режим", Объект.Режим);
	ПараметрыПечати.Вставить("СтраницаПечати", Объект.СтраницаПечати);
	ПараметрыПечати.Вставить("ЦеныНаДату", Объект.ЦеныНаДату);
	ПараметрыПечати.Вставить("ЦеныНазначенныеДействующие", Объект.ЦеныНазначенныеДействующие);
...
#Область ПодборШтрихкодаНоменклатуры

&НаКлиенте
Процедура ПодобратьШтрихкодНоменклатуры(Команда)
	
	Если Не Элементы.Товары.ТекущиеДанные = Неопределено Тогда
		Номенклатура = Элементы.Товары.ТекущиеДанные.Номенклатура;
		
		ПараметрыФормы = Новый Структура();
		ОтборНоменклатура = Новый Структура("Номенклатура", Номенклатура);
		ПараметрыФормы.Вставить("Отбор", ОтборНоменклатура);
		
		Оповещение = Новый ОписаниеОповещения("ВыборШтрихкодаНоменклатурыЗавершение", ЭтотОбъект);
		ОткрытьФорму("Обработка.ПечатьЭтикетокИЦенников.Форма.ФормаВыбораШтрихкодаНоменклатуры", ПараметрыФормы,,,,,Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура ВыборШтрихкодаНоменклатурыЗавершение(ВыбранныйШК, ДополнительныеПараметры) Экспорт
	
	Если ВыбранныйШК <> "" И ВыбранныйШК <> Неопределено Тогда
		
		Элементы.Товары.ТекущиеДанные.Штрихкод = ВыбранныйШК;
