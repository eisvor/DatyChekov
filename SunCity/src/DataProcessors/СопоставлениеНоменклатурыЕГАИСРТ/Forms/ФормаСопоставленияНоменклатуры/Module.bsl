...
Процедура ДополнитьРеквизитыНоменклатуры()
	...
	Попытка
		...
		Если ВыборкаАлкогольнаяПродукция.Следующий() Тогда
			...
			|	Номенклатура.ВидАлкогольнойПродукцииЕГАИС,
			...
			Если ВыборкаНоменклатура.Следующий() Тогда
				...
				Если НЕ ЗначениеЗаполнено(ВыборкаНоменклатура.ВидАлкогольнойПродукцииЕГАИС) Тогда
					НоменклатураОбъект.ВидАлкогольнойПродукцииЕГАИС = ВыборкаАлкогольнаяПродукция.ВидПродукции;
					НеобходимостьЗаписи = Истина;
				КонецЕсли;
...
Процедура ПриАктивацииСтрокиТаблицаНеСопоставленнойНоменклатурыНаСервере(Номенклатура)
	ИспользуютсяХарактеристики = Справочники.Номенклатура.ПроверитьИспользованиеХарактеристик(Номенклатура);
	УстановитьВидимостьЭлементовФормы();
КонецПроцедуры
