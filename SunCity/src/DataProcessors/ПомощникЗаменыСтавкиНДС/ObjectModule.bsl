...
Функция ТекстЗапроса(ТипыИзменяемыхОбъектов, ОграничитьВыборку = Ложь) Экспорт
	...
	Возврат СтрСоединить(Результат, ОбщегоНазначенияРТ.РазделительЗапросовВОбъединении());
	...
	Попытка
		...
		Для Каждого Ссылка Из ОбрабатываемыеОбъекты Цикл
			...
			Попытка
				...
				ИзменяемыйОбъект.СтавкаНДС = УчетНДС.СтавкаНДСПоУмолчанию(ДатаПереходногоПериода);
