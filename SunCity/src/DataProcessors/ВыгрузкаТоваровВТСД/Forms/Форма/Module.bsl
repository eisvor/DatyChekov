...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Объект.Магазин = ЗначениеНастроекПовтИсп.МагазинПоУмолчанию();
...
Процедура Заполнить(Команда)
	...
	Если ВариантЗаполнения = "ПоОтбору" 
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Магазин""'"), , "Объект.Магазин", , Отказ);
		Возврат;
	КонецЕсли;

	Если ВариантЗаполнения = "ПоОтбору" И ИспользоватьАссортимент 
		И НЕ ЗначениеЗаполнено(Объект.ФорматМагазина) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'У выбранного магазина не заполнено поле ""Формат магазина""'"), , "Объект.Магазин", , Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ПересчитатьЦеныСервер()
	...
	Запрос.УстановитьПараметр("ФорматМагазина", Объект.ФорматМагазина);
...
Процедура ЗаполнитьПоТоварамДокументаНаСервере()
	...
	ИначеЕсли ДокументИмяТипа = "ИзменениеАссортимента" Тогда
		...
		|	Штрихкоды.Штрихкод
		|ИЗ
		|	Документ.ИзменениеАссортимента.Товары КАК ТаблицаТоваров
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК Штрихкоды
		|		ПО ТаблицаТоваров.Номенклатура = Штрихкоды.Владелец
		|ГДЕ
		|	ТаблицаТоваров.Ссылка = &Документ
	...
	ИначеЕсли ДокументИмяТипа = "ВводНачальныхОстатковУзла" Тогда
		...
		Запрос = Новый Запрос;
		Если ИмяТаблицыВыборки = "Ассортимент" Тогда
			ТекстЗапроса = 
			"ВЫБРАТЬ
			|	ТаблицаТоваров.Номенклатура КАК Номенклатура,
			|	Штрихкоды.Характеристика КАК Характеристика,
			...
			|	ТаблицаТоваров.НомерСтроки,
			|	Штрихкоды.Штрихкод";
		ИначеЕсли ИмяТаблицыВыборки = "ЦеныНоменклатуры" Тогда
			ТекстЗапроса = 
			"ВЫБРАТЬ
			|	ТаблицаТоваров.Номенклатура КАК Номенклатура,
	...
	ИначеЕсли ДокументИмяТипа = "ОрдерНаОтражениеРезультатовПересчетовТоваров" Тогда
		...
		|	Штрихкоды.Штрихкод
		|ИЗ
		|	Документ.ОрдерНаОтражениеРезультатовПересчетовТоваров.Товары КАК ТаблицаТоваров
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК Штрихкоды
		|		ПО ТаблицаТоваров.Номенклатура = Штрихкоды.Владелец
		|			И ТаблицаТоваров.Характеристика = Штрихкоды.Характеристика
		|ГДЕ
	...
	ИначеЕсли ДокументИмяТипа = "ПересортицаТоваров" Тогда
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаТоваров.Номенклатура" + ПрефиксРаздела + " КАК Номенклатура,
		|	ТаблицаТоваров.Характеристика" + ПрефиксРаздела + " КАК Характеристика,
		|	Штрихкоды.Упаковка КАК Упаковка,
		|	ТаблицаТоваров.Количество КАК КоличествоУпаковок,
		|	ТаблицаТоваров.Цена" + ПрефиксРаздела + " КАК Цена,
		|	Штрихкоды.Штрихкод
		|ИЗ
		|	Документ.ПересортицаТоваров.Товары КАК ТаблицаТоваров
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК Штрихкоды
		|		ПО ТаблицаТоваров.Номенклатура" + ПрефиксРаздела + " = Штрихкоды.Владелец
		|			И ТаблицаТоваров.Характеристика" + ПрефиксРаздела + " = Штрихкоды.Характеристика
		|ГДЕ
		|	ТаблицаТоваров.Ссылка = &Документ
		|
		|УПОРЯДОЧИТЬ ПО
	...
	ИначеЕсли ДокументИмяТипа = "ПрименениеЦенНоменклатуры" Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаТоваров.Номенклатура КАК Номенклатура,
		...
		|	Штрихкоды.Штрихкод
		|ИЗ
		|	Документ.ПрименениеЦенНоменклатуры.Товары КАК ТаблицаТоваров
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК Штрихкоды
		|		ПО ТаблицаТоваров.Номенклатура = Штрихкоды.Владелец
		|			И ТаблицаТоваров.Характеристика = Штрихкоды.Характеристика
		|			И ТаблицаТоваров.Упаковка = Штрихкоды.Упаковка
	...
	ИначеЕсли ДокументИмяТипа = "УстановкаСебестоимости" Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаТоваров.Номенклатура КАК Номенклатура,
		...
		|	Штрихкоды.Штрихкод
		|ИЗ
		|	Документ.УстановкаСебестоимости.Товары КАК ТаблицаТоваров
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.Штрихкоды КАК Штрихкоды
		|		ПО ТаблицаТоваров.Номенклатура = Штрихкоды.Владелец
		|			И ТаблицаТоваров.Характеристика = Штрихкоды.Характеристика
		|ГДЕ
...
Процедура МагазинПриИзмененииСервер()
	...
	Если ИспользоватьАссортимент И НЕ ЗначениеЗаполнено(Объект.ФорматМагазина) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'У выбранного магазина не заполнено поле ""Формат магазина""'"), , "Объект.Магазин", ,);
	КонецЕсли;
