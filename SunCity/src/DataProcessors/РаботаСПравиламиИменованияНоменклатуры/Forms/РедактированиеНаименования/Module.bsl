...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого СтрокаПараметр Из Параметры.ПараметрыПравилаИменования Цикл
		...
		МассивПараметровПравила.Добавить(
			Новый РеквизитФормы(СтрокаПараметр.ПутьКДанным, СтрокаПараметр.ТипРеквизита,, СтрокаПараметр.ИмяПараметра));
	КонецЦикла;
	...
	Если Параметры.Свойство("ЗначенияПараметровИменования") Тогда
		Для Каждого СтрокаПараметр Из ПараметрыПравилаИменования Цикл
			СтрокиЗначенияПараметров = Параметры.ЗначенияПараметровИменования.НайтиСтроки(
				Новый Структура("ПутьКДанным", СтрокаПараметр.ПутьКДанным));
			Если СтрокиЗначенияПараметров.Количество() > 0 Тогда
				ЭтаФорма[СтрокаПараметр.ПутьКДанным] = СтрокиЗначенияПараметров[0].Значение;
				СтрокаПараметр.ДанныеПараметраСтрокой =
					Формат(СтрокиЗначенияПараметров[0].Значение, "ДЛФ=Д; БЛ='Ложь'; БИ='" + СтрокаПараметр.ИмяПараметра + "'");
			КонецЕсли;
		КонецЦикла;
	Иначе
		РезультатЗапроса = РаботаСПравиламиИменования.ЗначенияПараметровНоменклатуры(
			Номенклатура.ВидНоменклатуры,ПараметрыПравилаИменования,Номенклатура);
		...
		Если ВыборкаДанных.Количество() > 0 Тогда
			...
			Для Каждого СтрокаПараметр Из ПараметрыПравилаИменования Цикл
				...
				СтрокаПараметр.ДанныеПараметраСтрокой = Формат(
					ЭтаФорма[СтрокаПараметр.ПутьКДанным], "ДЛФ=Д; БЛ='Ложь'; БИ='" + СтрокаПараметр.ИмяПараметра + "'");
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого СтрокаПараметр Из ПараметрыПравилаИменования Цикл
		...
		Иначе
			...
			ИначеЕсли СтрокаПараметр.ПутьКДанным = "СтавкаНДС" Тогда
				...
				Элементы[СтрокаПараметр.ИмяЭлемента].СписокВыбора.Добавить(Справочники.СтавкиНДС.ПолучитьСтавкуНДСБезНДС());
				Элементы[СтрокаПараметр.ИмяЭлемента].СписокВыбора.Добавить(Справочники.СтавкиНДС.ПолучитьСтавкуНДСНоль());
			КонецЕсли;
...
Процедура Подключаемый_ПараметрИменованияПриИзменении(Элемент)
	...
	Для Каждого Строка Из Строки Цикл
		Строка.ДанныеПараметраСтрокой = Формат(
			ЭтаФорма[Строка.ПутьКДанным], "ДЛФ=Д; БЛ='Ложь'; БИ='" + Строка.ИмяПараметра + "'");
	КонецЦикла;
...
Процедура ОК(Команда)
	...
	Если МассивНеЗаполненныхПараметров.Количество() = 0 Тогда
		ЗакрытьФорму();
	Иначе
		МассивНеЗаполненныхОбязательныхПараметров = ПараметрыПравилаИменования.НайтиСтроки(
			Новый Структура("ДанныеПараметраСтрокой,ПроверятьЗаполнение", "", Истина));
		Если МассивНеЗаполненныхОбязательныхПараметров.Количество() > 0 Тогда
			Для Каждого СтрокаПараметр Из МассивНеЗаполненныхОбязательныхПараметров Цикл
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Поле ""%1"" не заполнено.'"), СтрокаПараметр.ИмяПараметра);
				ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, СтрокаПараметр.ПутьКДанным);
			КонецЦикла;
		Иначе
...
Процедура СформироватьНаименование()
	...
	Элементы.ДекорацияПримерНаименования.Заголовок =
		РаботаСПравиламиИменованияКлиентСервер.ПреобразоватьСтрокуПоПравилу(
		ПравилоИменования,ПараметрыПравилаИменования, "ИмяПараметра", "ДанныеПараметраСтрокой");
	...
	Если СтруктураПоискаДублей.РеквизитыЗаполнены Тогда
		...
		ЗаголовокНадписи = НСтр("ru='Обнаружены карточки с одинаковым наименованием (%1)'");
		Элементы.НадписьИнформацияДублирующиеПозиции.Заголовок = СтрШаблон(ЗаголовокНадписи, КоличествоДублирующихПозиций);
		Элементы.НадписьИнформацияДублирующиеПозиции.Видимость = КоличествоДублирующихПозиций > 0;
		
		Пока Выборка.Следующий() Цикл
			СтрокаДубли = ДублиНоменклатуры.Добавить();
			СтрокаДубли.Номенклатура = Выборка.Номенклатура;
		КонецЦикла;
	Иначе
