...
Процедура Подключаемый_СоздатьКонтрагентов()
	...
	Если СозданныеКонтрагенты.Количество() = 1 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, СозданныеКонтрагенты[0]);
	Иначе
...
Функция СоздатьКонтрагентовНаСервере(АдресПараметровСозданияКонтрагентов)
	...
	Для каждого ОрганизацияЕГАИС Из ПараметрыСозданияКонтрагентов.МассивЭлементов Цикл
		...
		Попытка
			...
			ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("Справочник.Контрагенты");
			ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("Справочник.КлассификаторОрганизацийЕГАИС");
