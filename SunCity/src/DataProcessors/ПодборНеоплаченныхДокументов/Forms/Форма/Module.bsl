...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("Операция") Тогда
		Отказ = Истина;
		ТекстИсключения = НСтр("ru = 'Форма открывается из финансовых документов'");
	...
	Если Параметры.Операция = Перечисления.ХозяйственныеОперации.ОплатаПоставщику Тогда
		...
		ОплатОтПокурателя = Ложь;
	Иначе
		Элементы.ГруппаОтбор.Видимость = Ложь;
		ОплатаПоставщику = Ложь;
		Если Параметры.Операция = Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента Тогда
			ОплатОтПокурателя = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьКорзину()
	...
	Если ОплатаПоставщику Тогда
		...
		|	ЗаказПоставщикуЭтапыОплат.Ссылка.СуммаДокумента КАК СуммаДокумента,
		|	ЗаказПоставщикуЭтапыОплат.Ссылка.Дата КАК Дата
		...
		|	ДолгПоЭтапамОплат.Дата КАК Дата
		|ПОМЕСТИТЬ Результат
		|ИЗ
		|	НеОплаченныеДокументы КАК НеОплаченныеДокументы
		...
		|ПОМЕСТИТЬ РасшифровкаПлатежаПоДокументам
		|ИЗ
		|	РасшифровкаПлатежа КАК РасшифровкаПлатежа
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ВЫБОР
		|		КОГДА РасшифровкаПлатежаПоДокументам.СуммаИзДокумента ЕСТЬ NULL
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК Пометка,
		|	ЕСТЬNULL(РасшифровкаПлатежаПоДокументам.СуммаИзДокумента, 0) КАК Сумма
		|ИЗ
		|	Результат КАК Результат
		|		ЛЕВОЕ СОЕДИНЕНИЕ РасшифровкаПлатежаПоДокументам КАК РасшифровкаПлатежаПоДокументам
		|		ПО Результат.ДокументРасчета = РасшифровкаПлатежаПоДокументам.Документ
		|ГДЕ
		|	Результат.НеОплаченнаяСумма > 0
		|
		|УПОРЯДОЧИТЬ ПО
		|	Результат.Дата";
	...
	ИначеЕсли ОплатОтПокурателя Тогда
		Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|	-РасчетыСКлиентамиОстатки.СуммаОстаток КАК КОплате
		...
		|				И &УсловиеЗаказ) КАК РасчетыСКлиентамиОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	РасчетыСКлиентам.ДокументРасчета КАК ДокументРасчета,
		|	РасчетыСКлиентам.КОплате КАК КОплате,
		|	РасчетыСКлиентам.ДокументРасчета.СуммаДокумента КАК СуммаДокумента,
		|	РасчетыСКлиентам.ДокументРасчета.Дата КАК ДокументРасчетаДата
		|ПОМЕСТИТЬ Результат
		|ИЗ
		|	РасчетыСКлиентам КАК РасчетыСКлиентам
		|ГДЕ
		|	РасчетыСКлиентам.КОплате > 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	РасшифровкаПлатежа.ДокументРасчета КАК ДокументРасчета,
		...
		|ВЫБРАТЬ
		|	Результат.ДокументРасчета КАК ДокументРасчета,
		|	Результат.КОплате КАК КОплате,
		|	Результат.СуммаДокумента КАК СуммаДокумента,
		|	Результат.ДокументРасчетаДата КАК ДокументРасчетаДата,
		|	ЕСТЬNULL(РасшифровкаПлатежаПоДокументам.Сумма, 0) КАК Сумма,
		|	ВЫБОР
		|		КОГДА РасшифровкаПлатежаПоДокументам.Сумма ЕСТЬ NULL
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК Пометка
		|ИЗ
		|	Результат КАК Результат
		|		ЛЕВОЕ СОЕДИНЕНИЕ РасшифровкаПлатежаПоДокументам КАК РасшифровкаПлатежаПоДокументам
		|		ПО Результат.ДокументРасчета = РасшифровкаПлатежаПоДокументам.ДокументРасчета
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДокументРасчетаДата";
		
		ТекстУсловияЗаказ = ?(ЗначениеЗаполнено(Заказ),
			"ЗаказПокупателя = &Заказ",
			"Истина");
		
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеЗаказ", ТекстУсловияЗаказ);
