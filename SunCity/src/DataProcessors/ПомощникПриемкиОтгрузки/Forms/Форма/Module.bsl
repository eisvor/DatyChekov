...
Процедура ПродолжитьОбработатьДанныеПоКодуКлиент(СтруктураРезультат) Экспорт
	...
	Если НЕ СтруктураРезультат.Свойство("ДействиеОтменено") Тогда
		...
		ИначеЕсли СтрокаРезультата.Свойство("СерийныйНомер") Тогда
			...
			СтруктураПоиска.Вставить("СерийныйНомер", СтрокаРезультата.СерийныйНомер);
			...
			Иначе
				...
				Если СтрокаРезультата.Свойство("Количество") Тогда
					СтрокаРезультата.Количество = 1;
				КонецЕсли;
				...
				Если ИдентификаторСтроки <> Неопределено Тогда
					...
					Если ТекущаяСтрока.КлючСвязиСерийныхНомеров = 0 Тогда
						...
						ДобавляемыеНомера.Добавить(СтрокаРезультата.СерийныйНомер);
					...
					Иначе
						...
						НоваяСтрока.СерийныйНомер = СтрокаРезультата.СерийныйНомер;
		...
		ИначеЕсли СтрокаРезультата.Свойство("Серия") Тогда
			...
			СтруктураИменФормы.Вставить("ИмяТЧСерии"            , "СерииФакт");
			СтруктураИменФормы.Вставить("ИмяТЧЭлементаВладельца", "ТоварыФакт");
			СтруктураИменФормы.Вставить("ИмяТЧТовары"           , "ТоварыФакт");
...
Функция ДобавитьНайденныеПозицииТоваровСервер(СтруктураПараметров, ДополнительныеПараметры = Неопределено)
	...
	ТекущаяСтрока = ПодключаемоеОборудованиеРТ.ИнициализацияСтрокиТоваров(ЭтотОбъект, 
		СтруктураПараметров, 
		ДобавленаСтрока, 
		"ТоварыФакт");
	...
	СтруктураОбновления = СтруктураОбновленияТоваровНаСервере(, Истина, Истина, Истина);
...
Процедура ОбработатьИзменениеСтроки(ИдентификаторСтроки, Действие) Экспорт
	...
	ТекущаяСтрока = Объект.ТоварыФакт.НайтиПоИдентификатору(ИдентификаторСтроки);
	
	Если Действие = "ИзмененоКоличество" Тогда
		ПриИзмененииТоварыКоличестваУпаковок("ТоварыДеревоКоличествоУпаковокФакт",,Ложь);
...
&НаКлиенте
Процедура ПриИзмененииТоварыКоличестваУпаковок(ИмяЭлемента, ТекущаяСтрока = Неопределено, ОбновитьТЧПоДеревуТоваров = Истина)
	...
	Если ОбновитьТЧПоДеревуТоваров Тогда
	...
	ПересчитатьСтатусыСервер(ТекущиеДанные.Номенклатура, ТекущиеДанные.Характеристика, КэшированныеЗначения);
...
Процедура ПриИзмененииТоварыНоменклатура(ТекущаяСтрока = Неопределено)
	...
	Если ТекущаяСтрока = Неопределено Тогда
		ТекущаяСтрока = Элементы.ТоварыФакт.ТекущиеДанные;
	КонецЕсли;
	
	ПересчитатьСтатусыСервер(ТекущаяСтрока.Номенклатура, ТекущаяСтрока.Характеристика, КэшированныеЗначения);
...
&НаСервере
Процедура ПересчитатьСтатусыСервер(Номенклатура, Характеристика, КэшированныеЗначения)
	...
	СтрокиТЧ = Объект.ТоварыФакт.НайтиСтроки(СтруктураПоиска);
	Для каждого СтрокаТЧ Из СтрокиТЧ Цикл
		СтруктураТЧ.СтрокиТЧ.Добавить(СтрокаТЧ);
	КонецЦикла;
...
Функция ВыполнитьРаспределениеВФоне()
	...
	ПараметрыЗадания.Вставить("Ответственный", Пользователи.ТекущийПользователь());
...
Процедура ПечатьОписьНоменклатурыПоВсемСкладам(Команда)
	...
	Для Каждого СтрокаСклада Из СкладыРаспределения Цикл
		...
		Основание = Нстр("ru='Распределение товаров по документу '") + Объект.ДокументРаспоряжение;
		
		ИмяДокумента = "Обработка.ПомощникПриемкиОтгрузки";
		
		СтруктураПечати = Новый Структура();
		СтруктураПечати.Вставить("Представление", Нстр("ru='Опись номенклатуры'"));
...
Процедура ПечатьОписьНоменклатурыПоРаспоряжению(Команда)
	...
	Основание = Нстр("ru='Приемка товаров по документу'")+ Символы.НПП + Объект.ДокументРаспоряжение;
	ИмяДокумента = "Обработка.ПомощникПриемкиОтгрузки";
...
Процедура Подключаемый_ПечатьОписьНоменклатурыПоПеремещению(Команда)
	...
	Основание = Нстр("ru='Распределение товаров по документу'") + Символы.НПП + Объект.ДокументРаспоряжение;
...
Процедура РаспределитьПоОстаткамНаСервере(РезультатЗакрытия = Неопределено)
	...
	Если ТаблицаПриоритетаСкладов = Неопределено Тогда
		ВызватьИсключение Нстр("ru='Не настроено распределение товаров по остаткам. Обратитесь к Администратору'");
	КонецЕсли;
...
Процедура РаспределитьПоПравиламРаспределенияНаСервере()
	...
	|	ЕСТЬNULL(РаспределениеНоменклатуры.СкладПоступления, Магазины.СкладПоступления) КАК Склад
	|ИЗ
	...
	|		ПО Номенклатура.ТоварнаяГруппа = РаспределениеНоменклатуры.ТоварнаяГруппа
	|			И (РаспределениеНоменклатуры.Магазин = &Магазин)
	|			И (РаспределениеНоменклатуры.СкладПоступления <> ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка))
	|ГДЕ
...
Процедура РаспределитьПоПроизвольномуВариантуНаСервере(ВариантРаспределения, РезультатЗакрытия = Неопределено)
	...
	МассивВыделенныхИдентификаторовТоварыПоФакту = Новый Массив();
	Для Каждого ИдентификаторВыделенной Из МассивИдентификаторовСтрокДляОбработки Цикл
		...
		Если Не НайденныеСтроки.Количество() = 0 Тогда
			МассивВыделенныхИдентификаторовТоварыПоФакту.Добавить(НайденныеСтроки[0].ПолучитьИдентификатор());
		КонецЕсли;
		
	КонецЦикла;
	
	Справочники.ВариантыРаспределенияТоваров.ВыполнитьРаспределениеТоваров(Объект.Магазин, ВариантРаспределения, Объект[ПараметрыРаспределения.БазоваяТаблицаДляРаспределения], Объект.СерийныеНомераФакт, Объект.ТоварыПеремещение, Объект.СерийныеНомераПеремещение, ЭтаФорма, МассивВыделенныхИдентификаторовТоварыПоФакту, НастройкиВарианта);
...
Процедура ЗаполнитьПараметрыРаспределения(Параметры, Отказ)
	...
	Если Объект.ДокументРаспоряжение = Неопределено Тогда
	
		ТекстИсключения = НСтр("ru = 'Обработка открывается из списков документов.
									 |Склад -> Приемка. Склад -> Отгрузка.'");
	...
	ИначеЕсли ТипЗнч(Объект.ДокументРаспоряжение) = Тип("ДокументСсылка.ЗаказПокупателя")
		Или ТипЗнч(Объект.ДокументРаспоряжение) = Тип("ДокументСсылка.РеализацияТоваров")
		Или ТипЗнч(Объект.ДокументРаспоряжение) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
		ПараметрыРаспределения.Операция = "Отгрузка";
	...
	ИначеЕсли ТипЗнч(Объект.ДокументРаспоряжение) = Тип("ДокументСсылка.ЗаказПоставщику")
		Или ТипЗнч(Объект.ДокументРаспоряжение) = Тип("ДокументСсылка.ПоступлениеТоваров")
		Или ТипЗнч(Объект.ДокументРаспоряжение) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
...
Процедура ИнициализироватьСкладыРаспределения()
	...
	НоменклатураРаспоряжения = Объект.ДокументРаспоряжение.Товары.Выгрузить(, "Номенклатура");
	НоменклатураРаспоряжения.Свернуть("Номенклатура");
	МассивНоменклатуры = НоменклатураРаспоряжения.ВыгрузитьКолонку("Номенклатура");
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПорядокСкладов.Склад КАК Склад,
	|	ПорядокСкладов.Порядок КАК Порядок
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОрдерНаПеремещениеТоваров КАК ОрдерНаПеремещениеТоваров
	|		ПО ТоварыНаСкладах.Регистратор = ОрдерНаПеремещениеТоваров.Ссылка
	|			И (НЕ ОрдерНаПеремещениеТоваров.ДокументОснование.Ссылка ЕСТЬ NULL)
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втЧастотныеСкладыНоменклатуры.Номенклатура КАК Номенклатура,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СкладНоменклатура.Склад КАК Склад
...
Функция ДобавитьСкладРаспределения(СкладСсылка, Видимость, Порядок) Экспорт
	...
	СуффиксИменЭлементов = СтрЗаменить(Строка(СкладСсылка.УникальныйИдентификатор()), "-", "");
	...
	НовыйСклад.ИмяГруппы = "Группа" + СуффиксИменЭлементов;
	НовыйСклад.ИмяКолонкиКоличество = "Количество" + СуффиксИменЭлементов;
	НовыйСклад.ИмяКолонкиКоличествоУпаковок = "КоличествоУпаковок" + СуффиксИменЭлементов;
	НовыйСклад.ИмяКолонкиОстаток = "КоличествоОстаток" + СуффиксИменЭлементов;
	НовыйСклад.ИмяКолонкиСтатусыСерий = "СтатусыСерий" + СуффиксИменЭлементов;
	НовыйСклад.ИмяЭлементаПечатьОписьНоменклатуры = "ПечатьОпись" + СуффиксИменЭлементов;
	НовыйСклад.ИмяЭлементаРаспределитьНаОдинСклад = "КнопкаВсеНаСклад" + СуффиксИменЭлементов;
	НовыйСклад.ИмяЭлементаПереместитьОстатокНаСклад = "КнопкаОстатокНаСклад" + СуффиксИменЭлементов;
...
//		СообщениеПользователю - Сообщение текст которого необходимо заполнить
// Возвращаемое значение:
//		Структура - структура с данными необходимыми для проверки
&НаКлиенте
...
// Получает сохраненную видимость складов из регистра сведений
&НаСервере
...
&НаСервере
Функция УдалитьСохраненныеДанные()
...
Процедура ОповещениеПоискаПоШтрихкоду(Штрихкод, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ПустаяСтрока(Штрихкод) Тогда
		Если НЕ(КэшированныеЗначения <> Неопределено
			...
			ПараметрыУказанияСерийРасширенный = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ПараметрыУказанияСерий);
			ПараметрыУказанияСерийРасширенный.Вставить("ИмяКолонкиКоличество"        , "КоличествоФакт");
			ПараметрыУказанияСерийРасширенный.Вставить("ИмяКолонкиКоличествоУпаковок", "КоличествоУпаковокФакт");
			ОбработкаТабличнойЧастиТоварыКлиент.ОбновитьКэшированныеЗначенияДляУчетаСерий(
				ЭтотОбъект.Элементы.ТоварыДерево,
				КэшированныеЗначения,
				ПараметрыУказанияСерийРасширенный);
		КонецЕсли;
