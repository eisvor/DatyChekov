...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПрефиксEAN13ПодарочногоСертификата = Константы.ПрефиксВнутреннегоШтрихкодаПодарочногоСертификата.Получить();
	ПрефиксEAN13УзлаРИБ = Константы.ПрефиксШтрихкодаУзлаРИБ.Получить();
	
	ПрефиксEAN13 = "2" + Строка(ПрефиксEAN13ПодарочногоСертификата) + ?(ПрефиксEAN13УзлаРИБ < 10, "0" + Строка(ПрефиксEAN13УзлаРИБ), Строка(ПрефиксEAN13УзлаРИБ));
...
Функция НайтиСуществующиеШтрихКоды()
	...
	Если Объект.ТипШтрихкодаРТ = ПланыВидовХарактеристик.ТипыШтрихкодов.EAN13 Тогда
		ПервыйШтрихкод = ПрефиксEAN13 + Объект.ПервыйШтрихкодEAN13;
...
Процедура РассчитатьШтрихкод(ИмяСтраницы = "")
	...
	Если Объект.ТипШтрихкодаРТ = ПланыВидовХарактеристик.ТипыШтрихкодов.EAN13 Тогда
		ИмяСтраницы = "СтраницаEAN13";
		ШтрихкодEAN13 = ПрефиксEAN13 + Объект.ПервыйШтрихкодEAN13;
		Если СтрДлина(ШтрихкодEAN13) >= 12 Тогда
			ШтрихКод = Лев(ШтрихкодEAN13, 12);
		Иначе
			ЕстьОшибкиПоШтрихКодам = ЗначениеЗаполнено(ШтрихкодEAN13);
		КонецЕсли;
	ИначеЕсли Объект.ТипШтрихкодаРТ = ПланыВидовХарактеристик.ТипыШтрихкодов.EAN8 Тогда
		ИмяСтраницы = "СтраницаEAN8";
	...
	Если (КоличествоСертификатов > 0) И ЗначениеЗаполнено(ШтрихКод) Тогда
		...
		ИначеЕсли  Объект.ТипШтрихкодаРТ = ПланыВидовХарактеристик.ТипыШтрихкодов.EAN8 Тогда
			ПоследнийШтрихкод = ПоследнийШтрихкод + ПодключаемоеОборудованиеРТ.КонтрольныйСимволEAN(ПоследнийШтрихкод, 8)
		КонецЕсли; 
...
Функция СоздатьСертификаты(ПроверкаПройдена = Истина)
	...
	Если Объект.ЕстьШтрихкоды Тогда
		...
		Если Объект.ТипШтрихкодаРТ = ПланыВидовХарактеристик.ТипыШтрихкодов.EAN13 Тогда
			...
			Объект.ПервыйШтрихкод = ПрефиксEAN13 + Объект.ПервыйШтрихкодEAN13;
	...
	Для Счетчик = 1 По КоличествоСертификатов Цикл
		...
		Если Объект.ЕстьШтрихкоды Тогда
			...
			ИначеЕсли Объект.ТипШтрихкодаРТ = ПланыВидовХарактеристик.ТипыШтрихкодов.EAN8 Тогда
				...
				ТекущийШтрихкод        = ТекущийШтрихкод + ПодключаемоеОборудованиеРТ.КонтрольныйСимволEAN(ТекущийШтрихкод, 8);
...
Процедура ПечатьПроверкиИспользуемыхКодов()
	...
	СтруктураТаблиц = ПолучитьСтруктуруТаблицДляПечатиНаСервере();
	
	ПараметрКоманды = Новый Массив;
	ПараметрКоманды.Добавить(Параметры.Номенклатура);
...
&НаСервере
Функция ПолучитьСтруктуруТаблицДляПечатиНаСервере()
	
	СтруктураТаблиц = Новый Структура;
	
	Если ТаблицаМагнитныхКодов.Количество() > 0 Тогда
		
		МассивМагнитныхКодов = Новый Массив;
		ТаблицаЗначенийМагнитныхКодов = ТаблицаМагнитныхКодов.Выгрузить();
		Для Каждого СтрокаМагнитныхКодов Из ТаблицаЗначенийМагнитныхКодов Цикл
			СтруктураМагнитныхКодов = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтрокаМагнитныхКодов);
			ЗаполнитьЗначенияСвойств(СтруктураМагнитныхКодов, СтрокаМагнитныхКодов);
			МассивМагнитныхКодов.Добавить(СтруктураМагнитныхКодов);
		КонецЦикла;
		
		СтруктураТаблиц.Вставить("ТаблицаМагнитныхКодов", МассивМагнитныхКодов);
		
	КонецЕсли;
	
	Если ТаблицаШтриховыхКодов.Количество() > 0 Тогда
		
		МассивШтриховыхКодов = Новый Массив;
		ТаблицаЗначенийШтриховыхКодов = ТаблицаШтриховыхКодов.Выгрузить();
		Для Каждого СтрокаШтриховыхКодов Из ТаблицаЗначенийШтриховыхКодов Цикл
			СтруктураШтриховыхКодов = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(СтрокаШтриховыхКодов);
			ЗаполнитьЗначенияСвойств(СтруктураШтриховыхКодов, СтрокаШтриховыхКодов);
			МассивШтриховыхКодов.Добавить(СтруктураШтриховыхКодов);
		КонецЦикла;
		
		СтруктураТаблиц.Вставить("ТаблицаШтриховыхКодов", МассивШтриховыхКодов);
