...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	Если НавигационнаяСсылкаФорматированнойСтроки = "КассоваяДисциплинаВыемкиПриЗакрытииСмены" Тогда
		СтандартнаяОбработка = Ложь;
		ПараметрыФормы = Новый Структура;
		ДанныеОтбора   = Новый Структура;
		Если ЗначениеЗаполнено(Объект.Магазин) Тогда
			ДанныеОтбора.Вставить("Магазин", Объект.Магазин);
		КонецЕсли;
		...
		ОткрытьФорму("РегистрСведений.КассоваяДисциплинаВыемкиПриЗакрытииСмены.ФормаСписка", ПараметрыФормы, ЭтотОбъект);
	КонецЕсли;
КонецПроцедуры
...
Процедура НесколькоОрганизацийПриИзменении(Элемент)
	...
	Элементы.СтраницаНесколькоОрганизаций.Видимость = НесколькоОрганизаций;
...
Процедура ЗаполнитьТаблицуУжеПодключенногоОборудования()
	...
	|	ЗНАЧЕНИЕ(Перечисление.ТипыПодключаемогоОборудования.ФискальныйРегистратор),
	|	КассыККМ.Владелец,
	|	КассыККМ.ПодключаемоеОборудование.СерийныйНомер
	|ИЗ
	|	Справочник.КассыККМ КАК КассыККМ
	|ГДЕ
...
Процедура ОбновитьСообщениеДрайвера()
	...
	ПараметрыВыполнения = Новый Структура("ИмяДействия", "ПолучитьОписаниеДрайвера");
	ПараметрыУстройства = Неопределено;
	ОповещениеПриЗавершении = Новый ОписаниеОповещения("ОбновитьСообщениеДрайвераЗавершение", ЭтотОбъект);
	...
	Иначе
		МенеджерОборудованияКлиент.НачатьВыполнениеДополнительнойКоманды(ОповещениеПриЗавершении,
...
&НаКлиенте
Процедура ОбновитьСообщениеДрайвераЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ЗаполнитьКассыККМ()
	...
	|	КассыККМ.ШаблонЧекаККМ КАК ШаблонЧекаККМ,
	|	КассыККМ.ШаблонЧекаККМВозврат КАК ШаблонЧекаККМВозврат,
	...
	|	ТаблицаКассККМ.Наименование КАК Наименование,
	|	ТаблицаКассККМ.РегистрационныйНомер КАК РегистрационныйНомер,
	|	ТаблицаКассККМ.СерийныйНомер КАК СерийныйНомер,
	|	ТаблицаКассККМ.ИспользоватьБезПодключенияОборудования КАК ИспользоватьБезПодключенияОборудования,
	|	ТаблицаКассККМ.ШаблонЧекаККМ КАК ШаблонЧекаККМ,
	|	ТаблицаКассККМ.ШаблонЧекаККМВозврат КАК ШаблонЧекаККМВозврат,
	|	ТаблицаКассККМ.ШиринаЛенты КАК ШиринаЛенты,
	|	ТаблицаКассККМ.ПодключаемоеОборудование КАК ПодключаемоеОборудование,
	|	ТаблицаКассККМ.ТипКассы КАК ТипКассы,
	|	ТаблицаКассККМ.НастройкаРаспределенияВыручкиПоСекциям КАК НастройкаРаспределенияВыручкиПоСекциям
	|ИЗ
	|	ТаблицаОрганизации КАК ТаблицаОрганизации
...
Функция СоздатьИзменитьКассуККМНаСервере(ИндексСтроки)
	...
	Попытка
		...
		БлокировкаДанных = Новый БлокировкаДанных;
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("Справочник.КассыККМ");
		БлокировкаДанных.Заблокировать();
...
Процедура ЗаполнитьОтчет()
	...
	|	Справочник.КассыККМ КАК КассыККМ
	|ГДЕ
	|	КассыККМ.ТипКассы = ЗНАЧЕНИЕ(Перечисление.ТипыКассККМ.СетевоеОборудование)
	|	И НЕ КассыККМ.ТипКассы = ЗНАЧЕНИЕ(Перечисление.ТипыКассККМ.ККМOffline)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
