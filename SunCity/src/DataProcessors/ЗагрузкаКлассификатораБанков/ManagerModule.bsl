...
Процедура ЗагрузитьКлассификаторБанков() Экспорт
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Справочник.КлассификаторБанков.КлассификаторБанков");
	Область = Макет.Области["ОбластьРасшифровки"];
	
	ТзБанков = Новый ТаблицаЗначений;
	ТзБанков.Колонки.Добавить("БИК", Новый ОписаниеТипов("Строка",,, Новый КвалификаторыСтроки(9)));
	ТзБанков.Колонки.Добавить("Наименование", Новый ОписаниеТипов("Строка"));
	ТзБанков.Колонки.Добавить("Город", Новый ОписаниеТипов("Строка"));
	
	Для Ном = Область.Верх По Область.Низ Цикл
		
		НоваяСтрока = ТзБанков.Добавить();
		
		НоваяСтрока.БИК           = СокрЛП(Макет.Область(Ном, 2).Текст);
		НоваяСтрока.Наименование  = СокрЛП(Макет.Область(Ном, 4).Текст);
		НоваяСтрока.Город         = СокрЛП(Макет.Область(Ном, 5).Текст);
	...
	|	ТаблицаКлассификатора.БИК,
	|	ТаблицаКлассификатора.Город
	...
	|	Классификатор.БИК КАК Код,
	|	Классификатор.Город КАК Город,
	...
	|		ПО Классификатор.БИК = КлассификаторБанковРФ.Код
	|ГДЕ
	|	КлассификаторБанковРФ.Ссылка ЕСТЬ NULL ";
	
	Запрос.УстановитьПараметр("ТаблицаКлассификатора", ТзБанков);
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			НовыйЭлемент = Справочники.КлассификаторБанков.СоздатьЭлемент();
	...
	ТекстСообщения = НСтр("ru = 'Заполнение классификатора банков завершено'");
