...
#Область Новости
	...
	ОбработкаНовостей.КонтекстныеНовости_ПриСозданииНаСервере(
		ЭтаФорма,
		"Обработка.АнализПродажФормированиеЗаказов",
		"Форма",
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если Поставщик = Неопределено Тогда
		...
		Поставщик						= ЗначениеНастроекПовтИсп.ПоставщикПоУмолчанию(Ответственный, Поставщик);
	КонецЕсли;
...
Процедура РезультатВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ИмяПоле = "ОжидаемоеПоступлениеКоличество" Тогда
		...
		Если Не Отказ Тогда
			ТабличныйДокументРезультат.Показать(Нстр("ru='Товары к поступлению'"));
		КонецЕсли;
...
Процедура ПоказатьОтбор(Команда)
	...
	ПараметрыФормы.Вставить("АдресНастроекКомпоновкиДанных", АдресНастроек);
...
Процедура ЗаполнитьЗаказыЗакрытие(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ДатаЗакрытияЗаказов) Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(Текст,, "ДатаЗакрытияЗаказов",,);
		Возврат;
	КонецЕсли;
			...
			Если ДанныеПрогноза <> Неопределено И ЗначениеЗаполнено(СтрокаДереваФормы.Номенклатура) Тогда
				...
				Если СтрокиПрогноза.Количество() > 0 Тогда
					...
					Если ИтоговаяПотребность >= 0 Тогда
						...
						СтрокаДереваФормы.КЗаказуКоличествоУпаковок =
							ИтоговаяПотребность / ?(СтрокаДереваФормы.Коэффициент = 0, 1, СтрокаДереваФормы.Коэффициент);
...
Функция ПоверитьЗаполнение()
	...
	Если Не ЗначениеЗаполнено(ЖелаемаяДатаПоставки) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(Текст,, "ЖелаемаяДатаПоставки",,);
	...
	Если НачалоДня(ДатаРасчета) > НачалоДня(ТекущаяДатаСеанса()) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(Текст,, "ДатаРасчета");
		РезультатПроверки = Истина;
	КонецЕсли;
...
Процедура СоздатьДокументыСервер(МассивДанных, ТипДокумента, Отказ, ТекстОшибки)
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Если ТипДокумента = "ЗаказПоставщику" Тогда
			ТипДокументаОписание = Нстр("ru='Заказ поставщику'");
		ИначеЕсли ТипДокумента = "ВозвратПоставщику" Тогда
			ТипДокументаОписание = Нстр("ru='Возврат поставщику'");
		ИначеЕсли ТипДокумента = "ЗаказНаПеремещение" Тогда
			ТипДокументаОписание = Нстр("ru='Заказ на перемещение'");
		ИначеЕсли ТипДокумента = "ЗаказНаПеремещениеВозврат" Тогда
			ТипДокументаОписание = Нстр("ru='Заказ на перемещение (возврат)'");
		КонецЕсли;
...
Процедура СоздатьДокументы(ТипДокумента)
	...
	Если Результат.ПолучитьЭлементы().Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю( ТекстОшибки,, "Результат",, Отказ);
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьЗаполненностьРасписания()
	...
	Если РезультатПроверки.Пустой() Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, , "Магазин");
	КонецЕсли;
...
#Область СлужебныеПроцедурыИФункцииМодуляПрогнозирования
...
Функция ТаблицаНачальныхЗначений(ДеревоЗначенийРезультат)
	...
	ОписаниеЧислаКоличество = ОбщегоНазначенияРТКлиентСервер.ОписаниеТиповЧисла(17, 3);
...
Процедура РассчитатьТаблицыДанныхТренда(СтруктураДанныхПрогноза)
	...
	ОписаниеЧислаТренда = ОбщегоНазначенияРТКлиентСервер.ОписаниеТиповЧисла(25, 10);
	...
	ОписаниеДаты = ОбщегоНазначенияРТКлиентСервер.ОписаниеТиповДаты();
...
Функция СформироватьГрафикТренда()
	...
	СерияПродажи.Текст = Нстр("ru='Продажи'");
	...
	СерияПрогноз.Текст = Нстр("ru='Прогноз'");
	...
	СерияТренд.Текст = Нстр("ru='Линия тренда'");
