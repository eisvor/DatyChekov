...
&НаКлиенте
Функция Подключаемый_СтраницаСписокНастроек_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
...
&НаКлиенте
Функция Подключаемый_СтраницаСписокНастроек_ПриПереходеДалее(Отказ)
...
Функция СтруктураДанныеНастроекОбмена()
	...
	СтруктураНастроек.Вставить("ПалитраБыстрыхТоваров", ПалитраБыстрыхТоваров);
	...
	Если МодельСервиса Тогда
		...
		НаименованиеЭтойБазы = МодульОбменДаннымиВМоделиСервиса.СформироватьНаименованиеПредопределенногоУзла();
	Иначе
		// Представление этой информационной базы.
		НаименованиеЭтойБазы = ОбменДаннымиСервер.НаименованиеПредопределенногоУзлаПланаОбмена(ИмяПланаОбмена);
		Если ПустаяСтрока(НаименованиеЭтойБазы) Тогда
			НаименованиеЭтойБазы = ОбменДаннымиПовтИсп.ИмяЭтойИнформационнойБазы();
		КонецЕсли;
	КонецЕсли;
	...
	СтруктураНастроек.Вставить("ВерсияФорматаОбмена", "1.15");
	...
	Если РезультатФоновогоЗадания.Статус = "Выполнено" Тогда
		...
		Иначе
			...
			Если Результат.Свойство("НумерацияЗакончилась") Тогда
				ИзменитьПорядковыйНомерПерехода(-1);
				ОбщегоНазначенияКлиент.СообщитьПользователю(
					НСтр("ru = 'Закончился дипазон префиксов узлов УРМК'"));
...
Процедура ПриНачалеСохраненияНастройкиСинхронизацииНаСервере(ПродолжитьОжидание)
	...
	НастройкиСинхронизации.ДанныеЗаполнения.Вставить("ПалитраБыстрыхТоваров", Настройки.ПалитраБыстрыхТоваров);
...
Процедура ПриОжиданииСохраненияНастройки()
	...
	Если ПродолжитьОжидание Тогда
		ОбменДаннымиКлиент.ОбновитьПараметрыОбработчикаОжидания(ПараметрыОбработчикаОжиданияСохраненияНастроек);
...
Процедура ЗаполнитьТаблицуПереходов()
	...
	Если МодельСервиса Тогда
		НовыйПереход = ДобавитьСтрокуТаблицыПереходов("СтраницаСписокНастроек", "СтраницаНавигацияНачало");
		НовыйПереход.ИмяОбработчикаПриОткрытии = "СтраницаСписокНастроек_ПриОткрытии";
		НовыйПереход.ИмяОбработчикаПриПереходеДалее = "СтраницаСписокНастроек_ПриПереходеДалее";
	ИначеЕсли ИспользоватьСинхронизациюДанных Тогда
		...
		НовыйПереход = ДобавитьСтрокуТаблицыПереходов("СтраницаСписокНастроек", "СтраницаНавигацияПродолжение");
		НовыйПереход.ИмяОбработчикаПриОткрытии = "СтраницаСписокНастроек_ПриОткрытии";
		НовыйПереход.ИмяОбработчикаПриПереходеДалее = "СтраницаСписокНастроек_ПриПереходеДалее";
	Иначе
...
Процедура НастроитьДинамическийСписок(Список)
	...
	ТекстЗапроса =
		...
		|	ОбменСУРМККассыККМ.Ссылка.Код КАК Код
		|ПОМЕСТИТЬ втУзлыНастроены
		|ИЗ
		...
		|	[Организация]
		|	ВЫБОР
		|		КОГДА втУзлыНастроены.Код ЕСТЬ NULL
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК УзелНастроен
		|ИЗ
		|	[КассаККМУРМК] КАК КассыККМ
		|		ЛЕВОЕ СОЕДИНЕНИЕ втУзлыНастроены КАК втУзлыНастроены
		|		ПО КассыККМ.Ссылка = втУзлыНастроены.КассаККМ
		|";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "[КассаККМУРМК]", МетаданныеКассыККМ.ПолноеИмя());
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "[ТорговыйОбъект]", "КассыККМ." +ИмяРеквизитаТорговыйОбъект+ " КАК ТорговыйОбъект,");

	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "[Организация]", "КассыККМ." +ИмяРеквизитаОрганизация+ " КАК Организация,");
