...
// Получает продолжительность дня.
//
// Параметры:
//  РабочаяСмена - СправочникСсылка.ИнтервалыРаботыМагазинов - интервал работы.
//
...
// Получает список сотрудников смены.
//
// Параметры:
//  Объект - ДокументОбъект.ЕжедневныйОтчет - смена сотрудников.
//
Функция ПолучитьСписокСотрудниковСмены(Объект) Экспорт
	...
	Если ЗначениеЗаполнено(РабочаяСмена) Тогда
		...
		|	ДОБАВИТЬКДАТЕ(ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0), МИНУТА, СУММА(РАЗНОСТЬДАТ(ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0), ПланируемоеРабочееВремяСотрудников.ДлинаРабочегоВремени, СЕКУНДА) / 60)) КАК ДлинаРабочегоВремени
		|ИЗ
