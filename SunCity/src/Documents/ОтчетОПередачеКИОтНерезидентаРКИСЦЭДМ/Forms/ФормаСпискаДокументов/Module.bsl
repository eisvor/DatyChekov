...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокНаправленийАктов.Добавить(Перечисления.НаправленияДокументовИСМПТК.Входящий,  НСтр("ru = 'Отчеты о передаче КИ импортеру (входящие) (ИС ЦЭДМ)'"));
	СписокНаправленийАктов.Добавить(Перечисления.НаправленияДокументовИСМПТК.Исходящий, НСтр("ru = 'Отчеты о передаче КИ импортеру (исходящие) (ИС ЦЭДМ)'"));
...
Процедура ОткрытьСписокАктов(НаправлениеАкт)
	...
	Если НаправлениеАкт = ПредопределенноеЗначение("Перечисление.НаправленияДокументовИСМПТК.Входящий") Тогда
		ОткрытьФорму("Документ.ОтчетОПередачеКИОтНерезидентаРКИСЦЭДМ.Форма.ФормаСпискаВходящих", ПараметрыФормы);
	Иначе
		ОткрытьФорму("Документ.ОтчетОПередачеКИОтНерезидентаРКИСЦЭДМ.Форма.ФормаСпискаИсходящих", ПараметрыФормы);
	КонецЕсли;
