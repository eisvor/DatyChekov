...
Процедура ОбработатьДанныеПоКодуСервер(СтруктураРезультат) Экспорт
	...
	Если СтрокаРезультата.Свойство("Карта") Тогда
		...
		Иначе
			ПодключаемоеОборудованиеРТ.УстановитьКонтрагентаПоДисконтнойКарте(ЭтотОбъект, СтруктураРезультат, СтрокаРезультата);
		КонецЕсли;
...
Функция ДобавитьНайденныеПозицииТоваров(СтруктураПараметров) Экспорт 
	...
	СтруктураДействий.Вставить("ПересчитатьСуммуЗакупки");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьСистемуНалогообложения(Объект.Дата, Объект.Организация, Объект.Магазин);
...
Процедура ТоварыСуммаЗакупкиПриИзменении(Элемент)
	...
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПересчитатьЦенуЗакупкиПоСуммеЗакупки");
...
&НаКлиенте
Процедура ПроверитьЗаполнениеДляПолученияРезультатов(ЕстьОшибки)
	
	Если НЕ ЗначениеЗаполнено(Объект.НачалоПериода) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено начало периода'"), Объект, "НачалоПериода", "Объект", ЕстьОшибки);
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.КонецПериода) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено окончание периода'"), Объект, "КонецПериода", "Объект", ЕстьОшибки);
	КонецЕсли;   
	
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнена организация'"), Объект, "Организация", "Объект", ЕстьОшибки);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.Магазин) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнен магазин'"), Объект, "Магазин", "Объект", ЕстьОшибки);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнен поставщик'"), Объект, "Контрагент", "Объект", ЕстьОшибки);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.Договор) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнен договор'"), Объект, "Договор", "Объект", ЕстьОшибки);
	КонецЕсли;
...
Процедура ЗаполнитьПоРезультатамПродаж(Команда)
	...
	ЕстьОшибки = Ложь;
	ПроверитьЗаполнениеДляПолученияРезультатов(ЕстьОшибки);
...
&НаКлиенте
Процедура ЗаполнитьПоРезультатамПродажПоМагазину(Команда)
	...
	ЗаполнитьПоРезультатамПродажНаСервере(Истина);
...
Процедура ОрганизацияПриИзмененииСервер()
	...
	Объект.УчитыватьНДС = УчетНДС.ПрименяетсяНДС(
		Объект.Дата,
		Объект.Организация,
		Объект.Магазин);
	Объект.ЦенаВключаетНДС = Объект.УчитыватьНДС;
...
Процедура СообщитьОбОшибкахОткрытияПодбора(Отказ)
	...
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Поле ""Поставщик"" не заполнено'"), Объект, "Объект.Контрагент",,Отказ);
	КонецЕсли;
...
Процедура ЗаполнитьЭтапыОплатНаСервере(Результат, ЗаполнитьПоВыбранномуГрафику = Ложь) Экспорт
	...
	ПараметрыЭтаповОплат.Вставить("СуммаДокумента",  Объект.Товары.Итог("Сумма"));
...
Процедура ОбновитьИтоговыеПоказатели(ЗапросДанных = Ложь, ПересчитатьВзаимозачет = Ложь)
	...
	Объект.СуммаЗакупки   = Объект.Товары.Итог("СуммаЗакупки");
	Объект.СуммаПродажи   = Объект.Товары.Итог("Сумма");
	ПроизвестиРасчетКомиссионногоВознагражденияНаСервере();
...
&НаСервере
Процедура ЗаполнитьПоРезультатамПродажНаСервере(ПоМагазину = Ложь)
	...
	УчетНДСУКомитента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "УчетНДСУКомитента");;
	ОбнулятьНДС = Ложь;
	...
	|	И ВЫБОР
	|			КОГДА &ПоМагазину = ИСТИНА
	|				ТОГДА ТоварыОтчетовКомитенту.Склад.Магазин = &Магазин
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	...
	Запрос.УстановитьПараметр("ПоМагазину", ПоМагазину);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОбнулятьНДС Тогда
			НоваяСтрока.СтавкаНДС = Справочники.СтавкиНДС.ПолучитьСтавкуНДСБезНДС();
		КонецЕсли;
		
		НоваяСтрока.СтавкаНДС = УчетНДС.СкорректироватьСтавкуНДС(НоваяСтрока.СтавкаНДС, Выборка.ДатаРеализации);
		НоваяСтрока.ЦенаЗакупки = ЗапасыСервер.ЦенаПоследнейЗакупкиПоДоговоруКомиссии(ПараметрыЦеныЗакупки);
...
&НаСервере
Процедура ЗаполнитьСистемуНалогообложения(ДатаДокумента, Организация, Магазин)
	
	СистемаНалогообложения = ПодключаемоеОборудованиеРТ.ПолучитьСистемуНалогообложения(ДатаДокумента, Организация, Магазин);
