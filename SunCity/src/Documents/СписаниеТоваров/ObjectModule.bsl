...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ИнтеграцияГИСМРТ.ЗаполнитьПризнакиЕстьМаркируемаяПродукцияИЕстьКиЗ(ЭтотОбъект, "Серии"));
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриказНаПроведениеИнвентаризацииТоваров") 
	 ...
	 ИЛИ ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.АктОРасхожденияхПриПриемкеТоваров") Тогда
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.АктОРасхожденияхПриПриемкеТоваров") Тогда
			Реквизиты = Новый Структура("СкладОтправитель, СкладПолучатель, Магазин, Организация", "СкладОтправитель", "СкладПолучатель", "МагазинОтправитель", "Организация");
			
			ВариантУчетаРасхождений = Перечисления.ВариантыУчетаРасхождений.НаСкладеОтправителя;
			Для Каждого СтрокаТоваров Из ДанныеЗаполнения.Товары Цикл
				Если ЗначениеЗаполнено(СтрокаТоваров.ВариантУчетаРасхождения) Тогда
					ВариантУчетаРасхождений = СтрокаТоваров.ВариантУчетаРасхождения;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			ЗначенияРеквизитов = ОбщегоНазначенияРТ.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения, Реквизиты);
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, ЗначенияРеквизитов);
			Если ВариантУчетаРасхождений = Перечисления.ВариантыУчетаРасхождений.НаСкладеПолучателя Тогда
				Склад = ЗначенияРеквизитов.СкладПолучатель;
			Иначе
				Склад = ЗначенияРеквизитов.СкладОтправитель;
			КонецЕсли;
		Иначе
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЧекКоррекции") Тогда
		...
		|	ЧекКоррекцииПозицииЧека.ЦенаСоСкидками КАК Цена,
		|	ЧекКоррекцииПозицииЧека.СуммаСоСкидками КАК Сумма,
		|	ЧекКоррекцииПозицииЧека.Количество КАК КоличествоУпаковок,
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Если ЗначениеЗаполнено(ДокументОснование) 
		...
		АналитикаХозяйственнойОперации = ЗначениеНастроекПовтИсп.АналитикаХозяйственнойОперацииПоУмолчанию(АналитикаХозяйственнойОперации, Перечисления.ХозяйственныеОперации.СписаниеПоИнвентаризации);
	Иначе
		АналитикаХозяйственнойОперации = ЗначениеНастроекПовтИсп.АналитикаХозяйственнойОперацииПоУмолчанию(АналитикаХозяйственнойОперации, Перечисления.ХозяйственныеОперации.СписаниеНаЗатраты);
	КонецЕсли;
