...
Функция ПараметрыПередачиВЕГАИС(ДокументСсылка)
	...
	СтруктураТаблиц = ПродажиСервер.ДанныеДляВскрытияТары(Объект, ДанныеЕГАИСДостаточны);
...
Процедура ДополнитьМассивИзТаблицы(МассивТоваров, ТаблицаТоваровЧека)
	...
	Для Каждого СтрокаТЧ Из ТаблицаТоваровЧека Цикл
		...
		СтавкаНДС = ОбработкаТабличнойЧастиТоварыСерверПовтИсп.СтавкаНДСЧислом(СтрокаТЧ.СтавкаНДС);
