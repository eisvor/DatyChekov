...
Функция ПолучитьДанныеДляЭД(СсылкаНаОбъект) Экспорт
	...
	|				ТОГДА Товары.Номенклатура.ЕдиницаИзмерения
	|			ИНАЧЕ Товары.Упаковка
	|		КОНЕЦ) КАК БазоваяЕдиницаСсылка,
	|	ЕСТЬNULL(втНоменклатураКонтрагента.ЕдиницаИзмеренияКод, ВЫБОР
	|			КОГДА Товары.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|				ТОГДА &ШтукаКод
	|			КОГДА Товары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	...
	|				ТОГДА &Штука
	|			КОГДА Товары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|				ТОГДА Товары.Номенклатура.ЕдиницаИзмерения.НаименованиеПолное
	|			ИНАЧЕ Товары.Упаковка.Наименование
	|		КОНЕЦ) КАК БазоваяЕдиницаНаименованиеПолное,
	|	ВЫБОР
	|		КОГДА втНоменклатураКонтрагента.ЕдиницаИзмерения ЕСТЬ NULL
	|				И Товары.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|				И НЕ Товары.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА Товары.Номенклатура.ЕдиницаИзмерения.МеждународноеСокращение
	|		ИНАЧЕ &ПустаяСтрока
	...
	|ГДЕ
	|	Товары.Ссылка = &Ссылка
	|	И Товары.ИдентификаторНоменклатурыПоставщика = &ПустаяСтрока
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	&Валюта КАК Валюта,
	|	&ПустаяСтрока КАК НомерСчета,
	|	&ПустаяСтрока КАК АдресДоставки,
	|	&ПустаяСтрока КАК АдресДоставкиЗначенияПолей,
	...
	Запрос.УстановитьПараметр("ЕдиницаИзмеренияШтука", Справочники.БазовыеЕдиницыИзмерения.НайтиПоКоду("796"));
	...
	Запрос.УстановитьПараметр("Шт", НСтр("ru = 'Шт'"));
	Запрос.УстановитьПараметр("Штука", НСтр("ru = 'Штука'"));
	Запрос.УстановитьПараметр("ШтукаКод", НСтр("ru = '796'"));
	Запрос.УстановитьПараметр("УпаковкаКод", НСтр("ru = '0000'"));
