...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = РаботаСДокументамиИСМПТККлиентСервер.ИмяСобытияЗаписьУведомлениеОВвозеИзЕАЭС() Тогда		
...
Процедура ПоставщикПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.Поставщик) Тогда
		СтранаОтправления = РаботаСДокументамиИСМПТКПереопределяемый.ПолучитьСтрануОтправленияВвозИзЕАЭС(Объект.Поставщик);
...
Процедура УправлениеФормой()
	...
	НеОтправлен = Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыДокументовИСМПТК.НеОпределен");
	Исходный = Объект.ТипУведомления = ПредопределенноеЗначение("Перечисление.ВидыДокументаИСМПТК.Исходный");
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТипУведомления", "Доступность", НеОтправлен);
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДеревоМаркированнойПродукцииПроверитьСостояниеКодовНаСервере", 
																	   "Видимость", НеОтправлен);
	//Отражение в учете
	ДокументМожноОтразитьВБазе = РаботаСДокументамиИСМПТКПереопределяемый.ПроверитьВозможностьОтраженияДокументаВУчете(ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВвозеИзЕАЭСИСМПТК());
...
Функция ПроверитьСвязанноеУведомление(ВыбранноеЗначение)
	...
	Возврат РаботаСДокументамиИСМПТК.ПроверитьНаличиеСвязанного_Общий(ВыбранноеЗначение, Истина, "", ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВвозеИзЕАЭСИСМПТК());
...
Функция ПолучитьДанныеДокументаОснованияНаСервере() Экспорт
	...
	Возврат РаботаСДокументамиИСМПТКПереопределяемый.ПолучитьДанныеДокументаОснованияДляПерезаполненияИСМПТ_Общий(Объект.ДокументОснование, ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВвозеИзЕАЭСИСМПТК());
...
Процедура ИзменитьКоманднуюПанельДокумента()
	...
	////ПРОВЕРКИ////
	Первичный = Объект.ТипУведомления = ПредопределенноеЗначение("Перечисление.ВидыДокументаИСМПТК.Исходный");
	...
	//Действия по АПИ
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВвозеИзЕАЭСИСМПТКОтправить", 
																"Видимость", МожноОтправить);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДокументУведомлениеОВвозеИзЕАЭСИСМПТКОтозвать", 
																"Видимость", МожноОтозвать);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВвозеИзЕАЭСИСМПТКПодтвердить", 
																"Видимость", МожноОтозвать);
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаСоздатьКорректировку", 
																"Видимость", Первичный И МожноОтозвать);
...
Функция ПодготовитьДанныеДляРегистрацииНоменклатуры()
	...
	Для Каждого ЭлементМассива Из ТаблицаДляПроверки Цикл
		Если ЭлементМассива.ВидУпаковки = ПредопределенноеЗначение("Перечисление.ВидыУпаковокИСМПТК.Групповая")
			...
			ДанныеНоменклатуры = ОбщегоНазначенияИСМПТКПереопределяемый.ПолучитьДанныеНоменклатурыИзРегистраШтрихкодовПоШтрихкоду(, ЕАН,,СообщенияОбОшибках);
...
#Область СвязьСУведомлениемПервичнымКорректировочным

&НаКлиенте
Процедура ЗаполнитьСведенияОСвязанныхДокументах()
	
	ЭтаФорма.ПредставлениеВвозИсправленный = Неопределено;
	ЭтаФорма.ПредставлениеВвозИсходный 	   = Неопределено;
		
	Если Объект.ТипУведомления = ПредопределенноеЗначение("Перечисление.ВидыДокументаИСМПТК.Исходный") Тогда
		Если ЗначениеЗаполнено(Объект.Идентификатор) Тогда
			Если ЗначениеЗаполнено(Объект.ИдентификаторВвозИсправленный) Тогда
				// Этим способом ищем связанный документ, когда в текущем документе у нас заполнен его идентификатор.
				СвязанныйВвозИсправленный = НайтиДокументПоИдентификатору(ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВвозеИзЕАЭСИСМПТК(), Объект.ИдентификаторВвозИсправленный);
			Иначе
				...
				// При этом искать корректировку имеет смысл только для уже зарегистрированного документа.
				СвязанныйВвозИсправленный = НайтиДокументПоИдентификаторуОтОбратного("ИдентификаторВвозПервичный");
			КонецЕсли;
			СформироватьПредставлениеСвязанногоДокумента(ЭтаФорма, СвязанныйВвозИсправленный, "ПредставлениеВвозИсправленный");
		КонецЕсли;
	Иначе
		Если ЗначениеЗаполнено(Объект.ИдентификаторВвозПервичный) Тогда
			СвязанныйВвозИсходный = НайтиДокументПоИдентификатору(ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВвозеИзЕАЭСИСМПТК(), Объект.ИдентификаторВвозПервичный);
		Иначе
			СвязанныйВвозИсходный = НайтиДокументПоИдентификаторуОтОбратного("ИдентификаторВвозИсправленный");
		КонецЕсли;
		СформироватьПредставлениеСвязанногоДокумента(ЭтаФорма, СвязанныйВвозИсходный, "ПредставлениеВвозИсходный");
	КонецЕсли;
...
Функция НайтиДокументПоИдентификаторуОтОбратного(ИмяРеквизитаИдентификатора)
	...
	СтруктураДанныхДляПоиска.ИмяДокументаДляЗапроса	  	= ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВвозеИзЕАЭСИСМПТК();
	СтруктураДанныхДляПоиска.ИмяРеквизитаВидДокумента 	= "ТипУведомления";
	СтруктураДанныхДляПоиска.ОтборПоВидуДокумента		= Истина;
	СтруктураДанныхДляПоиска.ИмяРеквизитаИдентификатора = ИмяРеквизитаИдентификатора;
	
	Если ИмяРеквизитаИдентификатора = "ИдентификаторВвозИсправленный" Тогда
...
Процедура СформироватьПредставлениеСвязанногоДокумента(Форма, СсылкаДокумент, ИмяЭлемента)
	...
	Если ЗначениеЗаполнено(СсылкаДокумент) Тогда
		...
		Если ИмяЭлемента = "ПредставлениеВвозИсправленный" Тогда
			Форма.СвязанныйВвозИсправленныйНомерИСМПТ = ИнтеграцияИСМПТКПереопределяемый.ЗначениеРеквизитаОбъекта(СсылкаДокумент, "НомерИСМПТ"); 
		КонецЕсли;
	Иначе
...
&НаКлиенте
Процедура ПредставлениеУведомлениеВвозИсходныйНажатие(Элемент, СтандартнаяОбработка)
	
	Если ЗначениеЗаполнено(ПредставлениеВвозИсходный) Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьЗначение(, СвязанныйВвозИсходный);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПредставлениеУведомлениеВвозИсправленныйНажатие(Элемент, СтандартнаяОбработка)
	
	Если ЗначениеЗаполнено(ПредставлениеВвозИсправленный) Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьЗначение(, СвязанныйВвозИсправленный);
	КонецЕсли;
...
Процедура УстановитьВидимостьПолейСвязанныхДокументов()
	...
	// Информация о Ввозе исходном
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СвязанноеУведомлениеИсходный", 
																	   "Видимость", Не Первичный И ((ЗначениеЗаполнено(ПредставлениеВвозИсходный) 
																								 И НЕ ПредставлениеВвозИсходный = РаботаСТекстамиИСМПТККлиентСервер.ПредставлениеНеНайден())
																								 ИЛИ ЗначениеЗаполнено(Объект.ИдентификаторВвозПервичный)));
	// Информация о Ввозе исправленном
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СвязанноеУведомлениеИсправленный", 
																	   "Видимость", Первичный И ((ЗначениеЗаполнено(ПредставлениеВвозИсправленный) 
																							  И НЕ ПредставлениеВвозИсправленный = РаботаСТекстамиИСМПТККлиентСервер.ПредставлениеНеНайден())
																							  ИЛИ ЗначениеЗаполнено(Объект.ИдентификаторВвозИсправленный)));
	
КонецПроцедуры

&НаСервере
Функция ПолучитьПредставлениеУведомленияОВвозе()
	
	Возврат Метаданные.Документы.УведомлениеОВвозеИзЕАЭСИСМПТК.Представление() + НСтр("ru = ' (корректирвоочное)'");
...
Процедура СоздатьКорректировку(Команда)
	...
	Если НЕ ЗначениеЗаполнено(ЭтаФорма.Параметры.Ключ) ИЛИ ЭтаФорма.Модифицированность Тогда
		...
		ТекстВопроса = СтрЗаменить(ТекстВопроса, "%НаименованиеДокумента%", ПолучитьПредставлениеУведомленияОВвозе());
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТекстВопроса) Тогда
		ДополнительныеПараметры = Новый Структура("Форма", ЭтаФорма);
		Режим = РежимДиалогаВопрос.ДаНет;
		Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопросаЗаписьПриСозданииУведомления", ЭтотОбъект, ДополнительныеПараметры);
		ПоказатьВопрос(Оповещение, ТекстВопроса, Режим, 0);
	Иначе
		ПараметрыФормы = Новый Структура;
...
&НаКлиенте
Процедура ПослеЗакрытияВопросаЗаписьПриСозданииУведомления(Результат, Параметры) Экспорт
