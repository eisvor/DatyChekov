...
&НаКлиенте
Процедура ЗаполнитьПоРезультатамСписанияПоМагазину(Команда)
	...
	ЗаполнитьПоРезультатамСписанийНаСервере(Истина);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПоРезультатамСписанийНаСервере(ПоМагазину = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.Цена = ЗапасыСервер.ЦенаПоследнейЗакупкиПоДоговоруКомиссии(ПараметрыЦеныЗакупки);
