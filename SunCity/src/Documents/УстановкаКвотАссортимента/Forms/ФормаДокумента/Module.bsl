...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если ЗначениеЗаполнено(Объект.ОбъектПланирования) Тогда
			ДоступностьПравила = ОбщегоНазначенияРТВызовСервера.РеквизитДоступен(
...
Процедура ОбъектПланированияПриИзменении(Элемент)
	...
	Если НЕ ДоступностьПравила Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "ОбъектПланирования");
		Объект.ОбъектПланирования = Неопределено;
	КонецЕсли;
...
Процедура УстановитьПроцентОтклоненияВсемСтрокам(Команда)
	...
	Если ЭлементыДерева.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(СтрокаСообщения, , "ДеревоКатегорий");
	Иначе
		ДополнительныеПараметры = Новый Структура;
		ОбработчикОповещения = Новый ОписаниеОповещения(
