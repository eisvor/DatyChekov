...
Процедура ПриИзмененииМагазина()
	...
	Объект.СкладОтправитель = ЗначениеНастроекПовтИсп.СкладПоступленияПоУмолчанию(Объект.Магазин,,Объект.СкладОтправитель, Пользователи.ТекущийПользователь());
	Объект.СкладПолучатель  = ЗначениеНастроекПовтИсп.СкладПродажиПоУмолчанию(Объект.Магазин,,Объект.СкладПолучатель, Пользователи.ТекущийПользователь());
...
Процедура ПроверитьВыводНапоминанияЕГАИСОбработчикОжидания()
	...
	Если ЗначениеЗаполнено(ТекстНапоминания) Тогда
		ПоказатьОповещениеПользователя(Нстр("ru='Обмен данными с ЕГАИС:'"),, ТекстНапоминания, БиблиотекаКартинок.Изменить);
