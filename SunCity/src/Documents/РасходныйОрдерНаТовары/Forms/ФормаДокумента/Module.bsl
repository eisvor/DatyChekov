...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(Документы.РасходныйОрдерНаТовары.ПараметрыУказанияСерий(Объект));
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Замер = ОценкаПроизводительностиРТКлиент.НачатьЗамер(Ложь, 
	                                            "Документ.РасходныйОрдерНаТовары.ФормаДокумента.Запись",
                                                          Ложь);
...
Процедура ПроверкаКоличестваТоваров(Команда)
    ...
    ОценкаПроизводительностиРТКлиент.НачатьЗамер(
             Истина, "Обработка.ПроверкаКоличестваТоваров.Форма.Форма.Открытие");
...
Процедура ОткрытьПодбор(Команда)
	...
	Иначе
		ПараметрЗаголовок = СтрЗаменить(ПараметрЗаголовок, "%Документ%", НСтр("ru = 'расходный ордер на товары'"));
	КонецЕсли;
...
Процедура ЗаполнитьПоОснованию(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ДокументОснование)Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не указано основание.'"), ЭтотОбъект);		Возврат;
	КонецЕсли;
		
	Если Объект.Товары.Количество() > 0 Тогда
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ОповещениеЗаполнитьПоОснованию", ЭтотОбъект);
...
&НаКлиенте
Процедура ОповещениеЗаполнитьПоОснованию(Ответ, ДополнительныеПараметры) Экспорт
