...
Процедура ДобавитьТоварыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ИначеЕсли ДополнительныеПараметры.РежимДобавления = "ДобавитьНоменклатуру" Тогда
			...
			Элементы.ДеревоЦен.ТекущаяСтрока = ИдентификаторСтроки;
			ДобавитьНоменклатуруСервер(ИдентификаторСтроки);
	...
	Если Характеристика <> Неопределено Тогда
		...
		Иначе
			ЗаполнитьЗначенияСвойств(
				НоваяСтрока, 
				ТекущийРодитель,
				"Номенклатура, ХарактеристикиИспользуются, ИспользоватьЦенуНоменклатуры, ЦеноваяГруппа, ЕдиницаИзмерения");
		КонецЕсли;
		...
		ИдентификаторСтроки = НоваяСтрока.ПолучитьИдентификатор();
		Элементы.ДеревоЦен.ТекущаяСтрока = ИдентификаторСтроки;
		ДеревоЦенХарактеристикаПриИзмененииСервер(ИдентификаторСтроки);
...
&НаКлиенте
Процедура УказатьЦеныВручнуюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	Если Результат <> Неопределено Тогда
		ОчиститьСообщения();
		УказатьЦеныВручнуюНаСервере(Результат);
	КонецЕсли;
...
&НаКлиенте
Процедура УказатьЦеныВручную(Команда)
	
	ПараметрыОткрытия = ПараметрыОткрытияФормыВыбораПараметровРасчета(Ложь, Истина);
	ДополнительныеПараметры = Новый Структура;
	ОбработчикОповещения = Новый ОписаниеОповещения(
									"УказатьЦеныВручнуюЗавершение",
									ЭтотОбъект,
									ДополнительныеПараметры);
	ОткрытьФорму(
		"Документ.УстановкаЦенНоменклатуры.Форма.ИзменениеЦен",
		ПараметрыОткрытия,
		ЭтаФорма,
		,
		,
...
Процедура ДобавитьХарактеристику(Команда)
	...
	Если ТекущиеДанные = Неопределено
		...
		Текст = НСтр("ru = 'Для того, чтобы выбрать характеристику необходимо предварительно выбрать номенклатуру.'");
...
Процедура ДеревоЦенНоменклатураПриИзмененииСервер(ИдентификаторСтроки, РассчитатьЦены = Истина)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		...
		|	СправочникНоменклатура.ВидНоменклатуры.ИспользоватьЦенуНоменклатуры КАК ИспользоватьЦенуНоменклатуры
		|ИЗ
...
Процедура ДобавитьНоменклатуруСервер(ИдентификаторСтроки)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		Если ТекущаяСтрока.Номенклатура.ВидНоменклатуры.ИспользованиеХарактеристик <> Перечисления.ВариантыВеденияДополнительныхДанныхПоНоменклатуре.НеИспользовать Тогда
			...
			ТекущаяСтрока.ИспользоватьЦенуНоменклатуры = ТекущаяСтрока.Номенклатура.ВидНоменклатуры.ИспользоватьЦенуНоменклатуры;
		КонецЕсли;
	КонецЕсли;
	...
	Если ТаблицаНоменклатуры.Количество() > 0 Тогда
		...
		Если СтрокаВидЦены.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБ
			...
			Если Ценообразование.ПроверитьСхемуКомпоновкиДанных(СхемаКомпоновкиДанных, ТекстОшибкиПриПроверке) Тогда
				...
				ДобавитьПолеНабораДанныхСКД(НаборДанныхНоменклатура, "ИдентификаторСтроки", "Число");
...
Процедура ДобавитьТоварыПоОтборуНаСервере(АдресВоВременномХранилище)
	...
	|   	ИНАЧЕ ЛОЖЬ 
	|	КОНЕЦ КАК ХарактеристикиИспользуются,
	|	ТаблицаНоменклатура.Номенклатура.ВидНоменклатуры.ИспользоватьЦенуНоменклатуры КАК ИспользоватьЦенуНоменклатуры
	|ИЗ
...
Процедура ЗагрузитьЗначенияБазовыхЦен(
	...
	Если МассивСтрокВидовЦен.Количество() > 0 Тогда
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры КАК ЦеныНоменклатуры
		|		ПО (ЦеныНоменклатуры.Характеристика = Товары.Характеристика)
		|			И (ЦеныНоменклатуры.Номенклатура = Товары.Номенклатура)
		|			И ((ЦеныНоменклатуры.Период, ЦеныНоменклатуры.ВидЦены) В
		|				(ВЫБРАТЬ
		|					МАКСИМУМ(Цены.Период),
		|					Цены.ВидЦены
		|				ИЗ
...
Процедура ЗагрузитьТабличнуюЧастьТовары()
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ХарактеристикиИспользуются,
	|	Товары.Номенклатура.ВидНоменклатуры.ИспользоватьЦенуНоменклатуры КАК ИспользоватьЦенуНоменклатуры,
...
Процедура ЗагрузитьЦеныИзТаблицыЗначений(
	...
	ТаблицаЗначенийДополнена = Ложь;
	
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.ДокументОснование) И ТипЗнч(ЭтотОбъект.Объект.ДокументОснование) = Тип("ДокументСсылка.ПоступлениеТоваров") Тогда
		Штрихкоды = ЭтотОбъект.Объект.ДокументОснование.ШтрихкодыУпаковок.Выгрузить(,"ШтрихкодУпаковки");
		Если Штрихкоды.Количество()>0 Тогда
			
			ДобавитьКодыМаркировкиКНоменклатуре(Таблица, Штрихкоды);
			ТаблицаЗначенийДополнена = Истина;
	...
	Для Каждого СтрокаИсточник Из Таблица Цикл
		...
		Если СтрокаТаблицыЦен <> Неопределено Тогда
			...
			Если (СтрокаТаблицыЦен[ИмяКолонки] = 0 ИЛИ НЕ ТолькоНезаполненные) И СтрокаИсточник.Цена <> Null Тогда
				...
				Если ТаблицаЗначенийДополнена Тогда
					Если СтрокаИсточник.ВидПродукции = Перечисления.ОсобенностиУчетаНоменклатуры.ТабачнаяПродукция Тогда
						
						СтруктураЦены = ШтрихкодированиеОбщегоНазначенияИС.НоваяСтруктураОбработкиШтрихкода(СтрокаИсточник.ЗначениеШтрихкода);
						СоставКода = СтруктураЦены.СоставКодаМаркировки;
						
						Если ЗначениеЗаполнено(СоставКода.МРЦСтрокой) Тогда
							
							МРЦ = СоставКода.МРЦ;
							МинимумОтМРЦ = МРЦ*0.75;
							
							Если СтрокаИсточник.Цена > МРЦ Тогда
								Цена = МРЦ;
							ИначеЕсли СтрокаИсточник.Цена < МинимумОтМРЦ Тогда
								Цена = МинимумОтМрц;
							КонецЕсли;
						КонецЕсли;
...
Процедура ПроверитьКорректностьЗаполненияДокумента(Отказ)
	...
	Для Каждого ТекЭлементНоменклатура Из ЭлементыНоменклатура Цикл
		...
		Если ТекЭлементНоменклатура.ХарактеристикиИспользуются 
			И НЕ ТекЭлементНоменклатура.ИспользоватьЦенуНоменклатуры Тогда
				...
				Если ТекущаяЦена = 0 ИЛИ НЕ ТолькоНезаполненные Тогда
					...
					Если ЗначениеЗаполнено(АлгоритмРасчетаЦены) Тогда
						...
						Попытка
							...
							ЗначениеЦены = ОкруглитьЦену(ОбщегоНазначения.ВычислитьВБезопасномРежиме(АлгоритмРасчетаЦены, СтрокаТаблицыЦен), ВидЦены);
...
Функция СоздатьПустоеДеревоНоменклатуры()
	...
	ДеревоЗначений.Колонки.Добавить("ИспользоватьЦенуНоменклатуры", Новый ОписаниеТипов("Булево"));
...
Функция СоздатьПустуюТаблицуНоменклатуры()
	...
	ТаблицаЗначений.Колонки.Добавить("ИдентификаторСтроки", ОбщегоНазначенияРТКлиентСервер.ОписаниеТиповЧисла(10, 0));
	ТаблицаЗначений.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаЗначений.Колонки.Добавить("Характеристика", Новый ОписаниеТипов("СправочникСсылка.ХарактеристикиНоменклатуры"));
...
Процедура УстановитьДоступностьЭлементовСервер()
	...
	МассивЭлементов.Добавить("ДеревоЦенУказатьЦеныВручную");
...
&НаСервере
Процедура УказатьЦеныВручнуюНаСервере(ПараметрыРасчета)
	...
	Для Каждого СтрокаМассива Из ПараметрыРасчета.ВидыЦен Цикл
		...
		Для Каждого ВыделеннаяСтрока Из Элементы.ДеревоЦен.ВыделенныеСтроки Цикл
			...
			СтрокаТаблицыЦен = ДеревоЦен.НайтиПоИдентификатору(ВыделеннаяСтрока);
			ОбработатьСтрокуПриУказанииЦеныВручнуюНаСервере(СтрокаТаблицыЦен, СтрокаМассива, ИмяКолонки, ПараметрыРасчета);
...
&НаСервере
Процедура ОбработатьСтрокуПриУказанииЦеныВручнуюНаСервере(СтрокаТаблицы, СтрокаМассива, ИмяКолонки, ПараметрыРасчета)
	
	Если СтрокаМассива.Цена <= 0 Тогда
		Возврат;
	КонецЕсли;
		
	СтрокаТаблицы[ИмяКолонки] = СтрокаМассива.Цена;
	Если СтрокаТаблицы["СтараяЦена" + ИмяКолонки] <> 0 Тогда
		СтрокаТаблицы["ПроцентИзменения" + ИмяКолонки] = Окр(
		100
		* (СтрокаТаблицы[ИмяКолонки] - СтрокаТаблицы["СтараяЦена" + ИмяКолонки])
					...
					Если СтрокаВыбраннойЦены.Выбрана 
						...
						Если СтрокаВыбраннойЦены.НепосредственноВлияющиеЦены.Количество() = 1 Тогда
							...
							Если ЗначениеЗаполнено(Формула) Тогда
								...
								Попытка
									УстановитьБезопасныйРежим(Истина);
									РасчетнаяЦена = ОкруглитьЦену(ОбщегоНазначения.ВычислитьВБезопасномРежиме(Формула), СтрокаВыбраннойЦены.НепосредственноВлияющиеЦены[0].Значение);
								Исключение
...
&НаСервере
Процедура ДобавитьКодыМаркировкиКНоменклатуре(Таблица, Штрихкоды)
	...
	Если ИспользоватьУпаковкиНоменклатуры Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|	ЕСТЬNULL(Таблица.Упаковка, ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)) КАК Упаковка,
			|	Таблица.Коэффициент КАК Коэффициент,";
	КонецЕсли;
	ТекстЗапроса = ТекстЗапроса +
	"
	|	Таблица.Цена КАК Цена,
	|	ЕстьNull(Таблица.ВидЦены, Значение(Справочник.ВидыЦен.ПустаяСсылка)) ВидЦены,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Штрихкоды.ШтрихкодУпаковки КАК КодМаркированнойПродукции
	|ПОМЕСТИТЬ КодыМаркированнойПродукции
	|ИЗ
	...
	|	ЕСТЬNULL(ШтрихкодыУпаковокТоваров.Характеристика, ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)) КАК Характеристика,
	|	ВЫБОР
	|		КОГДА ШтрихкодыУпаковокТоваров.Упаковка = НЕОПРЕДЕЛЕНО
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|		ИНАЧЕ ШтрихкодыУпаковокТоваров.Упаковка
	|	КОНЕЦ КАК Упаковка
	|ПОМЕСТИТЬ КодыМаркированнойПродукцииИНоменклатура
	|ИЗ
	|	Справочник.ШтрихкодыУпаковокТоваров КАК ШтрихкодыУпаковокТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КодыМаркированнойПродукции КАК КодыМаркированнойПродукции
	|		ПО ШтрихкодыУпаковокТоваров.Ссылка = КодыМаркированнойПродукции.КодМаркированнойПродукции
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ТаблицаТоваров.ВидЦены ВидЦены,
	|	ВЫРАЗИТЬ(ТаблицаТоваров.ИдентификаторСтроки КАК Число(10)) КАК ИдентификаторСтроки,
	|	МАКСИМУМ(КодыМаркированнойПродукцииИНоменклатура.ЗначениеШтрихкода) КАК ЗначениеШтрихкода
	|ПОМЕСТИТЬ НоменклатураИЗначениеМаркировки
	|ИЗ
	|	ТаблицаТоваров КАК ТаблицаТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ КодыМаркированнойПродукцииИНоменклатура КАК КодыМаркированнойПродукцииИНоменклатура
	|		ПО ТаблицаТоваров.Номенклатура = КодыМаркированнойПродукцииИНоменклатура.Номенклатура";
	
	Если ИспользоватьХарактеристикиНоменклатуры Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|			И ТаблицаТоваров.Характеристика = КодыМаркированнойПродукцииИНоменклатура.Характеристика";
	КонецЕсли;
	
	Если ИспользоватьУпаковкиНоменклатуры Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|			И ТаблицаТоваров.Упаковка = КодыМаркированнойПродукцииИНоменклатура.Упаковка";
	КонецЕсли;
	...
	|	ВЫРАЗИТЬ(ТаблицаТоваров.ИдентификаторСтроки КАК Число(10))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НоменклатураИЗначениеМаркировки.Номенклатура КАК Номенклатура,";
	Если ИспользоватьХарактеристикиНоменклатуры Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|	НоменклатураИЗначениеМаркировки.Характеристика КАК Характеристика,";
	КонецЕсли;
	
	Если ИспользоватьУпаковкиНоменклатуры Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|	НоменклатураИЗначениеМаркировки.Упаковка КАК Упаковка,
		|	НоменклатураИЗначениеМаркировки.Коэффициент КАК Коэффициент,";
	КонецЕсли;
	ТекстЗапроса = ТекстЗапроса + "
	|	НоменклатураИЗначениеМаркировки.Цена КАК Цена,
	|	НоменклатураИЗначениеМаркировки.ВидЦены КАК ВидЦены,
	|	НоменклатураИЗначениеМаркировки.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	НоменклатураИЗначениеМаркировки.ЗначениеШтрихкода КАК ЗначениеШтрихкода,
	|	&ОпределениеВидаПродукции КАК ВидПродукции
	|ИЗ
	|	НоменклатураИЗначениеМаркировки КАК НоменклатураИЗначениеМаркировки";
	
	Если Таблица.Колонки.Найти("ВидЦены") = Неопределено Тогда
		Таблица.Колонки.Добавить("ВидЦены", Новый ОписаниеТипов("СправочникСсылка.ВидыЦен"));
	КонецЕсли;
	
	ИнтеграцияИСРТ.ОпределитьВидПродукцииТекстаЗапроса(ТекстЗапроса, "НоменклатураИЗначениеМаркировки.Номенклатура");
