...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВидыЦенСправочник.Ссылка КАК Ссылка,
	|	ВидыЦенСправочник.ПометкаУдаления КАК ПометкаУдаления
	|ИЗ
	|	Справочник.ВидыЦен КАК ВидыЦенСправочник
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВидыЦен КАК ВидыЦен
	|		ПО (ВидыЦен.ВидЦены = ВидыЦенСправочник.Ссылка)
	|ГДЕ
	|	ВидыЦенСправочник.ПометкаУдаления = Истина";

	Запрос.УстановитьПараметр("ТаблицаВидовЦен", ЭтотОбъект.ВидыЦен);
	РезультатЗапроса = Запрос.Выполнить();
	Если Не РезультатЗапроса.Пустой() Тогда
		
		ТекстОповещения= НСтр("ru = 'Используются помеченные на удаление виды цен.'");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
    ...
    ОценкаПроизводительности.ЗакончитьЗамерВремени("УстановкаЦенНоменклатурыПроведение",ВремяНачалаЗамера,Товары.Количество(), Нстр("ru='Вес по табличной части ""Товары""'"));
