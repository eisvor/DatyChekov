...
Процедура ВыбратьДокументОснование()
	...
	ДокументыВыбора = РаботаСДокументамиИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДокументовОснованияДляДляУведомленияОВвозеИмпортИСЦЭДМ();
...
Процедура УправлениеФормой()
	...
	Элементы.ГруппаДССтраницы.ТекущаяСтраница = Элементы.СтраницаДСОдин;
	...
	ИнтерфейсИСМПТК.УстановитьЗаголовокФормыДокумента(, Объект.Ссылка, ЭтаФорма);
	//Ручной ввод ШК	
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыПоискПоШтрихкоду", "Доступность", НеОтправлен);
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СтраницаДСОдин",   "Доступность", НеОтправлен);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СтраницаДССписок", "Доступность", НеОтправлен);
...
Функция ПроверитьСвязанноеУведомление(ВыбранноеЗначение)
	...
	Возврат РаботаСДокументамиИСМПТК.ПроверитьНаличиеСвязанного_Общий(ВыбранноеЗначение, Истина, "", ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВвозеИзТретьихСтранИСЦЭДМ());
...
Функция ПолучитьДанныеДокументаОснованияНаСервере() Экспорт
	...
	Возврат РаботаСДокументамиИСМПТКПереопределяемый.ПолучитьДанныеДокументаОснованияДляПерезаполненияИСЦЭДМ_Общий(Объект.ДокументОснование, ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВвозеИзТретьихСтранИСЦЭДМ());
...
Процедура ИзменитьКоманднуюПанельДокумента()
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВвозеИзТретьихСтранИСЦЭДМОбновить",
																"Видимость", УПользователяЕстьНастройкаОбмена И Не НеОтправлен);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВвозеИзТретьихСтранИСЦЭДМОтправить",
																"Видимость", УПользователяЕстьНастройкаОбмена И НеОтправлен);
...
Процедура ВыполнитьПроверкуСтатусаИВладельцаПоДаннымЦЭДМ(СоответствиеСтатусаПроверки, КодИдентификации, СтатусСервер, ВладелецСервер)
	...
	Ошибочный = Ложь;
	МассивДопустимыхСтатусовКМ = ИнтеграцияИСМПТК.ДопустимыеСтатусыКМДляДокументаИСЦЭДМ("УведОВвозеИмпорт");
...
Процедура ПодобратьСоответствующийСкладЦЭДМНаСервере()
	...
	Если ДанныеСкладаЦЭДМ = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Для выбранного склада не удалось автоматически определить соответствие по зарегистрированным складам ИС ЦЭДМ! Возможно, для данного склада не задано соответствие в настройках ЦЭДМ или зарегистрировано несколько значений складов.'");
		ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
	Иначе
...
Процедура СкладПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Склад) 
		...
		ТекстВопроса = НСтр("ru = 'Выполнить поиск и заполнение данных соответствующего склада ИС ЦЭДМ?'");
