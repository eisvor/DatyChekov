		...
		Если ИзменитьДокументОснование Тогда
			...
			Если Структура_СвязанныйАкт = Неопределено Тогда
				...
				Если Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияДокументовИСМПТК.Исходящий") 
					И Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыДокументовИСЦЭДМ.НеОпределен")
...
Процедура ВыбратьДокументОснование()
	...
	ДокументыВыбора = РаботаСДокументамиИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДокументовОснованияДляАктаПриемаПередачиИСЦЭДМ(Объект.Направление, Объект.ВидОперации, "Выбрать");
...
Процедура ДокументОснованиеСоздать(Команда)
	...
	Если Объект.Направление = ПредопределенноеЗначение("Перечисление.НаправленияДокументовИСМПТК.Входящий") Тогда
		...
		Если НужноСообщитьОНезаполненнойНоменклатуре Тогда
			ТекстСообщения = НСтр("ru = 'В таблице ""Итоги"" есть строки с незаполненной Номенклатурой! Создание документа-основания невозможно!'")
						   + Символы.ПС
						   + НСтр("ru = 'Сначала зарегистрируйте все неопознанные штрихкоды номенклатуры.'");
			ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура СоздатьДокументОснованиеНаКлиентеПослеЗаписи(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.ОК Тогда
		...
		ДокументыВыбора = РаботаСДокументамиИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДокументовОснованияДляАктаПриемаПередачиИСЦЭДМ(Объект.Направление, Объект.ВидОперации, "Создать");
...
Функция ПроверитьЗаполненностьНоменклатурыТоварахВоВходящемАПП() 
	...
	Для Каждого Строка Из Объект.Итоги Цикл
		//Проверяем, зарегистрирован ли ШК в регистре на случай, если между заполнением Акта и созданием основания
		//данные регистра Штрихкоды были изменены. В этом случае может возникнуть ситуация, когда в самом Акте
		//номенклатура не заполнена (т.к. в момент получения ШК еще не был зарегистрирован),
		//но в момент создания основания уже можно определить товар.
		//Но если при проверке в ТЧ документа находятся штрихкоды, по которым так и не удается
		//определить товар, в вводе нового документа отказываем, чтобы не создавать пустые строки товаров.
		EAN = Строка.EAN;
		Если Не ЗначениеЗаполнено(EAN)
			И ЗначениеЗаполнено(Строка.GTIN) Тогда
			EAN = РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.EANПоGTIN(Строка.GTIN);
		КонецЕсли;
		ДанныеНоменклатуры = ПолучитьНоменклатуруПоШтрихкоду(EAN, СообщенияОбОшибках);
...
Процедура УправлениеФормой()
	...
	ДокументМожноОтразитьВБазе		 = РаботаСДокументамиИСМПТКПереопределяемый.ПроверитьВозможностьОтраженияДокументаВУчете(ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаАктПриемаПередачиИСЦЭДМ());
	ЕдиныйДистрибьютор   = Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперацииИСЦЭДМ.Реализация");
	...
	ЭтоКорректировка = Объект.ТипАкта = ПредопределенноеЗначение("Перечисление.ВидыДокументаИСМПТК.Исправленный");	
	//////////////////////////////	
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ВидОперации", "Доступность", Исходящий);
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Дата", "Доступность", Исходящий);
	//////////////////////////////	
	...
	Иначе
		ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДеревоМаркированнойПродукцииЗагрузитьКодыМаркировкиИзМобильногоПриложения", "Видимость", Ложь);
	КонецЕсли;
	//////////////////////////////	
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументОснованиеСоздать",  
																	   "Видимость", Исходящий ИЛИ (Входящий И Объект.Итоги.Количество()>0));
	...
	КонецЕсли;
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДеревоМаркированнойПродукцииГруппаКоличествоОснования", "Видимость", ВидимостьКоличествоОснования);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаОбщаяКомандаПерезаполнитьПоПервичномуДокументу", 
																"Видимость", Исходящий и НеОтправлен И ЗначениеЗаполнено(Объект.ДокументОснование));
	//////////////////////////////	
	
	//////// Этапы оформления документов ////////
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЭтапОформления", "Видимость", ИспользуютсяЭтапы И Исходящий);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЭтапОформления", "Доступность", 
																Не Объект.ЭтапОформления = ПредопределенноеЗначение("Перечисление.ЭтапыОформленияДокументовИСЦЭДМ.Отправлен"));
	//////////////////////////////	
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТаблицаИтоговЗаполнитьЦеныПоОснованию", "Видимость", Исходящий);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТаблицаИтоговЗаполнитьЦеныПоОснованию", 
																"Доступность", Исходящий И НеОтправлен И ЗначениеЗаполнено(Объект.ДокументОснование) И Объект.Итоги.Количество() > 0);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаМарки",  "Видимость", Исходящий И Не ЭтоКорректировка);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппаТовары", "Видимость", Исходящий ИЛИ (Входящий И ЗначениеЗаполнено(Объект.Итоги)));
	...
	//////// Складской учет ////////
	//Типовой склад физического хранения товаров
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СкладОтправитель", 
																"Видимость", ВедетсяУчетПоСкладам И Исходящий);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "СкладПолучатель",  
	...
	//////// Связь документов в схеме корректировки ////////
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДеревоМаркированнойПродукцииПолучитьДополнительныеСведенияПоКодамВИСЦЭДМ", 
																	   "Доступность", УПользователяЕстьНастройкаОбмена);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТаблицаИтоговЗапроситьСоставМарокДокумента", "Видимость",   Входящий);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТаблицаИтоговЗапроситьСоставМарокДокумента", "Доступность", УПользователяЕстьНастройкаОбмена);
	//////////////////////////////	
		
	//////// Запросы к ЦЭДМ по контрагенту ////////
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПроверитьРегистрациюКонтрагентаВЦЭДМ", "Видимость", Исходящий);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПроверитьРегистрациюКонтрагентаВЦЭДМ", "Доступность", 
																ЗначениеЗаполнено(Объект.Организация) И ЗначениеЗаполнено(Объект.ПолучательИдентификационныйНомер));
	
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПодобратьСкладКонтрагента", "Видимость", Исходящий);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПодобратьСкладКонтрагента", "Доступность", 
																НеОтправлен И ЗначениеЗаполнено(Объект.Организация) И ЗначениеЗаполнено(Объект.ПолучательИдентификационныйНомер));
	//////////////////////////////
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ВидОперации) Тогда
			Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперацииИСЦЭДМ.Реализация");
		КонецЕсли;
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперацииИСЦЭДМ.Реализация") Тогда
		ЗаполнитьСведенияЕД();
	КонецЕсли;
...
Функция ПроверитьСвязанныйАкт(ВыбранноеЗначение)
	...
	Возврат РаботаСДокументамиИСМПТК.ПроверитьНаличиеСвязанного_Общий(ВыбранноеЗначение, Истина, "", ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаАктПриемаПередачиИСЦЭДМ());
...
Функция ПолучитьДанныеДокументаОснованияНаСервере() Экспорт
	...
	Возврат РаботаСДокументамиИСМПТКПереопределяемый.ПолучитьДанныеДокументаОснованияДляПерезаполненияИСЦЭДМ_Общий(Объект.ДокументОснование, ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаАктПриемаПередачиИСЦЭДМ());
...
Процедура ИзменитьКоманднуюПанельДокумента()
	...
	Если Объект.Направление = Перечисления.НаправленияДокументовИСМПТК.Входящий Тогда
		...
		//Действия по АПИ
		ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументАктПриемаПередачиИСЦЭДМОтправить",   
																	"Видимость", Ложь);
		ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументАктПриемаПередачиИСЦЭДМПодтвердить", 
																	"Видимость", УПользователяЕстьНастройкаОбмена И МожноПодтвердить);
		ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументАктПриемаПередачиИСЦЭДМОтклонить",
																	"Видимость", УПользователяЕстьНастройкаОбмена И МожноОтклонить);
		ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументАктПриемаПередачиИСЦЭДМОбновить",
																	"Видимость", УПользователяЕстьНастройкаОбмена И МожноОбновить);
		ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументАктПриемаПередачиИСЦЭДМОтозвать",
																	"Видимость", Ложь);
		//Внутренние действия
...
Функция ПолучитьУВИО() Экспорт
	...
	|ГДЕ
	|	УведомлениеОВыводеИзОборотаИСЦЭДМ.ДокументОснование = &Ссылка";
	...
	Возврат ПредопределенноеЗначение("Документ.УведомлениеОВыводеИзОборотаИСЦЭДМ.ПустаяСсылка");
...
Процедура ВыполнитьПроверкуСтатусаИВладельцаПоДаннымЦЭДМ(СоответствиеСтатусаПроверки, КодИдентификации, СтатусСервер, ВладелецСервер)
	...
	Ошибочный = Ложь;
	МассивДопустимыхСтатусовКМ = ИнтеграцияИСМПТК.ДопустимыеСтатусыКМДляДокументаИСЦЭДМ("АктПриемаПередачи");
...
//При сканировании КМ в форме Сверки во входящем документе
//
&НаКлиенте
	...
	Если Не СтруктураКодовМаркировки = Неопределено Тогда
		...
		Если Не ВидПродукцииКода = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИСМПТК.ЛекарственныеПрепараты")
			...
			ТекстСообщения = РаботаСТекстамиИСМПТККлиентСервер.ТекстСообщенияКМПринадлежитНоменклатуреСОсобенностямиУчетаСПараметрами()
	        			   + " " + РаботаСТекстамиИСМПТККлиентСервер.ТекстСообщенияДокументНеПредназначенДляРаботыСЭтойТГ();
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НаименованиеПодсистемы%", РаботаСТекстамиИСМПТККлиентСервер.НаименованиеИСЦЭДМ());
			ТекстСообщения = ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, ВидПродукцииКода); 
			ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
			Возврат;
		КонецЕсли;
...
Процедура НайтиПервичныйДокументПоИдентификатору()
	...
	ИсходныйДокумент = РаботаСДокументамиИСМПТК.НайтиДокументПоИдентификаторуДляУстановкиСвязи(ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаАктПриемаПередачиИСЦЭДМ(), Объект.Идентификатор);
