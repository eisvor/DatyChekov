...
Функция ПолучитьДанныеДляЭД(СсылкаНаОбъект) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаОбъектОснование)
		И (ТипЗнч(СсылкаНаОбъектОснование) = Тип("ДокументСсылка.ВозвратТоваровПоставщику")
		ИЛИ ТипЗнч(СсылкаНаОбъектОснование) = Тип("ДокументСсылка.ПеремещениеТоваров")
		ИЛИ ТипЗнч(СсылкаНаОбъектОснование) = Тип("ДокументСсылка.РеализацияТоваров")) Тогда
		
		СтруктураДанных = Документы[СсылкаНаОбъектОснование.Метаданные().Имя].ПолучитьДанныеДляЭД(СсылкаНаОбъектОснование);
		Возврат СтруктураДанных;
	ИначеЕсли ЗначениеЗаполнено(СсылкаНаОбъектОснование)
		И ТипЗнч(СсылкаНаОбъектОснование) = Тип("ДокументСсылка.ОтчетКомитентуОПродажах") Тогда
		
		СтруктураДанных = Документы.ОтчетКомитентуОПродажах.ПолучитьДанныеДляЭДСФ(СсылкаНаОбъектОснование);
		Возврат СтруктураДанных;
	Иначе
...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	МассивОбъектовОснований = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	
	СтруктураТипов = ОбщегоНазначенияРТ.СоответствиеМассивовПоТипамОбъектов(МассивОбъектовОснований);
	
	СтруктураПервогоТипа = Неопределено;
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		СтруктураПервогоТипа = СтруктураОбъектов;
		Прервать;
	КонецЦикла;
	
	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(СтруктураПервогоТипа.Ключ);
	
	Возврат МенеджерОбъекта.ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, СтруктураПервогоТипа.Значение);
