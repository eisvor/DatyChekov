...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если НЕ Отказ Тогда
		...
		ОбменССайтомРТ.ЗаписатьСостояниеОплатыЗаказаПокупателя(Ссылка, Ссылка);
		ОбменССайтомРТ.ЗаписатьСостояниеДоставкиЗаказаПокупателя(Ссылка, Ссылка);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ЗначениеЗаполнено(Магазин) Тогда
		АссортиментСервер.ПроверитьАссортиментТаблицыТоваровДокументаПродажи(ЭтотОбъект,
			Отказ,
			Истина,
			,);
	КонецЕсли;
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Если НЕ Отказ Тогда
		ОбменССайтомРТ.ЗаписатьСостояниеОплатыЗаказаПокупателя(Ссылка, Ссылка, Ссылка);
		ОбменССайтомРТ.ЗаписатьСостояниеДоставкиЗаказаПокупателя(Ссылка, Ссылка, Истина);
	КонецЕсли;
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	ПравилоЦенообразования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Магазин, "ПравилоЦенообразования");
	Если ЗначениеЗаполнено(ПравилоЦенообразования) Тогда
		ЦенаВключаетНДС = ОбщегоНазначенияРТ.ЗначениеРеквизитаВПривилегированномРежиме(
			ПравилоЦенообразования, "ЦенаВключаетНДС");
	КонецЕсли;
