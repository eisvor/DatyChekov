...
Процедура ПродолжитьОбработатьДанныеПоКодуКлиент(СтруктураРезультат) Экспорт
	...
	Если ЭтотОбъект.ТолькоПросмотр Тогда
		СтруктураРезультат.Вставить("ТекстПредупреждения",
			Нстр("ru = 'Документ не доступен для редактирования.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
			Объект.Контрагент = ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("КонтрагентРозничныйПокупатель");
		КонецЕсли;
		
		УстановитьДоступностьЭлементовПоСтатусуСервер();
		
		Элементы.ОбновитьСостояния.Видимость = Ложь;
	КонецЕсли;
	...
	ИспользоватьПричиныОтменыЗаказовПокупателей =
		ПолучитьФункциональнуюОпцию("ИспользоватьПричиныОтменыЗаказовПокупателей");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "КопированиеСтрокВБуферОбмена" Или ИмяСобытия = "ВставкаСтрокИзБуфераОбмена" Тогда
		УстановитьДоступностьКомандБуфераОбмена();
	ИначеЕсли ИмяСобытия = "ЗаказПокупателя_Состояния" И Объект.Ссылка = Параметр Тогда
		ОбновитьСостояниеЗаказаСервер();
	КонецЕсли;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	// СтандартныеПодсистемы.КонтактнаяИнформация
	УправлениеКонтактнойИнформацией.ПриЧтенииНаСервере(ЭтотОбъект, КонтактноеЛицоКонтрагента, "СтраницаКонтактнаяИнформация");
	// Конец СтандартныеПодсистемы.КонтактнаяИнформация
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	УдалитьДобавленныеСтрокиКИ(ТекущийОбъект);
	
	ОбновитьКонтактнуюИнформациюКонтрагента(КонтактноеЛицоКонтрагента, Отказ);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	// СтандартныеПодсистемы.КонтактнаяИнформация
	УправлениеКонтактнойИнформацией.ОбработкаПроверкиЗаполненияНаСервере(ЭтаФорма, КонтактноеЛицоКонтрагента, Отказ);
	// Конец СтандартныеПодсистемы.КонтактнаяИнформация
КонецПроцедуры
...
Процедура ОповещениеРазбитьСтроку(Количество, ДополнительныеПараметры) Экспорт
	...
	Если НЕ НоваяСтрока = Неопределено Тогда
		...
		ОбработкаТабличнойЧастиТоварыКлиент.ВыделитьАгентскоеВознаграждение(Объект, ЭтотОбъект, СтруктураДействий, НоваяСтрока, КэшированныеЗначения);
...
Процедура НастроитьФормуПоДополнительнымПравам()
	...
	УправлениеПользователями.УстановитьДоступностьДляРеквизитовТабличнойЧасти(
		Элементы.СкидкиНазначитьРучнуюСкидку.Доступность,
		ПланыВидовХарактеристик.ПраваПользователей.ИзменятьРучнуюСкидку);
		
	УправлениеПользователями.УстановитьДоступностьДляРеквизитовТабличнойЧасти(
		Элементы.СкидкиОтменитьРучныеСкидки.Доступность,
		ПланыВидовХарактеристик.ПраваПользователей.ИзменятьРучнуюСкидку);
...
Процедура УстановитьДоступностьЭлементовНаСервере(ТолькоПросмотрЭлементов)
	...
	Если НЕ ТолькоПросмотрЭлементов Тогда
		...
		ДоступностьРезервирования =
			Объект.Проведен И Объект.Статус =
				Перечисления.СтатусыЗаказовПокупателей.Согласован И Объект.НаличиеНезарезервированныхСтрок;
		...
		УстановитьВидимостьДоступностьЭлементовКИ(ТолькоПросмотрЭлементов);
...
&НаСервере
Процедура УстановитьВидимостьДоступностьЭлементовКИ(ТолькоПросмотрЭлементов)
	
	Если НЕ ТолькоПросмотрЭлементов Тогда
			
		Если ТипЗнч(КонтактноеЛицоКонтрагента) = Тип("СправочникСсылка.Контрагенты") Тогда
			ЭтоРозничныйПокупатель = Ложь;
			ДоступностьКИ = УправлениеДоступомРТ.ЕстьПравоДобавленияИзмененияКонтрагента();
		Иначе
			ЭтоРозничныйПокупатель =
				Объект.Контрагент = ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("КонтрагентРозничныйПокупатель");
			ДоступностьКИ = УправлениеДоступомРТ.ЕстьПравоДобавленияИзмененияФизическогоЛица()
				И НЕ ЭтоРозничныйПокупатель;
		КонецЕсли;
...
Процедура УстановитьДоступностьЭлементовПоСтатусуСервер(РазрешитьРедактирование = Ложь)
	...
	МассивЭлементов.Добавить("Телефон");
	МассивЭлементов.Добавить("СтраницаКонтактнаяИнформация");
...
#Область ИзменениеКонтактнойИнформацииЗаказа
...
Процедура КонтрагентПриИзмененииНаСервере()
	...
	УдалитьДобавленныеСтрокиКИ();
	
	ВидКИТелефон = Справочники.ВидыКонтактнойИнформации.ТелефонФизическогоЛица;
	ВидКИАдрес = Справочники.ВидыКонтактнойИнформации.ФактАдресФизическогоЛица;
	
	Если ТипЗнч(КонтактноеЛицоКонтрагента) = Тип("СправочникСсылка.Контрагенты") Тогда
		ВидКИТелефон = Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента;
		ВидКИАдрес = Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента;
	КонецЕсли;
	
	СтрокиКИ = ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(
		Новый Структура("Вид", ВидКИТелефон));
	Если СтрокиКИ.Количество() >0 Тогда
		Объект.КонтрагентТелефон = СтрокиКИ[0].Представление;
		Объект.КонтрагентТелефонЗначение = СтрокиКИ[0].Значение;
	КонецЕсли;
	
	СтрокиКИ = ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(
		Новый Структура("Вид", ВидКИАдрес));
	Если СтрокиКИ.Количество() >0 Тогда
		Объект.АдресДоставки = СтрокиКИ[0].Представление;
		Объект.АдресДоставкиЗначение = СтрокиКИ[0].Значение;
	КонецЕсли;
	
	ДополнитьПоляКонтактнойИнформации();
	
	УстановитьВидимостьДоступностьЭлементовКИ(Объект.Статус = Перечисления.СтатусыЗаказовПокупателей.Закрыт);
	
КонецПроцедуры

&НаСервере
Процедура ДополнитьПоляКонтактнойИнформации()
	
	АдресДоставки = Объект.АдресДоставки;
	Телефон = Объект.КонтрагентТелефон;
	
	ПараметрыАдреса = Новый Структура("Индекс, Страна, ТипПомещения");
	СтраницаОсновная = Новый Структура();
	СтраницаОсновная.Вставить("ПараметрыАдреса", ПараметрыАдреса);
	
	ДобавленныеЭлементы = Новый СписокЗначений();
	ДобавленныеЭлементы.Добавить("АдресДоставки");
	ДобавленныеЭлементы.Добавить("Телефон");
	
	СтраницаОсновная.Вставить("ДобавленныеЭлементы", ДобавленныеЭлементы);
	СтраницаОсновная.Вставить("Владелец", КонтактноеЛицоКонтрагента);
	СтраницаОсновная.Вставить("МожноОтправлятьSMS",
		ЭтотОбъект.ПараметрыКонтактнойИнформации.СтраницаКонтактнаяИнформация.МожноОтправлятьSMS);
	
	ЭтотОбъект.ПараметрыКонтактнойИнформации.Вставить("СтраницаОсновная", СтраницаОсновная);
	...
	СтруктураТелефона = Новый Структура("comment", "");
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(Объект.КонтрагентТелефонЗначение) Тогда
		ДанныеАдреса = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(
				Объект.КонтрагентТелефонЗначение,
				Перечисления.ТипыКонтактнойИнформации.Адрес);
		
		ЗаполнитьЗначенияСвойств(СтруктураТелефона, ДанныеАдреса);
	КонецЕсли;
	
	СтрокаКИ = ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.Добавить();
	
	СтрокаКИ.Значение = Объект.КонтрагентТелефонЗначение;
	СтрокаКИ.Представление = Объект.КонтрагентТелефон;
	СтрокаКИ.Вид = ВидКИТелефон;
	СтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон;
	СтрокаКИ.ИмяРеквизита = "Телефон";
	СтрокаКИ.ИмяЭлементаДляРазмещения = "СтраницаОсновная";
	СтрокаКИ.Комментарий = СтруктураТелефона.comment;
		
	Элементы.ТелефонРасширеннаяПодсказка.Заголовок = СтруктураТелефона.comment;
	
	СтруктураАдреса = Новый Структура("comment", "");
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(Объект.АдресДоставкиЗначение) Тогда
		ДанныеАдреса = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(
				Объект.АдресДоставкиЗначение,
				Перечисления.ТипыКонтактнойИнформации.Адрес);
		
		ЗаполнитьЗначенияСвойств(СтруктураАдреса, ДанныеАдреса);
	КонецЕсли;
	
	СтрокаКИ = ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.Добавить();
	
	СтрокаКИ.Значение = Объект.АдресДоставкиЗначение;
	СтрокаКИ.Представление = Объект.АдресДоставки;
	СтрокаКИ.Вид = ВидКИАдрес;
	СтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес;
	СтрокаКИ.ИмяРеквизита = "АдресДоставки";
	СтрокаКИ.ИмяЭлементаДляРазмещения = "СтраницаОсновная";
	СтрокаКИ.Комментарий = СтруктураАдреса.comment;
		
	Элементы.АдресДоставкиРасширеннаяПодсказка.Заголовок = СтруктураАдреса.comment;
КонецПроцедуры

&НаСервере
Процедура ОбновитьКонтактноеЛицоКонтрагента()
	
	КонтактноеЛицоКонтрагента = Объект.Контрагент;
	Если ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ИнформацияОКонтрагенте = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Контрагент, "ЮрФизЛицо, ФизЛицо");
		Если ИнформацияОКонтрагенте.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо
			И ЗначениеЗаполнено(ИнформацияОКонтрагенте.ФизЛицо) Тогда
			КонтактноеЛицоКонтрагента = ИнформацияОКонтрагенте.ФизЛицо;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура УдалитьДобавленныеСтрокиКИ(ТекущийОбъект = Неопределено)
	
	СтрокиКИ = ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(
		Новый Структура("ИмяРеквизита", "Телефон"));
	Если СтрокиКИ.Количество() >0 Тогда
		Если Не ТекущийОбъект = Неопределено Тогда
			ТекущийОбъект.КонтрагентТелефон = СтрокиКИ[0].Представление;
			ТекущийОбъект.КонтрагентТелефонЗначение = СтрокиКИ[0].Значение;
		КонецЕсли;
		ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.Удалить(СтрокиКИ[0]);
	КонецЕсли;
	
	СтрокиКИ = ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.НайтиСтроки(
		Новый Структура("ИмяРеквизита", "АдресДоставки"));
	Если СтрокиКИ.Количество() >0 Тогда
		Если Не ТекущийОбъект = Неопределено Тогда
			ТекущийОбъект.АдресДоставки = СтрокиКИ[0].Представление;
			ТекущийОбъект.АдресДоставкиЗначение = СтрокиКИ[0].Значение;
		КонецЕсли;
		ЭтотОбъект.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов.Удалить(СтрокиКИ[0]);
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Процедура ОбновитьКонтактнуюИнформациюКонтрагента(КонтактноеЛицоКонтрагента, Отказ)
	...
	Если ЗначениеЗаполнено(КонтактноеЛицоКонтрагента) Тогда
		...
		Иначе
			ДоступностьКИ = УправлениеДоступомРТ.ЕстьПравоДобавленияИзмененияФизическогоЛица()
				И НЕ Объект.Контрагент = ЗначениеНастроекПовтИсп.ПолучитьЗначениеКонстанты("КонтрагентРозничныйПокупатель");
		КонецЕсли;
			
		Если НЕ ДоступностьКИ Тогда
			Возврат;
		КонецЕсли;
			
		Попытка
			КонтактноеЛицоКонтрагентаОбъект = КонтактноеЛицоКонтрагента.ПолучитьОбъект();
		Исключение
			Возврат;
		КонецПопытки;
		
		// СтандартныеПодсистемы.КонтактнаяИнформация
		УправлениеКонтактнойИнформацией.ПередЗаписьюНаСервере(ЭтотОбъект, КонтактноеЛицоКонтрагентаОбъект);
		// Конец СтандартныеПодсистемы.КонтактнаяИнформация
		
		Попытка
			КонтактноеЛицоКонтрагентаОбъект.Записать();
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление контактной информации.'"),
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбщегоНазначения.СообщитьПользователю(
				КраткоеПредставлениеОшибки(ИнформацияОбОшибке()), КонтактноеЛицоКонтрагента,,, Отказ);
		КонецПопытки;
	КонецЕсли;
КонецПроцедуры
...
#Область РаботаССостояниямиЗаказа

&НаКлиенте
Процедура СостояниеОплатыЗаказаНажатие(Элемент, СтандартнаяОбработка)
	ОткрытьФормуСостояния("СостояниеОплатыЗаказаПокупателя", СтандартнаяОбработка);
КонецПроцедуры

&НаКлиенте
Процедура СостояниеДоставкиЗаказаНажатие(Элемент, СтандартнаяОбработка)
	ОткрытьФормуСостояния("СостояниеДоставкиЗаказаПокупателя", СтандартнаяОбработка);
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьФормуСостояния(ИмяРегистра, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Отбор = Новый Структура;
	Отбор.Вставить("ЗаказПокупателя", Объект.Ссылка);
	ОткрытьФорму("РегистрСведений." + ИмяРегистра + ".ФормаСписка",Новый Структура("Отбор", Отбор), ЭтотОбъект);
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьСостояниеЗаказаСервер()
	
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Элементы.ОбновитьСостояния.Видимость = Истина;
		ПродажиСервер.ОбновитьСостояниеЗаказа(Объект.Ссылка, СостояниеЗаказа, СостояниеОплатыЗаказа, СостояниеДоставкиЗаказа);
	Иначе
		Элементы.ОбновитьСостояния.Видимость = Ложь;
	КонецЕсли;
