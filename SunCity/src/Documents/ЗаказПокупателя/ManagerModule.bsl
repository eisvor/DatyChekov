...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	КомандаОтчет = Отчеты.ОстаткиПоЗаказу.ДобавитьКомандуОтчета(КомандыОтчетов);
...
// Функция формирует массив данных для заполнения документов оплаты.
// с учетом распределения номенклатуры
//
// Возвращаемое значение:
//  Массив - массив структур для заполнения документов регистрации безналичной оплаты.
//
Функция ПодготовитьСтруктурыДляЗаполненияРегистрацииБезналичнойОплаты(ЗаказПокупателя) Экспорт
	
	Результат = Новый Массив;
	
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	ПодготовитьВременнуюТаблицуРаспределенияНоменклатуры(ЗаказПокупателя, МенеджерВременныхТаблиц);
	
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТРаспределениеНоменклатуры.Организация КАК Организация
	|ИЗ
	|	ВТРаспределениеНоменклатуры КАК ВТРаспределениеНоменклатуры";
	...
	Пока ВыборкаОрганизаций.Следующий() Цикл
		...
		СтруктураЗаполнения = ПолучитьСтруктуруЗаполненияРегистрацииБезналичнойОплаты();
		
		СтруктураЗаполнения.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента;
		СтруктураЗаполнения.ДокументОснование     = ЗаказПокупателя;
		СтруктураЗаполнения.Организация           = ВыборкаОрганизаций.Организация;
		
		ЗаполнитьЗначенияСвойств(СтруктураЗаполнения, ЗаказПокупателя, "Магазин, Контрагент");
...
Функция ПечатьСчетаНаОплату(МассивОбъектов, ОбъектыПечати)
	...
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Документы.ЗаказПокупателя.ПодготовитьВременнуюТаблицуРаспределенияНоменклатуры(МассивОбъектов, МенеджерВременныхТаблиц);
	...
	|	Документ.Дата КАК Дата,
	|	Документ.Магазин КАК Магазин,
	|	ЕСТЬNULL(ВТРаспределениеНоменклатуры.Организация, Документ.Организация) КАК Организация,
	|	Документ.Организация КАК ОрганизацияЗаказа,
	|	Документ.СуммаДокумента КАК СуммаДокумента,
	|	ПРЕДСТАВЛЕНИЕ(Документ.Магазин) КАК МагазинПредставление,
	|	Документ.Ответственный.ФизическоеЛицо КАК Ответственный,
	...
	|ИЗ
	|	Документ.ЗаказПокупателя КАК Документ
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРаспределениеНоменклатуры КАК ВТРаспределениеНоменклатуры
	|		ПО Документ.Ссылка = ВТРаспределениеНоменклатуры.ЗаказПокупателя
	|ГДЕ
	|	Документ.Ссылка В(&МассивОбъектов)
	|	И Документ.Проведен
	|;
	...
	|	ВТЗаказыПокупателей.Номер КАК Номер,
	|	ВТЗаказыПокупателей.Дата КАК Дата,
	|	ВТЗаказыПокупателей.Магазин КАК Магазин,
	|	ВТЗаказыПокупателей.ОрганизацияЗаказа Как ОрганизацияЗаказа,
	|	ВТЗаказыПокупателей.Организация КАК Поставщик,
	|	ВТЗаказыПокупателей.СуммаДокумента КАК СуммаДокумента,
	|	ВТЗаказыПокупателей.МагазинПредставление КАК МагазинПредставление,
	|	ВТЗаказыПокупателей.Ответственный КАК Ответственный,
	|	ВТЗаказыПокупателей.УчитыватьНДС КАК УчитыватьНДС,
	|	ВТЗаказыПокупателей.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	ВТЗаказыПокупателей.Покупатель КАК Покупатель,
	|	ВТЗаказыПокупателей.БанковскийСчет КАК БанковскийСчет,
	|	Организации.ИНН КАК ИНН,
	|	Организации.КПП КАК КПП,
	|	ВЫБОР
	|		КОГДА ВТЗаказыПокупателей.Организация.ЮрФизЛицо = ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ЮрЛицо)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОрганизацияЮридическоеЛицо,
	|	ПРЕДСТАВЛЕНИЕ(ВТЗаказыПокупателей.Организация) КАК ПредставлениеПоставщикаДляПлатПоручения,
	|	ПРЕДСТАВЛЕНИЕ(ВТЗаказыПокупателей.Организация) КАК ПредставлениеПоставщика
	|ИЗ
	|	ВТЗаказыПокупателей КАК ВТЗаказыПокупателей
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
	|		ПО ВТЗаказыПокупателей.Организация = Организации.Ссылка
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	Дата
	|;
	...
	|	ЕСТЬNULL(ВТРаспределениеНоменклатуры.Организация, ТаблицаТовары.Ссылка.Организация) КАК Поставщик
	|ИЗ
	|	Документ.ЗаказПокупателя.Товары КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРаспределениеНоменклатуры КАК ВТРаспределениеНоменклатуры
	|		ПО ТаблицаТовары.Ссылка = ВТРаспределениеНоменклатуры.ЗаказПокупателя
	|			И ТаблицаТовары.Номенклатура = ВТРаспределениеНоменклатуры.Номенклатура
	|ГДЕ
	|	ТаблицаТовары.Ссылка В(&МассивОбъектов)
	|	И ТаблицаТовары.Ссылка.Проведен
	|	И НЕ ТаблицаТовары.Отменено
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		БанковскийСчет = ВыборкаПоДокументам.БанковскийСчет;
		Если Не ВыборкаПоДокументам.Поставщик = ВыборкаПоДокументам.ОрганизацияЗаказа Тогда
			БанковскийСчет = Справочники.БанковскиеСчета.ПолучитьБанковскийСчетПоУмолчанию(ВыборкаПоДокументам.Поставщик);
		КонецЕсли;
		...
		Если Не ЗначениеЗаполнено(ОбластьЗаголовок.Параметры.Банк) Тогда 
			ОбластьЗаголовок.Параметры.Банк = Строка(БанковскийСчет);
		КонецЕсли;
		...
		// Строки ТЧ
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			Если НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамТЧ.Номенклатура) 
				ИЛИ НЕ ВыборкаПоСтрокамТЧ.Поставщик = ВыборкаПоДокументам.Поставщик Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ПодготовитьВременнуюТаблицуРаспределенияНоменклатуры(ЗаказПокупателя, МенеджерВременныхТаблиц) Экспорт
	...
	|	ЗаказПокупателяТовары.Номенклатура.ТоварнаяГруппа КАК ТоварнаяГруппа,
	|	ЗаказПокупателяТовары.Ссылка.Магазин КАК Магазин,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТЗаказПокупателя.Ссылка КАК ЗаказПокупателя,
	|	ВТЗаказПокупателя.Номенклатура КАК Номенклатура,
	|	ВТЗаказПокупателя.ТоварнаяГруппа КАК ТоварнаяГруппа,
	|	ВТЗаказПокупателя.Магазин КАК Магазин,
	|	ВЫБОР
	|		КОГДА РаспределениеНоменклатуры.Организация ЕСТЬ NULL
	|			ТОГДА ВТЗаказПокупателя.Организация
	|		ИНАЧЕ РаспределениеНоменклатуры.Организация
	|	КОНЕЦ КАК Организация
	|ПОМЕСТИТЬ ВТРаспределениеНоменклатуры
	|ИЗ
	...
	|		ПО ВТЗаказПокупателя.Магазин = РаспределениеНоменклатуры.Магазин
	|			И ВТЗаказПокупателя.ТоварнаяГруппа = РаспределениеНоменклатуры.ТоварнаяГруппа
...
Функция ПолучитьСтруктуруЗаполненияРегистрацииБезналичнойОплаты()
	
	Возврат Новый Структура("Организация, Магазин, Контрагент, ХозяйственнаяОперация, ДокументОснование");
