...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(МассивДанныхДляЗаполнения) = Тип("Массив") И МассивДанныхДляЗаполнения.Количество() > 0 Тогда
		...
		Для Каждого СтруктураДокумента Из МассивДанныхДляЗаполнения Цикл
			...
			СтруктураПереопределяемыхРеквизитов = РаботаСДокументамиИСМПТКПереопределяемый.ПолучитьЗначенияПоУмолчаниюДляСозданияУведомленияОВыводеИСМПТ(МассивДокументов);
			...
			НовыйДокументВИО.Статус    = Перечисления.СтатусыДокументовИСМПТК.НеОпределен;
			
			Если СтруктураДокумента.Свойство("Марки")
				И Не СтруктураДокумента.Марки = Неопределено 
				И РаботаСДокументамиИСМПТКПереопределяемый.ВДокументеНужноЗаполнятьМаркиИзОснованияИСМПТ(ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВыводеИзОборотаИСМПТК(), ДокументОснованиеДляЗаполнения) Тогда
				
				//Если заполняем Марки, занчит это ввод из полученного АПП. Нужно изменить данные поставщика
				НовыйДокументВИО.Поставщик = СтруктураДокумента.Реквизиты[0].Получатель;
				НовыйДокументВИО.ПоставщикНаименование = СтруктураДокумента.Реквизиты[0].ПолучательНаименование;
				НовыйДокументВИО.ПоставщикИдентификационныйНомер = СтруктураДокумента.Реквизиты[0].ПолучательИдентификационныйНомер;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	РаботаСДокументамиИСМПТКПереопределяемый.ДокументВИО_ОбработкаПроверкиЗаполнения(ЭтотОбъект, Отказ, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов);
