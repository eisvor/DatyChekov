...
//
// Параметры:
//  Объект - ДокументОбъект.ЗаменаДисконтныхКарт - документ для замены владельца.
...
Процедура ЗаменитьВладельца(Объект, Замещать) Экспорт
	...
	Попытка
		...
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить("Справочник.ИнформационныеКарты");
		...
		Если ТаблицаЗапроса.Количество() = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Обработка заполнения не потребовалась'"), Объект);
			Возврат;
		КонецЕсли;
...
//
// Параметры:
//  Объект - ДокументОбъект.ЗаменаДисконтныхКарт - документ для переноса данных.
//  Замещать - Булево - Если Истина, то владелец перезаписывается.
//
