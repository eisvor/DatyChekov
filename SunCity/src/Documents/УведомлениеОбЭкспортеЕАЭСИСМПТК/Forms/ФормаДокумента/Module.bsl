...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если ВременнаяСхемаПризнания Тогда
			...
			Объект.СтранаПолучатель = РаботаСДокументамиИСМПТКПереопределяемый.ЗаполнитьСтрануПоУмолчаниюПризнаниеКодовЕАЭС();
		...
		Иначе
			...
			Элементы.СтранаПолучатель.СписокВыбора.ЗагрузитьЗначения(РаботаСДокументамиИСМПТКПереопределяемый.ПолучитьСписокСтранЕАЭС());
		КонецЕсли;
		///////////
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ИнтеграцияИСМПТККлиент.ОповеститьФормы(РаботаСДокументамиИСМПТККлиентСервер.ИмяСобытияЗаписьУведомлениеОбЭкспортеЕАЭС());
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если Не КонтрагентНерезидент(ВыбранноеЗначение) Тогда
		ТекстСообщения = НСтр("ru = 'В выбранном документе %ДокументОснование% указан контрагент, который не соответствует требованиям заполняемого документа ИС МПТ!'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДокументОснование%", ВыбранноеЗначение);
		Сообщить(ТекстСообщения);
		Возврат;
	КонецЕсли;
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаРедактироватьНедоступныеРеквизиты", "Доступность", НеОтправлен);
...
Процедура СоздатьДокументОснованиеНаКлиентеПослеЗаписи(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.ОК Тогда
		...
		ДокументыВыбора = РаботаСДокументамиИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДокументовОснованияУведомленияОбЭкспортеИСМПТ(Направление);
...
Процедура ВыбратьДокументОснование()
	...
	ДокументыВыбора = РаботаСДокументамиИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДокументовОснованияУведомленияОбЭкспортеИСМПТ();
...
Процедура СформироватьПредставлениеСостояния()
	...
	Иначе
		...
		Иначе
			ПредставлениеДокументаОснования = НСтр("ru = 'Документ %Состояние%'");
...
Процедура УправлениеФормой()
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДеревоМаркированнойПродукцииПолучитьДанныеПоКоличеству",
																	   "Видимость", НеОтправлен); 
	//Отражение в учете
	ДокументМожноОтразитьВБазе = РаботаСДокументамиИСМПТКПереопределяемый.ПроверитьВозможностьОтраженияДокументаВУчете(ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОбЭкспортеЕАЭСИСМПТК());
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ДокументыСоответствияДобавитьСертификат", 
																	   "Доступность", Не Объект.Товары.Количество() = 0);
...
Функция ПроверитьСвязанноеУведомление(ВыбранноеЗначение)
	...
	Возврат РаботаСДокументамиИСМПТК.ПроверитьНаличиеСвязанного_Общий(ВыбранноеЗначение, Истина, "", ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОбЭкспортеЕАЭСИСМПТК());
...
Функция ПолучитьДанныеДокументаОснованияНаСервере() Экспорт
	...
	Возврат РаботаСДокументамиИСМПТКПереопределяемый.ПолучитьДанныеДокументаОснованияДляПерезаполненияИСМПТ_Общий(Объект.ДокументОснование, ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОбЭкспортеЕАЭСИСМПТК());
...
Процедура ИзменитьКоманднуюПанельДокумента()
	...
	//Действия по АПИ
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОбЭкспортеЕАЭСИСМПТКОтправить", 
																"Видимость", МожноОтправить);
	//Внутренние действия
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОбЭкспортеЕАЭСИСМПТКПроверить", 
																"Видимость", МожноОтправить);
...
Функция СформироватьДеревоКодовМаркировки() Экспорт
	...
	Для Каждого ГруппаНом Из ТаблицаТоваров Цикл
		ПустаяСтруктураКМ.Количество = ГруппаНом.КоличествоИсточник;
...
&НаСервере
Функция КонтрагентНерезидент(ВыбранныйДокумент) 

	Если Не ЗначениеЗаполнено(ВыбранныйДокумент) Тогда 
		Возврат Ложь;
	Иначе
		Возврат ИнтеграцияИСМПТКПереопределяемый.КонтрагентНерезидент(ВыбранныйДокумент);
	КонецЕсли;
...
Процедура ЗаполнитьНалоговыйНомерКонтрагента() 
	...
	ОбщегоНазначенияИСМПТКПереопределяемый.ПолучитьДанныеКонтрагентаИзЕАЭС(ПерезаполнятьИИНБИНПолучателя, НалоговыйНомер, Объект.Получатель);
	Если ПерезаполнятьИИНБИНПолучателя Тогда
		Объект.ПолучательИдентификационныйНомер = НалоговыйНомер;
	КонецЕсли;
...
Процедура ДобавитьКодМаркировкиВДерево(Номенклатура, СтруктураКодовМаркировки) Экспорт
	...
	Иначе
		...
		Иначе 
			НоваяСтрокаНоменклатура.Количество = 0;
		КонецЕсли;
