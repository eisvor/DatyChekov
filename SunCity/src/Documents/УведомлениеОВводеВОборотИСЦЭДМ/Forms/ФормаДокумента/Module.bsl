...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Объект.ПричинаВводаВОборот = ПредопределенноеЗначение("Перечисление.ПричиныПовторногоВводаКМВОборотИСЦЭДМ.Излишки");
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	//Сначала проверяем, если ли в выбранном основании товары, по которым необходимо заполнять Акт (фарма)
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = РаботаСДокументамиИСМПТККлиентСервер.ИмяСобытияЗаписьУведомлениеОВводеВОборотИСЦЭДМ() Тогда		
...
Функция ПолучитьПредставлениеУведомления() 
	...
	Возврат Метаданные.Документы.УведомлениеОВводеВОборотИСЦЭДМ.Представление();
...
Процедура ВыбратьДокументОснование()
	...
	ДокументыВыбора = РаботаСДокументамиИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДокументовОснованияДляВводаВОборотИСЦЭДМ(, "Выбрать");
...
Процедура СоздатьДокументОснованиеНаКлиентеПослеЗаписи(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.ОК Тогда
		...
		ДокументыВыбора = РаботаСДокументамиИСМПТККлиентСерверПереопределяемый.ПолучитьСписокДокументовОснованияДляВводаВОборотИСЦЭДМ(, "Создать");
...
Процедура ИзменитьКоманднуюПанельДокумента()
	...
	//Действия по АПИ
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВводеВОборотИСЦЭДМОтправить", 
																"Видимость", УПользователяЕстьНастройкаОбмена И МожноОтправить И ТекущийЭтапДопускаетОтправку);
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДокументУведомлениеОВводеВОборотИСЦЭДМОбновить", 
	...
	Если Не ВыбранныеФайлы = Неопределено Тогда
		...
		Для Каждого СтрокаТЧ Из Объект.Марки Цикл
			КодДляЗаписи = ?(ЗначениеЗаполнено(СтрокаТЧ.КодМаркировки), 
							РазборИОбработкаКодовМаркировкиИСМПТКСлужебныйКлиентСервер.Base64ВШтрихкод(СтрокаТЧ.КодМаркировки), 
							СтрокаТЧ.КодИдентификации);
...
Процедура УправлениеФормой()
	...
	//Отражение в учете
	ДокументМожноОтразитьВБазе = РаботаСДокументамиИСМПТКПереопределяемый.ПроверитьВозможностьОтраженияДокументаВУчете(ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВводеВОборотИСЦЭДМ());
	...
	ОбщегоНазначенияИСМПТККлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПодобратьСкладЦЭДМ", "Доступность", МожноРедактироватьДанные);
...
Функция ПроверитьСвязанноеУведомление(ВыбранноеЗначение)
	...
	Возврат РаботаСДокументамиИСМПТК.ПроверитьНаличиеСвязанного_Общий(ВыбранноеЗначение, Истина, "", ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВводеВОборотИСЦЭДМ());
...
Функция ПолучитьДанныеДокументаОснованияНаСервере() Экспорт
	...
	Возврат РаботаСДокументамиИСМПТКПереопределяемый.ПолучитьДанныеДокументаОснованияДляПерезаполненияИСЦЭДМ_Общий(Объект.ДокументОснование, ОбщегоНазначенияИСМПТКВызовСервера.ИмяДокументаУведомлениеОВводеВОборотИСЦЭДМ());
...
Процедура ВыполнитьПроверкуСтатусаИВладельцаПоДаннымЦЭДМ(СоответствиеСтатусаПроверки, КодИдентификации, СтатусСервер, ВладелецСервер)
	...
	Ошибочный = Ложь;
	МассивДопустимыхСтатусовКМ = ИнтеграцияИСМПТК.ДопустимыеСтатусыКМДляДокументаИСЦЭДМ("УведОВводеВОборот");
...
&НаКлиенте
Процедура ПодобратьСкладЦЭДМ(Команда)
	
	Отказ = Ложь;
	Если Не ЗначениеЗаполнено(Объект.Склад) Тогда
		ТекстСообщения = НСтр("ru = 'Перед выполнением необходимо указать склад отправителя!'");
		ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Объект.Поставщик) Тогда
		ТекстСообщения = НСтр("ru = 'Перед выполнением необходимо указать организацию отправителя!'");
		ОбщегоНазначенияИСМПТККлиентСерверПереопределяемый.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
	КонецЕсли;
