...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ТипДокументаИСЦЭДМ = Документы.УведомлениеОВводеВОборотИСЦЭДМ.ПустаяСсылка();
	...
	Если ТипЗнч(МассивДанныхДляЗаполнения) = Тип("Массив") И МассивДанныхДляЗаполнения.Количество() > 0 Тогда
		...
		Для Каждого СтруктураДокумента Из МассивДанныхДляЗаполнения Цикл
			...
			Если МассивДанныхДляЗаполнения.Найти(СтруктураДокумента) > 0 Тогда
				НовыйДокументВВО = Документы.УведомлениеОВводеВОборотИСЦЭДМ.СоздатьДокумент();
			Иначе
			...
			СтруктураПереопределяемыхРеквизитов = РаботаСДокументамиИСМПТКПереопределяемый.ПолучитьЗначенияПоУмолчаниюДляСозданияУведомленияОВводеИСЦЭДМ(ПараметрыЗаполнения.МассивДокументов);
			...
			Если ОбщегоНазначенияИСМПТКВызовСервера.ПроверитьЗначениеКонстанты("ВестиУчетПоСкладамИСЦЭДМ") Тогда
				СкладЦЭДМПоСоответствию = РаботаСДокументамиИСМПТКПереопределяемый.ПолучитьДанныеСкладаЦЭДМПоСкладуУчета(НовыйДокументВВО.Склад, НовыйДокументВВО.Организация);
				Если Не СкладЦЭДМПоСоответствию = Неопределено Тогда
					НовыйДокументВВО.СкладЦЭДМ = СкладЦЭДМПоСоответствию.СкладЦЭДМ;
					НовыйДокументВВО.ПоставщикСкладЦЭДМИдентификационныйНомер = СкладЦЭДМПоСоответствию.ИдентификаторСклада;
				КонецЕсли;
			КонецЕсли;
			
			НовыйДокументВВО.Статус = Перечисления.СтатусыДокументовИСЦЭДМ.НеОпределен; 
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	РаботаСДокументамиИСМПТКПереопределяемый.ДокументВВО_ОбработкаПроверкиЗаполненияИСЦЭДМ(ЭтотОбъект, Отказ, ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов);
