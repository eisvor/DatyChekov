...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.АктОРасхожденияхПриПриемкеТоваров") Тогда
		...
		Если ЗначениеЗаполнено(Магазин)
			...
			ТекстСообщения = НСтр("ru='В магазине ""%1%"" не используется ордерная схема для отражения излишков и недостач. Для отражения излишков нужно вводить документ ""Оприходование товаров"".'");
...
Процедура ЗаполнитьТоварыПоАктуОРасхождениях()
	...
	|		АктОРасхожденияхПриПриемкеТоваровТовары.Количество - АктОРасхожденияхПриПриемкеТоваровТовары.КоличествоВДокументе КАК Расхождения
	|	ИЗ
	...
	|		И АктОРасхожденияхПриПриемкеТоваровТовары.Количество > АктОРасхожденияхПриПриемкеТоваровТовары.КоличествоВДокументе
	...
	|	ГДЕ
	|		ОрдерНаОтражениеИзлишковТоваровТовары.Ссылка.ДокументОснование = &АктОРасхождениях
	|		И ОрдерНаОтражениеИзлишковТоваровТовары.Ссылка.Проведен) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Номенклатура,
	|	ВложенныйЗапрос.Характеристика
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаСерийАкта.Номенклатура КАК Номенклатура,
	|	ТаблицаСерийАкта.Характеристика КАК Характеристика,
	|	ТаблицаСерийАкта.Серия КАК Серия,
	|	ТаблицаСерийАкта.Количество - ЕСТЬNULL(ТаблицаСерийПеремещения.Количество, 0) КАК Расхождения
	|ПОМЕСТИТЬ ВтТаблицаСерийРасхождения
	|ИЗ
	|	ВтТаблицаСерийАкта КАК ТаблицаСерийАкта
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтТаблицаСерийПеремещения КАК ТаблицаСерийПеремещения
	|		ПО ТаблицаСерийАкта.Серия = ТаблицаСерийПеремещения.Серия
	|			И ТаблицаСерийАкта.Номенклатура = ТаблицаСерийПеремещения.Номенклатура
	|			И ТаблицаСерийАкта.Характеристика = ТаблицаСерийПеремещения.Характеристика
	|ГДЕ
	|	ТаблицаСерийАкта.Количество - ЕСТЬNULL(ТаблицаСерийПеремещения.Количество, 0) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
