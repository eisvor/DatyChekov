...
Процедура ЗаполнитьРеквизитыПогашенияПодарочныхСертификатов()
	...
	РассчитатьСуммуПогашенияСертификатовВсего();
...
Процедура РассчитатьСуммуНоминала(ТекущаяСтрока)
	...
	ТекущаяСтрока.СуммаПогашенияСертификата = ТекущаяСтрока.Количество * ТекущаяСтрока.Номинал;
	
	РассчитатьСуммуПогашенияСертификатовВсего();

КонецПроцедуры

// Рассчитать сумму погашения подарочных сертификатов всего.
//
// Параметры:
//  СуммаПогашенияСертификатовВсего - Число;
//
&НаСервере
Процедура РассчитатьСуммуПогашенияСертификатовВсего()

	СуммаПогашенияСертификатовВсего =  Объект.ПогашениеПодарочныхСертификатов.Итог("СуммаПогашенияСертификата");
...
Процедура УстановитьДоступностьЦенаВключаетНДС(ПриИзменении = Ложь)
	...
	ИначеЕсли ПриИзменении И Объект.УчитыватьНДС Тогда
		Объект.ЦенаВключаетНДС = ОбщегоНазначенияРТ.ЗначениеРеквизитаВПривилегированномРежиме(Объект.Магазин.ПравилоЦенообразования, "ЦенаВключаетНДС");
	КонецЕсли;
...
Функция ПродолжитьОбработатьДанныеПоКодуКлиент(СтруктураРезультат)
	...
	Иначе // Номенклатура.
		...
		ИначеЕсли ИмяТаблицыВыборки = "ПогашениеПодарочныхСертификатов" Тогда
			ТипНоменклатуры = ОбщегоНазначенияРТВызовСервера.ЗначениеРеквизитаОбъекта(СтрокаРезультата.Номенклатура, "ТипНоменклатуры");
			Если ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.ПодарочныйСертификат") Тогда
				ИдентификаторСтроки = ДобавитьНайденныеСерийныеНомераПогашение(СтрокаРезультата);
			Иначе
