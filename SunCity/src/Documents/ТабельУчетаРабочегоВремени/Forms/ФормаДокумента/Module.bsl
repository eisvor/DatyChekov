...
Процедура ДанныеОВремениПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Копирование Тогда
		...
		НезаполняемыеСвойства = "НомерСтрокиСотрудник, Сотрудник, ИстинаКогдаЭтоНоваяСтрока";

		Для Индекс = 0 По СтрокиИсточникиДанных.Количество() - 1 Цикл 		
			СтрокаПриемник = ДобавленныеСтроки[Индекс]; 
			ЗаполнитьЗначенияСвойств(СтрокаПриемник, СтрокиИсточникиДанных[Индекс], ,НезаполняемыеСвойства);
		КонецЦикла;		
	КонецЕсли;	
...
Процедура ДанныеОВремениАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Текст) Тогда
		...
		ОписаниеВидаВремени = ОписаниеВидовВремени.Получить(ВРег(БуквенноеОбозначение));
		
		Если НЕ ОписаниеВидаВремени = Неопределено Тогда
			
			ЗначениеВыбора = Новый Структура;
			ЗначениеВыбора.Вставить("БуквенноеОбозначение", БуквенноеОбозначение);
			ЗначениеВыбора.Вставить("Часы", ДанныеОВремени.Часы);
			ЗначениеВыбора.Вставить("ВидВремени", ОписаниеВидаВремени);
			
			ЧасыСтрока = ?(ДанныеОВремени.Часы = 0, "", " " + Формат(ДанныеОВремени.Часы, "ЧГ="));
			
			ПредставлениеЗначенияВыбора = СокрЛП(СтрШаблон(НСтр("ru = '%1 (%2) %3'"),
				Строка(ОписаниеВидаВремени), БуквенноеОбозначение, ЧасыСтрока));
...
Функция ТабельДанныеОВремениИзПредставления(Знач ПредставлениеДанныхОВремени, ДанныеТекущейСтроки, НомерДня, Отказ = Ложь)
	...
	ДанныеОВремениИзПредставления = Новый Структура("ВидВремени, БуквенноеОбозначение, Часы",
		ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.ВидыИспользованияРабочегоВремени.ПустаяСсылка"), "", 0);
	...
	Если ДанныеОВремени = Неопределено Или ДанныеОВремени.БуквенноеОбозначение = "" И ДанныеОВремени.Часы = 0 Тогда
		...
		Возврат ДанныеОВремениИзПредставления;
		
	ИначеЕсли ДанныеОВремени.БуквенноеОбозначение = "" Тогда
		Если ПустаяСтрока(ДанныеТекущейСтроки["ВидВремени" + НомерДня]) Тогда
			
			ДанныеОВремениИзПредставления.Вставить("ВидВремени",
				ОбщегоНазначенияКлиент.ПредопределенныйЭлемент("Перечисление.ВидыИспользованияРабочегоВремени.Явка"));
			ДанныеОВремениИзПредставления.Вставить("БуквенноеОбозначение", НСтр("ru = 'Я'"));
			ДанныеОВремениИзПредставления.Вставить("Часы", ДанныеОВремени.Часы);
			
			Возврат ДанныеОВремениИзПредставления;
			
		ИначеЕсли ДанныеТекущейСтроки.НомерСтрокиСотрудник = 1 Тогда
			
			БуквенноеОбозначение = ОписаниеВидовВремени.Получить(ДанныеТекущейСтроки["ВидВремени" + НомерДня]);
			
			ДанныеОВремениИзПредставления.Вставить("ВидВремени", ДанныеТекущейСтроки["ВидВремени" + НомерДня]);
			ДанныеОВремениИзПредставления.Вставить("БуквенноеОбозначение", БуквенноеОбозначение);
			ДанныеОВремениИзПредставления.Вставить("Часы", ДанныеОВремени.Часы);
	...
	Иначе
		...
		Если ОписаниеВидаВремени = Неопределено Тогда
			...
			Возврат Неопределено;
		Иначе
			ДанныеОВремениИзПредставления.Вставить("ВидВремени", ОписаниеВидаВремени.ВидВремени);
			ДанныеОВремениИзПредставления.Вставить("БуквенноеОбозначение", ДанныеОВремени.БуквенноеОбозначение);
			ДанныеОВремениИзПредставления.Вставить("Часы", ДанныеОВремени.Часы);
