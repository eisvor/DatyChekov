...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	СкрытиеПерсональныхДанных = ПолучитьФункциональнуюОпцию("ИспользоватьСкрытиеПерсональныхДанныхСубъектов");
...
Процедура АдресатыКонтактПриИзмененииНаСервере(ИдентификаторСтроки)
	...
	Если ТекущаяСтрока <> Неопределено Тогда
		СтруктураКонтактнойИнформации = ПолучитьСтруктуруКонтактнойИнформациейНаСервере(ТекущаяСтрока.Контакт);
		Если Не (ЗначениеЗаполнено(СтруктураКонтактнойИнформации)
			И ЗначениеЗаполнено(СтруктураКонтактнойИнформации[0].КонтактнаяИнформация)) Тогда
			ТекущаяСтрока.ТекстОшибки = "Не указана контактная информация";
		Иначе
			ЗаполнитьЗначенияСвойств(ТекущаяСтрока, СтруктураКонтактнойИнформации[0]);
	...
	Для Каждого СтрокаАдресата Из Объект.Адресаты Цикл
		...
		Если ЗначениеЗаполнено(ТекстСообщения) Тогда
			...
			Иначе
				...
				ПараметрыПисьма.Вставить("Кому", КонтактнаяИнформация);
...
Процедура ОбновлениеВидимостиИДоступностиРеквизитов()
	...
	Элементы.АдресатыПредставлениеКонтакта.Видимость = Не СкрытиеПерсональныхДанных;
...
Процедура ЗаполнитьПоГруппеПолучателейСкидкиНаСервере(ВыбраннаяГруппа)
	...
	ЭтоSMS = Объект.ТипРассылки = Перечисления.ТипыИнформационныхРассылок.SMS;
	...
	Запрос.УстановитьПараметр("ЭтоSMS", ЭтоSMS);
	Запрос.УстановитьПараметр("СкрытиеПерсональныхДанных", СкрытиеПерсональныхДанных);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(Выборка.КонтактнаяИнформация) Тогда
			НоваяСтрока.ТекстОшибки = "Не указана контактная информация";
		КонецЕсли; 
	КонецЦикла;
...
Функция ПредварительныйПросмотрСообщенияНаСервере(ИдентификаторСтроки)
	...
	Если Объект.ТипРассылки = Перечисления.ТипыИнформационныхРассылок.SMS И Объект.ОтправлятьВТранслите Тогда
		НовыйТекст = СтроковыеФункции.СтрокаЛатиницей(НовыйТекст);
	КонецЕсли;
...
Функция ЗначениеПараметраСообщения(ИмяПараметра, СтрокаАдресата)
	...
	ИначеЕсли НРег(ИмяПараметра) = "[остатокбаллов]" Тогда
		...
		Если ЗначениеЗаполнено(СтруктураКарты.ДисконтнаяКарта)
			...
			ДисконтныеКарты = МаркетинговыеАкцииСервер.ИсторияДисконтнойКарты(Объект.Дата, СтруктураКарты.ДисконтнаяКарта, Истина);
...
&НаСервере
Функция ПолучитьСтруктуруКонтактнойИнформациейНаСервере(Контакт)
	...
	|	ГДЕ
	|		ФизическиеЛицаКонтактнаяИнформация.Ссылка В(&ФизЛицо)
	...
	|	ГДЕ
	|		ФИОФизЛицСрезПоследних.ФизЛицо В(&ФизЛицо)) КАК Подзапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	Подзапрос.ФизЛицо
	|
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТКонтактнаяИнформация.ФизЛицо КАК Контакт,
	...
	|		ПО ВТКонтактнаяИнформация.ФизЛицо = СубъектыДляСкрытияПерсональныхДанных.Субъект
	|ГДЕ
	...
	Запрос.УстановитьПараметр("ФизЛицо", Контакт);
	Запрос.УстановитьПараметр("ЭтоSMS", ЭтоSMS);
	Запрос.УстановитьПараметр("СкрытиеПерсональныхДанных", СкрытиеПерсональныхДанных);
	Запрос.УстановитьПараметр("Телефон", Перечисления.ТипыКонтактнойИнформации.Телефон);
	Запрос.УстановитьПараметр("АдресЭлектроннойПочты", Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
	...
	Если ЭтоSMS Тогда
		Для Каждого СтрокаКонтактнойИнформации Из Результат Цикл
			ТелефонДляОтправки = СтрокаКонтактнойИнформации.КонтактнаяИнформация;
			ТелефонДляОтправки = СтрЗаменить(ТелефонДляОтправки, "+", "");
			ТелефонДляОтправки = СтрЗаменить(ТелефонДляОтправки, "-", "");
			ТелефонДляОтправки = СтрЗаменить(ТелефонДляОтправки, "(", "");
			ТелефонДляОтправки = СтрЗаменить(ТелефонДляОтправки, ")", "");
			ТелефонДляОтправки = СтрЗаменить(ТелефонДляОтправки, " ", "");
			СтрокаКонтактнойИнформации.КонтактнаяИнформация = ТелефонДляОтправки;
		КонецЦикла;
	КонецЕсли;
