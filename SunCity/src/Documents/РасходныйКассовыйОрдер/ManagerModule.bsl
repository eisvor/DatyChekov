...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.УстановитьПараметр("СтатьяДвиженияДенежныхСредств", ЗначениеНастроекПовтИсп.СтатьяДвиженияДенежныхСредств(Реквизиты.ХозяйственнаяОперация));
...
Функция ПодготовитьДанныеДляПробитияЧека(ДокументСсылка, РаспределениеВыручкиПоСекциям, НомерЧека) Экспорт
	...
	Если ТипЗнч(РеквизитыДокумента.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
	...
	Для Каждого СтрокаПлатежа Из РасшифровкаПлатежа Цикл
		...
		Если ИспользоватьРасчетыСКлиентами
			И ЗначениеЗаполнено(СтрокаПлатежа.ДокументРасчетовСКонтрагентом)
			И ОбщегоНазначенияРТ.ЕстьТЧОбъекта("Товары", СтрокаПлатежа.ДокументРасчетовСКонтрагентом.Метаданные())
			// Чек в качестве основания используется только при возврате аванса.
	...
	Если СуммаДокументовРасчетов <> РеквизитыДокумента.СуммаДокумента Тогда
		...
		Для Каждого СтрокаПлатежа Из РасшифровкаПлатежа Цикл
			...
			СтрокаПозицииЧека.Наименование = НСтр("ru = 'Возврат :'") + " " + РеквизитыДокумента.Выдать;			
...
Процедура ЗаполнитьИменаРеквизитовПоХозяйственнойОперации(ХозяйственнаяОперация, МассивВсехРеквизитов, МассивРеквизитовОперации) Экспорт
	...
	МассивВсехРеквизитов.Добавить("КассаККМПолучатель");
	...
	МассивВсехРеквизитов.Добавить("ГруппаОписьНаличных");
	
	МассивВсехРеквизитов.Добавить("РазбитьПлатеж");
	
	// Реквизиты для обработки проверки заполнения.
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СдачаДенежныхСредствВБанк Тогда
		...
		МассивРеквизитовОперации.Добавить("ГруппаОписьНаличных");
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствВКассуККМ Тогда
		...
		МассивРеквизитовОперации.Добавить("Касса");
		МассивРеквизитовОперации.Добавить("Организация");
		МассивРеквизитовОперации.Добавить("КассаККМПолучатель");
		МассивРеквизитовОперации.Добавить("ДоговорКонтрагента");
...
Процедура ЗаполнитьПризнакСпособаРасчетаСтрокиРасшифровки(Объект, СтрокаРасшифровки) Экспорт
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту Тогда
		Если ЗначениеЗаполнено(СтрокаРасшифровки.ДокументРасчетовСКонтрагентом) Тогда
			Если ТипЗнч(СтрокаРасшифровки.ДокументРасчетовСКонтрагентом) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда
...
Процедура ЗаполнитьМагазин(Объект, Магазин) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.ПлатежнаяВедомость) Тогда
		Источник = Объект.ПлатежнаяВедомость;
	КонецЕсли;
...
Процедура ЗаполнитьРеквитыПечатнойФормы(Объект, ДанныеЗаполнения = Неопределено) Экспорт
	...
	Если ДанныеЗаполнения = Неопределено Тогда
		...
		Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратОплатыКлиенту
			И ЗначениеЗаполнено(Объект.ДокументОснование) И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") Тогда
			ДанныеОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ДокументОснование, "Дата, ФизЛицо, ЧекНомер, ЧекДата");
			
			ДанныеЗаполнения.Вставить("Выдать", ДанныеОснования.ФизЛицо);
			
			Если ЗначениеЗаполнено(ДанныеОснования.ЧекНомер)  Тогда
				ШаблонПриложение = НСтр("ru = 'Заявление от %1, Чек №%2 от %3'");
				Приложение = СтрШаблон(ШаблонПриложение,
					Формат(ДатаСведений, "ДЛФ = DD"),
					ДанныеОснования.ЧекНомер,
					Формат(ДанныеОснования.ЧекДата, "ДЛФ=DD"));
				
				ДанныеЗаполнения.Вставить("Приложение", Приложение);
			КонецЕсли;
		ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗаработнойПлатыПоВедомостям
			И ЗначениеЗаполнено(Объект.ДокументОснование)  И ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ЗарплатаКВыплатеОрганизаций") Тогда
			ДанныеОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ДокументОснование,
				"Представление, Номер, Дата");
			
			ДанныеЗаполнения.Вставить("Приложение", ФормированиеПечатныхФормСервер.СформироватьЗаголовокДокумента(ДанныеОснования));
		КонецЕсли;
	...
	Если ДанныеЗаполнения.Свойство("Основание") Тогда
		...
		Если ЕстьРасчетыСКонтрагентами И ЗначениеЗаполнено(Объект.Контрагент) Тогда
			...
			Если НЕ ЗначениеЗаполнено(Объект.Выдать) И НЕ ДанныеЗаполнения.Свойство("Выдать") Тогда
				...
				Если ДанныеКонтрагента.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
					ДанныеЗаполнения.Вставить("Выдать", ДанныеКонтрагента.ФизЛицо);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	// Выдать.
	Если ДанныеЗаполнения.Свойство("Выдать") И ЗначениеЗаполнено(ДанныеЗаполнения.Выдать) Тогда
		Если ТипЗнч(ДанныеЗаполнения.Выдать) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			ДанныеФизЛица = ФормированиеПечатныхФормСервер.ФамилияИмяОтчество(ДанныеЗаполнения.Выдать, ДатаСведений);
			ФИО = ДанныеФизЛица.Фамилия + " " + ДанныеФизЛица.Имя + " " + ДанныеФизЛица.Отчество;
			
			ПоДокументу = РегистрыСведений.ДокументыФизическихЛиц.ДокументУдостоверяющийЛичностьФизлица(
				ДанныеЗаполнения.Выдать, ДатаСведений);
			ДанныеЗаполнения.Вставить("ПоДокументу", ПоДокументу);
			
		Иначе
			ФИО = ДанныеЗаполнения.Выдать;
		КонецЕсли;
		Объект.Выдать = ПолучитьСклоненияСтроки(ФИО, "Л=ru_RU", "ПД=Дательный")[0];
	КонецЕсли;
	
	// Приложение.
	Если ДанныеЗаполнения.Свойство("Приложение") И ЗначениеЗаполнено(ДанныеЗаполнения.Приложение) Тогда
		Объект.Приложение = ДанныеЗаполнения.Приложение;
	КонецЕсли;
	
	// По документу.
	Если ДанныеЗаполнения.Свойство("ПоДокументу") И ЗначениеЗаполнено(ДанныеЗаполнения.ПоДокументу) Тогда
		Объект.ПоДокументу = ДанныеЗаполнения.ПоДокументу;
	КонецЕсли;
...
// Получает структуру массивов (макет) для чека РКО.
//
// Параметры:
...
// Формирует структуру шаблона чека по умолчанию из макета документа Расходный кассовый ордер.
//
// Параметры:
...
//
// Возвращаемое значение:
//  ДокументСсылка.РасходныйКассовыйОрдер- документ для получения макета.
//
...
// Получает работников по ведомости которые не получили зарплату.
//
// Параметры:
//  ПлатежнаяВедомость - ДокументСсылка.ЗарплатаКВыплатеОрганизаций - ведомость зарплаты.
//
// Возвращаемое значение:
//  ВыборкаРезультатаЗапроса - Работники по ведомости.
//
...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
	...
	|ГДЕ
	|	ЗначениеРазрешено(Касса.Магазин) ИЛИ ЗначениеРазрешено(КассаПолучатель.Магазин)";
...
Функция ТекстЗапросаТаблицаДенежныеСредстваНаличные()
	...
	|	ЕСТЬNULL(ТабличнаяЧастьРасшифровкаПлатежа.ДоговорКонтрагента, ДанныеДокумента.ДоговорКонтрагента) КАК ДоговорКонтрагента,
	|	ВЫБОР
...
Функция ТекстЗапросаТаблицаДенежныеСредстваККМ()
	...
	|	ЕСТЬNULL(ТабличнаяЧастьРасшифровкаПлатежа.ДоговорКонтрагента, ДанныеДокумента.ДоговорКонтрагента),
...
Функция ТекстЗапросаТаблицаРасчетыСКлиентами()
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ТабличнаяЧастьРасшифровкаПлатежа.ДокументРасчетовСКонтрагентом ССЫЛКА Документ.ВозвратТоваровОтПокупателя
	|						ТОГДА ТабличнаяЧастьРасшифровкаПлатежа.ДокументРасчетовСКонтрагентом.ДокументОснование.ЗаказПокупателя
	...
	|					КОГДА ТабличнаяЧастьРасшифровкаПлатежа.ДокументРасчетовСКонтрагентом ССЫЛКА Документ.ПриходныйКассовыйОрдер
	|						ТОГДА ТабличнаяЧастьРасшифровкаПлатежа.ДокументРасчетовСКонтрагентом.ЗаказПокупателя
...
Функция СформироватьШтрихкодПечатнойФормы(XMLШтрихКода, Рисунок)
	...
	Возврат ГенерацияШтрихкода.ИзображениеШтрихкода(ПараметрыШтрихкода).Картинка;
...
Функция СформироватьПечатнуюФормуКвитанцияКСумке(МассивОбъектов, ОбъектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если НЕ ЕстьОшибкиПечатиИнкассации(ДанныеПечати) Тогда
			...
			ОбластьМакетаСекцияОсновнаяЧасть.Параметры.НомерСумки    		 = Сред(ДанныеПечати.НомерСумки, Найти(ДанныеПечати.НомерСумки, "/") + 1.00);
...
Функция СформироватьПечатнуюФормуКО2(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	РасходныйКассовыйОрдер.Номер КАК НомерДокумента,
	|	РасходныйКассовыйОрдер.Дата КАК ДатаДокумента,
	|	РасходныйКассовыйОрдер.Организация КАК Организация,
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		ОбластьМакетаШапка.Параметры.СуммаПрописью     		 	= ФормированиеПечатныхФормСервер.СформироватьСуммуПрописью(ВыборкаПоДокументам.Сумма);
		...
		ОбластьМакетаШапка.Параметры.ДатаДокумента     		 	= Формат(ВыборкаПоДокументам.ДатаДокумента, "ДФ = 'дд.ММ.гггг'")+" г.";
		...
		ОбластьМакетаШапка.Параметры.Основание             = СокрЛП(ВыборкаПоДокументам.Основание);

		ТекстВыдать      = СокрЛП(ВыборкаПоДокументам.Выдать);
		ТекстПоДокументу = СокрЛП(ВыборкаПоДокументам.ПоДокументу);
		
		Если ТекстВыдать = "" Тогда
			Если ВыборкаПоДокументам.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПрочиеРасходы Тогда
				ТекстВыдать = ?(ЗначениеЗаполнено(ВыборкаПоДокументам.Организация), ВыборкаПоДокументам.Организация.Наименование, "");
			ИначеЕсли ЗначениеЗаполнено(ВыборкаПоДокументам.Контрагент) Тогда
				Если ЕстьРасчетыСКонтрагентами  Тогда
					ТекстВыдать = ?(ВыборкаПоДокументам.Контрагент.НаименованиеПолное = "", ВыборкаПоДокументам.Контрагент.Наименование, ВыборкаПоДокументам.Контрагент.НаименованиеПолное);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
			
		ОбластьМакетаШапка.Параметры.Получатель = ТекстВыдать;
		ОбластьМакетаШапка.Параметры.РеквизитыДокументаУдостоверяющегоЛичность = ТекстПоДокументу;
