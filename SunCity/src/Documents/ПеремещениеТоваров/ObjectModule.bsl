...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ДополнительныеСвойства.ИспользуетсяКомиссионнаяТорговля И Организация <> ОрганизацияПолучатель Тогда
	...
	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени("РегистрНакопления.ОстаткиАлкогольнойПродукцииЕГАИС");
	МенеджерОбъекта.ОтразитьДвижения(ДополнительныеСвойства, Движения, Отказ);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ОбщегоНазначенияРТ.УстановитьНовоеЗначениеРеквизита(
		ЭтотОбъект,
		ОбработкаТабличнойЧастиТоварыКлиентСервер.СуммаДокумента(Товары, Истина),
		"СуммаДокумента");
		
	// ИнтеграцияЕГАИС - отключенный функционал
	Если Ложь И ЗначениеЗаполнено(ТТНВходящаяЕГАИС) Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
		...
		МагазинОтправитель = ДанныеЗаполнения.МагазинПолучатель;
		...
		Если ДанныеЗаполнения.МагазинОтправитель = ДанныеЗаполнения.МагазинПолучатель Тогда
			ЕдинственныеЗначения = Документы.ПеремещениеТоваров.ЕдинственныеЗначения(МагазинОтправитель);
			Если ЕдинственныеЗначения.ЕдинственныйМагазин Тогда
				МагазинПолучатель = МагазинОтправитель;
			...
			Если ЕдинственныеЗначения.ЕдинственнаяОрганизация Тогда
				ОрганизацияПолучатель = ЕдинственныеЗначения.Организация;
			Иначе
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ИзменениеАссортимента") Тогда
		...
		МагазинОтправитель = ЗначениеНастроекПовтИсп.МагазинПоУмолчанию(МагазинОтправитель);
		СкладОтправитель = ЗначениеНастроекПовтИсп.СкладПоступленияПоУмолчанию(МагазинОтправитель,,СкладОтправитель, Ответственный);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ СкладОтправитель.Магазин = МагазинОтправитель
		И НЕ МагазинОтправитель.СкладУправляющейСистемы Тогда
		
		ТекстСообщения = НСтр("ru = 'Склад-отправитель задан не верно'");
	...
	Если НЕ СкладПолучатель.Магазин = МагазинПолучатель
		И НЕ МагазинПолучатель.СкладУправляющейСистемы Тогда
		
		ТекстСообщения = НСтр("ru = 'Склад-получатель задан не верно'");
	...
	Если СкладОтправитель = СкладПолучатель
		...
		И МагазинОтправитель = МагазинПолучатель 
		И НЕ МагазинОтправитель.СкладУправляющейСистемы Тогда
		
		Поле = "СкладПолучатель";
		ЕдинственныеЗначения = Документы.ПеремещениеТоваров.ЕдинственныеЗначения(МагазинОтправитель);
		Если ЕдинственныеЗначения.ЕдинственныйМагазин Тогда
			Если ЕдинственныеЗначения.ЕдинственныйСклад Тогда
				Если ЕдинственныеЗначения.ЕдинственнаяОрганизация Тогда
					Поле = "МагазинПолучатель";
				Иначе
					Поле = "ОрганизацияПолучатель";
				КонецЕсли;
			Иначе
				Поле = "СкладПолучатель";
			КонецЕсли;
		...
		ТекстСообщения = НСтр("ru = 'Получатель равен отправителю'");
	...
	Если ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойПродукцииИСМП")
		И НЕ ЭтотОбъект.Организация = ЭтотОбъект.ОрганизацияПолучатель Тогда
		// Проверим заполнение марок ИСМП.
		ИнтеграцияИСРТ.ПроверитьЗаполнениеАкцизныхМарок(ЭтотОбъект, Отказ);
	КонецЕсли;
	
	ОперацияОтражаетсяВЕГАИС = Документы.ПеремещениеТоваров.ОперацияОтражаетсяВЕГАИС(ЭтотОбъект);
	Если ОперацияОтражаетсяВЕГАИС Тогда
		// Проверим заполнение марок ЕГАИС.
		МодульИнтеграцияЕГАИСВызовСервера = ОбщегоНазначения.ОбщийМодуль("ИнтеграцияЕГАИСВызовСервера");
		Если МодульИнтеграцияЕГАИСВызовСервера.ИспользуетсяРегистрацияРозничныхПродажВЕГАИС(Организация, МагазинОтправитель, Дата) Тогда
	...
	Если ЭтоПрослеживаемыйДокумент И ФиксироватьНомераГТД Тогда
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	МагазинОтправитель = ЗначениеНастроекПовтИсп.МагазинПоУмолчанию(МагазинОтправитель);
	СкладОтправитель   = ЗначениеНастроекПовтИсп.СкладПродажиПоУмолчанию(МагазинОтправитель,,СкладОтправитель, Ответственный);
	СкладПолучатель    = ЗначениеНастроекПовтИсп.СкладПоступленияПоУмолчанию(МагазинПолучатель,,СкладПолучатель, Ответственный);
...
Процедура ЗаполнитьПоПоступлению(ДокументОснование)
	...
	|			И ТоварыОснования.КлючСвязи = ТоварыКПеремещению.КлючСвязи
	|ГДЕ
