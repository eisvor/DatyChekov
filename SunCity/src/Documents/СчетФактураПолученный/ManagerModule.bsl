...
//	РеквизитыСчетаФактуры - Структура - Возвращаемый. Данные счета-фактуры (Ссылка, Номер, ДатаСоставления,
//	                                   ВИностраннойВалюте);
//	Проведен - Булево - Поиск только проведенных счетов-фактур.
//	Исправление - Булево - Используется при поиске основания для исправления.
//
// Возвращаемое значение:
//	Массив - Массив найденных счетов-фактур.
//
Функция СчетаФактурыПоОснованию(Основание, Организация = Неопределено, РеквизитыСчетаФактуры = Неопределено, Проведен = Истина, Исправление = Ложь) Экспорт
	...
	|	И ТаблицаОснований.Ссылка.Исправление = ЛОЖЬ
	|
	|УПОРЯДОЧИТЬ ПО
	|	Номер УБЫВ,
	...
	Если Не Исправление Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ТаблицаОснований.Ссылка.Исправление = ЛОЖЬ", "");
	КонецЕсли;
