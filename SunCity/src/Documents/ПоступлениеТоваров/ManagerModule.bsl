...
//
// Параметры:
//  СсылкаНаОбъект - ДокументСсылка.ПоступлениеТоваров - ссылка на документ.
// 
...
Функция ПолучитьДанныеДляЭД(СсылкаНаОбъект) Экспорт
	...
	|	&ПустаяСтрока КАК Идентификатор,
	...
	|
	|ВЫБРАТЬ
	|	ПоступлениеТоваровТоварыПоДаннымПоставщика.Ссылка.Контрагент,
	|	&ПустаяСтрока,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТНоменклатураДокумента.Владелец КАК Владелец,
	|	ВТНоменклатураДокумента.Идентификатор КАК Идентификатор,
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ВТНоменклатураДокумента.Номенклатура КАК Справочник.Номенклатура) КАК Номенклатура,
	|	ВЫРАЗИТЬ(ВТНоменклатураДокумента.Характеристика КАК Справочник.ХарактеристикиНоменклатуры) КАК Характеристика,
	|	ВЫРАЗИТЬ(ВТНоменклатураДокумента.Упаковка КАК Справочник.УпаковкиНоменклатуры) КАК Упаковка,
	|	МАКСИМУМ(втНоменклатураКонтрагента.Идентификатор) КАК Идентификатор
	|ПОМЕСТИТЬ ВТСопоставленнаяНоменклатура
	|ИЗ
	|	ВТНоменклатураДокумента КАК ВТНоменклатураДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ втНоменклатураКонтрагента КАК втНоменклатураКонтрагента
	|		ПО ВТНоменклатураДокумента.Номенклатура = втНоменклатураКонтрагента.Номенклатура
	|			И ВТНоменклатураДокумента.Характеристика = втНоменклатураКонтрагента.Характеристика
	|			И ВТНоменклатураДокумента.Упаковка = втНоменклатураКонтрагента.Упаковка
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТНоменклатураДокумента.Номенклатура,
	|	ВТНоменклатураДокумента.Характеристика,
	|	ВТНоменклатураДокумента.Упаковка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Шапка.ДатаВходящегоДокумента КАК ДатаЭД,
	|	Шапка.НомерВходящегоДокумента КАК НомерЭД,
	|	Шапка.Организация КАК Организация,
	|	Шапка.Контрагент КАК Контрагент,
	|	Шапка.Договор КАК ДоговорКонтрагента,
	|	&ПустаяСтрока КАК Грузоотправитель,
	|	&ПустаяСтрока КАК Грузополучатель,
	|	&ПустаяСтрока КАК Перевозчик,
	|	&Валюта КАК Валюта,
	...
	|	ЕСТЬNULL(втНоменклатураКонтрагента.Наименование, ВТСопоставленнаяНоменклатура.Номенклатура.НаименованиеПолное) КАК НоменклатураНаименование,
	|	ВТСопоставленнаяНоменклатура.Номенклатура.Код КАК НоменклатураКод,
	|	ЕСТЬNULL(втНоменклатураКонтрагента.Артикул, ВТСопоставленнаяНоменклатура.Номенклатура.Артикул) КАК НоменклатураАртикул,
	|	ВТСопоставленнаяНоменклатура.Номенклатура.СтавкаНДС КАК НоменклатураСтавкаНДС,
	|	ВТСопоставленнаяНоменклатура.Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры,
	|	ЕСТЬNULL(втНоменклатураКонтрагента.Характеристика, ВТСопоставленнаяНоменклатура.Характеристика) КАК Характеристика,
	|	ЕСТЬNULL(втНоменклатураКонтрагента.НаименованиеХарактеристики, ВТСопоставленнаяНоменклатура.Характеристика.Наименование) КАК ХарактеристикаНаименование,
	|	ЕСТЬNULL(втНоменклатураКонтрагента.ЕдиницаИзмерения, ВЫБОР
	|			КОГДА ВТСопоставленнаяНоменклатура.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|				ТОГДА ВТСопоставленнаяНоменклатура.Номенклатура.ЕдиницаИзмерения
	|			ИНАЧЕ ВТСопоставленнаяНоменклатура.Упаковка
	|		КОНЕЦ) КАК ЕдиницаИзмерения,
	|	ВЫБОР
	|		КОГДА ВТСопоставленнаяНоменклатура.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|			ТОГДА ВТСопоставленнаяНоменклатура.Номенклатура.ЕдиницаИзмерения.Наименование
	|		ИНАЧЕ ВТСопоставленнаяНоменклатура.Упаковка.Наименование
	|	КОНЕЦ КАК ЕдиницаИзмеренияНаименование,
	|	ЕСТЬNULL(втНоменклатураКонтрагента.ЕдиницаИзмеренияКод, ВЫБОР
	|			КОГДА ВТСопоставленнаяНоменклатура.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|				ТОГДА ВТСопоставленнаяНоменклатура.Номенклатура.ЕдиницаИзмерения.Код
	|			ИНАЧЕ ""0000""
	|		КОНЕЦ) КАК ЕдиницаИзмеренияКод,
	|	ВЫБОР
	|		КОГДА ВТСопоставленнаяНоменклатура.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|			ТОГДА ВТСопоставленнаяНоменклатура.Номенклатура.ЕдиницаИзмерения.Наименование
	|		ИНАЧЕ ВТСопоставленнаяНоменклатура.Упаковка.ЕдиницаИзмерения.Наименование
	|	КОНЕЦ КАК ВидУпаковки,
	|	ЕСТЬNULL(втНоменклатураКонтрагента.Идентификатор, &ПустаяСтрока) КАК ИдентификаторНоменклатурыПоставщика,
	|	ЕСТЬNULL(ТоварыПоДаннымПоставщика.КоличествоУпаковок, 0) КАК КоличествоПоДокументу,
	|	ЕСТЬNULL(ТоварыПоДаннымПоставщика.Цена, 0) КАК ЦенаПоДокументу,
	|	ЕСТЬNULL(ТоварыПоДаннымПоставщика.Сумма, 0) КАК СуммаПоДокументу,
	|	ЕСТЬNULL(ТоварыПоДаннымПоставщика.СтавкаНДС, 0) КАК СтавкаНДСПоДокументу,
	|	ЕСТЬNULL(ТоварыПоДаннымПоставщика.СуммаНДС, 0) КАК СуммаНДСПоДокументу,
	|	ЕСТЬNULL(ВЫБОР
	|			КОГДА ЕСТЬNULL(Товары.Ссылка.ЦенаВключаетНДС, ИСТИНА)
	|				ТОГДА ТоварыПоДаннымПоставщика.Сумма
	|			ИНАЧЕ ТоварыПоДаннымПоставщика.Сумма + ТоварыПоДаннымПоставщика.СуммаНДС
	|		КОНЕЦ, 0) КАК СуммаСНДСПоДокументу,
	|	ЕСТЬNULL(Товары.КоличествоУпаковок, 0) КАК Количество,
	|	ЕСТЬNULL(Товары.СтавкаНДС, 0) КАК СтавкаНДС,
	|	ЕСТЬNULL(Товары.Цена, 0) КАК Цена,
	|	ЕСТЬNULL(Товары.Сумма, 0) КАК Сумма,
	|	ЕСТЬNULL(Товары.СуммаНДС, 0) КАК СуммаНДС,
	|	ЕСТЬNULL(ВЫБОР
	|			КОГДА ЕСТЬNULL(Товары.Ссылка.ЦенаВключаетНДС, ИСТИНА)
	|				ТОГДА Товары.Сумма
	|			ИНАЧЕ Товары.Сумма + Товары.СуммаНДС
	|		КОНЕЦ, 0) КАК СуммаСНДС,
	|	ЕСТЬNULL(Товары.Ссылка.ЦенаВключаетНДС, ИСТИНА) КАК ЦенаВключаетНДС,
	|	ЕСТЬNULL(ТоварыПоДаннымПоставщика.КоличествоУпаковок, 0) + ЕСТЬNULL(Товары.КоличествоУпаковок, 0) КАК КоличествоИтого,
	|	ВЫБОР
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ втНоменклатураКонтрагента КАК втНоменклатураКонтрагента
	|		ПО ВТСопоставленнаяНоменклатура.Номенклатура = втНоменклатураКонтрагента.Номенклатура
	|			И ВТСопоставленнаяНоменклатура.Характеристика = втНоменклатураКонтрагента.Характеристика
	|			И ВТСопоставленнаяНоменклатура.Упаковка = втНоменклатураКонтрагента.Упаковка
	|			И ВТСопоставленнаяНоменклатура.Идентификатор = втНоменклатураКонтрагента.Идентификатор
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваров.Товары КАК Товары
	|		ПО ВТСопоставленнаяНоменклатура.Номенклатура = Товары.Номенклатура
	|			И ВТСопоставленнаяНоменклатура.Характеристика = Товары.Характеристика
	|			И ВТСопоставленнаяНоменклатура.Упаковка = Товары.Упаковка
	|			И (Товары.Ссылка = &Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваров.ТоварыПоДаннымПоставщика КАК ТоварыПоДаннымПоставщика
	|		ПО ВТСопоставленнаяНоменклатура.Номенклатура = ТоварыПоДаннымПоставщика.Номенклатура
	|			И ВТСопоставленнаяНоменклатура.Характеристика = ТоварыПоДаннымПоставщика.Характеристика
	|			И ВТСопоставленнаяНоменклатура.Упаковка = ТоварыПоДаннымПоставщика.Упаковка
	|			И (ТоварыПоДаннымПоставщика.Ссылка = &Ссылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТСопоставленнаяНоменклатура.Номенклатура КАК Номенклатура,
	|	ЕСТЬNULL(втНоменклатураКонтрагента.НаименованиеХарактеристики, ВТСопоставленнаяНоменклатура.Характеристика) КАК Характеристика,
	|	ЕСТЬNULL(втНоменклатураКонтрагента.ЕдиницаИзмерения, ВЫБОР
	|			КОГДА ВТСопоставленнаяНоменклатура.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиНоменклатуры.ПустаяСсылка)
	|				ТОГДА ВТСопоставленнаяНоменклатура.Номенклатура.ЕдиницаИзмерения
	|			ИНАЧЕ ВТСопоставленнаяНоменклатура.Упаковка
	...
	|		ПО ВТСопоставленнаяНоменклатура.Номенклатура = Штрихкоды.Владелец
	|			И ВТСопоставленнаяНоменклатура.Характеристика = Штрихкоды.Характеристика
	|			И ВТСопоставленнаяНоменклатура.Упаковка = Штрихкоды.Упаковка
	|		ЛЕВОЕ СОЕДИНЕНИЕ втНоменклатураКонтрагента КАК втНоменклатураКонтрагента
	|		ПО ВТСопоставленнаяНоменклатура.Номенклатура = втНоменклатураКонтрагента.Номенклатура
	|			И ВТСопоставленнаяНоменклатура.Характеристика = втНоменклатураКонтрагента.Характеристика
	|			И ВТСопоставленнаяНоменклатура.Упаковка = втНоменклатураКонтрагента.Упаковка
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПоступлениеТоваровШтрихкодыУпаковок.Ссылка КАК Ссылка,
	|	ПоступлениеТоваровШтрихкодыУпаковок.ШтрихкодУпаковки КАК Штрихкод
	|ИЗ
	|	Документ.ПоступлениеТоваров.ШтрихкодыУпаковок КАК ПоступлениеТоваровШтрихкодыУпаковок
	|ГДЕ
	|	ПоступлениеТоваровШтрихкодыУпаковок.Ссылка В(&Ссылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПоступлениеТоваровШтрихкодыУпаковокРасхождения.Ссылка КАК Ссылка,
	|	ПоступлениеТоваровШтрихкодыУпаковокРасхождения.ТипРасхождения КАК ТипРасхождения,
	|	ПоступлениеТоваровШтрихкодыУпаковокРасхождения.Номенклатура КАК Номенклатура,
	|	ПоступлениеТоваровШтрихкодыУпаковокРасхождения.Характеристика КАК Характеристика,
	|	ПоступлениеТоваровШтрихкодыУпаковокРасхождения.Количество КАК Количество,
	|	ПоступлениеТоваровШтрихкодыУпаковокРасхождения.ЗначениеШтрихкода КАК ЗначениеШтрихкода
	|ИЗ
	|	Документ.ПоступлениеТоваров.ШтрихкодыУпаковокРасхождения КАК ПоступлениеТоваровШтрихкодыУпаковокРасхождения
	|ГДЕ
	|	ПоступлениеТоваровШтрихкодыУпаковокРасхождения.Ссылка В(&Ссылка)";
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	// З-1 (Приходный ордер)
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "З1";
	...
	Если ДоступноПечатьЭтикетокИЦенников Тогда
		...
		ПараметрыПечатиЦенников.Вставить("ИмяПроцедурыПодготовкиСтруктурыДанных", "УправлениеПечатьюРТВызовСервера.СтруктураДанныхЦенниковИЭтикеток");
		...
		ПараметрыПечатиЭтикеток.Вставить("ИмяПроцедурыПодготовкиСтруктурыДанных", "УправлениеПечатьюРТВызовСервера.СтруктураДанныхЦенниковИЭтикеток");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "З1") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,"З1",
...
// Перезаполняет табличную часть "ТоварыПоДаннымПоставщика" согласно
// фактически поступившим товарам.
//
// Параметры:
//  Объект - ДокументОбъект.ПоступлениеТоваров
//
Процедура ПерезаполнитьТоварыПоДаннымПоставщикаПоФакту(Объект) Экспорт
	...
	КолонкиКПереносу = КолонкиИзмерения + "," + Символы.ПС + КолонкиСуммовые;
	
	ТоварыПоДаннымПоставщика = Объект.Товары.Выгрузить(, КолонкиКПереносу);
	ТоварыПоДаннымПоставщика.Свернуть(КолонкиИзмерения, КолонкиСуммовые);
	
	Объект.ТоварыПоДаннымПоставщика.Загрузить(ТоварыПоДаннымПоставщика);
...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	КомандаОтчет = Отчеты.АнализРасхожденийПриПоступленииТоваров.ДобавитьКомандуОтчета(КомандыОтчетов);
...
Процедура СформироватьТекстЗапросаАктРасхождений(ТекстЗапроса)
	...
	|			И ПоступлениеТоваровТоварыПоДаннымПоставщика.Ссылка = ТоварыПоФактуНомерСтроки.Ссылка
	|ГДЕ
	|	(ПоступлениеТоваровТоварыПоДаннымПоставщика.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар)
	|			ИЛИ ПоступлениеТоваровТоварыПоДаннымПоставщика.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПодарочныйСертификат))
	|	И ПоступлениеТоваровТоварыПоДаннымПоставщика.Ссылка В(&МассивОбъектов)
...
Функция ХозяйственныеОперацииИмпорта() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетИмпортныхТоваров") Тогда
...
Функция ПечатьНакладная(МассивОбъектов, ОбъектыПечати, ВыводитьЦены = Ложь, ТолькоПоРозничнымЦенам = Ложь)
	...
	Иначе //НЕ УстанавливатьВидыЦенВАссортименте
		...
		|ПО (ТаблицаТовары.Ссылка.Магазин = РозничныеЦены.Магазин
...
Функция ПечатьТОРГ4(МассивОбъектов, ОбъектыПечати)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		Пока ДанныеПечати.Следующий() Цикл
			...
			РеквизитыСопроводительногоДокумента = СтрЗаменить(РеквизитыСопроводительногоДокумента, "%ДатаДокументаСопровождения%",  Формат(ДанныеПечати.ДатаДокументаСопровождения, Нстр("ru='ДФ=дд ММММ гггг'")));
...
Функция ПечатьЗ1(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст ="ВЫБРАТЬ РАЗРЕШЕННЫЕ
	              ...
	              |	ПоступлениеТоваровУслуг.Ответственный.ФизическоеЛицо КАК Ответственный,
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПоступлениеТоваровУслуг_З1";
	Макет = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_З1");
	
	//Опеределим области макета
    ...
    Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		СведенияОбПоставщике 	= ФормированиеПечатныхФормСервер.СведенияОЮрФизЛице(ВыборкаПоДокументам.Контрагент, ВыборкаПоДокументам.ДатаДокумента);
	
		// Выводим шапку накладной
		ТекстРНН_БИН = "";
		ОбластьМакетаЗаголовок = Макет.ПолучитьОбласть("Заголовок");
		ОбластьМакетаЗаголовок.Параметры.ПредставлениеОрганизации 	= ФормированиеПечатныхФормСервер.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,");
		ОбластьМакетаЗаголовок.Параметры.ПредставлениеСклада 	 	= ВыборкаПоДокументам.СкладНаименование;
		ОбластьМакетаЗаголовок.Параметры.ОрганизацияРНН_БИН 		= ФормированиеПечатныхФормСервер.ОписаниеОрганизации(СведенияОбОрганизации, "ИНН,", Ложь, "ru");
		
		ТабличныйДокумент.Вывести(ОбластьМакетаЗаголовок);

		ОбластьМакетаЗаголовокДокумента.Параметры.Заполнить(ВыборкаПоДокументам);
		ОбластьМакетаЗаголовокДокумента.Параметры.ПредставлениеПоставщика 	= ФормированиеПечатныхФормСервер.ОписаниеОрганизации(СведенияОбПоставщике, "ПолноеНаименование,");
		ОбластьМакетаЗаголовокДокумента.Параметры.НомерДокумента 			= ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ВыборкаПоДокументам.НомерДокумента, Ложь, Истина);
	
		ТабличныйДокумент.Вывести(ОбластьМакетаЗаголовокДокумента);
	
		ОбластьМакетаЗаголовокТаблицы.Параметры.ВалютаПредставление = "тенге";
		...
		Если ВыборкаПоТабличнымЧастям.НайтиСледующий(Новый Структура("Ссылка", ВыборкаПоДокументам.Ссылка)) Тогда
			...
			Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл 
				...
				Сумма    	= Сумма    		+ ВыборкаПоСтрокамТЧ.ВсегоСНДС;
				Количество 	= Количество 	+ ВыборкаПоСтрокамТЧ.КоличествоПоДокументу;
		...
		// Вывести Итого
		ОбластьМакетаИтого.Параметры.ИтогВсегоСНДС 				= ФормированиеПечатныхФормСервер.ФорматСумм(Сумма);
		...
		Иначе
			ОбластьМакетаПодвал.Параметры.МОЛ 	= "";		
		КонецЕсли;	
